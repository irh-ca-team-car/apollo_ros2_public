...
namespace apollo
{
  namespace audio
  {
    ...
    bool SirenDetection::Evaluate(const std::vector<std::vector<double>> &signals)
    {
      ...
      if (signals.size() == 0)
      {
        AERROR << "Got no channel in signals!" << CRLF;
        return false;
      }
      if (signals[0].size() == 0)
      {
        AERROR << "Got no signal in channel 0!" << CRLF;
        return false;
      }
      if (signals[0].size() != 72000)
      {
        AERROR << "signals[0].size() = " << signals[0].size() << ", skiping!" << CRLF;
        return false;
      }
      ...
      AINFO << "SirenDetection used time: " << diff.count() * 1000 << " ms." << CRLF;
      ...
      ADEBUG << "neg_score = " << neg_score << ", pos_score = " << pos_score << CRLF;
    ...
    void SirenDetection::LoadModel()
    {
      ...
      {
        AFATAL << "Missing Siren Detection Model at '"<<FLAGS_torch_siren_detection_model<<"', File not found or corrupted"<<CRLF;
        std::exit(-1);
      }
    }
