...
std::pair<Point3D, double> DirectionDetection::EstimateSoundSource(
  ...
  source_position_p3d.x=(source_position[0]);
  source_position_p3d.y=(source_position[1]);
  source_position_p3d.z=(source_position[2]);
...
bool DirectionDetection::LoadExtrinsics(const std::string& yaml_file,
  ...
  if (!apollo::cyber::common::PathExists(yaml_file)) {
    AINFO << yaml_file << " does not exist!"<<CRLF;
    return false;
  }
  ...
  try {
    if (node.IsNull()) {
      AINFO << "Load " << yaml_file << " failed! please check!"<<CRLF;
      return false;
    }
    ...
    AERROR << "load camera extrinsic file " << yaml_file
           << " with error, YAML exception:" << e.what()<<CRLF;
    return false;
  }
...
double DirectionDetection::GccPhat(const torch::Tensor& sig,
  ...
  psig = torch::fft::rfft(psig, 1, false, "forward");
  prefsig = torch::fft::rfft(prefsig, 1, false, "forward");

  ConjugateTensor(&prefsig);
  torch::Tensor r = ComplexMultiply(psig, prefsig);
  torch::Tensor cc =
      torch::fft::irfft(r / ComplexAbsolute(r), {interp * n}, false , "backward");
