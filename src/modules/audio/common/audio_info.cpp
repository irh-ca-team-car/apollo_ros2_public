...
namespace audio {
  ...
  {
    std::size_t index = 0;
    for (const auto &channel_data : audio_data.channeldata)
    {
      if (channel_data.channeltype == ChannelType::RAW)
      {
        InsertChannelData(index, channel_data, audio_data.microphoneconfig);
        ++index;
      }
    }
}
...
void AudioInfo::InsertChannelData(const std::size_t index,
  ...
  int width = microphone_config.samplewidth;
  const std::string& data =  std::string(channel_data.data.data.begin(),channel_data.data.data.end());
  ...
  std::size_t max_signal_length = static_cast<std::size_t>(
      FLAGS_cache_signal_time * microphone_config.samplerate);
