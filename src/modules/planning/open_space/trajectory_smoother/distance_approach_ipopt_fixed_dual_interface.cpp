  ...
  namespace planning
  {
          ...
          obstacles_b_(obstacles_b)
    {
      ...
      distance_approach_config_ =
          planner_open_space_config.distanceapproachconfig;
      weight_state_x_ = distance_approach_config_.weightx;
      weight_state_y_ = distance_approach_config_.weighty;
      weight_state_phi_ = distance_approach_config_.weightphi;
      weight_state_v_ = distance_approach_config_.weightv;
      weight_input_steer_ = distance_approach_config_.weightsteer;
      weight_input_a_ = distance_approach_config_.weighta;
      weight_rate_steer_ = distance_approach_config_.weightsteerrate;
      weight_rate_a_ = distance_approach_config_.weightarate;
      weight_stitching_steer_ = distance_approach_config_.weightsteerstitching;
      weight_stitching_a_ = distance_approach_config_.weightastitching;
      weight_first_order_time_ =
          distance_approach_config_.weightfirstordertime;
      weight_second_order_time_ =
          distance_approach_config_.weightsecondordertime;
      min_safety_distance_ = distance_approach_config_.minsafetydistance;
      max_steer_angle_ =
          vehicle_param_.maxsteerangle / vehicle_param_.steerratio;
      max_speed_forward_ = distance_approach_config_.maxspeedforward;
      max_speed_reverse_ = distance_approach_config_.maxspeedreverse;
      max_acceleration_forward_ =
          distance_approach_config_.maxaccelerationforward;
      max_acceleration_reverse_ =
          distance_approach_config_.maxaccelerationreverse;
      min_time_sample_scaling_ =
          distance_approach_config_.mintimesamplescaling;
      max_time_sample_scaling_ =
          distance_approach_config_.maxtimesamplescaling;
      max_steer_rate_ =
          vehicle_param_.maxsteeranglerate / vehicle_param_.steerratio;
      use_fix_time_ = distance_approach_config_.usefixtime;
      wheelbase_ = vehicle_param_.wheelbase;
      enable_constraint_check_ =
          distance_approach_config_.enableconstraintcheck;
      enable_jacobian_ad_ = distance_approach_config_.enablejacobianad;
    }
        ...
        bool init_lambda, double *lambda)
    {
      ...
      if (distance_approach_config_.enablecheckinitialstate)
      {
        int kM = m;
        double g[kM];
        eval_g(n, x, true, m, g);
        ...
        Eigen::MatrixXd *dual_n_result) const
    {
      ...
      *dual_n_result = dual_n_result_;
      ...
      if (!distance_approach_config_.enableinitialfinalcheck)
      {
        return;
      }
                                                           ...
                                                           T *obj_value)
    {
      ...
      ACHECK(ts_ != 0) << "ts in distance_approach_ is 0";
      int control_index = control_start_index_;
      int time_index = time_start_index_;
      int state_index = state_start_index_;
