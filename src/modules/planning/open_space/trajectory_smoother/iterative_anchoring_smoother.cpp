...
IterativeAnchoringSmoother::IterativeAnchoringSmoother(
  ...
  center_shift_distance_ =
      ego_length_ / 2.0 - vehicle_param.backedgetocenter;
...
bool IterativeAnchoringSmoother::Smooth(
  ...
  for (size_t i = 0; i < xWS_size; ++i) {
    ...
    path_point.set__x(xWS(0, i));
    path_point.set__y(xWS(1, i));
    path_point.set__theta(xWS(2, i));
    path_point.set__s(accumulated_s);
    warm_start_path.push_back(std::move(path_point));
    last_path_point = cur_path_point;
  }

  const double interpolated_delta_s =
      planner_open_space_config_.iterativeanchoringsmootherconfig
          .interpolateddeltas;
    ...
    for (size_t i = 0; i < reanchoring_trails_num; ++i) {
      ...
      if (is_colliding) {
        ...
        if (index == 1) {
          ...
          path_points->at(index).set__x(path_points->at(index - 1).x +
                                       adjusted_delta_s *
                                           std::cos(adjust_theta));
          path_points->at(index).set__y(path_points->at(index - 1).y +
                                       adjusted_delta_s *
          ...
          double rand_dev_y =
              common::math::Clamp(pos_dis(gen), 2.0 * reanchoring_pos_stddev,
                                  -2.0 * reanchoring_pos_stddev);
          path_points->at(index).set__x(path_points->at(index).x + rand_dev_x);
          path_points->at(index).set__y(path_points->at(index).y + rand_dev_y);
        }
        ...
        for (size_t i = index - 1; i < index + 2; ++i) {
          path_points->at(i).set__theta(CalcHeadings(*path_points, i));
        }
      } else {
...
bool IterativeAnchoringSmoother::SetPathProfile(
  ...
  for (size_t i = 0; i < points_size; ++i) {
    PathPoint path_point;
    path_point.set__x(point2d[i].first);
    path_point.set__y(point2d[i].second);
    path_point.set__theta(headings[i]);
    path_point.set__s(accumulated_s[i]);
    path_point.set__kappa(kappas[i]);
    path_point.set__dkappa(dkappas[i]);
...
bool IterativeAnchoringSmoother::SmoothSpeed(const double init_a,
  ...
  piecewise_jerk_problem.set_x_ref(s_curve_config.refsweight,
                                   std::move(x_ref));
  piecewise_jerk_problem.set_weight_ddx(s_curve_config.accweight);
  piecewise_jerk_problem.set_weight_dddx(s_curve_config.jerkweight);
...
bool IterativeAnchoringSmoother::CombinePathAndSpeed(
  ...
  for (double cur_rel_time = 0.0; cur_rel_time < time_horizon;
    ...
    trajectory_point.set__relativetime(speed_point.t);
...
void IterativeAnchoringSmoother::AdjustPathAndSpeedByGear(
  ...
  std::for_each(
      ...
      [](TrajectoryPoint& trajectory_point) {
        trajectory_point.pathpoint.set__theta(
            NormalizeAngle(trajectory_point.pathpoint.theta + M_PI));
        trajectory_point.pathpoint.set__s(
            -1.0 * trajectory_point.pathpoint.s);
        trajectory_point.pathpoint.set__kappa(
            -1.0 * trajectory_point.pathpoint.kappa);
        // dkappa stays the same as direction of both kappa and s are reversed
        trajectory_point.set__v(-1.0 * trajectory_point.v);
        trajectory_point.set__a(-1.0 * trajectory_point.a);
      });
}
