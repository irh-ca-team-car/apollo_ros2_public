...
class OpenSpaceROITest {
 public:
  bool OpenSpaceROI() {
    ...
    double start_s =
        center_line_s -
        planner_open_space_config_.roiconfig.roilongitudinalrangestart;
    double end_s =
        center_line_s +
        planner_open_space_config_.roiconfig.roilongitudinalrangeend;
    ...
    if (parking_spot_heading_ > kMathEpsilon) {
      if (planner_open_space_config_.roiconfig.parkinginwards) {
  ...
  bool LoadMap(const std::string& lane_id, const std::string& parking_id) {
    ...
    nearby_lane_id.set__id(lane_id);
    hdmap::Id target_lane_id;
    target_lane_id.set__id(parking_id);
