...
bool DistanceSmoothing(
  ...
  float ts_ = planner_open_space_config.deltat;
  ...
  if (horizon_ <= 10 && horizon_ > 2 &&
      planner_open_space_config.enablelinearinterpolation) {
...
bool DistancePlan(HybridAStar* hybridA_ptr, ObstacleContainer* obstacles_ptr,
  ...
  //std::string flag_file_path = "<root>/modules/planning/conf/planning.json";
  ...
  if (FLAGS_enable_parallel_trajectory_smoothing) {
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      if (planner_open_space_config_.enablecheckparalleltrajectory) {
