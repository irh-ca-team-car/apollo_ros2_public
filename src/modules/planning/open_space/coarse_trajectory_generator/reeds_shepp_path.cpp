...
ReedShepp::ReedShepp(const common::VehicleParam& vehicle_param,
  ...
  max_kappa_ = std::tan(vehicle_param_.maxsteerangle /
                        vehicle_param_.steerratio) /
               vehicle_param_.wheelbase;
  if(FLAGS_enable_parallel_hybrid_a) AINFO << "parallel REEDShepp";
}
...
bool ReedShepp::GenerateLocalConfigurations(
    ...
    const std::shared_ptr<Node3d> end_node, ReedSheppPath* shortest_path) {
      UNUSED(end_node);
  double step_scaled =
      planner_open_space_config_.warmstartconfig.stepsize * max_kappa_;
