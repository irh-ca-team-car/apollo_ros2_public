...
HybridAStar::HybridAStar(const PlannerOpenSpaceConfig& open_space_conf) {
  ...
  next_node_num_ =
      planner_open_space_config_.warmstartconfig.nextnodenum;
  max_steer_angle_ =
      vehicle_param_.maxsteerangle / vehicle_param_.steerratio;
  step_size_ = planner_open_space_config_.warmstartconfig.stepsize;
  xy_grid_resolution_ =
      planner_open_space_config_.warmstartconfig.xygridresolution;
  delta_t_ = planner_open_space_config_.deltat;
  traj_forward_penalty_ =
      planner_open_space_config_.warmstartconfig.trajforwardpenalty;
  traj_back_penalty_ =
      planner_open_space_config_.warmstartconfig.trajbackpenalty;
  traj_gear_switch_penalty_ =
      planner_open_space_config_.warmstartconfig.trajgearswitchpenalty;
  traj_steer_penalty_ =
      planner_open_space_config_.warmstartconfig.trajsteerpenalty;
  traj_steer_change_penalty_ = planner_open_space_config_.warmstartconfig
                                   .trajsteerchangepenalty;
}
...
std::shared_ptr<Node3d> HybridAStar::Next_node_generator(
  ...
  for (size_t i = 0; i < arc / step_size_; ++i) {
    ...
    const double next_phi = common::math::NormalizeAngle(
        last_phi +
        traveled_distance / vehicle_param_.wheelbase * std::tan(steering));
...
bool HybridAStar::GenerateSpeedAcceleration(HybridAStartResult* result) {
  ...
  for (size_t i = 0; i + 1 < x_size; ++i) {
    double discrete_steer = (result->phi[i + 1] - result->phi[i]) *
                            vehicle_param_.wheelbase / step_size_;
    if (result->v[i] > 0.0) {
...
bool HybridAStar::GenerateSCurveSpeedAcceleration(HybridAStartResult* result) {
  ...
  for (size_t i = 0; i < path_points_size; ++i) {
    ...
    path_point.set__x(result->x[i]);
    path_point.set__y(result->y[i]);
    path_point.set__theta(result->phi[i]);
    path_point.set__s(result->accumulated_s[i]);
