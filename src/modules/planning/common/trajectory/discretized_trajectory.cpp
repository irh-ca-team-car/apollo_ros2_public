...
DiscretizedTrajectory::DiscretizedTrajectory(const apollo_msgs::msg::ApolloplanningADCTrajectory& trajectory) {
  assign(trajectory.trajectorypoint.begin(),
         trajectory.trajectorypoint.end());
}

TrajectoryPoint DiscretizedTrajectory::Evaluate(
    const double relative_time) const {
  auto comp = [](const TrajectoryPoint& p, const double relative_time) {
    return p.relativetime < relative_time;
  };
...
size_t DiscretizedTrajectory::QueryLowerBoundPoint(const double relative_time,
  ...
  if (relative_time >= back().relativetime) {
    return size() - 1;
  }
  auto func = [&epsilon](const TrajectoryPoint& tp,
                         const double relative_time) {
    return tp.relativetime + epsilon < relative_time;
  };
...
size_t DiscretizedTrajectory::QueryNearestPoint(
  ...
  for (size_t i = 0; i < size(); ++i) {
    const common::math::Vec2d curr_point(data()[i].pathpoint.x,
                                         data()[i].pathpoint.y);
...
void DiscretizedTrajectory::AppendTrajectoryPoint(
  ...
  if (!empty()) {
    CHECK_GT(trajectory_point.relativetime, back().relativetime);
  }
...
double DiscretizedTrajectory::GetTemporalLength() const {
  ...
  return back().relativetime - front().relativetime;
}
...
double DiscretizedTrajectory::GetSpatialLength() const {
  ...
  return back().pathpoint.s - front().pathpoint.s;
}
