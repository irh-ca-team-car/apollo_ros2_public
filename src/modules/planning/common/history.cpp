...
void HistoryObjectDecision::Init(const ObjectDecision& object_decisions) {
  ...
  for (size_t i = 0; i < object_decisions.objectdecision.size(); i++) {
    object_decision_.push_back(object_decisions.objectdecision[i]);
  }
}

void HistoryObjectDecision::Init(
    const std::string& id,
    const std::vector<apollo_msgs::msg::ApolloplanningObjectDecisionType>& object_decision) {
  id_ = id;
  object_decision_.clear();
...
std::vector<const apollo_msgs::msg::ApolloplanningObjectDecisionType*>
HistoryObjectDecision::GetObjectDecision() const {
  std::vector<const apollo_msgs::msg::ApolloplanningObjectDecisionType*> result;
...
void HistoryFrame::Init(const apollo_msgs::msg::ApolloplanningADCTrajectory& adc_trajactory) {
  adc_trajactory_=adc_trajactory;
  const auto& object_decisions = adc_trajactory.decision.objectdecision;
  for (size_t i = 0; i < object_decisions.decision.size(); i++)
  {
...
std::vector<const HistoryObjectDecision*> HistoryFrame::GetStopObjectDecisions()
  ...
  for (size_t i = 0; i < object_decisions_.size(); i++) {
    ...
    for (const apollo_msgs::msg::ApolloplanningObjectDecisionType* decision_type : obj_decision) {
      if (decision_type->objecttag.stop.reasoncode!=0 ) {

        std::vector<apollo_msgs::msg::ApolloplanningObjectDecisionType> object_decision;
...
int History::Add(const apollo_msgs::msg::ApolloplanningADCTrajectory& adc_trajectory_pb) {
