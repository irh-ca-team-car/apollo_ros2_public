...
bool IsVehicleStateValid(const VehicleState& vehicle_state) {
  if (std::isnan(vehicle_state.x) || std::isnan(vehicle_state.y) ||
      ...
      std::isnan(vehicle_state.linearvelocity) ||
      std::isnan(vehicle_state.linearacceleration)) {
    return false;
  }
  return true;
}
...
bool IsDifferentRouting(const RoutingResponse& first,
  ...
  if (first.apolloheader != apollo::common::Header() && second.apolloheader != apollo::common::Header())
  {
    return first.apolloheader.sequencenum != second.apolloheader.sequencenum;
  }
  return true;
}
