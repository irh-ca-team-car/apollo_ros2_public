 ...
 */
                                  ...
                                  ReferenceLineInfo *const reference_line_info)
            {
                ...
                ApolloplanningObjectDecisionType stop;
                auto *stop_decision = &stop.objecttag.stop;
                stop_decision->set__reasoncode(stop_reason_code);
                stop_decision->set__distances(-stop_distance);
                stop_decision->set__stopheading(stop_heading);
                stop_decision->stoppoint.set__x(stop_point.x());
                stop_decision->stoppoint.set__y(stop_point.y());
                stop_decision->stoppoint.set__z(0.0);

                for (size_t i = 0; i < wait_for_obstacles.size(); ++i)
                {
                    stop_decision->waitforobstacle.push_back(wait_for_obstacles[i]);
                }
                                  ...
                                  ReferenceLineInfo *const reference_line_info)
            {
                ...
                auto stop_point = reference_line.GetReferencePoint(
                    stop_wall->PerceptionSLBoundary().starts - stop_distance);

                ApolloplanningObjectDecisionType stop;
                auto *stop_decision = &stop.objecttag.stop;
                stop_decision->set__reasoncode(stop_reason_code);
                stop_decision->set__distances(-stop_distance);
                stop_decision->set__stopheading(stop_point.heading());
                stop_decision->stoppoint.set__x(stop_point.x());
                stop_decision->stoppoint.set__y(stop_point.y());
                stop_decision->stoppoint.set__z(0.0);
