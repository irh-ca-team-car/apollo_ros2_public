...
apollo_msgs::msg::ApolloplanningInternalSTGraphDebug* StGraphData::mutable_st_graph_debug() {
...
bool StGraphData::SetSTDrivableBoundary(
  ...
  for (size_t i = 0; i < s_boundary.size(); ++i) {
    auto st_bound_instance = &st_drivable_boundary_.stboundary.emplace_back();
    st_bound_instance->set__t(std::get<0>(s_boundary[i]));
    st_bound_instance->set__slower(std::get<1>(s_boundary[i]));
    st_bound_instance->set__supper(std::get<2>(s_boundary[i]));
    if (std::get<1>(v_obs_info[i]) > -kObsSpeedIgnoreThreshold) {
      st_bound_instance->set__vobslower(std::get<1>(v_obs_info[i]));
    }
    if (std::get<2>(v_obs_info[i]) < kObsSpeedIgnoreThreshold) {
      st_bound_instance->set__vobsupper(std::get<2>(v_obs_info[i]));
    }
  }
  return true;
}

const apollo_msgs::msg::ApolloplanningSTDrivableBoundary& StGraphData::st_drivable_boundary() const {
