...
const apollo_msgs::msg::ApolloperceptionPerceptionObstacle *PathDecision::FindPerceptionObstacle(
...
bool PathDecision::AddLateralDecision(const std::string &tag,
                                      ...
                                      const ApolloplanningObjectDecisionType &decision) {
...
bool PathDecision::MergeWithMainStop(const ObjectStop &obj_stop,
  ...
  common::PointENU stop_point = obj_stop.stoppoint;
  common::SLPoint stop_line_sl;
  reference_line.XYToSLPB(stop_point, &stop_line_sl);
  ...
  stop_line_s = std::fmax(
      stop_line_s, adc_sl_boundary.ends -
  ...
  main_stop_ = apollo::planning::MainStop();
  main_stop_.set__reasoncode(obj_stop.reasoncode);
  main_stop_.set__reason("stop by " + obj_id);
  main_stop_.stoppoint.set__x(obj_stop.stoppoint.x);
  main_stop_.stoppoint.set__y(obj_stop.stoppoint.y);
  main_stop_.set__stopheading(obj_stop.stopheading);
  stop_reference_line_s_ = stop_line_s;

  ADEBUG << " main stop obstacle id:" << obj_id
         << " stop_line_s:" << stop_line_s << " stop_point: ("
         << obj_stop.stoppoint.x << obj_stop.stoppoint.y
         << " ) stop_heading: " << obj_stop.stopheading;
  return true;
}
