...
bool IsNonmovableObstacle(const ReferenceLineInfo& reference_line_info,
  ...
  if (obstacle.PerceptionSLBoundary().starts >
      adc_sl_boundary.ends + kAdcDistanceThreshold) {
  ...
  for (const auto* other_obstacle :
    ...
    if (other_obstacle->PerceptionSLBoundary().startl >
            obstacle.PerceptionSLBoundary().endl ||
        other_obstacle->PerceptionSLBoundary().endl <
            obstacle.PerceptionSLBoundary().startl) {
      // not blocking the backside vehicle
      continue;
    }
    double delta_s = other_obstacle->PerceptionSLBoundary().starts -
                     obstacle.PerceptionSLBoundary().ends;
...
bool IsBlockingObstacleToSidePass(const Frame& frame, const Obstacle* obstacle,
  ...
  if (obstacle->PerceptionSLBoundary().starts >
      adc_sl_boundary.ends + kAdcDistanceSidePassThreshold) {
    ADEBUG << " - It is too far ahead.";
    return false;
  }

  // Obstacle is too close.
  if (adc_sl_boundary.ends + min_front_sidepass_distance >
      obstacle->PerceptionSLBoundary().starts) {
...
double GetDistanceBetweenADCAndObstacle(const Frame& frame,
  ...
  double distance_between_adc_and_obstacle =
      obstacle->PerceptionSLBoundary().starts - adc_sl_boundary.ends;
...
bool IsParkedVehicle(const ReferenceLine& reference_line,
  ...
  reference_line.GetRoadWidth(obstacle->PerceptionSLBoundary().starts,
                              &road_left_width, &road_right_width);
  max_road_right_width = road_right_width;
  reference_line.GetRoadWidth(obstacle->PerceptionSLBoundary().ends,
                              &road_left_width, &road_right_width);
  max_road_right_width = std::max(max_road_right_width, road_right_width);
  bool is_at_road_edge = std::abs(obstacle->PerceptionSLBoundary().startl) >
