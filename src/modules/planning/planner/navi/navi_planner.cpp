...
Status NaviPlanner::Init(const apollo_msgs::msg::ApolloplanningPlanningConfig& config) {
...
Status NaviPlanner::Plan(const TrajectoryPoint& planning_init_point,
                         Frame* frame, apollo_msgs::msg::ApolloplanningADCTrajectory* ptr_computed_trajectory) {
  AINFO_EVERY(60000) << "NaviPlanner Plan start ...";
...
Status NaviPlanner::PlanOnReferenceLine(
  ...
  ADEBUG << "planning start point:" << planning_init_point;
  ...
  *heuristic_speed_data = SpeedData(speed_profile);
  ...
  if (!reference_line_info->CombinePathAndSpeedProfile(
          planning_init_point.relativetime,
          planning_init_point.pathpoint.s, &trajectory)) {
  ...
  for (const auto* obstacle :
    ...
    if (obstacle->LongitudinalDecision().objecttag.stop != apollo_msgs::msg::ApolloplanningObjectStop())
    {
...
void NaviPlanner::RecordObstacleDebugInfo(
  ...
  for (const auto obstacle : path_decision->obstacles().Items()) {
    auto obstacle_debug = &ptr_debug->planningdata.obstacle.emplace_back();
    obstacle_debug->set__id(obstacle->Id());
    obstacle_debug->slboundary=(
    ...
    for (size_t i = 0; i < decider_tags.size(); ++i) {
      auto decision_tag = &obstacle_debug->decisiontag.emplace_back();
      decision_tag->set__decidertag(decider_tags[i]);
      decision_tag->decision=(decisions[i]);
    }
  }
}
...
void NaviPlanner::RecordDebugInfo(ReferenceLineInfo* reference_line_info,
  ...
  auto ptr_stats = &ptr_latency_stats->taskstats.emplace_back();
  ptr_stats->set__name(name);
  ptr_stats->set__timems(time_diff_ms);
}
...
std::vector<SpeedPoint> NaviPlanner::GenerateInitSpeedProfile(
  ...
  if (!last_speed_data.empty()) {
    const auto& last_init_point = last_frame->PlanningStartPoint().pathpoint;
    ...
    Vec2d xy_point(planning_init_point.pathpoint.x,
                   planning_init_point.pathpoint.y);
    SLPoint sl_point;
...
void NaviPlanner::GenerateFallbackPathProfile(
  ...
  double adc_s = reference_line_info->AdcSlBoundary().ends;
  ...
  //ACHECK(adc_point.has_path_point());
  const double dx = adc_point.pathpoint.x - adc_ref_point.x();
  const double dy = adc_point.pathpoint.y - adc_ref_point.y();
