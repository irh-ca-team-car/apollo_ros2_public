...
std::vector<PathPoint> ToDiscretizedReferenceLine(
  ...
  for (const auto& ref_point : ref_points) {
    ...
    path_point.set__x(ref_point.x());
    path_point.set__y(ref_point.y());
    path_point.set__theta(ref_point.heading());
    path_point.set__kappa(ref_point.kappa());
    path_point.set__dkappa(ref_point.dkappa());
...
void ComputeInitFrenetState(const PathPoint& matched_point,
  ...
  CartesianFrenetConverter::cartesian_to_frenet(
      ...
      cartesian_state.pathpoint.x, cartesian_state.pathpoint.y,
      cartesian_state.v, cartesian_state.a,
      cartesian_state.pathpoint.theta,
      cartesian_state.pathpoint.kappa, ptr_s, ptr_d);
}
...
Status LatticePlanner::PlanOnReferenceLine(
  ...
  PathPoint matched_point = PathMatcher::MatchToPath(
      *ptr_reference_line, planning_init_point.pathpoint.x,
  ...
  if (planning_target.stoppoint != apollo_msgs::msg::ApolloplanningStopPoint())
  {
    ADEBUG << "Planning target stop s: " << planning_target.stoppoint.s
  ...
  while (trajectory_evaluator.has_more_trajectory_pairs()) {
    ...
    for (uint i = 0; i < 10; ++i) {
      ADEBUG << combined_trajectory_points[i];
    }
  ...
  if (num_lattice_traj > 0) {
    ...
    if (FLAGS_enable_backup_trajectory) {
      ...
      BackupTrajectoryGenerator backup_trajectory_generator(
          init_s, init_d, planning_init_point.relativetime,
