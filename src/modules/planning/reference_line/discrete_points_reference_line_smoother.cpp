...
namespace planning {
      ...
      ReferenceLine *const smoothed_reference_line)
  {
    ...
    const auto &smoothing_method = config_.smootherconfig.discretepoints.smoothingmethod;
...
bool DiscretePointsReferenceLineSmoother::CosThetaSmooth(
  ...
  const auto &cos_theta_config =
      config_.smootherconfig.discretepoints.smootherconfig.costhetasmoothing;
...
bool DiscretePointsReferenceLineSmoother::FemPosSmooth(
  ...
  UNUSED(raw_point2d);
  UNUSED(bounds);
  UNUSED(ptr_smoothed_point2d);
#if true
  const auto& fem_pos_config =
      config_.smootherconfig.discretepoints.smootherconfig.femposdeviationsmoothing;
