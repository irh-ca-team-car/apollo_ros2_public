...
QpSplineReferenceLineSmoother::QpSplineReferenceLineSmoother(
  ...
  spline_solver_.reset(
      new OsqpSpline2dSolver(t_knots_, config.smootherconfig.qpspline.splineorder));
}
...
bool QpSplineReferenceLineSmoother::Smooth(
  ...
  spline_solver_->Reset(t_knots_, config_.smootherconfig.qpspline.splineorder);
  ...
  const double resolution =
      (end_t - start_t) / (config_.numoftotalpoints - 1);
  double t = start_t;
  std::vector<ReferencePoint> ref_points;
  const auto& spline = spline_solver_->spline();
  for (std::uint32_t i = 0; i < config_.numoftotalpoints && t < end_t;
       ++i, t += resolution) {
    ...
    ref_sl_point.set__s(std::max(ref_sl_point.s, 0.0));
...
bool QpSplineReferenceLineSmoother::Sampling() {
  ...
  uint32_t num_spline =
      std::max(1u, static_cast<uint32_t>(
                       length / config_.smootherconfig.qpspline.maxsplinelength + 0.5));
...
bool QpSplineReferenceLineSmoother::AddKernel() {
  ...
  // add spline kernel
  if (config_.smootherconfig.qpspline.secondderivativeweight > 0.0) {
    kernel->AddSecondOrderDerivativeMatrix(
        config_.smootherconfig.qpspline.secondderivativeweight);
  }
  if (config_.smootherconfig.qpspline.thirdderivativeweight > 0.0)
  {
    kernel->AddThirdOrderDerivativeMatrix(
        config_.smootherconfig.qpspline.thirdderivativeweight);
  }

  kernel->AddRegularization(config_.smootherconfig.qpspline.regularizationweight);
  return true;
}
