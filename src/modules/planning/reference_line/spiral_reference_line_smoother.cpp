...
SpiralReferenceLineSmoother::SpiralReferenceLineSmoother(
    const apollo_msgs::msg::ApolloplanningReferenceLineSmootherConfig& config)
...
bool SpiralReferenceLineSmoother::Smooth(
  ...
  if (anchor_points_.empty()) {
    const double piecewise_length = config_.smootherconfig.spiral.piecewiselength;
  ...
  for (const auto& p : smoothed_point2d) {
    ...
    if (!raw_reference_line.XYToSLPB(p, &ref_sl_point)) {
      return false;
    }
    if (ref_sl_point.s < 0 ||
...
int SpiralReferenceLineSmoother::SmoothStandAlone(
  ...
  ptop->set_default_max_point_deviation(config_.smootherconfig.spiral.maxdeviation);
  ptop->set_element_weight_curve_length(config_.smootherconfig.spiral.weightcurvelength);
  ptop->set_element_weight_kappa(config_.smootherconfig.spiral.weightkappa);
  ptop->set_element_weight_dkappa(config_.smootherconfig.spiral.weightdkappa);
  ...
  app->Options()->SetIntegerValue("max_iter", config_.smootherconfig.spiral.maxiteration);
  app->Options()->SetNumericValue("tol", config_.smootherconfig.spiral.opttol);
  app->Options()->SetNumericValue("acceptable_tol",
                                  config_.smootherconfig.spiral.optacceptabletol);
...
bool SpiralReferenceLineSmoother::Smooth(std::vector<Eigen::Vector2d> point2d,
  ...
  app->Options()->SetIntegerValue("acceptable_iter",
                                  config_.smootherconfig.spiral.optacceptableiteration);
  app->Options()->SetNumericValue("tol", config_.smootherconfig.spiral.opttol);
  app->Options()->SetNumericValue("acceptable_tol",
                                  config_.smootherconfig.spiral.optacceptabletol);
...
common::PathPoint SpiralReferenceLineSmoother::to_path_point(
  ...
  point.set__y(y);
  point.set__s(s);
  point.set__theta(theta);
  point.set__kappa(kappa);
  point.set__dkappa(dkappa);
  return point;
}
...
void SpiralReferenceLineSmoother::SetAnchorPoints(
  ...
  std::for_each(anchor_points_.begin(), anchor_points_.end(),
                [this](AnchorPoint& p) {
                  ...
                  p.path_point.set__x(curr_x - zero_x_);
                  p.path_point.set__y(curr_y - zero_y_);
                });
}
