...
ReferenceLineProvider::ReferenceLineProvider(
    ...
    const std::shared_ptr< apollo_msgs::msg::ApollorelativeMapMapMsg> &relative_map)
...
//   if (smoother_config_.smootherconfig.qpspline != apollo_msgs::msg::ApolloplanningQpSplineSmootherConfig())
//   {
//     smoother_.reset(new QpSplineReferenceLineSmoother(smoother_config_));
//   }
//   #if true
//   else if (smoother_config_.smootherconfig.spiral != apollo_msgs::msg::ApolloplanningSpiralSmootherConfig())
//   {
//     smoother_.reset(new SpiralReferenceLineSmoother(smoother_config_));
//   }
// #endif
//   else if (smoother_config_.smootherconfig.discretepoints != apollo_msgs::msg::ApolloplanningDiscretePointsSmootherConfig())
  {
...
bool ReferenceLineProvider::Start() {
  ...
  if (FLAGS_enable_reference_line_provider_thread) {
    task_future_ = std::async(&ReferenceLineProvider::GenerateThread, this);
  }
  return true;
}
...
bool ReferenceLineProvider::GetReferenceLinesFromRelativeMap(
  ...
  if (relative_map_->navigationpath.empty()) {
  ...
  for (const auto &path_pair : relative_map_->navigationpath) {
    const auto lane_id = path_pair.key;
  ...
  auto *adc_navigation_path = apollo::common::util::FindOrNull(
      relative_map_->navigationpath, adc_lane_id);
  if (adc_navigation_path == nullptr) {
    AERROR << "adc lane cannot be found in relative_map_->navigation_path";
    return false;
  }
  const uint32_t adc_lane_priority = adc_navigation_path->data.pathpriority;
  ...
  while (left_lane_ptr != nullptr &&
         left_lane_ptr->lane().leftneighborforwardlaneid.size() > 0) {
    auto neighbor_lane_id =
        left_lane_ptr->lane().leftneighborforwardlaneid.at(0);
  ...
  while (right_lane_ptr != nullptr &&
         right_lane_ptr->lane().rightneighborforwardlaneid.size() > 0) {
    auto neighbor_lane_id =
        right_lane_ptr->lane().rightneighborforwardlaneid.at(0);
  ...
  ADEBUG << "relative_map_->navigation_path_size = "
         << relative_map_->navigationpath.size();
  for (const auto &path_pair : relative_map_->navigationpath) {
    const auto lane_id = path_pair.key;
    const uint32_t priority = path_pair.data.pathpriority;
  ...
  if (is_lane_change_needed) {
    ...
    if (left_neighbor_lane_ids.end() !=
      ...
      nearest_neighbor_lane_id =
          adc_lane_way_point.lane->lane().leftneighborforwardlaneid.at(0).id;
      ...
      nearest_neighbor_lane_id = adc_lane_way_point.lane->lane()
                                     .rightneighborforwardlaneid.at(0)
                                     .id;
    }
  }
  ...
  for (const auto &path_pair : relative_map_->navigationpath) {
    const auto &lane_id = path_pair.key;
    const auto &path_points = path_pair.data.path.pathpoint;
    ...
    reference_lines->back().SetPriority(path_pair.data.pathpriority);
  }
...
bool ReferenceLineProvider::GetNearestWayPointFromNavigationPath(
    ...
    const std::unordered_set<std::string> &navigation_lane_ids,// 0_Path from navigation line index 0
    hdmap::LaneWaypoint *waypoint) {
  const double kMaxDistance = 20.0;
  ...
  if (status < 0) {
    AERROR << "failed to get lane from point " << point;
    return false;
  }
  ...
  if (waypoint->lane == nullptr) {
    AERROR << "failed to find nearest point " << point;
  }
  ...
  if (is_new_routing || !FLAGS_enable_reference_line_stitching) {
    for (auto iter = segments->begin(); iter != segments->end();) {
      ...
      if (!SmoothRouteSegment(*iter, &reference_lines->back())) {
        ...
        common::SLPoint sl;
        if (!reference_lines->back().XYToSLPB(vehicle_state, &sl)) {
...
bool ReferenceLineProvider::ExtendReferenceLine(const VehicleState &state,
  ...
  const double look_forward_required_distance =
      PncMap::LookForwardDistance(state.linearvelocity);
...
AnchorPoint ReferenceLineProvider::GetAnchorPoint(
  ...
  AnchorPoint anchor;
  anchor.longitudinal_bound = smoother_config_.longitudinalboundarybound;
  auto ref_point = reference_line.GetReferencePoint(s);
  if (ref_point.lane_waypoints().empty()) {
    anchor.path_point = ref_point.ToPathPoint(s);
    anchor.lateral_bound = smoother_config_.maxlateralboundarybound;
    return anchor;
  }
  ...
  if (hdmap::RightBoundaryType(waypoint) == hdmap::LaneBoundaryType::CURB) {
    safe_lane_width -= smoother_config_.curbshift;
    if (safe_lane_width < kEpislon) {
      ADEBUG << "lane width smaller than adc width and right curb shift";
      effective_width = kEpislon;
      is_lane_width_safe = false;
    } else {
      center_shift += 0.5 * smoother_config_.curbshift;
    }
  }
  ...
  const double buffered_width =
      safe_lane_width - 2.0 * smoother_config_.lateralbuffer;
  safe_lane_width =
  ...
  anchor.lateral_bound = common::math::Clamp(
      effective_width, smoother_config_.minlateralboundarybound,
      smoother_config_.maxlateralboundarybound);
  return anchor;
}
...
void ReferenceLineProvider::GetAnchorPoints(
  ...
  const double interval = smoother_config_.maxconstraintinterval;
  int num_of_anchors =
...
bool ReferenceLineProvider::SmoothPrefixedReferenceLine(
  ...
  for (auto &point : anchor_points) {
    common::SLPoint sl_point;
    if (!prefix_ref.XYToSLPB(point.path_point, &sl_point))
    {
      continue;
    }
    ...
    point.path_point.set__x(prefix_ref_point.x());
    point.path_point.set__y(prefix_ref_point.y());
    point.path_point.set__z(0.0);
    point.path_point.set__theta(prefix_ref_point.heading());
