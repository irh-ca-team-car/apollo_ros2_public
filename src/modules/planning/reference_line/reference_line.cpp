...
using apollo::common::util::DistanceXYPB;
...
ReferencePoint ReferenceLine::GetNearestReferencePoint(
  ...
  for (size_t i = 0; i < reference_points_.size(); ++i) {
    const double distance = DistanceXYPB(xy, reference_points_[i]);
...
common::FrenetFramePoint ReferenceLine::GetFrenetPoint(
  ...
  XYToSLPB(path_point, &sl_point);
  common::FrenetFramePoint frenet_frame_point;
  frenet_frame_point.set__s(sl_point.s);
  frenet_frame_point.set__l(sl_point.l);
  ...
  frenet_frame_point.set__dl(dl);
  frenet_frame_point.set__ddl(ddl);
...
ReferenceLine::ToFrenetFrame(const common::TrajectoryPoint& traj_point) const {
  ...
  common::SLPoint sl_point;
  XYToSLPB(traj_point.pathpoint, &sl_point);
  ...
  CartesianFrenetConverter::cartesian_to_frenet(
      ...
      ref_point.kappa(), ref_point.dkappa(), traj_point.pathpoint.x,
      traj_point.pathpoint.y, traj_point.v, traj_point.a,
      traj_point.pathpoint.theta, traj_point.pathpoint.kappa,
...
ReferencePoint ReferenceLine::GetReferencePoint(const double x,
  ...
  CHECK_GE((size_t)reference_points_.size(), (size_t)1);
...
ReferencePoint ReferenceLine::InterpolateWithMatchedIndex(
  ...
  CHECK_LE(s0 - 1.0e-6, s) << "s: " << s << " is less than s0 : " << s0;
  CHECK_LE(s, s1 + 1.0e-6) << "s: " << s << " is larger than s1: " << s1;
...
double ReferenceLine::GetDrivingWidth(const SLBoundary& sl_boundary) const {
  ...
  GetLaneWidth(sl_boundary.starts, &lane_left_width, &lane_right_width);

  double driving_width = std::max(lane_left_width - sl_boundary.endl,
                                  lane_right_width + sl_boundary.startl);
...
bool ReferenceLine::IsOnLane(const SLBoundary& sl_boundary) const {
  ...
  return sl_boundary.startl <= lane_left_width &&
         sl_boundary.endl >= -lane_right_width;
}
...
bool ReferenceLine::IsOnRoad(const SLBoundary& sl_boundary) const {
  ...
  return sl_boundary.startl <= road_left_width &&
         sl_boundary.endl >= -road_right_width;
}
...
bool ReferenceLine::GetApproximateSLBoundary(
  ...
  sl_boundary->set__starts(min_s);
  sl_boundary->set__ends(max_s);
  sl_boundary->set__startl(min_l);
  sl_boundary->set__endl(max_l);
  return true;
}
...
bool ReferenceLine::GetSLBoundary(const common::math::Box2d& box,
  ...
  for (size_t i = 0; i < corners.size(); ++i) {
    ...
    sl_boundary->boundarypoint.push_back(sl_corners[index0]);

    // sl_point is outside of polygon; add to the vertex list
    if (v0.CrossProd(v1) < 0.0) {
      sl_boundary->boundarypoint.push_back(sl_point_mid);
    }
  }

  for (const auto& sl_point : sl_boundary->boundarypoint) {
  ...
  sl_boundary->set__starts(start_s);
  sl_boundary->set__ends(end_s);
  sl_boundary->set__startl(start_l);
  sl_boundary->set__endl(end_l);
  return true;
}
...
bool ReferenceLine::GetSLBoundary(const hdmap::Polygon& polygon,
  ...
  for (const auto& point : polygon.point) {
    SLPoint sl_point;
    if (!XYToSLPB(point, &sl_point))
    {
      AERROR << "Failed to get projection for point: " << point
             << " on reference line.";
      return false;
    }
    start_s = std::fmin(start_s, sl_point.s);
...
bool ReferenceLine::HasOverlap(const common::math::Box2d& box) const {
  ...
  if (sl_boundary.startl * sl_boundary.endl < 0) {
    return false;
  }

  double lane_left_width = 0.0;
  double lane_right_width = 0.0;
  const double mid_s = (sl_boundary.starts + sl_boundary.ends) / 2.0;
  ...
  if (sl_boundary.startl > 0) {
    return sl_boundary.startl < lane_left_width;
  } else {
    return sl_boundary.endl > -lane_right_width;
  }
}
...
std::string ReferenceLine::DebugString() const {
 ...
 /* return absl::StrCat(
  ...
  return absl::StrCat("point_num: ", reference_points_.size());
}
...
double ReferenceLine::GetSpeedLimitFromS(const double s) const {
  ...
  for (const auto& lane_waypoint : map_path_point.lane_waypoints()) {
    ...
    speed_limit =
        std::fmin(lane_waypoint.lane->lane().speedlimit, speed_limit);
  }
