...
class SmootherUtil {
  ...
  bool Smooth() {
    ...
    for (; i < raw_points_.size(); ++i) {
      ...
      while (j > 0 && s < FLAGS_smooth_length / 2.0) {
        s += apollo::common::util::DistanceXYPB(ref_points_[j - 1], ref_points_[j]);
        --j;
      }
  ...
  void Export(const std::string& filename) {
    ...
    for (size_t i = 1; i + 1 < ref_points_.size(); ++i) {
      ...
      ofs << std::fixed << "{\"kappa\": " << point.kappa() << ", \"s\": " << s
          ...
          << "}";
      s += apollo::common::util::DistanceXYPB(point, ref_points_[i + 1]);
    }
    ofs.close();
 ...
 private:
  std::vector<AnchorPoint> CreateAnchorPoints(const ReferencePoint& init_point,
    ...
    int num_of_anchors = std::max(
        2, static_cast<int>(
               ref_line.Length() / config_.maxconstraintinterval + 0.5));
    ...
    for (const double s : anchor_s) {
      if (s + config_.maxconstraintinterval / 2.0 < sl.s) {
        continue;
      }
      ...
      AnchorPoint anchor;
      anchor.path_point.x=(ref_point.x());
      anchor.path_point.y=(ref_point.y());
      anchor.path_point.z=(0.0);
      anchor.path_point.s = (s);
      anchor.path_point.theta=(ref_point.heading());
      anchor.path_point.kappa=(ref_point.kappa());
      anchor.lateral_bound = config_.maxlateralboundarybound;
      anchor.longitudinal_bound = config_.longitudinalboundarybound;
 ...
 private:
  ...
  apollo_msgs::msg::ApolloplanningReferenceLineSmootherConfig config_;
};
