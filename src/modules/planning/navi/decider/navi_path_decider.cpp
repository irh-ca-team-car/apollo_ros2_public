...
bool NaviPathDecider::Init(const apollo_msgs::msg::ApolloplanningPlanningConfig &config)
{
  move_dest_lane_config_talbe_.clear();
  max_speed_levels_.clear();
  PlannerNaviConfig planner_navi_conf =
      config.planningconfig.navigationplanningconfig.plannernaviconfig;
  config_ = planner_navi_conf.navipathdeciderconfig;
  auto move_dest_lane_config_talbe = config_.movedestlaneconfigtalbe;
  for (const auto& item : move_dest_lane_config_talbe.lateralshift) {
    double max_speed_level = item.maxspeed;
    double max_move_dest_lane_shift_y = item.maxmovedestlaneshifty;
  ...
  max_keep_lane_distance_ = config_.maxkeeplanedistance;
  max_keep_lane_shift_y_ = config_.maxkeeplaneshifty;
  min_keep_lane_offset_ = config_.minkeeplaneoffset;
  keep_lane_shift_compensation_ = config_.keeplaneshiftcompensation;
  start_plan_point_from_ = config_.startplanpointfrom;
  move_dest_lane_compensation_ = config_.movedestlanecompensation;
...
apollo::common::Status NaviPathDecider::Process(
  ...
  start_plan_point_.set__x(vehicle_state_.x);
  start_plan_point_.set__y(vehicle_state_.y);
  start_plan_point_.set__theta(vehicle_state_.heading);
  start_plan_v_ = vehicle_state_.linearvelocity;
  start_plan_a_ = vehicle_state_.linearacceleration;
  if (start_plan_point_from_ == 1) {
    // start plan point from planning schedule
    start_plan_point_.set__x(init_point.pathpoint.x);
    start_plan_point_.set__y(init_point.pathpoint.y);
    start_plan_point_.set__theta(init_point.pathpoint.theta);
...
void NaviPathDecider::MoveToDestLane(
  ...
  std::transform(path_points->begin(), path_points->end(), path_points->begin(),
                 [lateral_shift_value](common::PathPoint& old_path_point) {
                   ...
                   new_path_point.set__y(new_path_point_y);
...
void NaviPathDecider::RecordDebugInfo(const PathData& path_data) {
  ...
  auto* ptr_optimized_path = &reference_line_info_->mutable_debug()
                                 ->planningdata.path.emplace_back();
...
bool NaviPathDecider::GetBasicPathData(
  ...
  double min_path_len = config_.minpathlength;
  // get min path plan length s = v0 * t + 1 / 2.0 * a * t^2
  double path_len =
      start_plan_v_ * config_.minlookforwardtime +
      start_plan_a_ * pow(config_.minlookforwardtime, 2) / 2.0;
  ...
  if (!reference_line.XYToSLPB(start_plan_point_project.ToPathPoint(0.0),
                             &sl_point)) {
    AERROR << "Failed to get start plan point s from reference "
              "line.";
    return false;
  }
  auto start_plan_point_project_s = sl_point.s;
...
bool NaviPathDecider::IsSafeChangeLane(const ReferenceLine& reference_line,
  ...
  Vec2d vec_to_center(
      (adc_param.frontedgetocenter - adc_param.backedgetocenter) /
          2.0,
      (adc_param.leftedgetocenter - adc_param.rightedgetocenter) /
          2.0);
  ...
  for (const auto* obstacle : path_decision.obstacles().Items()) {
    ...
    if (sl_boundary.startl < -kLateralShift ||
        sl_boundary.endl > kLateralShift) {
      continue;
    }
    ...
    const double kForwardSafeDistance = std::max(
        ...
        ((vehicle_state_.linearvelocity - obstacle->speed()) * kSafeTime));
    const double kBackwardSafeDistance = std::max(
        kBackwardMinSafeDistance,
        ((obstacle->speed() - vehicle_state_.linearvelocity) * kSafeTime));
    if (sl_boundary.ends >
            adc_sl_boundary.starts - kBackwardSafeDistance &&
        sl_boundary.starts <
            adc_sl_boundary.ends + kForwardSafeDistance) {
      return false;
    }
  }
