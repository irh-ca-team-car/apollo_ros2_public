...
bool NaviSpeedDecider::Init(const apollo_msgs::msg::ApolloplanningPlanningConfig &planning_config)
{
  CHECK_GT(FLAGS_planning_upper_speed_limit, 0.0);
  NavigationPlanningConfig config =
      planning_config.planningconfig.navigationplanningconfig;
 
  max_speed_ = FLAGS_planning_upper_speed_limit;
  preferred_accel_ = std::abs(config.plannernaviconfig.navispeeddeciderconfig.preferredaccel);
  preferred_decel_ = std::abs(config.plannernaviconfig
                                  .navispeeddeciderconfig
                                  .preferreddecel);
  preferred_jerk_ = std::abs(config.plannernaviconfig
                                 .navispeeddeciderconfig
                                 .preferredjerk);
  max_accel_ = std::abs(
      config.plannernaviconfig.navispeeddeciderconfig.maxaccel);
  max_decel_ = std::abs(
      config.plannernaviconfig.navispeeddeciderconfig.maxdecel);
  preferred_accel_ = std::min(max_accel_, preferred_accel_);
  preferred_decel_ = std::min(max_decel_, preferred_accel_);

  obstacle_buffer_ = std::abs(config.plannernaviconfig
                                  .navispeeddeciderconfig
                                  .obstaclebuffer);
  safe_distance_base_ = std::abs(config.plannernaviconfig
                                     .navispeeddeciderconfig
                                     .safedistancebase);
  safe_distance_ratio_ = std::abs(config.plannernaviconfig
                                      .navispeeddeciderconfig
                                      .safedistanceratio);
  following_accel_ratio_ = std::abs(config.plannernaviconfig
                                        .navispeeddeciderconfig
                                        .followingaccelratio);
  soft_centric_accel_limit_ = std::abs(config.plannernaviconfig
                                           .navispeeddeciderconfig
                                           .softcentricaccellimit);
  hard_centric_accel_limit_ = std::abs(config.plannernaviconfig
                                           .navispeeddeciderconfig
                                           .hardcentricaccellimit);
  soft_centric_accel_limit_ =
      std::min(hard_centric_accel_limit_, soft_centric_accel_limit_);
  hard_speed_limit_ = std::abs(config.plannernaviconfig
                                   .navispeeddeciderconfig
                                   .hardspeedlimit);
  hard_accel_limit_ = std::abs(config.plannernaviconfig
                                   .navispeeddeciderconfig
                                   .hardaccellimit);
  enable_safe_path_ = config.plannernaviconfig
                          .navispeeddeciderconfig
                          .enablesafepath;
  enable_planning_start_point_ = config.plannernaviconfig
                                     .navispeeddeciderconfig
                                     .enableplanningstartpoint;
  enable_accel_auto_compensation_ = config.plannernaviconfig
                                        .navispeeddeciderconfig
                                        .enableaccelautocompensation;
  kappa_preview_ =
      config.plannernaviconfig.navispeeddeciderconfig.kappapreview;
  kappa_threshold_ = config.plannernaviconfig
                         .navispeeddeciderconfig
                         .kappathreshold;
...
Status NaviSpeedDecider::Execute(Frame* frame,
  ...
  preferred_speed_ = std::abs(planning_target.cruisespeed);
  ...
  auto current_v = vehicle_state.linearvelocity;
  auto current_a = vehicle_state.linearacceleration;
...
Status NaviSpeedDecider::MakeSpeedDecision(
  ...
  auto start_s = path_points.front().s;
  auto end_s = path_points.back().s != 0 ? path_points.back().s : start_s;
...
Status NaviSpeedDecider::AddCentricAccelerationConstraints(
  ...
  for (size_t i = 1; i < path_points.size(); i++) {
    ...
    auto start_s = prev.s - bs;
    start_s = std::max(0.0, start_s);
    auto end_s = cur.s - bs ;
    end_s = std::max(0.0, end_s);
    auto start_k =  prev.kappa ;
    auto end_k =  cur.kappa ;
...
void NaviSpeedDecider::RecordDebugInfo(const SpeedData& speed_data) {
  ...
  for (auto ptr = speed_data.begin(); ptr < speed_data.end(); ptr++)
  {
    ptr_speed_plan->speedpoint.push_back(*ptr);
  }
}
