...
bool NaviObstacleDecider::Init(const apollo_msgs::msg::ApolloplanningPlanningConfig& config) {
  PlannerNaviConfig planner_navi_conf =
      config.planningconfig.navigationplanningconfig.plannernaviconfig;
  config_ = planner_navi_conf.naviobstacledeciderconfig;
  return true;
}
...
bool NaviObstacleDecider::IsNeedFilterObstacle(
  ...
  *projection_point_ptr = PathMatcher::MatchToPath(
  ...
  if ((projection_point_ptr->s - vehicle_projection_point.s) >
      (config_.judgediscoeff * vehicle_state.linearvelocity +
       config_.basisdisvalue)) {
    return is_filter;
  }
...
void NaviObstacleDecider::ProcessObstacle(
  ...
  for (const auto& current_obstacle : obstacles) {
    ...
    if (dist <
        (config_.maxnudgedistance + current_obstacle->Perception().width +
...
double NaviObstacleDecider::GetObstacleActualOffsetDistance(
  ...
  if (last_nudge_dist_ > 0) {
    if (lat_dist < 0) {
      if (actual_dist < std::fabs(right_nudge_lane) + config_.safedistance) {
        *lane_obstacles_num = *lane_obstacles_num + 1;
      }
    }
  } else if (last_nudge_dist_ < 0) {
    if (lat_dist > 0) {
      if (actual_dist < std::fabs(left_nudge_lane) + config_.safedistance) {
        *lane_obstacles_num = *lane_obstacles_num + 1;
      }
    }
  }
  ...
  if (is_obstacle_stable_) {
    ++statist_count_;
    if (statist_count_ > config_.cyclesnumber) {
...
void NaviObstacleDecider::RecordLastNudgeDistance(const double nudge_dist) {
  double tolerance = config_.nudgeallowtolerance;
  ...
  if (no_nudge_num_ >= config_.cyclesnumber) {
...
void NaviObstacleDecider::SmoothNudgeDistance(
  ...
  if (vehicle_state.linearvelocity < config_.maxallownudgespeed) {
    ++limit_speed_num_;
  } else {
    limit_speed_num_ = 0;
  }
  if (limit_speed_num_ < config_.cyclesnumber) {
    *nudge_dist = 0;
  }
  if (std::fabs(*nudge_dist) > config_.nudgeallowtolerance) {
    ++eliminate_clutter_num_;
  } else {
    eliminate_clutter_num_ = 0;
  }
  if (eliminate_clutter_num_ < config_.cyclesnumber) {
    *nudge_dist = 0;
  }
...
double NaviObstacleDecider::GetNudgeDistance(
  ...
  for (auto iter = obstacle_lat_dist_.begin(); iter != obstacle_lat_dist_.end();
    ...
    auto lat_dist = iter->second;
    if (actual_dist > config_.minnudgedistance &&
        actual_dist < config_.maxnudgedistance) {
      auto need_nudge_dist = config_.maxnudgedistance - actual_dist;
      if (lat_dist >= 0.0) {
...
void NaviObstacleDecider::KeepNudgePosition(const double nudge_dist,
  ...
  if (std::fabs(nudge_dist) > config_.nudgeallowtolerance &&
      std::fabs(last_nudge_dist_) < config_.nudgeallowtolerance &&
      !keep_nudge_flag_) {
    cycles_count_ = 0;
  ...
  if (keep_nudge_flag_) {
    ++cycles_count_;
    if (cycles_count_ > config_.maxkeepnudgecycles) {
...
void NaviObstacleDecider::GetUnsafeObstaclesInfo(
  ...
  if (reference_line_y < 0.0) {
    unsafe_refline_pos_y = reference_line_y -
                           VehicleParam().rightedgetocenter -
                           config_.speeddeciderdetectrange;
    unsafe_car_pos_y = VehicleParam().rightedgetocenter +
                       config_.speeddeciderdetectrange;
    ...
    unsafe_refline_pos_y = reference_line_y +
                           VehicleParam().leftedgetocenter +
                           config_.speeddeciderdetectrange;
    unsafe_car_pos_y = -1.0 * (VehicleParam().leftedgetocenter +
                               config_.speeddeciderdetectrange);
