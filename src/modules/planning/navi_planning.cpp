...
NaviPlanning::~NaviPlanning()
{
  ...
  *injector_->planning_context()->mutable_planning_status() = PlanningStatus();
}

std::string NaviPlanning::Name() const { return "navi_planning"; }

Status NaviPlanning::Init(const apollo_msgs::msg::ApolloplanningPlanningConfig &config)
{
  /*config_ = config;
  if (!CheckPlanningConfig(config_))
  {
    AERROR << "Planning config error.";
...
void NaviPlanning::RunOnce(const LocalView &local_view,
                           apollo_msgs::msg::ApolloplanningADCTrajectory *const trajectory_pb)
{
  ...
  CHECK_GE(start_timestamp, vehicle_state.timestamp);
      ...
      FLAGS_message_latency_threshold)
  {
    ...
    vehicle_state.set__x(future_xy.x());
    vehicle_state.set__y(future_xy.y());
    vehicle_state.set__timestamp(start_timestamp);
  }

  auto *not_ready = 
  &trajectory_pb->decision.maindecision.task.notready;

  if (!status.ok() || !util::IsVehicleStateValid(vehicle_state))
  {
    const std::string msg = "Update VehicleStateProvider failed";
    AERROR << msg;
    not_ready->set__reason(msg);
    //status.Save(trajectory_pb->mutable_header()->mutable_status());
    // TODO(all): integrate reverse gear
    trajectory_pb->set__gear(canbus::Chassis::GEAR_DRIVE);
  ...
  trajectory_pb->latencystats.set__initframetimems(
  ...
  if (!status.ok())
  {
    ...
    {
      trajectory_pb->decision.maindecision.task.notready.set__reason(status.ToString());
      //status.Save(trajectory_pb->mutable_header()->mutable_status());
      // TODO(all): integrate reverse gear
      trajectory_pb->set__gear(canbus::Chassis::GEAR_DRIVE);
      FillPlanningPb(start_timestamp, trajectory_pb);
  ...
  trajectory_pb->latencystats.set__totaltimems(time_diff_ms);
  ADEBUG << "Planning latency: "
         << trajectory_pb->latencystats;

  auto *ref_line_task =
      &trajectory_pb->latencystats.taskstats.emplace_back();
  ...
  if (!status.ok())
  {
    ...
    if (FLAGS_publish_estop)
    {
      ...
      // estop_ = estop_ || trajectory_.estop().is_estop();
      // we should add more information to ensure the estop being triggered.
      EStop *estop = &trajectory_pb->estop;
      estop->set__isestop(true);
      estop->set__reason(status.error_message());
    }
  }

  trajectory_pb->set__isreplan(stitching_trajectory.size() == 1);
  // TODO(all): integrate reverse gear
  trajectory_pb->set__gear(canbus::Chassis::GEAR_DRIVE);
  FillPlanningPb(start_timestamp, trajectory_pb);
  //ADEBUG << "Planning pb:" << trajectory_pb->apolloheader().DebugString();
...
{
  if (config_.planningconfig.navigationplanningconfig != apollo_msgs::msg::ApolloplanningNavigationPlanningConfig())
  {
...
void NaviPlanning::ExportReferenceLineDebug(planningInternal::Debug *debug)
{
    ...
    apollo_msgs::msg::ApolloplanningADCTrajectory *const trajectory_pb)
{
  ...
  trajectory_pb->set__rightofwaystatus(best_ref_info->GetRightOfWayStatus());
  for (const auto &id : best_ref_info->TargetLaneId())
  {
    trajectory_pb->laneid.push_back(id);
  }

  best_ref_info->ExportDecision(&trajectory_pb->decision,
  ...
  **/
  ...
  {
    if (last_publishable_trajectory_->TrajectoryPointAt(i).relativetime >
  ...
  best_ref_info->ExportEngageAdvice(&trajectory_pb->engageadvice,
                                    injector_->planning_context());
    ...
    const localization::LocalizationEstimate &localization) const
{
  ...
  if (localization.pose.position == apollo_msgs::msg::ApollocommonPointENU())
  {
...
/*bool NaviPlanning::CheckPlanningConfig(const apollo_msgs::msg::ApolloplanningPlanningConfig &config)
{
  if (config.planningconfig.navigationplanningconfig == apollo_msgs::msg::ApolloplanningNavigationPlanningConfig())
  {
    return false;
  }
