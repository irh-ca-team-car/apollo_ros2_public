...
bool CosThetaSmoother::Solve(
  ...
  const double weight_cos_included_angle = config_.weightcosincludedangle;
  const double weight_anchor_points = config_.weightanchorpoints;
  const double weight_length = config_.weightlength;
  const size_t print_level = config_.printlevel;
  const size_t max_num_of_iterations = config_.maxnumofiterations;
  const size_t acceptable_num_of_iterations =
      config_.acceptablenumofiterations;
  const double tol = config_.tol;
  const double acceptable_tol = config_.acceptabletol;
  const bool use_automatic_differentiation =
      config_.ipoptuseautomaticdifferentiation;
