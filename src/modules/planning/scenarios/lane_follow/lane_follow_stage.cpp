...
Status LaneFollowStage::PlanOnReferenceLine(
  ...
  ADEBUG << "planning start point:" << planning_start_point;
  ...
  if (!reference_line_info->CombinePathAndSpeedProfile(
          planning_start_point.relativetime,
          planning_start_point.pathpoint.s, &trajectory)) {
  ...
  for (const auto* obstacle :
    ...
    if (obstacle->LongitudinalDecision().objecttag.stop != apollo_msgs::msg::ApolloplanningObjectStop() &&
        obstacle->LongitudinalDecision().objecttag.stop.reasoncode ==
            STOP_REASON_DESTINATION)
    {
      SLPoint dest_sl = GetStopSL(obstacle->LongitudinalDecision().objecttag.stop,
  ...
  for (const auto* obstacle :
    ...
    if (obstacle->LongitudinalDecision().objecttag.stop != apollo_msgs::msg::ApolloplanningObjectStop())
    {
      ...
      if (dest_stop_s < 0.0) {
        ...
        SLPoint stop_sl = GetStopSL(obstacle->LongitudinalDecision().objecttag.stop,
                                    reference_line_info->reference_line());
...
void LaneFollowStage::GenerateFallbackPathProfile(
  ...
  ACHECK(adc_point.pathpoint != apollo::common::PathPoint());
  const auto adc_point_x = adc_point.pathpoint.x;
  const auto adc_point_y = adc_point.pathpoint.y;

  common::SLPoint adc_point_s_l;
  if (!reference_line.XYToSLPB(adc_point.pathpoint, &adc_point_s_l)) {
    AERROR << "Fail to project ADC to reference line when calculating path "
              "fallback. Straight forward path is generated";
    const auto adc_point_heading = adc_point.pathpoint.theta;
    const auto adc_point_kappa = adc_point.pathpoint.kappa;
    const auto adc_point_dkappa = adc_point.pathpoint.dkappa;
...
bool LaneFollowStage::RetrieveLastFramePathProfile(
  ...
  const auto adc_frenet_frame_point_ =
      reference_line_info->reference_line().GetFrenetPoint(
          frame->PlanningStartPoint().pathpoint);
...
SLPoint LaneFollowStage::GetStopSL(const ObjectStop& stop_decision,
  ...
  SLPoint sl_point;
  reference_line.XYToSLPB(stop_decision.stoppoint, &sl_point);
  return sl_point;
}
