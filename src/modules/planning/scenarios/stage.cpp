...
Stage::Stage(const StageConfig& config,
             const std::shared_ptr<DependencyInjector>& injector)
    : config_(config), injector_(injector) {
  // set stage_type in PlanningContext
  injector->planning_context()
      ->mutable_planning_status()
      ->scenario.set__stagetype(stage_type());

  name_ = ScenarioConfig::StageType_Name((ScenarioConfig::StageType)config_.stagetype);
  next_stage_ = (ScenarioConfig::StageType)config_.stagetype;
  std::unordered_map<TaskConfig::TaskType, const apollo_msgs::msg::ApolloplanningTaskConfig*, std::hash<int>>
      config_map;
  for (const auto& task_config : config_.taskconfig) {
    config_map[(TaskConfig::TaskType)task_config.tasktype] = &task_config;
  }
  for (size_t i = 0; i < config_.tasktype.size(); ++i) {
    auto task_type = (TaskConfig::TaskType)config_.tasktype.at(i);
