...
bool StageIntersectionCruiseImpl::CheckDone(
    ...
    const apollo_msgs::msg::ApolloplanningStageConfig& config, const PlanningContext* context,
    ...
    if (pnc_junction_overlaps.empty())
    {
        ...
        {
            // stop_sign scenarios
            const auto &stop_sign_status = context->planning_status().stopsign;
            const std::string traffic_sign_overlap_id =
                stop_sign_status.currentstopsignoverlapid;
                     ...
                     ScenarioConfig::TRAFFIC_LIGHT_UNPROTECTED_RIGHT_TURN)
        {
            ...
            const auto &traffic_light_status =
                context->planning_status().trafficlight;
            const std::string traffic_sign_overlap_id =
                traffic_light_status.currenttrafficlightoverlapid.size() > 0
                    ? traffic_light_status.currenttrafficlightoverlapid.at(0)
                    : "";
            traffic_sign_overlap = scenario::util::GetOverlapOnReferenceLine(
                reference_line_info, traffic_sign_overlap_id,
        ...
        else if (scenario_type == ScenarioConfig::YIELD_SIGN)
        {
            ...
            const auto &yield_sign_status = context->planning_status().yieldsign;
            const std::string traffic_sign_overlap_id =
                yield_sign_status.currentyieldsignoverlapid.size() > 0
                    ? yield_sign_status.currentyieldsignoverlapid.at(0)
                    : "";
            traffic_sign_overlap = scenario::util::GetOverlapOnReferenceLine(
                reference_line_info, traffic_sign_overlap_id,
        ...
        const double adc_back_edge_s =
            reference_line_info.AdcSlBoundary().starts;
