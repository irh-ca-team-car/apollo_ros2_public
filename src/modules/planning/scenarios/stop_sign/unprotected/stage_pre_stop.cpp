...
Stage::StageStatus StopSignUnprotectedStagePreStop::Process(
  ...
  for (const auto& perception_obstacle_id : watch_vehicle_ids) {
    injector_->planning_context()
        ...
        ->stopsign.waitforobstacleid.push_back(perception_obstacle_id);
  }
...
int StopSignUnprotectedStagePreStop::AddWatchVehicle(
  ...
  // check type
  if (obstacle_type != apollo::perception::PerceptionObstacle::UNKNOWN &&
      obstacle_type != apollo::perception::PerceptionObstacle::UNKNOWN_MOVABLE &&
      obstacle_type != apollo::perception::PerceptionObstacle::BICYCLE &&
      obstacle_type != apollo::perception::PerceptionObstacle::VEHICLE)
  {
  ...
  if (HDMapUtil::BaseMap().GetNearestLaneWithHeading(
    ...
    ADEBUG << "obstacle_id[" << perception_obstacle_id << "] type["
           ...
           << point << "; heading[" << perception_obstacle.theta
           << "]";
    return -1;
  }
  ...
  const double stop_line_s = over_lap_info->overlapinfo.laneoverlapinfo.starts;
  ...
  if (distance_to_stop_line >
      scenario_config_.watchvehiclemaxvalidstopdistance) {
    ADEBUG << "obstacle_id[" << perception_obstacle_id << "] type["
...
bool StopSignUnprotectedStagePreStop::CheckADCStop(
  ...
  if (distance_stop_line_to_adc_front_edge >
      scenario_config_.maxvalidstopdistance) {
