...
PullOverStatus CheckADCPullOver(
  ...
  const auto& pull_over_status =
      planning_context->planning_status().pullover;
  ...
  if (distance >= scenario_config.passdestinationthreshold) {
...
bool CheckPullOverPositionBySL(const ReferenceLineInfo& reference_line_info,
  ...
  bool ret = (l_diff <= scenario_config.maxlerrortoendpoint &&
              theta_diff <= scenario_config.maxthetaerrortoendpoint);
  if (check_s) {
    ret = (ret && s_diff >= 0 &&
           s_diff <= scenario_config.maxserrortoendpoint);
  }
...
bool CheckADCReadyToCruise(
  ...
  return ((vehicle_status->gear() == canbus::Chassis::GEAR_DRIVE ||
           std::fabs(vehicle_status->vehicle_state().linearvelocity) <
               kMinSpeed) &&
...
bool CheckADCSurroundObstacles(const common::math::Vec2d adc_position,
  ...
  const double adc_length = vehicle_config.vehicleparam.length;
  const double adc_width = vehicle_config.vehicleparam.width;
  // ADC box
