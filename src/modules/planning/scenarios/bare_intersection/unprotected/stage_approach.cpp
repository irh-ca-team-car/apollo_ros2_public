...
Stage::StageStatus BareIntersectionUnprotectedStageApproach::Process(
  ...
  if (scenario_config_.enableexplicitstop) {
    bool stop = false;
    ...
    if (distance_adc_to_pnc_junction <= kCheckClearDistance &&
      ...
      auto* bare_intersection_status = &injector_->planning_context()
                                                ...
                                                ->bareintersection;
      int clear_counter = bare_intersection_status->clearcounter;
      ...
      bare_intersection_status->set__clearcounter(clear_counter);
    }
