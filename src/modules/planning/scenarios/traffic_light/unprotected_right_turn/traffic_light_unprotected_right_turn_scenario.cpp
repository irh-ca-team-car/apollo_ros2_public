...
void TrafficLightUnprotectedRightTurnScenario::Init() {
  ...
  if (traffic_light_status.currenttrafficlightoverlapid.empty()) {
  ...
  for (size_t i = 0;
       i < traffic_light_status.currenttrafficlightoverlapid.size(); i++)
  {
    const std::string traffic_light_overlap_id =
        traffic_light_status.currenttrafficlightoverlapid.at(i);
...
apollo::common::util::Factory<
    ...
    Stage* (*)(const apollo_msgs::msg::ApolloplanningStageConfig& stage_config,
...
std::unique_ptr<Stage> TrafficLightUnprotectedRightTurnScenario::CreateStage(
    const apollo_msgs::msg::ApolloplanningStageConfig& stage_config,
    const std::shared_ptr<DependencyInjector>& injector) {
  if (s_stage_factory_.Empty()) {
    RegisterStages();
  }
  auto ptr = s_stage_factory_.CreateObjectOrNull((apollo::planning::ScenarioConfig::StageType)stage_config.stagetype,
...
bool TrafficLightUnprotectedRightTurnScenario::GetScenarioConfig() {
  if (config_.scenarioconfig.trafficlightunprotectedrightturnconfig == apollo_msgs::msg::ApolloplanningScenarioTrafficLightUnprotectedRightTurnConfig())
  {
    AERROR << "miss scenario specific config";
    return false;
  }
  context_.scenario_config=(
      config_.scenarioconfig.trafficlightunprotectedrightturnconfig);
  return true;
}
