...
void YieldSignScenario::Init() {
  ...
  const auto& yield_sign_status =
      injector_->planning_context()->planning_status().yieldsign;

  if (yield_sign_status.currentyieldsignoverlapid.empty()) {
    AERROR << "Could not find yield-sign(s)";
    return;
  }

  context_.current_yield_sign_overlap_ids.clear();
  for (size_t i = 0; i < yield_sign_status.currentyieldsignoverlapid.size();
       i++) {
    const std::string yield_sign_overlap_id =
        yield_sign_status.currentyieldsignoverlapid.at(i);
...
bool YieldSignScenario::GetScenarioConfig() {
  if (config_.scenarioconfig.yieldsignconfig==apollo_msgs::msg::ApolloplanningScenarioYieldSignConfig())
  {
    AERROR << "miss scenario specific config";
    return false;
  }
  context_.scenario_config=(config_.scenarioconfig.yieldsignconfig);
  return true;
}
