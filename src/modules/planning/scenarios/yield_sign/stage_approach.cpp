    ...
    if (distance_adc_to_stop_line <
      ...
      for (const auto* obstacle : path_decision.obstacles().Items()) {
        ...
        injector_->planning_context()
            ...
            ->yieldsign.waitforobstacleid.push_back(obstacle->Id());
...
Stage::StageStatus YieldSignStageApproach::FinishStage() {
  ...
  yield_sign_status->doneyieldsignoverlapid.clear();
  for (const auto& yield_sign_overlap_id :
       GetContext()->current_yield_sign_overlap_ids) {
    yield_sign_status->doneyieldsignoverlapid.push_back(yield_sign_overlap_id);
  }
  yield_sign_status->waitforobstacleid.clear();
