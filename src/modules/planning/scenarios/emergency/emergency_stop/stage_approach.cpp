...
Stage::StageStatus EmergencyStopStageApproach::Process(
  ...
  const double stop_distance = scenario_config_.stopdistance;

  bool stop_fence_exist = false;
  double stop_line_s;
  const auto& emergency_stop_status =
      injector_->planning_context()->planning_status().emergencystop;
  if (emergency_stop_status.stopfencepoint != apollo::common::PointENU())
    {
      common::SLPoint stop_fence_sl;
      reference_line.XYToSLPB(emergency_stop_status.stopfencepoint,
                            &stop_fence_sl);
  ...
  if (!stop_fence_exist) {
    const double deceleration = scenario_config_.maxstopdeceleration;
    ...
    auto* emergency_stop_fence_point = &injector_->planning_context()
                                           ...
                                           ->emergencystop
                                           .stopfencepoint;
    emergency_stop_fence_point->set__x(stop_fence_point.x());
    emergency_stop_fence_point->set__y(stop_fence_point.y());
  }
