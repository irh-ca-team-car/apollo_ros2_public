...
LearningModelInferenceTrajectoryTask::LearningModelInferenceTrajectoryTask(
    ...
    ACHECK(config.taskconfig.learningmodelinferencetrajectorytaskconfig != apollo_msgs::msg::ApolloplanningLearningModelInferenceTrajectoryTaskConfig());
}
...
Status LearningModelInferenceTrajectoryTask::Process(
  ...
  const auto &config =
      config_.taskconfig.learningmodelinferencetrajectorytaskconfig;
  ...
  const double first_point_relative_time =
      adc_future_trajectory_points.front().relativetime;
  const double last_point_relative_time =
      adc_future_trajectory_points.back().relativetime;
  const auto learning_data_frame = injector_->learning_based_data()
                                            ->GetLatestLearningDataFrame();
  const int frame_num =
      learning_data_frame ? learning_data_frame->framenum : -1;
  ...
  if (adc_future_trajectory_points.size() <= 0 ||
      ...
      last_point_relative_time <
          config.minadcfuturetrajectorytimelength) {
