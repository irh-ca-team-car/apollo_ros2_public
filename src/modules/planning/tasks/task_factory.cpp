...
namespace apollo
{
  namespace planning
  {
    ...
    apollo::common::util::Factory<
        ...
        Task *(*)(const apollo_msgs::msg::ApolloplanningTaskConfig &config,
    ...
    std::unordered_map<TaskConfig::TaskType, apollo_msgs::msg::ApolloplanningTaskConfig, std::hash<int>>
        TaskFactory::default_task_configs_;

    void TaskFactory::Init(const apollo_msgs::msg::ApolloplanningPlanningConfig &config,
                           const std::shared_ptr<DependencyInjector> &injector)
    {
      UNUSED(injector);
      ///////////////////////////
      ...
      for (const auto &default_task_config : config.defaulttaskconfig)
      {
        default_task_configs_[(apollo::planning::TaskConfig::TaskType)default_task_config.tasktype] =
            default_task_config;
      }
    }

    std::unique_ptr<Task> TaskFactory::CreateTask(
        const apollo_msgs::msg::ApolloplanningTaskConfig &task_config,
        const std::shared_ptr<DependencyInjector> &injector)
    {
      apollo_msgs::msg::ApolloplanningTaskConfig merged_config;
      if (default_task_configs_.find((apollo::planning::TaskConfig::TaskType)task_config.tasktype) !=
          default_task_configs_.end())
      {
        merged_config = default_task_configs_[(apollo::planning::TaskConfig::TaskType)task_config.tasktype];
      }

      if (task_config.taskconfig.creepdeciderconfig != apollo_msgs::msg::ApolloplanningCreepDeciderConfig())
        merged_config.taskconfig.creepdeciderconfig = task_config.taskconfig.creepdeciderconfig;
      if (task_config.taskconfig.lanechangedeciderconfig != apollo_msgs::msg::ApolloplanningLaneChangeDeciderConfig())
        merged_config.taskconfig.lanechangedeciderconfig = task_config.taskconfig.lanechangedeciderconfig;
      if (task_config.taskconfig.learningmodelinferencetaskconfig != apollo_msgs::msg::ApolloplanningLearningModelInferenceTaskConfig())
        merged_config.taskconfig.learningmodelinferencetaskconfig = task_config.taskconfig.learningmodelinferencetaskconfig;
      if (task_config.taskconfig.learningmodelinferencetrajectorytaskconfig != apollo_msgs::msg::ApolloplanningLearningModelInferenceTrajectoryTaskConfig())
        merged_config.taskconfig.learningmodelinferencetrajectorytaskconfig = task_config.taskconfig.learningmodelinferencetrajectorytaskconfig;
      if (task_config.taskconfig.openspacefallbackdeciderconfig != apollo_msgs::msg::ApolloplanningOpenSpaceFallBackDeciderConfig())
        merged_config.taskconfig.openspacefallbackdeciderconfig = task_config.taskconfig.openspacefallbackdeciderconfig;
      if (task_config.taskconfig.openspaceprestopdeciderconfig != apollo_msgs::msg::ApolloplanningOpenSpacePreStopDeciderConfig())
        merged_config.taskconfig.openspaceprestopdeciderconfig = task_config.taskconfig.openspaceprestopdeciderconfig;
      if (task_config.taskconfig.openspaceroideciderconfig != apollo_msgs::msg::ApolloplanningOpenSpaceRoiDeciderConfig())
        merged_config.taskconfig.openspaceroideciderconfig=task_config.taskconfig.openspaceroideciderconfig;
      if (task_config.taskconfig.openspacetrajectorypartitionconfig != apollo_msgs::msg::ApolloplanningOpenSpaceTrajectoryPartitionConfig())
        merged_config.taskconfig.openspacetrajectorypartitionconfig=task_config.taskconfig.openspacetrajectorypartitionconfig;
      if (task_config.taskconfig.openspacetrajectoryproviderconfig != apollo_msgs::msg::ApolloplanningOpenSpaceTrajectoryProviderConfig())
        merged_config.taskconfig.openspacetrajectoryproviderconfig=task_config.taskconfig.openspacetrajectoryproviderconfig;
      if (task_config.taskconfig.pathassessmentdeciderconfig != 0)
        merged_config.taskconfig.pathassessmentdeciderconfig=task_config.taskconfig.pathassessmentdeciderconfig;

      return task_factory_.CreateObject((apollo::planning::TaskConfig::TaskType)task_config.tasktype, merged_config,
                                        injector);
    }
