...
Status OpenSpaceTrajectoryOptimizer::Plan(
  ...
  double init_x = trajectory_stitching_point.pathpoint.x;
  double init_y = trajectory_stitching_point.pathpoint.y;
  double init_phi = trajectory_stitching_point.pathpoint.theta;
  ...
  if (FLAGS_enable_parallel_trajectory_smoothing) {
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      if (config_.planneropenspaceconfig
              .enablecheckparalleltrajectory) {
  ...
  if (FLAGS_enable_record_debug) {
    open_space_debug_ = apollo::planningInternal::OpenSpaceDebug();
...
void OpenSpaceTrajectoryOptimizer::RecordDebugInfo(
  ...
  open_space_debug_.trajectorystitchingpoint=(
      trajectory_stitching_point);
  // load translation origin and heading angle
  auto* roi_shift_point = &open_space_debug_.roishiftpoint;
  // pathpoint
  roi_shift_point->pathpoint.set__x(translate_origin.x());
  roi_shift_point->pathpoint.set__y(translate_origin.y());
  roi_shift_point->pathpoint.set__theta(rotate_angle);

  // load end_pose into debug
  auto* end_point = &open_space_debug_.endpoint;
  end_point->pathpoint.set__x(end_pose[0]);
  end_point->pathpoint.set__y(end_pose[1]);
  end_point->pathpoint.set__theta(end_pose[2]);
  end_point->set__v(end_pose[3]);

  // load warm start trajectory
  size_t horizon = xWS.cols() - 1;
  auto* warm_start_trajectory =
      &open_space_debug_.warmstarttrajectory;
  for (size_t i = 0; i < horizon; ++i) {
    auto* warm_start_point = &warm_start_trajectory->vehiclemotionpoint.emplace_back();
    warm_start_point->trajectorypoint.pathpoint.set__x(
        xWS(0, i));
    warm_start_point->trajectorypoint.pathpoint.set__y(
        xWS(1, i));
    warm_start_point->trajectorypoint.pathpoint.set__theta(xWS(2, i));
    warm_start_point->trajectorypoint.set__v(xWS(3, i));
    warm_start_point->set__steer(uWS(0, i));
    warm_start_point->trajectorypoint.set__a(uWS(1, i));
  }
  auto* warm_start_point = &warm_start_trajectory->vehiclemotionpoint.emplace_back();
  warm_start_point->trajectorypoint.pathpoint.set__x(
      xWS(0, horizon));
  warm_start_point->trajectorypoint.pathpoint.set__y(
      xWS(1, horizon));
  warm_start_point->trajectorypoint.pathpoint.set__theta(
      xWS(2, horizon));
  warm_start_point->trajectorypoint.set__v(xWS(3, horizon));

  // load warm start dual variables
  size_t l_warm_up_rows = l_warm_up.rows();
  for (size_t i = 0; i < horizon; ++i) {
    for (size_t j = 0; j < l_warm_up_rows; j++) {
      open_space_debug_.warmstartduallambda.push_back(l_warm_up(j, i));
    }
  }
  size_t n_warm_up_rows = n_warm_up.rows();
  for (size_t i = 0; i < horizon; ++i) {
    for (size_t j = 0; j < n_warm_up_rows; j++) {
      open_space_debug_.warmstartdualmiu.push_back(n_warm_up(j, i));
    }
  }
  ...
  for (size_t i = 0; i < horizon; ++i) {
    for (size_t j = 0; j < dual_l_result_ds_rows; j++) {
      open_space_debug_.optimizedduallambda.push_back(dual_l_result_ds(j, i));
    }
  }
  size_t dual_n_result_ds_rows = dual_n_result_ds.rows();
  for (size_t i = 0; i < horizon; ++i) {
    for (size_t j = 0; j < dual_n_result_ds_rows; j++) {
      open_space_debug_.optimizeddualmiu.push_back(dual_n_result_ds(j, i));
    }
  }
  ...
  auto* smoothed_trajectory = &open_space_debug_.smoothedtrajectory;
  for (size_t i = 0; i < horizon; ++i) {
    auto* smoothed_point = &smoothed_trajectory->vehiclemotionpoint.emplace_back();
    smoothed_point->trajectorypoint.pathpoint.set__x(
        state_result_ds(0, i));
    smoothed_point->trajectorypoint.pathpoint.set__y(
        state_result_ds(1, i));
    smoothed_point->trajectorypoint.pathpoint.set__theta(
        state_result_ds(2, i));
    smoothed_point->trajectorypoint.set__v(state_result_ds(3, i));
    smoothed_point->set__steer(control_result_ds(0, i));
    smoothed_point->trajectorypoint.set__a(control_result_ds(1, i));
    relative_time += time_result_ds(0, i);
    smoothed_point->trajectorypoint.set__relativetime(
        relative_time);
  }
  auto* smoothed_point = &smoothed_trajectory->vehiclemotionpoint.emplace_back();
  smoothed_point->trajectorypoint.pathpoint.set__x(
  ...
  smoothed_point->trajectorypoint.set__v(
      state_result_ds(3, horizon));

  // load xy boundary (xmin, xmax, ymin, ymax)
  open_space_debug_.xyboundary.push_back(XYbounds[0]);
  open_space_debug_.xyboundary.push_back(XYbounds[1]);
  open_space_debug_.xyboundary.push_back(XYbounds[2]);
  open_space_debug_.xyboundary.push_back(XYbounds[3]);

  // load obstacles
  for (const auto& obstacle_vertices : obstacles_vertices_vec) {
    auto* obstacle_ptr = &open_space_debug_.obstacles.emplace_back();
    for (const auto& vertex : obstacle_vertices) {
      obstacle_ptr->verticesxcoords.push_back(vertex.x());
      obstacle_ptr->verticesycoords.push_back(vertex.y());
    }
  }
}

void OpenSpaceTrajectoryOptimizer::UpdateDebugInfo(
    planningInternal::OpenSpaceDebug* open_space_debug) {
      UNUSED(open_space_debug);
...
bool OpenSpaceTrajectoryOptimizer::IsInitPointNearDestination(
  ...
  const common::PathPoint path_point = planning_init_point.pathpoint;
  ...
  if (distance_to_init_point <
      config_.planneropenspaceconfig.isneardestinationthreshold) {
    return true;
  }
  return false;
}
...
void OpenSpaceTrajectoryOptimizer::LoadTrajectory(
  ...
  for (size_t i = 0; i < states_size; ++i) {
    ...
    point.pathpoint.set__x(state_result(0, i));
    point.pathpoint.set__y(state_result(1, i));
    point.pathpoint.set__theta(state_result(2, i));
    point.set__v(state_result(3, i));
    Vec2d cur_path_point(state_result(0, i), state_result(1, i));
    relative_s += cur_path_point.DistanceTo(last_path_point);
    point.pathpoint.set__s(relative_s);
    // TODO(Jinyun): Evaluate how to set end states control input
    if (i == controls_size) {
      point.set__steer(0.0);
      point.set__a(0.0);
    } else {
      point.set__steer(control_result(0, i));
      point.set__a(control_result(1, i));
    }

    if (i == 0) {
      point.set__relativetime(relative_time);
    } else {
...
void OpenSpaceTrajectoryOptimizer::UseWarmStartAsResult(
  ...
  *time_result_ds = Eigen::MatrixXd::Constant(
      1, time_result_horizon, config_.planneropenspaceconfig.deltat);
}
...
bool OpenSpaceTrajectoryOptimizer::GenerateDistanceApproachTraj(
  ...
  const common::VehicleParam& vehicle_param_ =
      common::VehicleConfigHelper::GetConfig().vehicleparam;
  double front_to_center = vehicle_param_.frontedgetocenter;
  double back_to_center = vehicle_param_.backedgetocenter;
  double left_to_center = vehicle_param_.leftedgetocenter;
  double right_to_center = vehicle_param_.rightedgetocenter;
  ...
  double ts = config_.planneropenspaceconfig.deltat;
...
void OpenSpaceTrajectoryOptimizer::LoadResult(
  ...
  *time_result_dc = Eigen::MatrixXd::Zero(1, points_size - 1);
  ...
  for (size_t i = 0; i < points_size; ++i) {
    state_result(0, i) = discretized_trajectory[i].pathpoint.x;
    state_result(1, i) = discretized_trajectory[i].pathpoint.y;
    state_result(2, i) = discretized_trajectory[i].pathpoint.theta;
  ...
  for (size_t i = 0; i + 1 < points_size; ++i) {
    control_result(0, i) =
        std::atan(discretized_trajectory[i].pathpoint.kappa * wheel_base);
    control_result(1, i) = discretized_trajectory[i].a;
    time_result(0, i) = discretized_trajectory[i + 1].relativetime -
                        discretized_trajectory[i].relativetime;
  }
}
