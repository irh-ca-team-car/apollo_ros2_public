...
OpenSpaceTrajectoryProvider::OpenSpaceTrajectoryProvider(
  ...
  open_space_trajectory_optimizer_.reset(new OpenSpaceTrajectoryOptimizer(
      config.taskconfig.openspacetrajectoryproviderconfig
...
Status OpenSpaceTrajectoryProvider::Process() {
  ...
  if (!IsVehicleStopDueToFallBack(
    ...
    auto* open_space_status = &injector_->planning_context()
                                  ->mutable_planning_status()
                                  ->openspace;
    open_space_status->set__positioninit(false);
  }
    ...
    if (trajectory_updated_) {
      ...
      if (FLAGS_enable_record_debug) {
        ...
        open_space_trajectory_optimizer_->UpdateDebugInfo(
            &ptr_debug->planningdata.openspace);
...
bool OpenSpaceTrajectoryProvider::IsVehicleNearDestination(
  ...
  ADEBUG << "is_near_destination_threshold"
         << config_.taskconfig.openspacetrajectoryproviderconfig
                .openspacetrajectoryoptimizerconfig
                .planneropenspaceconfig
                .isneardestinationthreshold; // which config file
  ADEBUG << "is_near_destination_theta_threshold"
         << config_.taskconfig.openspacetrajectoryproviderconfig
                .openspacetrajectoryoptimizerconfig
                .planneropenspaceconfig
                .isneardestinationthetathreshold;
  if (distance_to_vehicle < config_.taskconfig.openspacetrajectoryproviderconfig
                                .openspacetrajectoryoptimizerconfig
                                .planneropenspaceconfig
                                .isneardestinationthreshold &&
      theta_to_vehicle < config_.taskconfig.openspacetrajectoryproviderconfig
                             .openspacetrajectoryoptimizerconfig
                             .planneropenspaceconfig
                             .isneardestinationthetathreshold)
  {
...
bool OpenSpaceTrajectoryProvider::IsVehicleStopDueToFallBack(
  ...
  const double adc_speed = vehicle_state.linearvelocity;
  const double adc_acceleration = vehicle_state.linearacceleration;
...
void OpenSpaceTrajectoryProvider::GenerateStopTrajectory(
  ...
  double standstill_acceleration =
      frame_->vehicle_state().linearvelocity >= -vEpsilon
  ...
  for (size_t i = 0; i < stop_trajectory_length; i++) {
    ...
    point.pathpoint.set__kappa(0.0);
    point.set__relativetime(relative_time);
    point.set__v(0.0);
    point.set__a(standstill_acceleration);
...
void OpenSpaceTrajectoryProvider::LoadResult(
  ...
  double stitching_point_relative_time =
      stitching_trajectory_ptr->back().relativetime;
  double stitching_point_relative_s =
      stitching_trajectory_ptr->back().pathpoint.s;
  for (size_t i = 0; i < optimizer_trajectory_size; ++i) {
    optimizer_trajectory_ptr->at(i).set__relativetime(
        optimizer_trajectory_ptr->at(i).relativetime +
        stitching_point_relative_time);
    optimizer_trajectory_ptr->at(i).pathpoint.set__s(
        optimizer_trajectory_ptr->at(i).pathpoint.s +
...
void OpenSpaceTrajectoryProvider::ReuseLastFrameDebug(const Frame* last_frame) {
  ...
  UNUSED(ptr_debug);
  /* ptr_debug->planningdata.openspace.MergeFrom(
