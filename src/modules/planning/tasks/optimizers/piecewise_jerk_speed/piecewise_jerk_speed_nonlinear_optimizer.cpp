...
PiecewiseJerkSpeedNonlinearOptimizer::PiecewiseJerkSpeedNonlinearOptimizer(
  ...
  ACHECK(config_.taskconfig.piecewisejerknonlinearspeedoptimizerconfig != apollo_msgs::msg::ApolloplanningPiecewiseJerkNonlinearSpeedOptimizerConfig());
}
...
Status PiecewiseJerkSpeedNonlinearOptimizer::Process(
  ...
  if (speed_limit_check_status) {
    ...
    auto* speed_constraint =
        &st_graph_data->mutable_st_graph_debug()->speedconstraint;
    for (int i = 0; i < num_of_knots_; ++i) {
      double t = i * delta_t_;
      speed_constraint->t.push_back(t);
      speed_constraint->upperbound.push_back(
...
Status PiecewiseJerkSpeedNonlinearOptimizer::SetUpStatesAndBounds(
  ...
  s_ddot_max_ = veh_param.maxacceleration;
  s_ddot_min_ = -1.0 * std::abs(veh_param.maxdeceleration);
...
Status PiecewiseJerkSpeedNonlinearOptimizer::OptimizeByQP(
  ...
  const auto& config =
      config_.taskconfig.piecewisejerknonlinearspeedoptimizerconfig;
  piecewise_jerk_problem.set_weight_x(0.0);
  piecewise_jerk_problem.set_weight_dx(0.0);
  piecewise_jerk_problem.set_weight_ddx(config.accweight);
  piecewise_jerk_problem.set_weight_dddx(config.jerkweight);
  ...
  piecewise_jerk_problem.set_x_ref(config.refsweight, std::move(x_ref));
...
Status PiecewiseJerkSpeedNonlinearOptimizer::OptimizeByNLP(
  ...
  if (FLAGS_use_smoothed_dp_guide_line) {
    ...
    ptr_interface->set_w_reference_spatial_distance(
        config.spotentialweight);
  }

  if (FLAGS_use_soft_bound_in_nonlinear_speed_opt) {
    ptr_interface->set_soft_safety_bounds(s_soft_bounds_);
    ptr_interface->set_w_soft_s_bound(config.softsboundweight);
  }

  ptr_interface->set_w_overall_a(config.accweight);
  ptr_interface->set_w_overall_j(config.jerkweight);
  ptr_interface->set_w_overall_centripetal_acc(config.lataccweight);

  ptr_interface->set_reference_speed(cruise_speed_);
  ptr_interface->set_w_reference_speed(config.refvweight);
