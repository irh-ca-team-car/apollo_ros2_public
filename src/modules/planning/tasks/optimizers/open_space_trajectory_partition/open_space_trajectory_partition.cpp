...
OpenSpaceTrajectoryPartition::OpenSpaceTrajectoryPartition(
  ...
  open_space_trajectory_partition_config_ =
      config_.taskconfig.openspacetrajectorypartitionconfig;
  heading_search_range_ =
      open_space_trajectory_partition_config_.headingsearchrange;
  heading_track_range_ =
      open_space_trajectory_partition_config_.headingtrackrange;
  distance_search_range_ =
      open_space_trajectory_partition_config_.distancesearchrange;
  heading_offset_to_midpoint_ =
      open_space_trajectory_partition_config_.headingoffsettomidpoint;
  lateral_offset_to_midpoint_ =
      open_space_trajectory_partition_config_.lateraloffsettomidpoint;
  longitudinal_offset_to_midpoint_ =
      open_space_trajectory_partition_config_.longitudinaloffsettomidpoint;
  vehicle_box_iou_threshold_to_midpoint_ =
      open_space_trajectory_partition_config_
          .vehicleboxiouthresholdtomidpoint;
  linear_velocity_threshold_on_ego_ = open_space_trajectory_partition_config_
                                          .linearvelocitythresholdonego;
  ...
  shift_distance_ = ego_length_ / 2.0 - vehicle_param_.backedgetocenter;
  wheel_base_ = vehicle_param_.wheelbase;
}
...
Status OpenSpaceTrajectoryPartition::Process() {
  ...
  const auto& open_space_status =
      injector_->planning_context()->planning_status().openspace;
  if (!open_space_status.positioninit &&
      frame_->open_space_info().open_space_provider_success()) {
    auto* open_space_status = &injector_->planning_context()
                                  ->mutable_planning_status()
                                  ->openspace;
    open_space_status->set__positioninit(true);
...
void OpenSpaceTrajectoryPartition::InterpolateTrajectory(
  ...
  size_t interpolated_pieces_num =
      open_space_trajectory_partition_config_.interpolatedpiecesnum;
  ...
  for (size_t i = 0; i < trajectory_to_be_partitioned_intervals_num; ++i) {
    double relative_time_interval =
        (stitched_trajectory_result.at(i + 1).relativetime -
         stitched_trajectory_result.at(i).relativetime) /
...
void OpenSpaceTrajectoryPartition::UpdateVehicleInfo() {
  ...
  ego_v_ = vehicle_state.linearvelocity;
...
bool OpenSpaceTrajectoryPartition::EncodeTrajectory(
  ...
  const auto& init_path_point = trajectory.front().pathpoint;
  const auto& last_path_point = trajectory.back().pathpoint;
...
bool OpenSpaceTrajectoryPartition::CheckTrajTraversed(
  ...
  const int index_history_size =
      open_space_status.partitionedtrajectoriesindexhistory.size();
  ...
  for (int i = 0; i < index_history_size - 1; i++) {
    const auto& index_history =
        open_space_status.partitionedtrajectoriesindexhistory.at(i);
...
void OpenSpaceTrajectoryPartition::UpdateTrajHistory(
  ...
  if (trajectory_history.empty()) {
    open_space_status->partitionedtrajectoriesindexhistory.push_back(
...
void OpenSpaceTrajectoryPartition::PartitionTrajectory(
  ...
  // Decide initial gear
  const auto& first_path_point = raw_trajectory.front().pathpoint;
  const auto& second_path_point = raw_trajectory[1].pathpoint;
  ...
  *gear =
  ...
  for (size_t i = 0; i < horizon - 1; ++i) {
    ...
    // Check gear change
    heading_angle = trajectory_point.pathpoint.theta;
    const Vec2d tracking_vector(next_trajectory_point.pathpoint.x -
                                    trajectory_point.pathpoint.x,
                                next_trajectory_point.pathpoint.y -
...
void OpenSpaceTrajectoryPartition::LoadTrajectoryPoint(
  ...
  point->set__relativetime(trajectory_point.relativetime);
  point->pathpoint.set__x(trajectory_point.pathpoint.x);
  point->pathpoint.set__y(trajectory_point.pathpoint.y);
  point->pathpoint.set__theta(trajectory_point.pathpoint.theta);
  point->set__v(trajectory_point.v);
  point->pathpoint.set__s(*distance_s);
  Vec2d cur_pos_vec(trajectory_point.pathpoint.x,
                    trajectory_point.pathpoint.y);
  *distance_s += (gear == canbus::Chassis::GEAR_REVERSE ? -1.0 : 1.0) *
                 (cur_pos_vec.DistanceTo(*last_pos_vec));
  *last_pos_vec = cur_pos_vec;
  point->pathpoint.set__kappa((is_trajectory_last_point ? -1 : 1) *
                                         std::tan(trajectory_point.steer) /
                                         wheel_base_);
  point->set__a(trajectory_point.a);
}
...
bool OpenSpaceTrajectoryPartition::CheckReachTrajectoryEnd(
  ...
  const PathPoint& path_end_point = trajectory_end_point.pathpoint;
  ...
  *(current_gear_status) = last_gear_status;
  ...
  if (flag_change_to_next || !current_gear_status->gear_shift_period_finished) {
    ...
    if (current_gear_status->gear_shift_period_time >
        open_space_trajectory_partition_config_.gearshiftperiodduration) {
      current_gear_status->gear_shift_period_finished = true;
      current_gear_status->gear_shift_period_started = true;
    } else {
...
void OpenSpaceTrajectoryPartition::GenerateGearShiftTrajectory(
  ...
  const double gear_shift_max_t =
      open_space_trajectory_partition_config_.gearshiftmaxt;
  const double gear_shift_unit_t =
      open_space_trajectory_partition_config_.gearshiftunitt;
  // TrajectoryPoint point;
  for (double t = 0.0; t < gear_shift_max_t; t += gear_shift_unit_t) {
    TrajectoryPoint point;
    point.pathpoint.set__x(frame_->vehicle_state().x);
    point.pathpoint.set__y(frame_->vehicle_state().y);
    point.pathpoint.set__theta(frame_->vehicle_state().heading);
    point.pathpoint.set__s(0.0);
    point.pathpoint.set__kappa(frame_->vehicle_state().kappa);
    point.set__relativetime(t);
    point.set__v(0.0);
    point.set__a(0.0);
...
void OpenSpaceTrajectoryPartition::AdjustRelativeTimeAndS(
  ...
  *(current_partitioned_trajectory) =
  ...
  double time_shift =
      trajectory->at(closest_trajectory_point_index).relativetime;
  double s_shift =
      trajectory->at(closest_trajectory_point_index).pathpoint.s;
  const size_t trajectory_size = trajectory->size();
  for (size_t i = 0; i < trajectory_size; ++i) {
    ...
    trajectory_point->set__relativetime(trajectory_point->relativetime -
                                        time_shift);
    trajectory_point->pathpoint.set__s(
        trajectory_point->pathpoint.s - s_shift);
  }
  ...
  time_shift =
      unpartitioned_trajectory_result->at(index_estimate).relativetime;
  s_shift =
      unpartitioned_trajectory_result->at(index_estimate).pathpoint.s;
