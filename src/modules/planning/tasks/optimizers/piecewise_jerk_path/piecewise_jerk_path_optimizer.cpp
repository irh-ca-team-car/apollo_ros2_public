...
PiecewiseJerkPathOptimizer::PiecewiseJerkPathOptimizer(
  ...
  ACHECK(config_.taskconfig.piecewisejerkpathoptimizerconfig != apollo_msgs::msg::ApolloplanningPiecewiseJerkPathOptimizerConfig());
}
...
common::Status PiecewiseJerkPathOptimizer::Process(
  ...
  ADEBUG << "Plan at the starting point: x = " << init_point.pathpoint.x
         << ", y = " << init_point.pathpoint.y
         << ", and angle = " << init_point.pathpoint.theta;
  ...
  const auto &config = reference_line_info_->IsChangeLanePath()
                           ? config_.taskconfig.piecewisejerkpathoptimizerconfig
  ...
  std::array<double, 5> w = {
      ...
      config.dlweight *
          std::fmax(init_frenet_state.first[1] * init_frenet_state.first[1],
                    5.0),
      config.ddlweight, config.dddlweight, 0.0};
  ...
  for (const auto& path_boundary : path_boundaries) {
    ...
    const double lat_acc_bound =
        std::tan(veh_param.maxsteerangle / veh_param.steerratio) /
        veh_param.wheelbase;
...
PiecewiseJerkPathOptimizer::ConvertPathPointRefFromFrontAxeToRearAxe(
  ...
  for (auto path_point : path_data.discretized_path()) {
    ...
    new_path_point.set__x(path_point.x - front_to_rear_axe_distance *
                                              std::cos(path_point.theta));
    new_path_point.set__y(path_point.y - front_to_rear_axe_distance *
...
bool PiecewiseJerkPathOptimizer::OptimizePath(
  ...
  if (is_valid_path_reference) {
    ...
    const double peak_value = config_.taskconfig.piecewisejerkpathoptimizerconfig
                                  .pathreferencelweight;
  ...
  const auto& veh_param =
      common::VehicleConfigHelper::GetConfig().vehicleparam;
  const double axis_distance = veh_param.wheelbase;
  const double max_yaw_rate =
      veh_param.maxsteeranglerate / veh_param.steerratio / 2.0;
...
FrenetFramePath PiecewiseJerkPathOptimizer::ToPiecewiseJerkPath(
  ...
  while (accumulated_s < piecewise_jerk_traj.ParamLength()) {
    ...
    frenet_frame_point.set__s(accumulated_s + start_s);
    frenet_frame_point.set__l(l);
