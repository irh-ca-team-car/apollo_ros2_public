...
double DpStCost::GetObstacleCost(const StGraphPoint& st_graph_point) {
  ...
  if (FLAGS_use_st_drivable_boundary) {
    ...
    const double lower_bound =
        st_drivable_boundary_.stboundary.at(index).slower;
    const double upper_bound =
        st_drivable_boundary_.stboundary.at(index).supper;
  ...
  for (const auto* obstacle : obstacles_) {
    ...
    if (s < s_lower) {
      const double follow_distance_s = config_.safedistance;
      if (s + follow_distance_s < s_lower) {
        continue;
      } else {
        auto s_diff = follow_distance_s - s_lower + s;
        cost += config_.obstacleweight * config_.defaultobstaclecost *
                s_diff * s_diff;
      }
    } else if (s > s_upper) {
...
double DpStCost::GetSpatialPotentialCost(const StGraphPoint& point) {
  return (total_s_ - point.point().s()) * config_.spatialpotentialpenalty;
}

double DpStCost::GetReferenceCost(const STPoint& point,
                                  const STPoint& reference_point) const {
  return config_.referenceweight * (point.s() - reference_point.s()) *
...
double DpStCost::GetSpeedCost(const STPoint& first, const STPoint& second,
  ...
  if (speed < max_adc_stop_speed && InKeepClearRange(second.s())) {
    // first.s in range
    cost += config_.keepclearlowspeedpenalty * unit_t_ *
            config_.defaultspeedcost;
  }

  double det_speed = (speed - speed_limit) / speed_limit;
  if (det_speed > 0) {
    cost += config_.exceedspeedpenalty * config_.defaultspeedcost *
            (det_speed * det_speed) * unit_t_;
  } else if (det_speed < 0) {
    cost += config_.lowspeedpenalty * config_.defaultspeedcost *
            -det_speed * unit_t_;
  }

  if (FLAGS_enable_dp_reference_speed) {
    double diff_speed = speed - cruise_speed;
    cost += config_.referencespeedpenalty * config_.defaultspeedcost *
...
double DpStCost::GetAccelCost(const double accel) {
  ...
  CHECK_LT(accel_key, accel_cost_.size());
  ...
  if (accel_cost_.at(accel_key) < 0.0) {
    ...
    double max_acc = config_.maxacceleration;
    double max_dec = config_.maxdeceleration;
    double accel_penalty = config_.accelpenalty;
    double decel_penalty = config_.decelpenalty;
...
double DpStCost::JerkCost(const double jerk) {
  ...
  if (jerk_cost_.at(jerk_key) < 0.0) {
    ...
    if (jerk > 0) {
      cost = config_.positivejerkcoeff * jerk_sq * unit_t_;
    } else {
      cost = config_.negativejerkcoeff * jerk_sq * unit_t_;
    }
