...
GriddedPathTimeGraph::GriddedPathTimeGraph(
  ...
  unit_t_ = gridded_path_time_graph_config_.unitt;
  total_length_s_ = st_graph_data_.path_length();
  dense_unit_s_ = gridded_path_time_graph_config_.denseunits;
  sparse_unit_s_ = gridded_path_time_graph_config_.sparseunits;
  dense_dimension_s_ = gridded_path_time_graph_config_.densedimensions;
  // Safety approach preventing unreachable acceleration/deceleration
  max_acceleration_ =
      std::min(std::abs(vehicle_param_.maxacceleration),
               std::abs(gridded_path_time_graph_config_.maxacceleration));
  max_deceleration_ =
      -1.0 *
      std::min(std::abs(vehicle_param_.maxdeceleration),
               std::abs(gridded_path_time_graph_config_.maxdeceleration));
}
    ...
    if (count > 0) {
      ...
      for (size_t r = next_lowest_row; r <= next_highest_row; ++r) {
        ...
        if (FLAGS_enable_multi_thread_in_dp_st_graph) {
          results.push_back(
              std::async(&GriddedPathTimeGraph::CalculateCostAt, this, msg));
        } else {
          CalculateCostAt(msg);
        }
      }
...
void GriddedPathTimeGraph::CalculateCostAt(
  ...
  if (c == 0) {
    CHECK_EQ(r, 0U) << "Incorrect. Row should be 0 with col = 0. row: " << r;
...
Status GriddedPathTimeGraph::RetrieveSpeedProfile(SpeedData* const speed_data) {
  ...
  while (cur_point != nullptr) {
    ...
    speed_point.set__s(cur_point->point().s());
    speed_point.set__t(cur_point->point().t());
