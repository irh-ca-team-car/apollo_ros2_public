...
Status STObstaclesProcessor::MapObstaclesToSTBoundaries(
  ...
  for (const auto* obs_item_ptr : path_decision->obstacles().Items()) {
    ...
    if (obs_ptr->Trajectory().trajectorypoint.empty()) {
      // Obstacle is static.
...
bool STObstaclesProcessor::GetLimitingSpeedInfo(
  ...
  for (auto it : obs_id_to_decision_) {
    ...
    if (obs_decision.objecttag.yieldd != apollo_msgs::msg::ApolloplanningObjectYield() || obs_decision.objecttag.stop != apollo_msgs::msg::ApolloplanningObjectStop())
    {
      if (obs_s_min <= s_max) {
        s_max = obs_s_min;
    ...
    else if (it.second.objecttag.overtake != apollo_msgs::msg::ApolloplanningObjectOvertake())
    {
      if (obs_s_max >= s_min) {
        s_min = obs_s_max;
...
bool STObstaclesProcessor::GetSBoundsFromDecisions(
    ...
    std::vector<std::vector<std::pair<std::string, apollo_msgs::msg::ApolloplanningObjectDecisionType>>>* const
  ...
  for (const auto& obs_id_to_decision_pair : obs_id_to_decision_) {
    ...
    auto obs_st_boundary = obs_id_to_st_boundary_[obs_id];
    if (obs_decision.objecttag.overtake !=  apollo_msgs::msg::ApolloplanningObjectOvertake() &&
  ...
  for (auto s_gap : s_gaps) {
    ...
    std::vector<std::pair<std::string, apollo_msgs::msg::ApolloplanningObjectDecisionType>> obs_decisions;
...
void STObstaclesProcessor::SetObstacleDecision(
    const std::string& obs_id, const apollo_msgs::msg::ApolloplanningObjectDecisionType& obs_decision) {
  obs_id_to_decision_[obs_id] = obs_decision;
  ObjectStatus object_status;
  object_status.motiontype.motiontag.dynamic=1;
  if (obs_decision.objecttag.yieldd != apollo_msgs::msg::ApolloplanningObjectYield() || obs_decision.objecttag.stop != apollo_msgs::msg::ApolloplanningObjectStop())
  {
    obs_id_to_st_boundary_[obs_id].SetBoundaryType(
        STBoundary::BoundaryType::YIELD);
    object_status.decisiontype.objecttag.yieldd.timebuffer=0.000000001;
  }
  else if (obs_decision.objecttag.overtake != apollo_msgs::msg::ApolloplanningObjectOvertake())
  {
    obs_id_to_st_boundary_[obs_id].SetBoundaryType(
        STBoundary::BoundaryType::OVERTAKE);
    object_status.decisiontype.objecttag.overtake.timebuffer=0.000000001;
  }
  history_->mutable_history_status()->SetObjectStatus(obs_id, object_status);
}

void STObstaclesProcessor::SetObstacleDecision(
    const std::vector<std::pair<std::string, apollo_msgs::msg::ApolloplanningObjectDecisionType>>&
        obstacle_decisions) {
...
bool STObstaclesProcessor::ComputeObstacleSTBoundary(
  ...
  *is_caution_obstacle = false;
  ...
  if (obs_trajectory.trajectorypoint.empty()) {
    ...
    *obs_caution_end_t = planning_time_;
    ...
    for (const auto& obs_traj_pt : obs_trajectory.trajectorypoint) {
      ...
      if (GetOverlappingS(adc_path_points, obs_box, kADCSafetyLBuffer,
        ...
        if ((*is_caution_obstacle)) {
          if (IsSWithinADCLowRoadRightSegment(overlapping_s.first) ||
              IsSWithinADCLowRoadRightSegment(overlapping_s.second)) {
            *obs_caution_end_t = obs_traj_pt.relativetime;
          }
        }
      }
...
bool STObstaclesProcessor::GetOverlappingS(
  ...
  int pt_before_idx = GetSBoundingPathPointIndex(
      adc_path_points, obstacle_instance, vehicle_param_.frontedgetocenter,
  ...
  int pt_after_idx = GetSBoundingPathPointIndex(
      adc_path_points, obstacle_instance, vehicle_param_.backedgetocenter,
...
apollo_msgs::msg::ApolloplanningObjectDecisionType STObstaclesProcessor::DetermineObstacleDecision(
    const double obs_s_min, const double obs_s_max, const double s) const
{
  apollo_msgs::msg::ApolloplanningObjectDecisionType decision;
  if (s <= obs_s_min) {
    decision.objecttag.yieldd.set__distances(0.0);
  } else if (s >= obs_s_max) {
    decision.objecttag.overtake.set__distances(0.0);
  }
  return decision;
}
