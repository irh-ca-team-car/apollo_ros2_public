...
using ObsDecSet = std::vector<std::pair<std::string, apollo_msgs::msg::ApolloplanningObjectDecisionType>>;
}  // namespace
...
STBoundsDecider::STBoundsDecider(
  ...
  ACHECK(config.taskconfig.stboundsdeciderconfig != apollo_msgs::msg::ApolloplanningSTBoundsDeciderConfig());
  st_bounds_config_ = config.taskconfig.stboundsdeciderconfig;
}
...
Status STBoundsDecider::Process(Frame* const frame,
  ...
  apollo::planningInternal::STGraphDebug *st_graph_debug = &reference_line_info->mutable_debug()->planningdata.stgraph.emplace_back();
...
void STBoundsDecider::InitSTBoundsDecider(
  ...
  st_obstacles_processor_.Init(path_data.discretized_path().Length(),
                               st_bounds_config_.totaltime, path_data,
...
Status STBoundsDecider::GenerateFallbackSTBound(STBound* const st_bound,
  ...
  for (double curr_t = 0.0; curr_t <= st_bounds_config_.totaltime;
...
void STBoundsDecider::RecordSTGraphDebug(
    ...
    planningInternal::STGraphDebug* const st_graph_debug) {
  ...
  for (const auto& boundary : st_graph_data) {
    auto boundary_debug = &st_graph_debug->boundary.emplace_back();
    boundary_debug->set__name(boundary.id());
    if (boundary.boundary_type() == STBoundary::BoundaryType::YIELD) {
      boundary_debug->set__type(apollo::planningInternal::StGraphBoundaryDebug::StBoundaryType::ST_BOUNDARY_TYPE_YIELD);
      ADEBUG << "Obstacle ID = " << boundary.id() << ", decision = YIELD";
    } else if (boundary.boundary_type() == STBoundary::BoundaryType::OVERTAKE) {
      boundary_debug->set__type(apollo::planningInternal::StGraphBoundaryDebug::StBoundaryType::ST_BOUNDARY_TYPE_OVERTAKE);
      ADEBUG << "Obstacle ID = " << boundary.id() << ", decision = OVERTAKE";
    } else {
      boundary_debug->set__type(apollo::planningInternal::StGraphBoundaryDebug::StBoundaryType::ST_BOUNDARY_TYPE_UNKNOWN);
      ADEBUG << "Obstacle ID = " << boundary.id() << ", decision = UNKNOWN";
    }

    for (const auto& point : boundary.points()) {
      auto point_debug = &boundary_debug->point.emplace_back();
      point_debug->set__t(point.x());
      point_debug->set__s(point.y());
    }
  }

  // Plot the chosen ST boundary.
  auto boundary_debug = &st_graph_debug->boundary.emplace_back();
  boundary_debug->set__name("Generated ST-Boundary");
  boundary_debug->set__type(apollo::planningInternal::StGraphBoundaryDebug::StBoundaryType::ST_BOUNDARY_TYPE_DRIVABLE_REGION);
  for (const auto& st_bound_pt : st_bound) {
    auto point_debug = &boundary_debug->point.emplace_back();
    double t = 0.0;
    double s_lower = 0.0;
    std::tie(t, s_lower, std::ignore) = st_bound_pt;
    point_debug->set__t(t);
    point_debug->set__s(s_lower);
    ADEBUG << "(" << t << ", " << s_lower << ")";
  }
  for (int i = static_cast<int>(st_bound.size()) - 1; i >= 0; --i) {
    ...
    point_debug->set__t(t);
    point_debug->set__s(s_upper);
    ADEBUG << "(" << t << ", " << s_upper << ")";
  }

  // Plot the used st_guide_line when generating the st_bounds
  for (const auto& st_points : st_guide_line) {
    auto* speed_point = &st_graph_debug->speedprofile.emplace_back();
    speed_point->set__t(st_points.first);
    speed_point->set__s(st_points.second);
  }
}
