...
CreepDecider::CreepDecider(const apollo_msgs::msg::ApolloplanningTaskConfig& config,
                           const std::shared_ptr<DependencyInjector>& injector)
    : Decider(config, injector) {
  ACHECK(config_.taskconfig.creepdeciderconfig != apollo_msgs::msg::ApolloplanningCreepDeciderConfig());
}
...
Status CreepDecider::Process(Frame* frame,
  ...
  if (injector_->planning_context()
          ->planning_status()
          .yieldsign
          .currentyieldsignoverlapid.size() > 0) {
    yield_sign_overlap_id = injector_->planning_context()
                                ->planning_status()
                                .yieldsign
                                .currentyieldsignoverlapid.at(0);
  }
  ...
  if (stop_line_s > 0.0) {
    ...
    util::BuildStopDecision(virtual_obstacle_id, creep_stop_s,
                            config_.taskconfig.creepdeciderconfig.stopdistance,
...
bool CreepDecider::CheckCreepDone(const Frame& frame,
  ...
  const auto& creep_config = config_.taskconfig.creepdeciderconfig;
  bool creep_done = false;
  double creep_stop_s = traffic_sign_overlap_end_s +
                        FindCreepDistance(frame, reference_line_info);

  const double distance =
      creep_stop_s - reference_line_info.AdcSlBoundary().ends;
  if (distance < creep_config.maxvalidstopdistance ||
    ...
    for (auto* obstacle :
      ...
      if (obstacle->reference_line_st_boundary().min_t() <
        ...
        if (obstacle_traveled_s < kepsilon &&
            obstacle->reference_line_st_boundary().min_t() <
                creep_config.ignoremaxstmint &&
            obstacle->reference_line_st_boundary().min_s() >
                creep_config.ignoreminstmins) {
          continue;
        }
        all_far_away = false;
        break;
    ...
    auto* creep_decider_status = &injector_->planning_context()
                                          ...
                                          ->creepdecider;
    int creep_clear_counter = creep_decider_status->creepclearcounter;
    ...
    creep_decider_status->set__creepclearcounter(creep_clear_counter);
  }
