...
PathDecider::PathDecider(const apollo_msgs::msg::ApolloplanningTaskConfig &config,
...
bool PathDecider::MakeStaticObstacleDecision(
  ...
  for (const auto *obstacle : path_decision->obstacles().Items()) {
    ...
    const std::string obstacle_type_name =
        apollo::perception::PerceptionObstacle::Type_Name((apollo::perception::PerceptionObstacle::Type)obstacle->Perception().type);
    ...
    if (obstacle->HasLongitudinalDecision() &&
        obstacle->LongitudinalDecision().objecttag.ignore != 0 &&
        obstacle->HasLateralDecision() &&
        obstacle->LateralDecision().objecttag.ignore != 0)
    {
      continue;
    }
    if (obstacle->HasLongitudinalDecision() &&
        obstacle->LongitudinalDecision().objecttag.stop != apollo_msgs::msg::ApolloplanningObjectStop())
    {
      // STOP decision
      continue;
    }
    ...
    if (obstacle->Id() == blocking_obstacle_id &&
      ...
      ApolloplanningObjectDecisionType object_decision;
      object_decision.objecttag.stop = GenerateObjectStopDecision(*obstacle);
    ...
    ApolloplanningObjectDecisionType object_decision;
    object_decision.objecttag.ignore=1;
    const auto &sl_boundary = obstacle->PerceptionSLBoundary();
    if (sl_boundary.ends < frenet_path.front().s ||
        sl_boundary.starts > frenet_path.back().s) {
    ...
    double min_nudge_l =
        half_width +
        config_.taskconfig.pathdeciderconfig.staticobstaclebuffer / 2.0;

    if (curr_l - lateral_radius > sl_boundary.endl ||
        curr_l + lateral_radius < sl_boundary.startl) {
      // 1. IGNORE if laterally too far away.
      path_decision->AddLateralDecision("PathDecider/not-in-l", obstacle->Id(),
                                        object_decision);
    } else if (sl_boundary.endl >= curr_l - min_nudge_l &&
               sl_boundary.startl <= curr_l + min_nudge_l) {
      // 2. STOP if laterally too overlapping.
      object_decision.objecttag.stop = GenerateObjectStopDecision(*obstacle);

      if (path_decision->MergeWithMainStop(
              object_decision.objecttag.stop, obstacle->Id(),
      ...
      if (sl_boundary.endl < curr_l - min_nudge_l) {  // &&
        // sl_boundary.end_l() > curr_l - min_nudge_l - 0.3) {
        // LEFT_NUDGE
        apollo_msgs::msg::ApolloplanningObjectNudge *object_nudge_ptr = &object_decision.objecttag.nudge;
        object_nudge_ptr->set__type(ObjectNudge::LEFT_NUDGE);
        object_nudge_ptr->set__distancel(
            config_.taskconfig.pathdeciderconfig.staticobstaclebuffer);
        path_decision->AddLateralDecision("PathDecider/left-nudge",
                                          obstacle->Id(), object_decision);
      } else if (sl_boundary.startl > curr_l + min_nudge_l) {  // &&
        // sl_boundary.start_l() < curr_l + min_nudge_l + 0.3) {
        // RIGHT_NUDGE
        apollo_msgs::msg::ApolloplanningObjectNudge *object_nudge_ptr = &object_decision.objecttag.nudge;
        object_nudge_ptr->set__type(ObjectNudge::RIGHT_NUDGE);
        object_nudge_ptr->set__distancel(
            -config_.taskconfig.pathdeciderconfig.staticobstaclebuffer);
...
ObjectStop PathDecider::GenerateObjectStopDecision(
  ...
  object_stop.set__reasoncode(StopReasonCode::STOP_REASON_OBSTACLE);
  object_stop.set__distances(-stop_distance);

  const double stop_ref_s =
      obstacle.PerceptionSLBoundary().starts - stop_distance;
  const auto stop_ref_point =
      reference_line_info_->reference_line().GetReferencePoint(stop_ref_s);
  object_stop.stoppoint.set__x(stop_ref_point.x());
  object_stop.stoppoint.set__y(stop_ref_point.y());
  object_stop.set__stopheading(stop_ref_point.heading());
  return object_stop;
}
