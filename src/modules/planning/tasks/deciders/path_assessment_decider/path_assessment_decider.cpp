  ...
  *(reference_line_info->mutable_path_data()) = valid_path_data.front();
  ...
  if (reference_line_info->GetBlockingObstacle() != nullptr) {
    int front_static_obstacle_cycle_counter =
        mutable_path_decider_status->frontstaticobstaclecyclecounter;
    mutable_path_decider_status->set__frontstaticobstaclecyclecounter(
        std::max(front_static_obstacle_cycle_counter, 0));
    mutable_path_decider_status->set__frontstaticobstaclecyclecounter(
        std::min(front_static_obstacle_cycle_counter + 1, 10));
    mutable_path_decider_status->set__frontstaticobstacleid(
  ...
  if (reference_line_info->path_data().path_label().find("self") !=
    ...
    int able_to_use_self_lane_counter =
        mutable_path_decider_status->abletouseselflanecounter;

    if (able_to_use_self_lane_counter < 0) {
      able_to_use_self_lane_counter = 0;
    }
    mutable_path_decider_status->set__abletouseselflanecounter(
        std::min(able_to_use_self_lane_counter + 1, 10));
  } else {
    mutable_path_decider_status->set__abletouseselflanecounter(0);
  }

  // Update side-pass direction.
  if (mutable_path_decider_status->isinpathlaneborrowscenario) {
    ...
    mutable_path_decider_status->decidedsidepassdirection.clear();
    if (right_borrow) {
      mutable_path_decider_status->decidedsidepassdirection.push_back(
  ...
  if ((lhs.path_label().find("left") != std::string::npos &&
    ...
    if (blocking_obstacle) {
      ...
      const double obstacle_l =
          (blocking_obstacle->PerceptionSLBoundary().startl +
           blocking_obstacle->PerceptionSLBoundary().endl) /
          2;
...
bool PathAssessmentDecider::IsCollidingWithStaticObstacles(
  ...
  for (const auto* obstacle : indexed_obstacles.Items()) {
    ...
    if ((obstacle_sl.ends - obstacle_sl.starts) *
            (obstacle_sl.endl - obstacle_sl.startl) <
        kMinObstacleArea) {
      continue;
    }
    // Convert into polygon and save it.
    obstacle_polygons.push_back(
        Polygon2d({Vec2d(obstacle_sl.starts, obstacle_sl.startl),
                   Vec2d(obstacle_sl.starts, obstacle_sl.endl),
                   Vec2d(obstacle_sl.ends, obstacle_sl.endl),
                   Vec2d(obstacle_sl.ends, obstacle_sl.startl)}));
  }
...
bool PathAssessmentDecider::IsStopOnReverseNeighborLane(
  ...
  for (const auto& frenet_path_point : path_data.frenet_frame_path()) {
    if (std::fabs(frenet_path_point.s - check_s) < kSDelta) {
      path_point_sl.set__s(frenet_path_point.s);
      path_point_sl.set__l(frenet_path_point.l);
    }
  }
...
void PathAssessmentDecider::SetPathPointType(
  ...
  const double ego_length = vehicle_config.vehicleparam.length;
  const double ego_width = vehicle_config.vehicleparam.width;
  const double ego_back_to_center =
      vehicle_config.vehicleparam.backedgetocenter;
  ...
  for (size_t i = 0; i < discrete_path.size(); ++i) {
    ...
    double lane_right_width = 0.0;
    double middle_s =
        (ego_sl_boundary.starts + ego_sl_boundary.ends) / 2.0;
    if (reference_line_info.reference_line().GetLaneWidth(
      ...
      if (is_lane_change_path) {
        ...
        // out-of-lane.
        if (ego_sl_boundary.startl > lane_left_width ||
            ego_sl_boundary.endl < -lane_right_width) {
          // This means that ADC hasn't started lane-change yet.
          std::get<1>((*path_point_decision)[i]) =
              PathData::PathPointType::IN_LANE;
        } else if (ego_sl_boundary.startl >
                       -lane_right_width + back_to_inlane_extra_buffer &&
                   ego_sl_boundary.endl <
...
void PathAssessmentDecider::RecordDebugInfo(
  ...
  ptr_optimized_path->set__name(debug_name);
