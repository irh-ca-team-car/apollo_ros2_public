...
OpenSpaceRoiDecider::OpenSpaceRoiDecider(
  ...
  vehicle_params_ =
      apollo::common::VehicleConfigHelper::GetConfig().vehicleparam;
}
...
Status OpenSpaceRoiDecider::Process(Frame *frame) {
  ...
  const auto &roi_type = config_.taskconfig.openspaceroideciderconfig.roitype;
  if (roi_type == OpenSpaceRoiDeciderConfig::PARKING) {
    const auto &routing_request =
        frame->local_view().routing->routingrequest;

    if (routing_request.parkinginfo != apollo_msgs::msg::ApolloroutingParkingInfo() &&
        routing_request.parkinginfo.parkingspaceid!= std::string()) {
      target_parking_spot_id_ =
          routing_request.parkinginfo.parkingspaceid;
    } else {
    ...
    if (injector_->planning_context()
             ->planning_status()
             .parkandgo
             .adcinitposition== apollo_msgs::msg::ApollocommonPointENU()) {
...
void OpenSpaceRoiDecider::SetOriginFromADC(Frame *const frame,
  ...
  const auto &park_and_go_status =
      injector_->planning_context()->planning_status().parkandgo;

  const double adc_init_x = park_and_go_status.adcinitposition.x;
  const double adc_init_y = park_and_go_status.adcinitposition.y;
  const double adc_init_heading = park_and_go_status.adcinitheading;
...
void OpenSpaceRoiDecider::SetParkingSpotEndPose(
  ...
  const double parking_depth_buffer =
      config_.taskconfig.openspaceroideciderconfig.parkingdepthbuffer;
  CHECK_GE(parking_depth_buffer, 0.0);
  const bool parking_inwards =
      config_.taskconfig.openspaceroideciderconfig.parkinginwards;
  ...
  if (config_.taskconfig.openspaceroideciderconfig.parkinginwards) {
...
void OpenSpaceRoiDecider::SetParkAndGoEndPose(Frame *const frame) {
  const double kSTargetBuffer =
      config_.taskconfig.openspaceroideciderconfig.endposesdistance;
  ...
  park_and_go_status->adcadjustendpose.set__x(target_x);
  park_and_go_status->adcadjustendpose.set__y(target_y);
...
void OpenSpaceRoiDecider::GetRoadBoundary(
  ...
  double start_s =
      center_line_s -
      config_.taskconfig.openspaceroideciderconfig.roilongitudinalrangestart;
  double end_s =
      center_line_s +
      config_.taskconfig.openspaceroideciderconfig.roilongitudinalrangeend;
  ...
  while (check_point_s <= end_s) {
    ...
    bool is_center_lane_heading_change =
        ...
        config_.taskconfig.openspaceroideciderconfig.roilinesegmentminangle;
    ...
    check_point_s =
        start_s +
        index *
            config_.taskconfig.openspaceroideciderconfig.roilinesegmentlength;
...
void OpenSpaceRoiDecider::GetRoadBoundaryFromMap(
  ...
  while (check_point_s <= end_s) {
    ...
    check_point_xy.set__x(check_point.x());
    check_point_xy.set__y(check_point.y());
    ...
    check_point_s = check_point_s + config_.taskconfig.openspaceroideciderconfig
...
void OpenSpaceRoiDecider::AddBoundaryKeyPoint(
  ...
  if (std::abs(current_curb_point_delta_theta) >
      config_.taskconfig.openspaceroideciderconfig
...
bool OpenSpaceRoiDecider::GetParkingSpot(Frame *const frame,
  ...
  int next_lanes_num = nearest_lane->lane().successorid.size();
  if (next_lanes_num != 0) {
    for (int i = 0; i < next_lanes_num; ++i) {
      auto next_lane_id = nearest_lane->lane().successorid.at(i);
...
bool OpenSpaceRoiDecider::GetPullOverSpot(
  ...
  const auto &pull_over_status =
      injector_->planning_context()->planning_status().pullover;
  if (pull_over_status.position == apollo_msgs::msg::ApollocommonPointENU() || pull_over_status.theta ==0) {
  ...
  *nearby_path =
  ...
  const double pull_over_length_front = pull_over_status.lengthfront;
  const double pull_over_length_back = pull_over_status.lengthback;
  const double pull_over_width_left = pull_over_status.widthleft;
  const double pull_over_width_right = pull_over_status.widthright;
...
bool OpenSpaceRoiDecider::CheckDistanceToParkingSpot(
  ...
  if (std::abs((left_bottom_point_s + right_bottom_point_s) / 2 -
               vehicle_point_s) <
      config_.taskconfig.openspaceroideciderconfig.parkingstartrange) {
    return true;
  } 
  return false;
}
...
bool OpenSpaceRoiDecider::LoadObstacleInVertices(
  ...
  if (config_.taskconfig.openspaceroideciderconfig.enableperceptionobstacles) {
    ...
    for (const auto &obstacle : obstacles_by_frame_->Items()) {
      ...
      original_box.LongitudinalExtend(
          config_.taskconfig.openspaceroideciderconfig.perceptionobstaclebuffer);
