...
Status PathBoundsDecider::Process(
  ...
  auto* pull_over_status = &injector_->planning_context()
                               ...
                               ->pullover;
  const bool plan_pull_over_path = pull_over_status->planpulloverpath;
  ...
  if (FLAGS_enable_smarter_lane_change &&
    ...
    if (config_.taskconfig.pathboundsdeciderconfig
            .isextendlaneboundstoincludeadc) {
  ...
  if (reference_line_info->is_path_lane_borrow()) {
    const auto& path_decider_status =
        injector_->planning_context()->planning_status().pathdecider;
    for (const auto& lane_borrow_direction :
         path_decider_status.decidedsidepassdirection) {
...
void PathBoundsDecider::InitPathBoundsDecider(
  ...
  ADEBUG << "Plan at the starting point: x = "
         << planning_start_point.pathpoint.x
         << ", y = " << planning_start_point.pathpoint.y
         << ", and angle = " << planning_start_point.pathpoint.theta;
...
common::TrajectoryPoint PathBoundsDecider::InferFrontAxeCenterFromRearAxeCenter(
  ...
  double front_to_rear_axe_distance =
      VehicleConfigHelper::GetConfig().vehicleparam.wheelbase;
  common::TrajectoryPoint ret = traj_point;
  ret.pathpoint.set__x(
      traj_point.pathpoint.x +
      front_to_rear_axe_distance * std::cos(traj_point.pathpoint.theta));
  ret.pathpoint.set__y(
      traj_point.pathpoint.y +
      front_to_rear_axe_distance * std::sin(traj_point.pathpoint.theta));
  return ret;
}
...
Status PathBoundsDecider::GeneratePullOverPathBound(
  ...
  if (curr_idx < 0) {
    auto pull_over_type = pull_over_status->pullovertype;
    *pull_over_status=apollo_msgs::msg::ApolloplanningPullOverStatus();
    pull_over_status->set__pullovertype(pull_over_type);
    pull_over_status->set__planpulloverpath(true);
    ...
    pull_over_status->position.set__x(
        std::get<0>(pull_over_configuration));
    pull_over_status->position.set__y(
        std::get<1>(pull_over_configuration));
    pull_over_status->position.set__z(0.0);
    pull_over_status->set__theta(std::get<2>(pull_over_configuration));
    pull_over_status->set__lengthfront(FLAGS_obstacle_lon_start_buffer);
    pull_over_status->set__lengthback(FLAGS_obstacle_lon_end_buffer);
    pull_over_status->set__widthleft(
        VehicleConfigHelper::GetConfig().vehicleparam.width / 2.0);
    pull_over_status->set__widthright(
        VehicleConfigHelper::GetConfig().vehicleparam.width / 2.0);
...
bool PathBoundsDecider::FindDestinationPullOverS(
  ...
  reference_line.XYToSLPB(routing_end.pose, &destination_sl);
  const double destination_s = destination_sl.s;
  const double adc_end_s = reference_line_info.AdcSlBoundary().ends;

  // Check if destination is some distance away from ADC.
  ADEBUG << "Destination s[" << destination_s << "] adc_end_s[" << adc_end_s
         << "]";
  if (destination_s - adc_end_s < config_.taskconfig.pathboundsdeciderconfig
                                      .pulloverdestinationtoadcbuffer) {
  ...
  const double destination_to_pathend_buffer =
      config_.taskconfig.pathboundsdeciderconfig
          .pulloverdestinationtopathendbuffer;
...
bool PathBoundsDecider::FindEmergencyPullOverS(
  ...
  const double adjust_factor =
      config_.taskconfig.pathboundsdeciderconfig
          .pulloverapproachlondistanceadjustfactor;
...
bool PathBoundsDecider::SearchPullOverPosition(
  ...
  if (pull_over_status.pullovertype ==
  ...
  } else if (pull_over_status.pullovertype == PullOverStatus::PULL_OVER) {
  ...
  const double pull_over_space_width =
      ...
      VehicleConfigHelper::GetConfig().vehicleparam.width;
  ...
  while ((search_backward && idx >= 0 &&
    ...
    pt_sl.set__s(pt_ref_line_s);
    pt_sl.set__l(pt_ref_line_l);
    common::math::Vec2d pt_xy;
    reference_line_info.reference_line().SLToXY(pt_sl, &pt_xy);
    common::PointENU hdmap_point;
    hdmap_point.set__x(pt_xy.x());
    hdmap_point.set__y(pt_xy.y());
    ...
    while ((search_backward && j >= 0 &&
      ...
      if (curr_road_right_width - (curr_right_bound + adc_half_width) >
          config_.taskconfig.pathboundsdeciderconfig.pulloverroadedgebuffer) {
    ...
    if (is_feasible_window) {
      ...
      pull_over_sl_point.set__s(pull_over_s);
      pull_over_sl_point.set__l(pull_over_l);
...
void PathBoundsDecider::UpdatePullOverBoundaryByLaneBoundary(
  ...
  const auto& pull_over_status =
      injector_->planning_context()->planning_status().pullover;
  const auto pull_over_type = pull_over_status.pullovertype;
...
void PathBoundsDecider::GetBoundaryFromLaneChangeForbiddenZone(
  ...
  auto* lane_change_status = &injector_->planning_context()
                                 ->mutable_planning_status()
                                 ->changelane;
  if (lane_change_status->iscleartochangelane) {
    ADEBUG << "Current position is clear to change lane. No need prep s.";
    lane_change_status->set__existlanechangestartposition(false);
    return;
  }
  double lane_change_start_s = 0.0;
  if (lane_change_status->existlanechangestartposition) {
    common::SLPoint point_sl;
    reference_line.XYToSLPB(lane_change_status->lanechangestartposition,
                          &point_sl);
    ...
    lane_change_start_sl.set__s(lane_change_start_s);
    lane_change_start_sl.set__l(0.0);
    common::math::Vec2d lane_change_start_xy;
    reference_line.SLToXY(lane_change_start_sl, &lane_change_start_xy);
    lane_change_status->set__existlanechangestartposition(true);
    lane_change_status->lanechangestartposition.set__x(
        lane_change_start_xy.x());
    lane_change_status->lanechangestartposition.set__y(
...
std::vector<ObstacleEdge> PathBoundsDecider::SortObstaclesForSweepLine(
  ...
  for (const auto* obstacle : indexed_obstacles.Items()) {
    ...
    if (obstacle->PerceptionSLBoundary().ends < adc_frenet_s_) {
      continue;
    }
    ...
    sorted_obstacles.emplace_back(
        1, obstacle_sl.starts - FLAGS_obstacle_lon_start_buffer,
        obstacle_sl.startl - FLAGS_obstacle_lat_buffer,
        obstacle_sl.endl + FLAGS_obstacle_lat_buffer, obstacle->Id());
    sorted_obstacles.emplace_back(
        0, obstacle_sl.ends + FLAGS_obstacle_lon_end_buffer,
        obstacle_sl.startl - FLAGS_obstacle_lat_buffer,
        obstacle_sl.endl + FLAGS_obstacle_lat_buffer, obstacle->Id());
  }
...
bool PathBoundsDecider::UpdatePathBoundaryWithBuffer(
  ...
  double left_adc_buffer_coeff =
      (is_left_lane_bound
           ? config_.taskconfig.pathboundsdeciderconfig.adcbuffercoeff
...
void PathBoundsDecider::RecordDebugInfo(
  ...
  for (const PathBoundPoint& path_bound_point : path_boundaries) {
    ...
    frenet_frame_point.set__s(std::get<0>(path_bound_point));
    frenet_frame_point.set__dl(0.0);
    frenet_frame_point.set__ddl(0.0);

    frenet_frame_point.set__l(std::get<1>(path_bound_point));
    frenet_frame_right_boundaries.push_back(frenet_frame_point);
    frenet_frame_point.set__l(std::get<2>(path_bound_point));
  ...
  auto* ptr_display_path_1 =
      &reference_line_info->mutable_debug()->planningdata.path.emplace_back();
  ptr_display_path_1->set__name("planning_path_boundary_1");
  ptr_display_path_1->pathpoint.clear();
  for(auto i=left_path_data.discretized_path().begin();i<
       left_path_data.discretized_path().end();i++)
       ptr_display_path_1->pathpoint.push_back(*i);
  auto* ptr_display_path_2 =
      &reference_line_info->mutable_debug()->planningdata.path.emplace_back();
  ptr_display_path_2->set__name("planning_path_boundary_2");
  ptr_display_path_2->pathpoint.clear();
  for(auto i=right_path_data.discretized_path().begin();i<
       right_path_data.discretized_path().end();i++)
       ptr_display_path_2->pathpoint.push_back(*i);
}
