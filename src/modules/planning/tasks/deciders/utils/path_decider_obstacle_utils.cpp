...
bool ComputeSLBoundaryIntersection(const SLBoundary& sl_boundary,
  ...
  *ptr_l_max = -std::numeric_limits<double>::max();
  ...
  // invalid polygon
  if (sl_boundary.boundarypoint.size() < 3) {
    return false;
  }

  bool has_intersection = false;
  for (size_t i = 0; i < sl_boundary.boundarypoint.size(); ++i) {
    auto j = (i + 1) % sl_boundary.boundarypoint.size();
    const auto& p0 = sl_boundary.boundarypoint.at(i);
    const auto& p1 = sl_boundary.boundarypoint.at(j);
