...
namespace apollo
{
  namespace planning
  {
                                         ...
                                         ReferenceLineInfo *reference_line_info)
    {
          ...
          frame->reference_line_info().size() > 1)
      {
        ...
        reference_line_info->mutable_debug()
            ->planningdata.hybridmodel.set__learningmodeloutputfailreason(err_msg);
        reference_line_info->mutable_debug()
            ->planningdata.hybridmodel.set__learningmodeloutputusageratio(
      ...
      reference_line_info->mutable_debug()
          ...
          .set__usinglearningmodeloutput(true);
      // export evaluated path reference
      reference_line_info->mutable_debug()->planningdata.hybridmodel.evaluatedpathreference.pathpoint.clear();

      for (auto i = evaluated_path_reference.begin(); i < evaluated_path_reference.end(); i++)
        reference_line_info->mutable_debug()
            ->planningdata.hybridmodel
            .evaluatedpathreference.pathpoint.push_back(*i);
        ...
        std::vector<PathPoint> *path_points)
    {
      ...
      {
        if (trajectory_point.pathpoint != PathPoint())
        {
          path_points->push_back(trajectory_point.pathpoint);
        }
      }
    }
        ...
        const PathBoundary &regular_path_bound)
    {
      ...
      for (const auto path_reference_point : path_reference)
      {
        ...
        const auto &path_point = path_reference_point.pathpoint;
        Box2d vehicle_box =
        ...
        std::vector<std::vector<LineSegment2d>> *path_bound_segments)
    {
      ...
      prev_left_sl_point.set__l(std::get<0>(path_bound_points[0]));
      prev_left_sl_point.set__s(start_s);
      prev_right_sl_point.set__l(std::get<1>(path_bound_points[0]));
      prev_right_sl_point.set__s(start_s);
      ...
      for (size_t i = 1; i < path_bound_size; ++i)
      {
        ...
        left_sl_point.set__l(std::get<0>(path_bound_points[i]));
        left_sl_point.set__s(current_s);
        right_sl_point.set__l(std::get<1>(path_bound_points[i]));
        right_sl_point.set__s(current_s);
        ...
        ReferenceLineInfo *const reference_line_info)
    {
      ...
      ptr_display_path->set__name(path_name);
      ptr_display_path->pathpoint.clear();
      for (auto i = path_points.begin(); i < path_points.end(); i++)
        ptr_display_path->pathpoint.push_back(*i);
    }
