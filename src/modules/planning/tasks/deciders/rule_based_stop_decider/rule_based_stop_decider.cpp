...
RuleBasedStopDecider::RuleBasedStopDecider(
  ...
  ACHECK(config.taskconfig.rulebasedstopdeciderconfig != apollo_msgs::msg::ApolloplanningRuleBasedStopDeciderConfig());
  rule_based_stop_decider_config_ = config.taskconfig.rulebasedstopdeciderconfig;
}
...
void RuleBasedStopDecider::CheckLaneChangeUrgency(Frame *const frame) {
  for (auto &reference_line_info : *frame->mutable_reference_line_info()) {
    ...
    if (reference_line->XYToSLPB(point, &sl_point) &&
      ...
      double distance_to_passage_end =
          sl_point.s - reference_line_info.AdcSlBoundary().ends;
      // If ADC is still far from the end of routing, no need to stop, skip
      if (distance_to_passage_end >
          rule_based_stop_decider_config_.approachdistanceforlanechange) {
        continue;
      }
      ...
      util::BuildStopDecision(
          ...
          rule_based_stop_decider_config_.urgentdistanceforlanechange,
...
void RuleBasedStopDecider::StopOnSidePass(
  ...
  if (path_data.path_label().find("self") != std::string::npos) {
    check_clear = false;
    change_lane_stop_path_point = apollo::common::PathPoint();
    return;
  }
  ...
  if (!check_clear &&
    ...
    if (!LaneChangeDecider::IsPerceptionBlocked(
            *reference_line_info,
            rule_based_stop_decider_config_.searchbeamlength,
            rule_based_stop_decider_config_.searchbeamradiusintensity,
            rule_based_stop_decider_config_.searchrange,
            rule_based_stop_decider_config_.isblockanglethreshold) &&
        LaneChangeDecider::IsClearToChangeLane(reference_line_info)) {
      return;
    }
...
bool RuleBasedStopDecider::CheckADCStop(
  ...
  if (adc_speed > rule_based_stop_decider_config_.maxadcstopspeed) {
  ...
  if (distance_stop_line_to_adc_front_edge >
      rule_based_stop_decider_config_.maxvalidstopdistance) {
...
bool RuleBasedStopDecider::CheckClearDone(
  ...
  const double adc_front_edge_s = reference_line_info.AdcSlBoundary().ends;
  const double adc_back_edge_s = reference_line_info.AdcSlBoundary().starts;
  const double adc_start_l = reference_line_info.AdcSlBoundary().startl;
  const double adc_end_l = reference_line_info.AdcSlBoundary().endl;
  ...
  reference_line_info.reference_line().XYToSLPB(stop_point, &stop_sl_point);
