...
STBoundaryMapper::STBoundaryMapper(
    ...
    : speed_bounds_config_(config),
      ...
      vehicle_param_(common::VehicleConfigHelper::GetConfig().vehicleparam),
...
Status STBoundaryMapper::ComputeSTBoundary(PathDecision* path_decision) const {
  ...
  for (const auto* ptr_obstacle_item : path_decision->obstacles().Items()) {
    ...
    const apollo_msgs::msg::ApolloplanningObjectDecisionType& decision = ptr_obstacle->LongitudinalDecision();
    if (decision.objecttag.stop != apollo_msgs::msg::ApolloplanningObjectStop())
    {
      ...
      reference_line_.XYToSLPB(decision.objecttag.stop.stoppoint, &stop_sl_point);
    ...
    else if (decision.objecttag.follow != apollo_msgs::msg::ApolloplanningObjectFollow() || decision.objecttag.overtake != apollo_msgs::msg::ApolloplanningObjectOvertake() ||
             decision.objecttag.yieldd != apollo_msgs::msg::ApolloplanningObjectYield())
    {
    ...
    else if (!decision.objecttag.ignore !=0)
    {
      // 3. Ignore those unrelated obstacles.
      AWARN << "No mapping for decision: " << decision;
    }
  }
  if (stop_obstacle) {
...
bool STBoundaryMapper::MapStopDecision(
    Obstacle *stop_obstacle, const apollo_msgs::msg::ApolloplanningObjectDecisionType &stop_decision) const
{
  ACHECK(stop_decision.objecttag.stop !=apollo_msgs::msg::ApolloplanningObjectStop()) << "Must have stop decision";
  common::SLPoint stop_sl_point;
  reference_line_.XYToSLPB(stop_decision.objecttag.stop.stoppoint, &stop_sl_point);

  double st_stop_s = 0.0;
  const double stop_ref_s =
      stop_sl_point.s - vehicle_param_.frontedgetocenter;
  ...
  point_pairs.emplace_back(
      ...
      STPoint(s_max + speed_bounds_config_.boundarybuffer,
              planning_max_time_));
  auto boundary = STBoundary(point_pairs);
  boundary.SetBoundaryType(STBoundary::BoundaryType::STOP);
  boundary.SetCharacteristicLength(speed_bounds_config_.boundarybuffer);
...
bool STBoundaryMapper::GetOverlapBoundaryPoints(
  ...
  // Sanity checks.
  ACHECK(upper_points->empty());
  ACHECK(lower_points->empty());
  ...
  if (trajectory.trajectorypoint.empty()) {
    ...
    for (const auto& curr_point_on_path : path_points) {
      ...
      if (CheckOverlap(curr_point_on_path, obs_box, l_buffer)) {
        ...
        const double backward_distance = -vehicle_param_.frontedgetocenter;
    ...
    for (size_t i = 0; i < trajectory.trajectorypoint.size(); ++i) {
      const auto& trajectory_point = trajectory.trajectorypoint.at(i);
      const Box2d obs_box = obstacle.GetBoundingBox(trajectory_point);

      double trajectory_point_time = trajectory_point.relativetime;
      static constexpr double kNegtiveTimeThreshold = -1.0;
      if (trajectory_point_time < kNegtiveTimeThreshold) {
        continue;
      }

      const double step_length = vehicle_param_.frontedgetocenter;
      auto path_len =
        ...
        if (CheckOverlap(curr_adc_path_point, obs_box, l_buffer)) {
          ...
          if (find_high && find_low) {
            lower_points->emplace_back(
                low_s - speed_bounds_config_.pointextension,
                trajectory_point_time);
            upper_points->emplace_back(
                high_s + speed_bounds_config_.pointextension,
                trajectory_point_time);
          }
          break;
        }
...
void STBoundaryMapper::ComputeSTBoundaryWithDecision(
    Obstacle* obstacle, const apollo_msgs::msg::ApolloplanningObjectDecisionType& decision) const {
  ACHECK(decision.objecttag.follow != apollo_msgs::msg::ApolloplanningObjectFollow() || decision.objecttag.yieldd != apollo_msgs::msg::ApolloplanningObjectYield() ||
         decision.objecttag.overtake != apollo_msgs::msg::ApolloplanningObjectOvertake())
  ...
  if (decision.objecttag.follow != apollo_msgs::msg::ApolloplanningObjectFollow())
  {
    characteristic_length = std::fabs(decision.objecttag.follow.distances);
    b_type = STBoundary::BoundaryType::FOLLOW;
  }
  else if (decision.objecttag.yieldd != apollo_msgs::msg::ApolloplanningObjectYield())
  {
    characteristic_length = std::fabs(decision.objecttag.yieldd.distances);
  ...
  else if (decision.objecttag.overtake != apollo_msgs::msg::ApolloplanningObjectOvertake())
  {
    characteristic_length = std::fabs(decision.objecttag.overtake.distances);
    b_type = STBoundary::BoundaryType::OVERTAKE;
  }
  else
  {
    ACHECK(false) << "Obj decision should be either yield or overtake: "
                  << decision;
  }
...
bool STBoundaryMapper::CheckOverlap(const PathPoint& path_point,
  ...
  Vec2d ego_center_map_frame((vehicle_param_.frontedgetocenter -
                              vehicle_param_.backedgetocenter) *
                                 0.5,
                             (vehicle_param_.leftedgetocenter -
                              vehicle_param_.rightedgetocenter) *
                                 0.5);
