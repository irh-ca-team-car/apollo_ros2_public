...
Status SpeedLimitDecider::GetSpeedLimits(
  ...
  for (uint32_t i = 0; i < discretized_path.size(); ++i) {
    ...
    const double speed_limit_from_centripetal_acc =
        std::sqrt(speed_bounds_config_.maxcentricaccelerationlimit /
                  std::fmax(std::fabs(discretized_path.at(i).kappa),
                            speed_bounds_config_.minimalkappa));
    ...
    const double collision_safety_range =
        speed_bounds_config_.collisionsafetyrange;
    for (const auto* ptr_obstacle : obstacles.Items()) {
      if (ptr_obstacle->IsVirtual()) {
        continue;
      }
      if (ptr_obstacle->LateralDecision().objecttag.nudge == apollo_msgs::msg::ApolloplanningObjectNudge())
      {
        continue;
      }
      ...
      const double vehicle_front_s =
          reference_line_s + vehicle_param_.frontedgetocenter;
      const double vehicle_back_s =
          reference_line_s - vehicle_param_.backedgetocenter;
      const double obstacle_front_s =
          ptr_obstacle->PerceptionSLBoundary().ends;
      const double obstacle_back_s =
          ptr_obstacle->PerceptionSLBoundary().starts;
      ...
      const auto& nudge_decision = ptr_obstacle->LateralDecision().objecttag.nudge;
      ...
      bool is_close_on_left =
          ...
          (frenet_point_l - vehicle_param_.rightedgetocenter -
               collision_safety_range <
           ptr_obstacle->PerceptionSLBoundary().endl);

      // obstacle is on the left of ego vehicle (at path point i)
      bool is_close_on_right =
          (nudge_decision.type == ObjectNudge::RIGHT_NUDGE) &&
          (ptr_obstacle->PerceptionSLBoundary().startl -
               collision_safety_range <
           frenet_point_l + vehicle_param_.leftedgetocenter);
      ...
      if (is_close_on_left || is_close_on_right) {
        ...
        if (ptr_obstacle->IsStatic()) {
          nudge_speed_ratio =
              speed_bounds_config_.staticobsnudgespeedratio;
        } else {
          nudge_speed_ratio =
              speed_bounds_config_.dynamicobsnudgespeedratio;
        }
    ...
    if (FLAGS_enable_nudge_slowdown) {
      curr_speed_limit =
          std::fmax(speed_bounds_config_.lowestspeed,
