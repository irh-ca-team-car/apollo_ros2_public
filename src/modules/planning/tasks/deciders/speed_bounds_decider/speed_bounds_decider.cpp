...
SpeedBoundsDecider::SpeedBoundsDecider(
  ...
  ACHECK(config.taskconfig.speedboundsdeciderconfig != apollo_msgs::msg::ApolloplanningSpeedBoundsDeciderConfig());
  speed_bounds_config_ = config.taskconfig.speedboundsdeciderconfig;
}
...
Status SpeedBoundsDecider::Process(
  ...
  STBoundaryMapper boundary_mapper(
      ...
      path_data.discretized_path().Length(), speed_bounds_config_.totaltime,
      injector_);
  ...
  const double total_time_by_conf = speed_bounds_config_.totaltime;
  ...
  STGraphDebug *st_graph_debug = &debug->planningdata.stgraph.emplace_back();
...
void SpeedBoundsDecider::RecordSTGraphDebug(
  ...
  for (const auto &boundary : st_graph_data.st_boundaries()) {
    ...
    switch (boundary->boundary_type()) {
      case STBoundary::BoundaryType::FOLLOW:
        boundary_debug->set__type(apollo::planningInternal::StGraphBoundaryDebug::StBoundaryType::ST_BOUNDARY_TYPE_FOLLOW);
        break;
      case STBoundary::BoundaryType::OVERTAKE:
        boundary_debug->set__type(
            apollo::planningInternal::StGraphBoundaryDebug::StBoundaryType::ST_BOUNDARY_TYPE_OVERTAKE);
        break;
      case STBoundary::BoundaryType::STOP:
        boundary_debug->set__type(apollo::planningInternal::StGraphBoundaryDebug::StBoundaryType::ST_BOUNDARY_TYPE_STOP);
        break;
      case STBoundary::BoundaryType::UNKNOWN:
        boundary_debug->set__type(
            apollo::planningInternal::StGraphBoundaryDebug::StBoundaryType::ST_BOUNDARY_TYPE_UNKNOWN);
        break;
      ...
      case STBoundary::BoundaryType::KEEP_CLEAR:
        boundary_debug->set__type(
            apollo::planningInternal::StGraphBoundaryDebug::StBoundaryType::ST_BOUNDARY_TYPE_KEEP_CLEAR);
        break;
    }
  ...
  for (const auto &point : st_graph_data.speed_limit().speed_limit_points()) {
    common::SpeedPoint *speed_point = &st_graph_debug->speedlimit.emplace_back();
    speed_point->set__s(point.first);
    speed_point->set__v(point.second);
  }
}
