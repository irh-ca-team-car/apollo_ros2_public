...
{
  ACHECK(config_.taskconfig.lanechangedeciderconfig!=apollo_msgs::msg::ApolloplanningLaneChangeDeciderConfig());
}
...
Status LaneChangeDecider::Process(
  ...
  const auto& lane_change_decider_config = config_.taskconfig.lanechangedeciderconfig;
  ...
  if (lane_change_decider_config.recklesschangelane) {
  ...
  if (!has_change_lane) {
    ...
    if (prev_status->status == ChangeLaneStatus::IN_CHANGE_LANE) {
      if (prev_status->pathid == current_path_id) {
        PrioritizeChangeLane(true, reference_line_info);
      } else {
      ...
      if (now - prev_status->timestamp <
          lane_change_decider_config.changelanefailfreezetime) {
        // RemoveChangeLane(reference_line_info);
        PrioritizeChangeLane(false, reference_line_info);
      ...
      if (now - prev_status->timestamp <
          lane_change_decider_config.changelanesuccessfreezetime) {
        // RemoveChangeLane(reference_line_info);
        PrioritizeChangeLane(false, reference_line_info);
...
void LaneChangeDecider::UpdatePreparationDistance(
  ...
  auto* lane_change_status =
      &planning_context->mutable_planning_status()->changelane;
  ...
  if (is_opt_succeed) {
    lane_change_status->set__lastsucceedtimestamp(
        Clock::NowInSeconds());
    lane_change_status->set__iscurrentoptsucceed(true);
    return;
  }
  // If path optimizer or speed optimizer failed, report the status
  lane_change_status->set__iscurrentoptsucceed(false);
  // If the planner just succeed recently, let's be more patient and try again
  if (Clock::NowInSeconds() -
          lane_change_status->lastsucceedtimestamp <
...
void LaneChangeDecider::UpdateStatus(double timestamp,
  ...
  auto* lane_change_status = &injector_->planning_context()
                                 ->mutable_planning_status()
                                 ->changelane;
  lane_change_status->set__timestamp(timestamp);
  lane_change_status->set__pathid(path_id);
  lane_change_status->set__status(status_code);
}
...
void LaneChangeDecider::PrioritizeChangeLane(
  ...
  if (!lane_change_decider_config.enableprioritizechangelane) {
    return;
  }
...
void LaneChangeDecider::RemoveChangeLane(
  ...
  if (!lane_change_decider_config.enableremovechangelane) {
    return;
  }
  ADEBUG << "removed change lane";
  auto iter = reference_line_info->begin();
...
bool LaneChangeDecider::IsClearToChangeLane(
  ...
  double ego_start_s = reference_line_info->AdcSlBoundary().starts;
  double ego_end_s = reference_line_info->AdcSlBoundary().ends;
  double ego_v =
      std::abs(reference_line_info->vehicle_state().linearvelocity);
  ...
  for (const auto* obstacle :
    ...
    if (obstacle->HasTrajectory()) {
      double obstacle_moving_direction =
          obstacle->Trajectory().trajectorypoint.at(0).pathpoint.theta;
