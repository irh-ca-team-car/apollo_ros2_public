...
Status PathLaneBorrowDecider::Process(
  ...
  if (Decider::config_.taskconfig.pathlaneborrowdeciderconfig
          .allowlaneborrowing &&
...
bool PathLaneBorrowDecider::IsNecessaryToBorrowLane(
  ...
  if (mutable_path_decider_status->isinpathlaneborrowscenario) {
    ...
    if (mutable_path_decider_status->abletouseselflanecounter >= 6) {
      // If have been able to use self-lane for some time, then switch to
      // non-lane-borrowing.
      mutable_path_decider_status->set__isinpathlaneborrowscenario(false);
    ...
    ADEBUG << "Blocking obstacle ID["
           << mutable_path_decider_status->frontstaticobstacleid << "]";
    ...
    if (path_decider_status.decidedsidepassdirection.empty()) {
      ...
      if (!left_borrowable && !right_borrowable) {
        ...
        mutable_path_decider_status->set__isinpathlaneborrowscenario(true);
        if (left_borrowable) {
  ...
  return mutable_path_decider_status->isinpathlaneborrowscenario;
}
...
bool PathLaneBorrowDecider::IsBlockingObstacleWithinDestination(
  ...
  const std::string blocking_obstacle_id =
      path_decider_status.frontstaticobstacleid;
  ...
  double blocking_obstacle_s =
      blocking_obstacle->PerceptionSLBoundary().starts;
  double adc_end_s = reference_line_info.AdcSlBoundary().ends;
...
bool PathLaneBorrowDecider::IsBlockingObstacleFarFromIntersection(
  ...
  double blocking_obstacle_s =
      blocking_obstacle->PerceptionSLBoundary().ends;
  ADEBUG << "Blocking obstacle is at s = " << blocking_obstacle_s;
...
void PathLaneBorrowDecider::CheckLaneBorrow(
  ...
  *right_neighbor_lane_borrowable = true;
  ...
  double check_s = reference_line_info.AdcSlBoundary().ends;
  ...
  while (check_s < lookforward_distance) {
    ...
    if (*left_neighbor_lane_borrowable) {
      ...
      ADEBUG << "s[" << check_s << "] left_lane_boundary_type["
             << apollo::hdmap::LaneBoundaryType::Type_Name(lane_boundary_type) << "]";
    }
