...
std::vector<common::FrenetFramePoint> LateralQPOptimizer::GetFrenetFramePath()
  ...
  for (size_t i = 0; i < opt_d_.size(); ++i) {
    ...
    frenet_frame_point.set__s(accumulated_s);
    frenet_frame_point.set__l(opt_d_[i]);
    frenet_frame_point.set__dl(opt_d_prime_[i]);
    frenet_frame_point.set__ddl(opt_d_pprime_[i]);
