...
TrajectoryEvaluator::TrajectoryEvaluator(
  ...
  {
    stop_point = planning_target.stoppoint.s;
  }
...
std::vector<double> TrajectoryEvaluator::ComputeLongitudinalGuideVelocity(
  ...
  double cruise_v = planning_target.cruisespeed;
  ...
  {
    double dist_s = planning_target.stoppoint.s - init_s_[0];
    ...
    std::shared_ptr<Trajectory1d> lon_ref_trajectory =
        PiecewiseBrakingTrajectoryGenerator::Generate(
            planning_target.stoppoint.s, init_s_[0],
            planning_target.cruisespeed, init_s_[1], a_comfort, d_comfort,
