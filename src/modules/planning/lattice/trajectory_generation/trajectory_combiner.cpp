...
DiscretizedTrajectory TrajectoryCombiner::Combine(
  ...
  while (t_param < FLAGS_trajectory_time_length) {
    ...
    if (prev_trajectory_point.x!=0 && prev_trajectory_point.y!=0) {
    ...
    trajectory_point.pathpoint.set__x(x);
    trajectory_point.pathpoint.set__y(y);
    trajectory_point.pathpoint.set__s(accumulated_trajectory_s);
    trajectory_point.pathpoint.set__theta(theta);
    trajectory_point.pathpoint.set__kappa(kappa);
    trajectory_point.set__v(v);
    trajectory_point.set__a(a);
    trajectory_point.set__relativetime(t_param + init_relative_time);

    combined_trajectory.AppendTrajectoryPoint(trajectory_point);

    t_param = t_param + FLAGS_trajectory_time_resolution;

    prev_trajectory_point = trajectory_point.pathpoint;
  }
