...
double PredictionQuerier::ProjectVelocityAlongReferenceLine(
  ...
  int num_traj_point = trajectory.trajectorypoint.size();
  if (num_traj_point < 2) {
    return 0.0;
  }

  if (t < trajectory.trajectorypoint.at(0).relativetime ||
      t > trajectory.trajectorypoint.at(num_traj_point - 1).relativetime) {
    return 0.0;
  }

  auto matched_it =
      std::lower_bound(trajectory.trajectorypoint.begin(),
                       trajectory.trajectorypoint.end(), t,
                       [](const common::TrajectoryPoint& p, const double t) {
                         return p.relativetime < t;
                       });

  double v = matched_it->v;
  double theta = matched_it->pathpoint.theta;
