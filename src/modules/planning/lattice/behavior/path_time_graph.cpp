...
void PathTimeGraph::SetStaticObstacle(
  ...
  ptr_reference_line_info_->reference_line().GetLaneWidth(
      sl_boundary.starts, &left_width, &right_width);
  if (sl_boundary.starts > path_range_.second ||
      sl_boundary.ends < path_range_.first ||
      sl_boundary.startl > left_width ||
      sl_boundary.endl < -right_width) {
  ...
  path_time_obstacle_map_[obstacle_id].set_bottom_left_point(
      SetPathTimePoint(obstacle_id, sl_boundary.starts, 0.0));
  path_time_obstacle_map_[obstacle_id].set_bottom_right_point(SetPathTimePoint(
      obstacle_id, sl_boundary.starts, FLAGS_trajectory_time_length));
  path_time_obstacle_map_[obstacle_id].set_upper_left_point(
      SetPathTimePoint(obstacle_id, sl_boundary.ends, 0.0));
  path_time_obstacle_map_[obstacle_id].set_upper_right_point(SetPathTimePoint(
      obstacle_id, sl_boundary.ends, FLAGS_trajectory_time_length));
  static_obs_sl_boundaries_.push_back(std::move(sl_boundary));
  ADEBUG << "ST-Graph mapping static obstacle: " << obstacle_id
         << ", start_s : " << sl_boundary.starts
         << ", end_s : " << sl_boundary.ends
         << ", start_l : " << sl_boundary.startl
         << ", end_l : " << sl_boundary.endl;
}
  ...
  while (relative_time < time_range_.second) {
    ...
    if (path_time_obstacle_map_.find(obstacle->Id()) ==
      ...
      path_time_obstacle_map_[obstacle->Id()].set_bottom_left_point(
          SetPathTimePoint(obstacle->Id(), sl_boundary.starts,
                           relative_time));
      path_time_obstacle_map_[obstacle->Id()].set_upper_left_point(
          SetPathTimePoint(obstacle->Id(), sl_boundary.ends, relative_time));
    }

    path_time_obstacle_map_[obstacle->Id()].set_bottom_right_point(
        SetPathTimePoint(obstacle->Id(), sl_boundary.starts, relative_time));
...
void PathTimeGraph::UpdateLateralBoundsByObstacle(
  ...
  if (sl_boundary.starts > s_end || sl_boundary.ends < s_start) {
    return;
  }
  auto start_iter = std::lower_bound(
      discretized_path.begin(), discretized_path.end(), sl_boundary.starts);
  ...
  if (sl_boundary.endl > -FLAGS_numerical_epsilon &&
      sl_boundary.startl < FLAGS_numerical_epsilon) {
    ...
    for (size_t i = start_index; i < std::min(end_index + 1, bounds->size());
      ...
      bounds->operator[](i).first =
          std::max(bounds->operator[](i).first,
                   sl_boundary.endl + FLAGS_nudge_buffer);
    }
    return;
  }
  if (sl_boundary.startl > -FLAGS_numerical_epsilon) {
    for (size_t i = start_index; i < std::min(end_index + 1, bounds->size());
         ++i) {
      bounds->operator[](i).second =
          std::min(bounds->operator[](i).second,
                   sl_boundary.startl - FLAGS_nudge_buffer);
    }
    return;
  }
}
