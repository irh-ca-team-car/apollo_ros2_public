...
Crosswalk::Crosswalk(const apollo_msgs::msg::ApolloplanningTrafficRuleConfig& config,
...
Status Crosswalk::ApplyRule(Frame* const frame,
  ...
  if (!FindCrosswalks(reference_line_info)) {
    injector_->planning_context()->mutable_planning_status()->crosswalk = apollo_msgs::msg::ApolloplanningCrosswalkStatus();
    return Status::OK();
  }
...
void Crosswalk::MakeDecisions(Frame* const frame,
  ...
  double adc_front_edge_s = reference_line_info->AdcSlBoundary().ends;
  ...
  for (const auto& stop_time : mutable_crosswalk_status->stoptime) {
    stop_times.emplace(stop_time.obstacleid, stop_time.stoptimestampsec);
  }
  crosswalk_stop_timer.emplace(mutable_crosswalk_status->crosswalkid,
                               stop_times);

  const auto& finished_crosswalks =
      mutable_crosswalk_status->finishedcrosswalk;
  ...
  for (auto crosswalk_overlap : crosswalk_overlaps_) {
    ...
    if (adc_front_edge_s - crosswalk_overlap->end_s >
        config_.config.crosswalk.minpasssdistance) {
      if (mutable_crosswalk_status->crosswalkid!=std::string() &&
          mutable_crosswalk_status->crosswalkid == crosswalk_id) {
        mutable_crosswalk_status->crosswalkid=std::string();
        mutable_crosswalk_status->stoptime.clear();
      }
    ...
    for (const auto* obstacle : path_decision->obstacles().Items()) {
      ...
      const apollo_msgs::msg::ApolloperceptionPerceptionObstacle& perception_obstacle = obstacle->Perception();
      apollo::perception::PerceptionObstacle::Type obstacle_type = (apollo::perception::PerceptionObstacle::Type)perception_obstacle.type;
        ...
        if (obstacle_speed <= kMaxStopSpeed) {
          if (crosswalk_stop_timer[crosswalk_id].count(obstacle_id) < 1) {
            ...
            ADEBUG << "stop_time: obstacle_id[" << obstacle_id << "] stop_time["
                   << stop_time << "]";
            if (stop_time >= config_.config.crosswalk.stoptimeout) {
              stop = false;
            }
          }
        }
  ...
  for (auto crosswalk_to_stop : crosswalks_to_stop) {
    ...
    util::BuildStopDecision(virtual_obstacle_id, crosswalk_overlap->start_s,
                            config_.config.crosswalk.stopdistance,
                            StopReasonCode::STOP_REASON_CROSSWALK,
                            crosswalk_to_stop.second,
                            TrafficRuleConfig::RuleId_Name((TrafficRuleConfig::RuleId)config_.ruleid),
  ...
  if (firsts_crosswalk_to_stop) {
    ...
    mutable_crosswalk_status->crosswalkid=(crosswalk);
    mutable_crosswalk_status->stoptime.clear();
    for (const auto& timer : crosswalk_stop_timer[crosswalk]) {
      //a revoir
      auto* stop_time = &mutable_crosswalk_status->stoptime.emplace_back();
      stop_time->obstacleid=(timer.first);
      stop_time->stoptimestampsec=(timer.second);
    ...
    mutable_crosswalk_status->finishedcrosswalk.clear();
    for (auto crosswalk_overlap : crosswalk_overlaps_) {
      if (crosswalk_overlap->start_s < firsts_crosswalk_to_stop->start_s) {
        // a revoir
        mutable_crosswalk_status->finishedcrosswalk.push_back(
  ...
  ADEBUG << "crosswalk_status: " << *mutable_crosswalk_status;
}
...
bool Crosswalk::CheckStopForObstacle(
  ...
  const std::string& obstacle_id = obstacle.Id();
  apollo::perception::PerceptionObstacle::Type obstacle_type = (apollo::perception::PerceptionObstacle::Type)perception_obstacle.type;
  std::string obstacle_type_name = apollo::perception::PerceptionObstacle::Type_Name(obstacle_type);
  double adc_end_edge_s = reference_line_info->AdcSlBoundary().starts;

  // check type
  if (obstacle_type != apollo::perception::PerceptionObstacle::Type::PEDESTRIAN &&
      obstacle_type != apollo::perception::PerceptionObstacle::Type::BICYCLE &&
      obstacle_type != apollo::perception::PerceptionObstacle::Type::UNKNOWN_MOVABLE &&
      obstacle_type != apollo::perception::PerceptionObstacle::Type::UNKNOWN)
  {
  ...
  const Polygon2d crosswalk_exp_poly =
      crosswalk_ptr->polygon().ExpandByDistance(
          config_.config.crosswalk.expandsdistance);
  ...
  reference_line.XYToSLPB(perception_obstacle.position, &obstacle_sl_point);
  auto& obstacle_sl_boundary = obstacle.PerceptionSLBoundary();
  const double obstacle_l_distance =
      std::min(std::fabs(obstacle_sl_boundary.startl),
               std::fabs(obstacle_sl_boundary.endl));
  ...
  bool stop = false;
  if (obstacle_l_distance >= config_.config.crosswalk.stoplooseldistance) {
    ...
    if (is_path_cross) {
      ...
      ADEBUG << "need_stop(>=l2): obstacle_id[" << obstacle_id << "] type["
             ...
             config_.config.crosswalk.stopstrictldistance) {
    if (is_on_road) {
      ...
      if (is_path_cross) {
        ...
        const auto adc_path_point =
            Vec2d(injector_->ego_info()->start_point().pathpoint.x,
                  injector_->ego_info()->start_point().pathpoint.y);
  ...
  if (stop) {
    if (stop_deceleration >= config_.config.crosswalk.maxstopdeceleration) {
      if (obstacle_l_distance > config_.config.crosswalk.stopstrictldistance) {
