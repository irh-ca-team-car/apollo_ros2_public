...
StopSign::StopSign(const apollo_msgs::msg::ApolloplanningTrafficRuleConfig& config,
...
void StopSign::MakeDecisions(Frame* const frame,
  ...
  if (!config_.config.stopsign.enabled) {
    return;
  }

  const auto &stop_sign_status =
      injector_->planning_context()->planning_status().stopsign;
  ...
  for (const auto& stop_sign_overlap : stop_sign_overlaps) {
    ...
    if (stop_sign_overlap.object_id ==
        stop_sign_status.donestopsignoverlapid) {
      continue;
    }
    ...
    const std::vector<std::string> wait_for_obstacle_ids(
        stop_sign_status.waitforobstacleid.begin(),
        stop_sign_status.waitforobstacleid.end());
    util::BuildStopDecision(virtual_obstacle_id, stop_sign_overlap.start_s,
                            config_.config.stopsign.stopdistance,
