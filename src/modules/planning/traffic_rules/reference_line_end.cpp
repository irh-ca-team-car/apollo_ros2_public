...
Status ReferenceLineEnd::ApplyRule(
  ...
  // check
  double remain_s =
      reference_line.Length() - reference_line_info->AdcSlBoundary().ends;
  if (remain_s >
      config_.config.referencelineend.minreferencelineremainlength) {
    return Status::OK();
  }
  ...
  const double stop_line_s =
      obstacle_start_s - config_.config.referencelineend.stopdistance;
  auto stop_point = reference_line.GetReferencePoint(stop_line_s);

  apollo_msgs::msg::ApolloplanningObjectDecisionType stop;
  auto stop_decision = &stop.objecttag.stop;
  stop_decision->reasoncode=(StopReasonCode::STOP_REASON_DESTINATION);
  stop_decision->distances=(-config_.config.referencelineend.stopdistance);
  stop_decision->stopheading=(stop_point.heading());
  stop_decision->stoppoint.x=(stop_point.x());
  stop_decision->stoppoint.y=(stop_point.y());
  stop_decision->stoppoint.z=(0.0);

  auto *path_decision = reference_line_info->path_decision();
  path_decision->AddLongitudinalDecision(
      TrafficRuleConfig::RuleId_Name((TrafficRuleConfig::RuleId)config_.ruleid), stop_wall->Id(), stop);
