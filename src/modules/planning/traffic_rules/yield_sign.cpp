...
YieldSign::YieldSign(const apollo_msgs::msg::ApolloplanningTrafficRuleConfig& config,
...
void YieldSign::MakeDecisions(Frame* const frame,
  ...
  if (!config_.config.yieldsign.enabled) {
    return;
  }
  ...
  for (const auto& yield_sign_overlap : yield_sign_overlaps) {
    ...
    for (const auto& done_yield_sign_overlap_id :
         yield_sign_status.doneyieldsignoverlapid) {
    ...
    const std::vector<std::string> wait_for_obstacle_ids(
        yield_sign_status.waitforobstacleid.begin(),
        yield_sign_status.waitforobstacleid.end());
    util::BuildStopDecision(virtual_obstacle_id, yield_sign_overlap.start_s,
                            config_.config.yieldsign.stopdistance,
