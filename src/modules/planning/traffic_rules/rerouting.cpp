...
Rerouting::Rerouting(const apollo_msgs::msg::ApolloplanningTrafficRuleConfig& config,
...
bool Rerouting::ChangeLaneFailRerouting() {
  ...
  if (!reference_line.XYToSLPB(point, &sl_point)) {
    AERROR << "Failed to project point: " << point;
    return false;
  }
  ...
  // * speed, no rerouting
  double adc_s = reference_line_info_->AdcSlBoundary().ends;
  ...
  const double prepare_rerouting_time =
      config_.config.rerouting.preparereroutingtime;
  ...
  auto *rerouting = &injector_->planning_context()->mutable_planning_status()->rerouting;
  if (rerouting == nullptr) {
    AERROR << "rerouting is nullptr.";
    return false;
  }
  double current_time = Clock::NowInSeconds();
  if (rerouting->lastreroutingtime!=0 &&
      ((current_time - rerouting->lastreroutingtime)<
       config_.config.rerouting.cooldowntime)) {
  ...
  rerouting->lastreroutingtime=(current_time);
  return true;
}
