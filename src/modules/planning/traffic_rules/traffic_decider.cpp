...
apollo::common::util::Factory<
    ...
    TrafficRule *(*)(const apollo_msgs::msg::ApolloplanningTrafficRuleConfig &config,
...
void TrafficDecider::RegisterRules() {
  s_rule_factory.Register(
      ...
      [](const apollo_msgs::msg::ApolloplanningTrafficRuleConfig &config,
...
void TrafficDecider::BuildPlanningTarget(
  ...
  apollo_msgs::msg::ApolloplanningStopPoint stop_point;
  for (const auto *obstacle :
    ...
    if (obstacle->IsVirtual() && obstacle->HasLongitudinalDecision() &&
        ...
        obstacle->PerceptionSLBoundary().starts < min_s)
    {
      min_s = obstacle->PerceptionSLBoundary().starts;
      ...
      if (stop_code == StopReasonCode::STOP_REASON_DESTINATION ||
        ...
        stop_point.type=(StopPoint::HARD);
        ADEBUG << "Hard stop at: " << min_s
               << "REASON: " << StopReasonCode_Name((apollo::planning::StopReasonCode)stop_code);
      } else if (stop_code == StopReasonCode::STOP_REASON_YELLOW_SIGNAL) {
        stop_point.type=(StopPoint::SOFT);
  ...
  if (min_s != std::numeric_limits<double>::infinity()) {
    ...
    stop_point.s=(min_s - front_edge_to_center +
...
Status TrafficDecider::Execute(
  ...
  for (const auto &rule_config : rule_configs_.config) {
    if (!rule_config.enabled) {
      ADEBUG << "Rule " << rule_config.ruleid << " not enabled";
      continue;
    }
    auto rule = s_rule_factory.CreateObject((apollo::planning::TrafficRuleConfig::RuleId)rule_config.ruleid, rule_config,
                                            injector);
    if (!rule) {
      AERROR << "Could not find rule " << rule_config;
      continue;
    }
    rule->ApplyRule(frame, reference_line_info);
    ADEBUG << "Applied rule "
           << TrafficRuleConfig::RuleId_Name((TrafficRuleConfig::RuleId)rule_config.ruleid);
  }
