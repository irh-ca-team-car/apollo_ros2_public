...
TrafficLight::TrafficLight(const apollo_msgs::msg::ApolloplanningTrafficRuleConfig &config,
...
void TrafficLight::MakeDecisions(Frame* const frame,
  ...
  if (!config_.config.trafficlight.enabled) {
    return;
  }

  const auto& traffic_light_status =
      injector_->planning_context()->planning_status().trafficlight;
  ...
  // debug info
  auto *signal_light_debug =
      &reference_line_info->mutable_debug()->planningdata.signallight;
  signal_light_debug->adcfronts=(adc_front_edge_s);
  signal_light_debug->adcspeed=(
  ...
  for (const auto& traffic_light_overlap : traffic_light_overlaps) {
    ...
    for (const auto& done_traffic_light_overlap_id :
         traffic_light_status.donetrafficlightoverlapid) {
    ...
    traffic_light_sl.s=(traffic_light_overlap.start_s);
    traffic_light_sl.l=(0);
    ...
    const double distance =
        common::util::DistanceXYPB(traffic_light_point, adc_position);
    ...
    // debug info
    // a revoir
    planningInternal::SignalDebug* signal_debug =
        &signal_light_debug->signal.emplace_back();
    signal_debug->adcstopdeceleration=(stop_deceleration);
    signal_debug->color=(signal_color);
    signal_debug->lightid=(traffic_light_overlap.object_id);
    signal_debug->lightstops=(traffic_light_overlap.start_s);
    ...
    if (stop_deceleration > config_.config.trafficlight.maxstopdeceleration) {
    ...
    util::BuildStopDecision(virtual_obstacle_id, traffic_light_overlap.start_s,
                            config_.config.trafficlight.stopdistance,
