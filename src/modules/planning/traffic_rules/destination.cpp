...
Destination::Destination(const apollo_msgs::msg::ApolloplanningTrafficRuleConfig& config,
...
int Destination::MakeDecisions(Frame* frame,
  ...
  if (adc_sl.starts > dest_sl.s && !dest.haspasseddestination) {
        ...
        pull_over_status.position.y != 0)
    {
      ...
      const double stop_line_s = pull_over_sl.s +
                                 VehicleConfigHelper::GetConfig()
                                     .vehicleparam
                                     .frontedgetocenter +
                                 config_.config.destination.stopdistance;
      util::BuildStopDecision(
          stop_wall_id, stop_line_s, config_.config.destination.stopdistance,
          StopReasonCode::STOP_REASON_PULL_OVER, wait_for_obstacle_ids,
          TrafficRuleConfig::RuleId_Name((TrafficRuleConfig::RuleId)config_.ruleid), frame,
          reference_line_info);
      return 0;
    }
  }
