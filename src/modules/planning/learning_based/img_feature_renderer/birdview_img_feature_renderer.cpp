...
static const char ROADMAP_IMG_PATH[] =
    "data/planning/semantic_map/sunnyvale_with_two_offices.png";
static const char SPEEDLIMITMAP_IMG_PATH[] =
    "data/planning/semantic_map/"
...
bool BirdviewImgFeatureRenderer::RenderMultiChannelEnv(
  ...
  const auto& current_traj_point = learning_data_frame.adctrajectorypoint.at(
      ego_trajectory_point_history_size - 1);
  const double current_time_sec = current_traj_point.timestampsec;
  const auto& current_path_point =
      current_traj_point.trajectorypoint.pathpoint;
...
bool BirdviewImgFeatureRenderer::RenderEgoCurrentPoint(
  ...
  (*img_feature).at<uchar>(config_.egoidxy, config_.egoidxx) = color[0];
  return true;
}
...
bool BirdviewImgFeatureRenderer::RenderEgoCurrentBox(
  ...
  const auto& param = ego_vehicle_config_.vehicleparam;
  std::vector<cv::Point2i> box_corner_points =
      GetAffinedBoxImgIdx(0.0, 0.0, M_PI_2,
                          {
                              std::make_pair(param.frontedgetocenter,
                                             param.leftedgetocenter),
                              std::make_pair(param.frontedgetocenter,
                                             -param.rightedgetocenter),
                              std::make_pair(-param.backedgetocenter,
                                             -param.rightedgetocenter),
                              std::make_pair(-param.backedgetocenter,
                                             param.leftedgetocenter),
                          },
...
bool BirdviewImgFeatureRenderer::RenderEgoPastPoint(
  ...
  const auto& ego_past = learning_data_frame.adctrajectorypoint;
  cv::Scalar gradual_change_color;
  for (const auto& ego_past_point : ego_past) {
    const double relative_time =
        current_time_sec - ego_past_point.timestampsec;
    if (relative_time > config_.maxegopasthorizon) {
      continue;
    }
    gradual_change_color =
        color * (1 - relative_time / config_.maxegopasthorizon);
    cv::circle(*img_feature,
               GetAffinedPointImgIdx(
                   ego_past_point.trajectorypoint.pathpoint.x,
                   ego_past_point.trajectorypoint.pathpoint.y,
...
bool BirdviewImgFeatureRenderer::RenderObsPastBox(
  ...
  for (const auto& obstacle : learning_data_frame.obstacle) {
    if (obstacle.obstacletrajectory.evaluatedtrajectorypoint.size() == 0) {
      continue;
    }
    const double obstacle_box_length = obstacle.length;
    const double obstacle_box_width = obstacle.width;
    const auto& past_traj =
        obstacle.obstacletrajectory.evaluatedtrajectorypoint;
    cv::Scalar gradual_change_color;
    for (const auto& traj_point : past_traj) {
      const double relative_time =
          current_time_sec - traj_point.timestampsec;
      if (relative_time > config_.maxobspasthorizon) {
        continue;
      }
      gradual_change_color =
          color * (1 - relative_time / config_.maxobspasthorizon);
      const auto& path_point = traj_point.trajectorypoint.pathpoint;
...
bool BirdviewImgFeatureRenderer::RenderObsFutureBox(
  ...
  for (const auto& obstacle : learning_data_frame.obstacle) {
    ...
    if (obstacle.obstacleprediction.isstatic) {
      const auto &past_traj_points_size =
          obstacle.obstacletrajectory.evaluatedtrajectorypoint.size();
      ...
      const auto& last_past_traj_point =
          obstacle.obstacletrajectory.evaluatedtrajectorypoint.at(
              past_traj_points_size - 1);
      const auto& path_point =
          last_past_traj_point.trajectorypoint.pathpoint;
      std::vector<cv::Point2i> box_corner_points = GetAffinedBoxImgIdx(
          path_point.x, path_point.y, M_PI_2 + path_point.theta,
          {
              std::make_pair(obstacle_box_length / 2, obstacle_box_width / 2),
    ...
    const auto& predicted_traj_size =
        obstacle.obstacleprediction.trajectory.size();
    ...
    int max_prob = 0;
    int max_prob_idx = 0;
    for (size_t i = 0; i < predicted_traj_size; ++i) {
      const auto& trajectory = obstacle.obstacleprediction.trajectory.at(i);
    ...
    const auto& predicted_traj = obstacle.obstacleprediction
                                     .trajectory.at(max_prob_idx)
                                     .trajectorypoint;
    cv::Scalar gradual_change_color;
    for (const auto& traj_point : predicted_traj) {
      const double relative_time =
          traj_point.timestampsec - current_time_sec;
      if (relative_time > config_.maxobsfuturehorizon) {
        break;
      }
      gradual_change_color =
          color * relative_time / config_.maxobspasthorizon;
      const auto& path_point = traj_point.trajectorypoint.pathpoint;
      std::vector<cv::Point2i> box_corner_points = GetAffinedBoxImgIdx(
          path_point.x, path_point.y, M_PI_2 + path_point.theta,
          {
...
bool BirdviewImgFeatureRenderer::RenderTrafficLight(
  ...
  for (const auto& traffic_light_status :
       learning_data_frame.trafficlightdetection.trafficlight) {
    ...
    for (const auto& overlap_id : traffic_light->signal().overlapid) {
      const auto& overlap =
      ...
      for (const auto& overlap_object : overlap->overlap().object) {
        if (overlap_object.overlapinfo.laneoverlapinfo == apollo_msgs::msg::ApollohdmapLaneOverlapInfo())
        {
          continue;
        }
        ...
        for (const auto& segment : lane->lane().centralcurve.segment) {
          const int segment_point_size = segment.curvetype.linesegment.point.size();
          for (int i = 0; i < segment_point_size - 1; ++i) {
            const auto &p0 = GetAffinedPointImgIdx(
                segment.curvetype.linesegment.point.at(i).x,
                segment.curvetype.linesegment.point.at(i).y, ego_current_x,
                ego_current_y, M_PI_2 - ego_current_heading);
            const auto &p1 = GetAffinedPointImgIdx(
                segment.curvetype.linesegment.point.at(i + 1).x,
                segment.curvetype.linesegment.point.at(i + 1).y, ego_current_x,
                ego_current_y, M_PI_2 - ego_current_heading);
...
bool BirdviewImgFeatureRenderer::RenderRouting(
  ...
  const int routing_lanes_size =
      learning_data_frame.routing.localroutinglaneid.size();
  ...
  for (int i = 0; i < routing_lanes_size; ++i) {
    ...
    const auto& lane =
        apollo::hdmap::HDMapUtil::BaseMap().GetLaneById(hdmap::MakeMapId(
            learning_data_frame.routing.localroutinglaneid.at(i)));
    if (lane == nullptr) {
      AERROR << "lane ["
             << learning_data_frame.routing.localroutinglaneid.at(i)
             << "] not found";
    ...
    for (const auto& segment : lane->lane().centralcurve.segment) {
      ...
      for (int i = 0; i < segment_point_size - 1; ++i) {
        const auto& p0 = GetAffinedPointImgIdx(
            segment.curvetype.linesegment.point.at(i).x,
            segment.curvetype.linesegment.point.at(i).y, ego_current_x, ego_current_y,
...
bool BirdviewImgFeatureRenderer::CropByPose(const double ego_x,
  ...
  cv::Rect fine_rect(rough_radius - config_.egoidxx,
                     rough_radius - config_.egoidxy, config_.height,
                     config_.width);
...
cv::Point2i BirdviewImgFeatureRenderer::GetAffinedPointImgIdx(
  ...
  return GetPointImgIdx(rotated_local_x, rotated_local_y, config_.egoidxx,
                        config_.egoidxy);
}
