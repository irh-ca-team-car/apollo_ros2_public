...
TrajectoryImitationLibtorchInference::TrajectoryImitationLibtorchInference(
    const apollo_msgs::msg::ApolloplanningLearningModelInferenceTaskConfig& config)
...
bool TrajectoryImitationLibtorchInference::LoadModel() {
  //if (config_.usecuda && torch::cuda::is_available()) {
  if (config_.usecuda) {
    ADEBUG << "CUDA is available";
    device_ = torch::Device(torch::kCUDA);
    try {
      model_ = torch::jit::load(config_.gpumodelfile, device_);
    } catch (const c10::Error& e) {
      AERROR << "Failed to load model on to device";
      return false;
    }
  } else {
    model_ = torch::jit::load(config_.cpumodelfile, device_);
  }
...
void TrajectoryImitationLibtorchInference::output_postprocessing(
  ...
  const int past_points_size = learning_data_frame->adctrajectorypoint.size();
  const auto& cur_traj_point =
      learning_data_frame->adctrajectorypoint.at(past_points_size - 1);
  const double cur_time_sec = cur_traj_point.timestampsec;
  const auto& cur_path_point = cur_traj_point.trajectorypoint.pathpoint;
  ...
  ADEBUG << "cur_x[" << cur_x << "], cur_y[" << cur_y << "], cur_heading["
         << cur_heading << "], cur_v[" << cur_traj_point.trajectorypoint.v
         << "]";

  learning_data_frame->output.adcfuturetrajectorypoint.clear();
  const double delta_t = config_.trajectorydeltat;
  ...
  for (int i = 0; i < torch_output_tensor.size(1); ++i) {
    ...
    auto* traj_point =   &learning_data_frame->output
                           .adcfuturetrajectorypoint.emplace_back();
    traj_point->set__timestampsec(time_sec);
    traj_point->trajectorypoint.pathpoint.set__x(x);
    traj_point->trajectorypoint.pathpoint.set__y(y);
    traj_point->trajectorypoint.pathpoint.set__theta(
        heading);
    traj_point->trajectorypoint.set__v(v);
    traj_point->trajectorypoint.set__relativetime(delta_t *
                                                              (i + 1));
  }
}
