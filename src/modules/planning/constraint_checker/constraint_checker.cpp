...
ConstraintChecker::Result ConstraintChecker::ValidTrajectory(
  ...
  for (const auto& p : trajectory) {
    ...
    double kappa = p.pathpoint.kappa;
  ...
  for (size_t i = 1; i < trajectory.NumOfPoints(); ++i) {
    ...
    if (p1.relativetime > kMaxCheckRelativeTime) {
      break;
    }

    double t = p0.relativetime;

    double dt = p1.relativetime - p0.relativetime;
    ...
    double lat_a = p1.v * p1.v * p1.pathpoint.kappa;
