...
bool CollisionChecker::InCollision(
  ...
  for (size_t i = 0; i < ego_trajectory.NumOfPoints(); ++i) {
    ...
    const auto relative_time = ego_point.relativetime;
    const auto ego_theta = ego_point.pathpoint.theta;

    Box2d ego_box({ego_point.pathpoint.x, ego_point.pathpoint.y},
...
bool CollisionChecker::IsObstacleBehindEgoVehicle(
  ...
  auto obstacle_reference_line_position = PathMatcher::GetPathFrenetCoordinate(
      discretized_reference_line, point.pathpoint.x,
      point.pathpoint.y);
