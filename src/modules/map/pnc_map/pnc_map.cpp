...
LaneSegment PncMap::ToLaneSegment(const routing::LaneSegment &segment) const {
  ...
  return LaneSegment(lane, segment.starts, segment.ends);
}
...
std::vector<routing::LaneWaypoint> PncMap::FutureRouteWaypoints() const {
  const auto &waypoints = routing_.routingrequest.waypoint;
...
bool PncMap::UpdateRoutingResponse(const routing::RoutingResponse &routing) {
  ...
  all_lane_ids_.clear();
  for (size_t road_index = 0; road_index < routing.road.size(); ++road_index)
  {
    const auto &road_segment = routing.road[road_index];
    for (size_t passage_index = 0; passage_index < road_segment.passage.size();
         ++passage_index)
    {
      const auto &passage = road_segment.passage[passage_index];
      for (size_t lane_index = 0; lane_index < passage.segment.size();
           ++lane_index) {
        ...
        route_indices_.back().index = {(int)road_index, (int)passage_index, (int)lane_index};
      }
    }
  }
  ...
  const auto &request_waypoints = routing.routingrequest.waypoint;
...
bool PncMap::ValidateRouting(const RoutingResponse &routing) {
  ...
  if (routing.routingrequest == apollo_msgs::msg::ApolloroutingRoutingRequest() ||
      routing.routingrequest.waypoint.size() < 2)
  {
    AERROR << "Routing does not have request.";
    return false;
  }
  for (const auto &waypoint : routing.routingrequest.waypoint) {
    if (waypoint.id == std::string() || waypoint.s == 0) {
...
bool PncMap::PassageToSegments(routing::Passage passage,
  ...
  for (const auto &lane : passage.segment) {
    ...
    segments->emplace_back(lane_ptr, std::max(0.0, lane.starts),
                           std::min(lane_ptr->total_length(), lane.ends));
  }
...
std::vector<int> PncMap::GetNeighborPassages(const routing::RoadSegment &road,
  ...
  CHECK_LE((size_t)start_passage, road.passage.size());
  std::vector<int> result;
  const auto &source_passage = road.passage.at(start_passage);
  result.emplace_back(start_passage);
  if (source_passage.changelanetype == routing::FORWARD)
  {
    return result;
  }
  if (source_passage.canexit) {  // No need to change lane
    return result;
  }
  ...
  if (source_passage.changelanetype == routing::LEFT)
  {
    for (const auto &segment : source_segments) {
      for (const auto &left_id :
           segment.lane->lane().leftneighborforwardlaneid) {
        neighbor_lanes.insert(left_id.id);
      }
    }
  }
  else if (source_passage.changelanetype == routing::RIGHT)
  {
    for (const auto &segment : source_segments) {
      for (const auto &right_id :
           segment.lane->lane().rightneighborforwardlaneid) {
        neighbor_lanes.insert(right_id.id);
      }
    }
  }

  for (size_t i = 0; i < road.passage.size(); ++i) {
    if (i == (size_t)start_passage)
    {
      continue;
    }
    const auto &target_passage = road.passage.at(i);
...
bool PncMap::GetRouteSegments(const VehicleState &vehicle_state,
  ...
  double look_forward_distance =
      LookForwardDistance(vehicle_state.linearvelocity);
...
bool PncMap::GetRouteSegments(const VehicleState &vehicle_state,
  ...
  const auto &road = routing_.road.at(road_index);
  // Raw filter to find all neighboring passages
  auto drive_passages = GetNeighborPassages(road, passage_index);
  for (const int index : drive_passages) {
    const auto &passage = road.passage.at(index);
    ...
    route_segments->back().SetCanExit(passage.canexit);
    route_segments->back().SetNextAction((apollo::routing::ChangeLaneType)passage.changelanetype);
...
LaneInfoConstPtr PncMap::GetRouteSuccessor(LaneInfoConstPtr lane) const {
  if (lane->lane().successorid.empty()) {
    return nullptr;
  }
  hdmap::Id preferred_id = lane->lane().successorid.at(0);
  for (const auto &lane_id : lane->lane().successorid) {
...
LaneInfoConstPtr PncMap::GetRoutePredecessor(LaneInfoConstPtr lane) const {
  if (lane->lane().predecessorid.empty()) {
    return nullptr;
  }

  std::unordered_set<std::string> predecessor_ids;
  for (const auto &lane_id : lane->lane().predecessorid) {
    predecessor_ids.insert(lane_id.id);
  }

  hdmap::Id preferred_id = lane->lane().predecessorid.at(0);
...
bool PncMap::ExtendSegments(const RouteSegments &segments,
  ...
  if (!segments.GetProjection(point, &sl, &waypoint)) {
    AERROR << "point: " << point << " is not on segment";
    return false;
  }
