...
LaneBoundaryType::Type LeftBoundaryType(const LaneWaypoint& waypoint) {
  ...
  for (const auto& type :
       waypoint.lane->lane().leftboundary.boundarytype) {
    if (type.s <= waypoint.s) {
      if (type.types.size() > 0) {
        return (LaneBoundaryType::Type)type.types[0];
      } else {
        return LaneBoundaryType::UNKNOWN;
      }
    }
...
LaneBoundaryType::Type RightBoundaryType(const LaneWaypoint& waypoint) {
  ...
  for (const auto& type :
       waypoint.lane->lane().rightboundary.boundarytype) {
    if (type.s <= waypoint.s) {
      if (type.types.size() > 0) {
        return (LaneBoundaryType::Type)type.types[0];
      } else {
...
LaneWaypoint LeftNeighborWaypoint(const LaneWaypoint& waypoint) {
  ...
  for (const auto& lane_id :
       waypoint.lane->lane().leftneighborforwardlaneid) {
...
LaneWaypoint RightNeighborWaypoint(const LaneWaypoint& waypoint) {
  ...
  CHECK_NOTNULL(map_ptr);
  for (const auto& lane_id :
       waypoint.lane->lane().rightneighborforwardlaneid) {
    auto lane = map_ptr->GetLaneById(lane_id);
    if (!lane) {
      return neighbor;
    }
...
void Path::InitPoints() {
  ...
  CHECK_EQ(accumulated_s_.size(), (size_t)num_points_);
  CHECK_EQ(unit_directions_.size(), (size_t)num_points_);
  CHECK_EQ(segments_.size(), (size_t)num_segments_);
}
...
void Path::InitLaneSegments() {
  ...
  CHECK_EQ(lane_segments_to_next_point_.size(), (size_t)num_segments_);
}
...
void Path::InitWidth() {
  ...
  CHECK_EQ(lane_left_width_.size(), (size_t)num_sample_points_);
  CHECK_EQ(lane_right_width_.size(), (size_t)num_sample_points_);

  CHECK_EQ(road_left_width_.size(), (size_t)num_sample_points_);
  CHECK_EQ(road_right_width_.size(), (size_t)num_sample_points_);
}
...
void Path::InitPointIndex() {
  ...
  CHECK_EQ(last_point_index_.size(), (size_t)num_sample_points_);
}
...
void Path::GetAllOverlaps(GetOverlapFromLaneFunc GetOverlaps_from_lane,
  ...
  for (const auto& lane_segment : lane_segments_) {
    ...
    for (const auto& overlap : GetOverlaps_from_lane(*(lane_segment.lane))) {
      ...
      const auto& lane_overlap_info = overlap_info->overlapinfo;
      if (lane_overlap_info.laneoverlapinfo.starts <= lane_segment.end_s &&
          lane_overlap_info.laneoverlapinfo.ends >= lane_segment.start_s) {
        const double ref_s = s - lane_segment.start_s;
        const double adjusted_start_s =
            std::max(lane_overlap_info.laneoverlapinfo.starts, lane_segment.start_s) + ref_s;
        const double adjusted_end_s =
            std::min(lane_overlap_info.laneoverlapinfo.ends, lane_segment.end_s) + ref_s;
...
void PathApproximation::InitProjections(const Path& path) {
  ...
  CHECK_EQ(sampled_max_original_projections_to_left_.size(),
           (size_t)num_projection_samples_);
}
