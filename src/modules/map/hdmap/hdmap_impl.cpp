...
int HDMapImpl::LoadMapFromFile(const std::string& map_filename) {
  ...
  else if (std::filesystem::exists(map_filename))
  {
    if (!cyber::common::GetProtoFromASCIIFile(map_filename, &map_))
    {
      return -1;
    }
  }
...
int HDMapImpl::LoadMapFromProto(const Map& map_proto) {
  ...
  for (const auto& road_ptr_pair : road_table_) {
    ...
    for (const auto& road_section : road_ptr_pair.second->sections()) {
      ...
      for (const auto& lane_id : road_section.laneid) {
    ...
    if (road_ptr->has_junction_id()) {
      ...
      for (const auto& section : road_ptr->sections()) {
        if (section.id.id == section_id.id) {
          road_boundary_ptr->roadboundaries.push_back(section.boundary);
        }
      }
    ...
    if (road_ptr->has_junction_id()) {
      ...
      for (const auto& temp_road_boundary : temp_road_boundaries) {
        apollo::hdmap::BoundaryPolygon boundary_polygon =
            temp_road_boundary.outerpolygon;
        for (const auto& edge : boundary_polygon.edge) {
          if (edge.type == apollo::hdmap::BoundaryEdge::LEFT_BOUNDARY) {
            for (const auto& s : edge.curve.segment) {
              for (const auto& p : s.curvetype.linesegment.point) {
        ...
        if (temp_road_boundary.hole.size() != 0) {
...
int HDMapImpl::GetRoi(const apollo::common::PointENU& point, double radius,
  ...
  for (const auto& lane_ptr : lanes) {
    ...
    for (const auto& overlap_id : lane_ptr->lane().overlapid) {
      OverlapInfoConstPtr overlap_ptr = GetOverlapById(overlap_id);
      for (size_t i = 0; i < overlap_ptr->overlap().object.size(); ++i)
      {
...
int HDMapImpl::GetForwardNearestSignalsOnLane(
  ...
  while (s < back_distance) {
    for (const auto& predecessor_lane_id : lane_ptr->lane().predecessorid) {
  ...
  while (lane_ptr != nullptr) {
      ...
      for (size_t i = 0; i < overlap_ptr->overlap().object.size(); ++i) {
        if (overlap_ptr->overlap().object[i].id.id == lane_ptr->id().id) {
          lane_overlap_offset_s =
              overlap_ptr->overlap().object[i].overlapinfo.laneoverlapinfo.starts -
              s_start;
          continue;
        }
    ...
    for (const auto& successor_lane_id : lane_ptr->lane().successorid) {
...
int HDMapImpl::GetLocalMap(const apollo::common::PointENU& point,
  ...
  for (auto& lane_ptr : lanes) {
    ...
    std::copy(lane_ptr->lane().overlapid.begin(),
              lane_ptr->lane().overlapid.end(),
              std::back_inserter(overlap_ids));
    local_map->lane.push_back(lane_ptr->lane());
  }

  for (auto& crosswalk_ptr : crosswalks) {
    map_element_ids.insert(crosswalk_ptr->id().id);
    std::copy(crosswalk_ptr->crosswalk().overlapid.begin(),
              crosswalk_ptr->crosswalk().overlapid.end(),
              std::back_inserter(overlap_ids));
    local_map->crosswalk.push_back(crosswalk_ptr->crosswalk());
  }

  for (auto& junction_ptr : junctions) {
    map_element_ids.insert(junction_ptr->id().id);
    std::copy(junction_ptr->junction().overlapid.begin(),
              junction_ptr->junction().overlapid.end(),
              std::back_inserter(overlap_ids));
    local_map->junction.push_back(junction_ptr->junction());
  }

  for (auto& signal_ptr : signals) {
    map_element_ids.insert(signal_ptr->id().id);
    std::copy(signal_ptr->signal().overlapid.begin(),
              signal_ptr->signal().overlapid.end(),
              std::back_inserter(overlap_ids));
    local_map->signal.push_back(signal_ptr->signal());
  }

  for (auto& stop_sign_ptr : stop_signs) {
    map_element_ids.insert(stop_sign_ptr->id().id);
    std::copy(stop_sign_ptr->stop_sign().overlapid.begin(),
              stop_sign_ptr->stop_sign().overlapid.end(),
              std::back_inserter(overlap_ids));
    local_map->stopsign.push_back(stop_sign_ptr->stop_sign());
  }

  for (auto& yield_sign_ptr : yield_signs) {
    std::copy(yield_sign_ptr->yield_sign().overlapid.begin(),
              yield_sign_ptr->yield_sign().overlapid.end(),
              std::back_inserter(overlap_ids));
    map_element_ids.insert(yield_sign_ptr->id().id);
    local_map->yieldd.push_back(yield_sign_ptr->yield_sign());
  }

  for (auto& clear_area_ptr : clear_areas) {
    map_element_ids.insert(clear_area_ptr->id().id);
    std::copy(clear_area_ptr->clear_area().overlapid.begin(),
              clear_area_ptr->clear_area().overlapid.end(),
              std::back_inserter(overlap_ids));
    local_map->cleararea.push_back(clear_area_ptr->clear_area());
  }

  for (auto& speed_bump_ptr : speed_bumps) {
    map_element_ids.insert(speed_bump_ptr->id().id);
    std::copy(speed_bump_ptr->speed_bump().overlapid.begin(),
              speed_bump_ptr->speed_bump().overlapid.end(),
              std::back_inserter(overlap_ids));
    local_map->speedbump.push_back(speed_bump_ptr->speed_bump());
  }

  for (auto& road_ptr : roads) {
    map_element_ids.insert(road_ptr->id().id);
    local_map->road.push_back(road_ptr->road());
  }

  for (auto& parking_space_ptr : parking_spaces) {
    map_element_ids.insert(parking_space_ptr->id().id);
    std::copy(parking_space_ptr->parking_space().overlapid.begin(),
              parking_space_ptr->parking_space().overlapid.end(),
              std::back_inserter(overlap_ids));
    local_map->parkingspace.push_back(parking_space_ptr->parking_space());
  }
  ...
  for (auto& overlap_id : overlap_ids) {
    ...
    if (!need_delete) {
      local_map->overlap.push_back(overlap_ptr->overlap());
    }
  }
...
int HDMapImpl::GetForwardNearestRSUs(const apollo::common::PointENU& point,
  ...
  while (s < real_distance) {
    ...
    for (size_t x = 0; x < lane_ptr->junctions().size(); ++x) {
      ...
      for (size_t i = 0; i < overlap_ptr->overlap().object.size(); ++i) {
        ...
        if (overlap_object.id.id == lane_ptr->id().id) {
          start_s = overlap_object.overlapinfo.laneoverlapinfo.starts;
          continue;
        }
    ...
    for (const auto& overlap_junction : overlap_junctions) {
      ...
      for (const auto& overlap_id : junction->junction().overlapid) {
        OverlapInfoConstPtr overlap_ptr = GetOverlapById(overlap_id);
        CHECK_NOTNULL(overlap_ptr);
        for (size_t i = 0; i < overlap_ptr->overlap().object.size(); ++i) {
          const auto& overlap_object = overlap_ptr->overlap().object[i];
          if (!overlap_object.overlapinfo.rsuoverlapinfo!=0) {
            continue;
          }
    ...
    for (const auto suc_lane_id : lane_ptr->lane().successorid) {
      LaneInfoConstPtr suc_lane_ptr = GetLaneById(suc_lane_id);
      if (lane_ptr->lane().successorid.size() > 1) {
...
void HDMapImpl::Clear() {
  ...
  map_.yieldd.clear();
  map_.overlap.clear();
  map_.cleararea.clear();
  map_.speedbump.clear();
  map_.road.clear();
  map_.parkingspace.clear();
  map_.pncjunction.clear();
  map_.rsu.clear();
  lane_table_.clear();
