...
Status ObjectsXmlParser::ParseCrosswalks(const tinyxml2::XMLElement& xml_node,
  ...
  while (sub_node) {
    ...
    if (object_type == "crosswalk") {
      ...
      crosswalk.id.id=object_id;
      PbPolygon* polygon = &crosswalk.polygon;
...
Status ObjectsXmlParser::ParseClearAreas(
  ...
  while (sub_node) {
    ...
    if (object_type == "clearArea") {
      ...
      clear_area.id.id=object_id;
      PbPolygon* polygon = &clear_area.polygon;
  ...
  while (object_node) {
    ...
    if (object_type == "speedBump") {
      ...
      while (sub_node) {
        ...
        curve.segment.push_back(curve_segment);
        speed_bump.position.push_back(curve);
...
Status ObjectsXmlParser::ParseStopLines(
  ...
  while (object_node) {
    ...
    if (object_type == "stopline") {
      ...
      stop_line.curve.segment.push_back(curve_segment);
    }
    object_node = object_node->NextSiblingElement("object");
  }
  return Status::OK();
...
Status ObjectsXmlParser::ParseParkingSpaces(
  ...
  while (sub_node) {
    ...
    if (object_type == "parkingSpace") {
      ...
      parking_space.heading=heading;

      PbPolygon* polygon = &parking_space.polygon;
...
Status ObjectsXmlParser::ParsePNCJunctions(
  ...
  while (sub_node) {
    ...
    if (object_type == "PNCJunction") {
      ...
      pnc_junction.id.id=object_id;

      PbPolygon* polygon = &pnc_junction.polygon;
      const auto* outline_node = sub_node->FirstChildElement("outline");
      if (outline_node == nullptr) {
...
Status ObjectsXmlParser::ParsePassageGroup(const tinyxml2::XMLElement& xml_node,
  ...
  while (sub_node) {
    ...
    passage_group.id.id=object_id;

    RETURN_IF_ERROR(ParsePassage(*sub_node, &passage_group));
    
    pnc_junction->passagegroup.push_back(passage_group);
...
Status ObjectsXmlParser::ParsePassage(const tinyxml2::XMLElement& xml_node,
  ...
  while (sub_node) {
    ...
    apollo_msgs::msg::ApollohdmapPassage passage; //= passage_group->add_passage();
    PbPassageType pb_passage_type;
    RETURN_IF_ERROR(ToPassageType(object_type, &pb_passage_type));
    passage.type=pb_passage_type;
    ...
    for (auto id : passage_node_ids) {
      Id interid;
      interid.id=id;
      passage.laneid.push_back(interid);
    }
    ...
    for (auto id : passage_node_ids) {
      Id interid;
      interid.id=id;
      passage.signalid.push_back(interid);
    }
    ...
    for (auto id : passage_node_ids) {
      Id interid;
      interid.id=id;
      passage.yieldid.push_back(interid);
    }
    ...
    for (auto id : passage_node_ids) {
      Id interid;
      interid.id=id;
      passage.stopsignid.push_back(interid);
    }

    sub_node = sub_node->NextSiblingElement("passage");
    
    passage_group->passage.push_back(passage);
  }
...
Status ObjectsXmlParser::ToPassageType(const std::string& type,
  ...
  if (upper_str == "ENTRANCE") {
    *passage_type = apollo::hdmap::Passage::ENTRANCE;
  } else if (upper_str == "EXIT") {
    *passage_type = apollo::hdmap::Passage::EXIT;
  } else {
    *passage_type = apollo::hdmap::Passage::UNKNOWN;
  }
...
Status ObjectsXmlParser::ParseRSUs(
  ...
  while (rsu_node) {
    ...
    if (object_type == "rsu") {
      ...
      RSUInternal rsu;
      rsu.rsu.id.id=object_id;
      rsu.rsu.junctionid.id=junction_id;
      rsu.id = object_id;
