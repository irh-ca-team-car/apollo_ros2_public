...
Status LanesXmlParser::ParseSectionBoundary(
  ...
  while (sub_node) {
    ...
    RETURN_IF_ERROR(
        UtilXmlParser::ParseCurve(*sub_node, &boundary_edge.curve));
    std::string type;
    ...
    boundary_edge.type=boundary_type;
    
    boundary->edge.push_back(boundary_edge);
...
Status LanesXmlParser::ParseLaneSection(const tinyxml2::XMLElement& xml_node,
  ...
  if (sub_node) {
    ...
    while (sub_node) {
      ...
      lane_internal.lane.leftboundary =
          lane_internal.lane.rightboundary;
      lane_internal.lane.rightboundary.curve.segment.clear();
      lane_internal.lane.rightboundary.boundarytype.clear();
      if (lanes->size() > 0) {
        PbLane& left_neighbor_lane = lanes->back().lane;
        left_neighbor_lane.rightboundary =
            lane_internal.lane.leftboundary;
      }
  ...
  reference_lane_internal.lane.leftboundary =
      reference_lane_internal.lane.rightboundary;
  if (lanes->size() > 0) {
    PbLane& left_neighbor_lane = lanes->back().lane;
    left_neighbor_lane.rightboundary =
        reference_lane_internal.lane.leftboundary;
...
Status LanesXmlParser::ParseLane(const tinyxml2::XMLElement& xml_node,
  ...
  if (sub_node) {
    PbLaneBoundary* lane_boundary = &lane->rightboundary;
    ACHECK(lane_boundary != nullptr);
    success =
        UtilXmlParser::ParseCurve(*sub_node, &lane_boundary->curve);
    if (!success.ok()) {
      std::string err_msg = "Error parse lane border";
      return Status(apollo::common::ErrorCode::HDMAP_DATA_ERROR, err_msg);
    }
    lane_boundary->length=
        UtilXmlParser::CurveLength(lane_boundary->curve);
    ...
    lane_boundary->virtuall=is_virtual;
  }
  ...
  while (sub_node) {
    ...
    apollo_msgs::msg::ApollohdmapLaneBoundaryType lane_boundary_type; //= lane_boundary->add_boundary_type();
    lane_boundary_type.s=s_offset;
    lane_boundary_type.types.push_back(boundary_type);
    sub_node = sub_node->NextSiblingElement("borderType");
    
    lane_boundary->boundarytype.push_back(lane_boundary_type);
  }
...
Status LanesXmlParser::ParseDirection(const tinyxml2::XMLElement& xml_node,
  ...
  lane->direction=pb_lane_direction;
...
Status LanesXmlParser::ParseCenterCurve(const tinyxml2::XMLElement& xml_node,
  ...
  PbCurve* center_curve = &lane->centralcurve;
  ...
  lane->length=UtilXmlParser::CurveLength(*center_curve);
...
Status LanesXmlParser::ParseSpeed(const tinyxml2::XMLElement& xml_node,
  ...
  if (sub_node) {
    ...
    lane->speedlimit=ToMPS(max_speed);
  }
...
Status LanesXmlParser::ParseSampleAssociates(
  ...
  while (sub_node) {
    ...
    LaneSampleAssociation left_sample; //= lane->add_left_sample();
    left_sample.s=s;
    left_sample.width=left_width;

    LaneSampleAssociation right_sample; //= lane->add_right_sample();
    right_sample.s=s;
    right_sample.width=right_width;
    
    lane->leftsample.push_back(left_sample);
    lane->rightsample.push_back(right_sample);
...
Status LanesXmlParser::ParseSingleSideRoadSampleAssociates(
  ...
  while (sub_node) {
    ...
    LaneSampleAssociation road_sample;
        
    if (bleft) {
      LaneSampleAssociation road_sample;
      road_sample.s=s;
      road_sample.width=width;
      lane->leftroadsample.push_back(road_sample);}
    else if (!bleft){
      LaneSampleAssociation road_sample;
      road_sample.s=s;
      road_sample.width=width;
      lane->rightroadsample.push_back(road_sample);}
...
Status LanesXmlParser::ParseRoadSampleAssociates(
  ...
  if (sub_node == nullptr) {
    ...
    while (sub_node) {
      ...
      LaneSampleAssociation left_road_sample; //= lane->add_left_sample();
      left_road_sample.s=s;
      left_road_sample.width=left_width;

      LaneSampleAssociation right_road_sample; //= lane->add_right_sample();
      right_road_sample.s=s;
      right_road_sample.width=right_width;
    
      lane->leftroadsample.push_back(left_road_sample);
      lane->rightroadsample.push_back(right_road_sample);
...
void LanesXmlParser::ParseLaneLink(const tinyxml2::XMLElement& xml_node,
  ...
  while (sub_node) {
    ...
    if (checker == tinyxml2::XML_SUCCESS) {
      ...
      pb_lane_id.id=lane_id;
      lane->predecessorid.push_back(pb_lane_id);
    }
  ...
  while (sub_node) {
    ...
    if (checker == tinyxml2::XML_SUCCESS) {
      ...
      pb_lane_id.id=lane_id;
      lane->successorid.push_back(pb_lane_id);
    }
    ...
    if (checker == tinyxml2::XML_SUCCESS) {
      if (side == "left") {
        if (direction == "same") {
          PbID pb_lane_id;
          pb_lane_id.id=lane_id;
          lane->leftneighborforwardlaneid.push_back(pb_lane_id);
        } else {
          PbID pb_lane_id;
          pb_lane_id.id=lane_id;
          lane->leftneighborreverselaneid.push_back(pb_lane_id);
        }
      } else if (side == "right") {
        if (direction == "same") {
          PbID pb_lane_id;
          pb_lane_id.id=lane_id;
          lane->rightneighborforwardlaneid.push_back(pb_lane_id);
        } else {
          PbID pb_lane_id;
          pb_lane_id.id=lane_id;
          lane->rightneighborreverselaneid.push_back(pb_lane_id);
        }
      }
    }
  ...
  while (sub_node) {
    ...
    if (checker == tinyxml2::XML_SUCCESS) {
      PbID pb_lane_id;
      pb_lane_id.id=lane_id;
      lane->selfreverselaneid.push_back(pb_lane_id);
    }
...
Status LanesXmlParser::ParseRegionOverlap(
  ...
  while (region_overlap_node != nullptr) {
    ...
    region_overlap.id.id=region_overlap_id;
    ...
    while (outline_node != nullptr) {
      ...
      RETURN_IF_ERROR(UtilXmlParser::ParseOutline(
          *outline_node, &interpoly));
      region_overlap.polygon.push_back(interpoly);
