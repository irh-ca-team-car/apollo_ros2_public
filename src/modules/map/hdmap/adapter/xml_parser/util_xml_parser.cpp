...
Status UtilXmlParser::ParseGeometry(const tinyxml2::XMLElement& xml_node,
  ...
  if (checker == tinyxml2::XML_SUCCESS) {
    ...
    curve_segment->startposition.x=output_x;
    curve_segment->startposition.y=output_y;
    curve_segment->length=length;
  }

  const auto sub_node = xml_node.FirstChildElement("pointSet");
  if (sub_node) {
    PbLineSegment* line_segment = &curve_segment->curvetype.linesegment;
...
Status UtilXmlParser::ParsePointSet(const tinyxml2::XMLElement& xml_node,
  ...
  while (sub_node) {
    ...
    line_segment->point.push_back(pt);
...
double UtilXmlParser::CurveLength(const PbCurve& curve) {
  double length = 0.0;
  for (size_t i = 0; i < curve.segment.size(); ++i) {
