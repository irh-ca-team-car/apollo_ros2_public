...
Status SignalsXmlParser::ParseTrafficLights(
  ...
  while (signal_node) {
    ...
    if (object_type == "trafficLight") {
      ...
      traffic_light.type=signal_layout_type;
      ...
      if (sign_infos_node != nullptr) {
        ...
        while (sign_info_node != nullptr) {
          ...
          apollo_msgs::msg::ApollohdmapSignInfo sign_info; //= traffic_light.add_sign_info();
          PbSignInfoType pb_sign_info_type;
          to_pb_sign_info_type(sign_info_type, &pb_sign_info_type);
          sign_info.type=pb_sign_info_type;
          
          traffic_light.signinfo.push_back(sign_info);

          sign_info_node = sign_info_node->NextSiblingElement("signInfo");
        }
      }

      PbPolygon* polygon = &traffic_light.boundary;
      ...
      while (sub_node) {
        ...
        sub_signal.id.id=sub_signal_id;
        sub_signal.type=sub_signal_type;
        PbPoint3D* pt = &sub_signal.location;
        RETURN_IF_ERROR(UtilXmlParser::ParsePoint(*sub_node, pt));
        
        traffic_light.subsignal.push_back(sub_signal);
  ...
  while (signal_node) {
    ...
    if (object_type == "stopSign") {
      ...
      if (sub_node) {
        ...
        stop_sign_internal.stop_sign.type=pb_stop_type;
      }
