...
Status HeaderXmlParser::Parse(const tinyxml2::XMLElement& xml_node,
  ...
  std::copy(version.begin(), version.end(), std::back_inserter(header->version.data));
  //header->set_date(date);
  std::copy(date.begin(),date.end(), std::back_inserter(header->date.data));
  header->projection.proj=to_coordinate;
  //header->set_district(database_name);
  std::copy(database_name.begin(),database_name.end(), std::back_inserter(header->district.data));
  ...
  std::copy(vendor.begin(),vendor.end(), std::back_inserter(header->vendor.data));
