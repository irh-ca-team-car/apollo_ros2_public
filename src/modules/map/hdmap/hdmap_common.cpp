...
void PointsFromCurve(const Curve &input_curve, std::vector<Vec2d> *points) {
  ...
  for (const auto &curve : input_curve.segment) {
    if (curve.curvetype.linesegment!=LineSegment()) {
      for (const auto &point : curve.curvetype.linesegment.point) {
...
apollo::common::math::Polygon2d ConvertToPolygon2d(const Polygon &polygon) {
  ...
  points.reserve(polygon.point.size());
...
LaneInfo::LaneInfo(const apollo_msgs::msg::ApollohdmapLane &lane) : lane_(lane) { Init(); }

void LaneInfo::Init() {
  PointsFromCurve(lane_.centralcurve, &points_);
  ...
  for (const auto &overlap_id : lane_.overlapid) {
  ...
  for (const auto &sample : lane_.leftsample) {
    sampled_left_width_.emplace_back(sample.s, sample.width);
  }
  for (const auto &sample : lane_.rightsample) {
  ...
  if (lane_.type!=0) {
    if (lane_.type == Lane::CITY_DRIVING) {
      ...
      AERROR << "lane_[id = " << "] type is NONE.";
    }
  } else {
    AERROR << "lane_[id = " << "] has NO type.";
  }

  sampled_left_road_width_.clear();
  sampled_right_road_width_.clear();
  for (const auto &sample : lane_.leftroadsample) {
    sampled_left_road_width_.emplace_back(sample.s, sample.width);
  }
  for (const auto &sample : lane_.rightroadsample) {
...
void JunctionInfo::Init() {
  ...
  for (const auto &overlap_id : junction_.overlapid) {
...
SignalInfo::SignalInfo(const apollo_msgs::msg::ApollohdmapSignal &signal) : signal_(signal) { Init(); }

void SignalInfo::Init() {
  for (const auto &stop_line : signal_.stopline) {
...
StopSignInfo::StopSignInfo(const apollo_msgs::msg::ApollohdmapStopSign &stop_sign) : stop_sign_(stop_sign) {
  init();
}

void StopSignInfo::init() {
  for (const auto &stop_line : stop_sign_.stopline) {
    SegmentsFromCurve(stop_line, &segments_);
  }
  ACHECK(!segments_.empty());

  for (const auto &overlap_id : stop_sign_.overlapid) {
    overlap_ids_.emplace_back(overlap_id);
  }
}
...
void StopSignInfo::UpdateOverlaps(const HDMapImpl &map_instance) {
  for (const auto &overlap_id : overlap_ids_) {
    ...
    for (const auto &object : overlap_ptr->overlap().object) {
      ...
      if (object.overlapinfo.junctionoverlapinfo!=0) {
        overlap_junction_ids_.push_back(object.id);
      } else if (object.overlapinfo.laneoverlapinfo!=LaneOverlapInfo()) {
...
void YieldSignInfo::Init() {
  for (const auto &stop_line : yield_sign_.stopline) {
    SegmentsFromCurve(stop_line, &segments_);
  }
...
const apollo_msgs::msg::ApollohdmapObjectOverlapInfo *OverlapInfo::GetObjectOverlapInfo(const Id &id) const {
...
RoadInfo::RoadInfo(const apollo_msgs::msg::ApollohdmapRoad &road) : road_(road) {
