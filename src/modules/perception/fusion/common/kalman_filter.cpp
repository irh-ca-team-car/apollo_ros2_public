...
bool KalmanFilter::Init(const Eigen::VectorXd &initial_belief_states,
  ...
  if (initial_uncertainty.rows() != initial_uncertainty.cols()) {
    AERROR_EVERY(60000) << "the cols and rows of uncertainty martix should be equal";
    return false;
  }
  states_num_ = static_cast<int>(initial_uncertainty.rows());

  if (states_num_ <= 0) {
    AERROR_EVERY(60000) << "state_num should be greater than zero";
    return false;
  }

  if (states_num_ != initial_belief_states.rows()) {
    AERROR_EVERY(60000) << "the rows of state should be equal to state_num";
    return false;
  }
...
bool KalmanFilter::Predict(const Eigen::MatrixXd &transform_matrix,
  ...
  if (!init_) {
    AERROR_EVERY(60000) << "Predict: Kalman Filter initialize not successfully";
    return false;
  }
  if (transform_matrix.rows() != states_num_) {
    AERROR_EVERY(60000) << "the rows of transform matrix should be equal to state_num";
    return false;
  }
  if (transform_matrix.cols() != states_num_) {
    AERROR_EVERY(60000) << "the cols of transform matrix should be equal to state_num";
    return false;
  }
  if (env_uncertainty_matrix.rows() != states_num_) {
    AERROR_EVERY(60000) << "the rows of env uncertainty should be equal to state_num";
    return false;
  }
  if (env_uncertainty_matrix.cols() != states_num_) {
    AERROR_EVERY(60000) << "the cols of env uncertainty should be equal to state_num";
    return false;
  }
...
bool KalmanFilter::Correct(const Eigen::VectorXd &cur_observation,
  ...
  if (!init_) {
    AERROR_EVERY(60000) << "Correct: Kalman Filter initialize not successfully";
    return false;
  }
  if (cur_observation.rows() != states_num_) {
    AERROR_EVERY(60000) << "the rows of current observation should be equal to state_num";
    return false;
  }
  if (cur_observation_uncertainty.rows() != states_num_) {
    AERROR_EVERY(60000) << "the rows of current observation uncertainty "
              "should be equal to state_num";
    return false;
  }
  if (cur_observation_uncertainty.cols() != states_num_) {
    AERROR_EVERY(60000) << "the cols of current observation uncertainty "
              "should be equal to state_num";
    return false;
  }
...
bool KalmanFilter::SetControlMatrix(const Eigen::MatrixXd &control_matrix) {
  if (!init_) {
    AERROR_EVERY(60000) << "SetControlMatrix: Kalman Filter initialize not successfully";
    return false;
  }
  if (control_matrix.rows() != states_num_ ||
      control_matrix.cols() != states_num_) {
    AERROR_EVERY(60000) << "the rows/cols of control matrix should be equal to state_num";
    return false;
  }
