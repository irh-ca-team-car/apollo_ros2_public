...
bool PbfGatekeeper::Init() {
  ...
  params_.publish_if_has_lidar = params.publishifhaslidar;
  params_.publish_if_has_radar = params.publishifhasradar;
  params_.publish_if_has_camera = params.publishifhascamera;
  params_.use_camera_3d = params.usecamera3d;
  params_.min_radar_confident_distance = params.minradarconfidentdistance;
  params_.max_radar_confident_angle = params.maxradarconfidentangle;
  params_.min_camera_publish_distance = params.mincamerapublishdistance;
  params_.invisible_period_threshold = params.invisibleperiodthreshold;
  params_.existence_threshold = params.existencethreshold;
  params_.radar_existence_threshold = params.radarexistencethreshold;
  params_.toic_threshold = params.toicthreshold;
  params_.use_track_time_pub_strategy = params.usetracktimepubstrategy;
  params_.pub_track_time_thresh = params.pubtracktimethresh;
  return true;
}
  ...
  if (params_.publish_if_has_camera && visible_in_camera &&
    ...
    if ((camera_object->GetBaseObject()->sub_type ==
      ...
      if (exist_prob > params_.existence_threshold) {
        ...
        AINFO_EVERY(60000) << "publish camera only object : cnt =  " << cnt_cam;
        cnt_cam++;
        return true;
      }
    }
