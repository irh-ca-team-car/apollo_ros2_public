...
DstTypeFusion::DstTypeFusion(TrackPtr track)
  ...
  if (sensor_obj == nullptr) {
    AERROR_EVERY(60000) << "track has no sensor_obj";
    return;
  }
  fused_dst_ =
...
bool DstTypeFusion::Init() {
  ...
  for (auto camera_param : params.cameraparams) {
    std::string camera_id = camera_param.name;
    options_.camera_max_valid_dist_[camera_id] = camera_param.validdist;
    options_.sensor_reliability_[camera_id] = camera_param.reliability;
    options_.sensor_reliability_for_unknown_[camera_id] =
        camera_param.reliabilityforunknown;
    AINFO_EVERY(60000) << "dst type fusion params: " << camera_id
  ...
  for (auto lidar_param : params.lidarparams) {
    ...
    options_.sensor_reliability_for_unknown_[lidar_id] =
        lidar_param.reliabilityforunknown;
    AINFO_EVERY(60000) << "dst type fusion params: " << lidar_id
...
Dst DstTypeFusion::TypeProbsToDst(const std::vector<float> &type_probs) {
  ...
  //    AINFO_EVERY(60000) << "unknonw_unmovable prob = " <<
  //    type_probs[(int)base::ObjectType::UNKNOWN_UNMOVABLE] << " > 0.0f";
  //}
