...
bool PbfTracker::InitParams() {
  ...
  AINFO_EVERY(60000) << "Config file : " << config;
  ...
  AINFO_EVERY(60000) << "Load PbfTrackerConfig: " << params.typefusionmethod << ","
        << params.motionfusionmethod << "," << params.shapefusionmethod
        << "," << params.existencefusionmethod;
  s_type_fusion_method_ = params.typefusionmethod;
  s_motion_fusion_method_ = params.motionfusionmethod;
  s_existence_fusion_method_ = params.existencefusionmethod;
  s_shape_fusion_method_ = params.shapefusionmethod;
...
bool PbfTracker::InitMethods() {
  if (s_type_fusion_method_ == "DstTypeFusion") {
    ...
    AERROR_EVERY(60000) << "Unknown type fusion : " << s_type_fusion_method_;
    return false;
  }
  ...
  if (s_motion_fusion_method_ == "KalmanMotionFusion") {
    ...
    AERROR_EVERY(60000) << "Unknown motion fusion : " << s_motion_fusion_method_;
    return false;
  }
  ...
  if (s_existence_fusion_method_ == "DstExistenceFusion") {
    ...
    AERROR_EVERY(60000) << "Unknown existence fusion : " << s_existence_fusion_method_;
    return false;
  }
  ...
  if (s_shape_fusion_method_ == "PbfShapeFusion") {
    ...
    AERROR_EVERY(60000) << "Unknown shape fusion : " << s_shape_fusion_method_;
    return false;
  }
