...
bool DstExistenceFusion::Init() {
  ...
  if (!GetFusionInitOptions("DstExistenceFusion", &options)) {
    AERROR_EVERY(60000) << "GetFusionInitOptions failed ";
    return false;
  }
  ...
  if (!cyber::common::GetProtoFromASCIIFile(config, &params)) {
    AERROR_EVERY(60000) << "Read config failed: " << config;
    return false;
  }
  for (auto valid_dist : params.cameravaliddist) {
    std::string camera_id = valid_dist.cameraname;
    options_.camera_max_valid_dist_[camera_id] = valid_dist.validdist;
    AINFO_EVERY(60000) << "dst existence fusion params: " << camera_id
          << " max valid dist: " << options_.camera_max_valid_dist_[camera_id];
  }

  options_.track_object_max_match_distance_ =
      params.trackobjectmaxmatchdistance;
  AINFO_EVERY(60000) << "dst existence fusion params: "
...
void DstExistenceFusion::UpdateExistenceState() {
  ...
  if (!(track_ref_->GetLidarObjects()).empty()) {
    ...
    AERROR_EVERY(60000) << boost::format("this fused object: %d has no sensor objects") %
