  ...
  namespace perception
  {
    namespace fusion
    {
      ...
      bool ProbabilisticFusion::Init(const FusionInitOptions &init_options)
      {
        ...
        params_.use_lidar = params.uselidar;
        params_.use_radar = params.useradar;
        params_.use_camera = params.usecamera;
        params_.tracker_method = params.trackermethod;
        params_.data_association_method = params.dataassociationmethod;
        params_.gate_keeper_method = params.gatekeepermethod;
        for (size_t i = 0; i < params.prohibitionsensors.size(); ++i)
        {
          params_.prohibition_sensors.push_back(params.prohibitionsensors[i]);
        }

        // static member initialization from PB config
        Track::SetMaxLidarInvisiblePeriod(params.maxlidarinvisibleperiod);
        Track::SetMaxRadarInvisiblePeriod(params.maxradarinvisibleperiod);
        Track::SetMaxCameraInvisiblePeriod(params.maxcamerainvisibleperiod);
        Sensor::SetMaxCachedFrameNumber(params.maxcachedframenum);
        ...
        {
          AERROR_EVERY(60000) << "Unknown association method: " << params_.data_association_method;
          return false;
        }
        if (!matcher_->Init())
        {
          AERROR_EVERY(60000) << "Failed to init matcher.";
          return false;
        }
        ...
        {
          AERROR_EVERY(60000) << "Unknown gate keeper method: " << params_.gate_keeper_method;
          return false;
        }
        if (!gate_keeper_->Init())
        {
          AERROR_EVERY(60000) << "Failed to init gatekeeper.";
          return false;
        }
                                     ...
                                     std::vector<base::ObjectPtr> *fused_objects)
      {
        ...
        {
          AERROR_EVERY(60000) << "fusion error: fused_objects is nullptr";
          return false;
        }
        ...
        // 1. save frame data
        {
          ...
          if (started_)
          {
            AINFO_EVERY(60000) << "add sensor measurement: " << sensor_frame->sensor_info.name
...

        {
          ...
          AINFO_EVERY(60000) << "Get " << frames.size() << " related frames for fusion";
      ...
      {
        AINFO_EVERY(60000) << "Fusing frame: " << frame->GetSensorId()
      ...
      void ProbabilisticFusion::RemoveLostTrack()
      {
        ...
        AINFO_EVERY(60000) << "Remove " << foreground_tracks.size() - foreground_track_count
                           << " foreground tracks";
        ...
        AINFO_EVERY(60000) << "Remove " << background_tracks.size() - background_track_count
                           << " background tracks";
          ...
          double timestamp, std::vector<base::ObjectPtr> *fused_objects)
      {
        ...
        AINFO_EVERY(60000) << "collect objects : fg_obj_cnt = " << fg_obj_num
