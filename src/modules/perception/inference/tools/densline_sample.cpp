...
int main(int argc, char **argv) {
  ...
  for (auto name : outputs) {
    AINFO_EVERY(60000) << "outputs name: " << name;
  }
  ...
  for (auto image_file : image_lists) {
    ...
    AINFO_EVERY(60000) << img.channels();
    ...
    for (auto output_name : outputs) {
      ...
      AINFO_EVERY(60000) << output_name << " " << blob->channels() << " " << blob->height()
            << " " << blob->width();
      double sum = 0;
      for (int i = 0; i < blob->count(); ++i) {
        sum += blob->cpu_data()[i];
        if (i < 100) {
          AINFO_EVERY(60000) << blob->cpu_data()[i];
        }
      }
      AINFO_EVERY(60000) << output_name << ", sum : \n";
      // end of if
