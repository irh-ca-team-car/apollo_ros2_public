...
int evaluate_image_list() {
  ...
  if (!fin.is_open()) {
    AERROR_EVERY(60000) << "Failed to open test list file: " << FLAGS_test_list;
    return -1;
  }
  ...
  if (!out_car.is_open()) {
    AERROR_EVERY(60000) << "Failed to open out car file: " << out_file;
    return -1;
  }
  ...
  for (size_t i = 0; i < img_list.size(); ++i) {
    ...
    if (img.data == 0) {
      AERROR_EVERY(60000) << "Failed to read image: " << image_path;
      return -1;
    }
