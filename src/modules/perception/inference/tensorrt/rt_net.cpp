...
class RTLogger : public nvinfer1::ILogger {
  void log(Severity severity, const char *msg) override {
    if (severity != Severity::kINFO && severity != Severity::kVERBOSE) {
      AINFO_EVERY(60000) << "Severity:"<<((int)severity) <<" ::" << msg;
    }
  }
} rt_gLogger;
...
void RTNet::ConstructMap(const LayerParameter &layer_param,
  ...
  for (size_t i = 0; i < layer_param.top.size(); i++) {
    std::string top_name = layer_param.top.at(i);
...
void RTNet::addConvLayer(const LayerParameter &layer_param,
                         ...
                         TensorModifyMap *tensor_modify_map) {
  auto p = layer_param.convolutionparam;

  int nbOutputs = p.numoutput;

  int kernelH = p.kernelh!=0 ? p.kernelh : p.kernelsize.at(0);
  int kernelW = p.kernelw!=0 ? p.kernelw
                                 : p.kernelsize.size() > 1 ? p.kernelsize.at(1)
                                                            : p.kernelsize.at(0);
  int C = getCHW(inputs[0]->getDimensions()).c();
  int G = p.group!=0 ? p.group : 1;

  int size = nbOutputs * kernelW * kernelH * C;
  auto wt = loadLayerWeights(p.weightfiller.value, size);
  ...
  if (convLayer) {
    int strideH =
        p.strideh!=0 ? p.strideh : p.stride.size() > 0 ? p.stride.at(0) : 1;
    int strideW = p.stridew!=0
                      ? p.stridew
                      : p.stride.size() > 1
                            ? p.stride.at(1)
                            : p.stride.size() > 0 ? p.stride.at(0) : 1;

    int padH = p.padh!=0 ? p.padh : p.pad.size() > 0 ? p.pad.at(0) : 0;
    int padW =
        p.padw!=0
            ? p.padw
            : p.pad.size() > 1 ? p.pad.at(1) : p.pad.size() > 0 ? p.pad.at(0) : 0;
    int dilationH = p.dilation.size() > 0 ? p.dilation.at(0) : 1;
    int dilationW = p.dilation.size() > 1
                        ? p.dilation.at(1)
                        : p.dilation.size() > 0 ? p.dilation.at(0) : 1;
  ...
  AINFO_EVERY(60000) << layer_param.name << dim_string;
#endif
}
...
void RTNet::addActiveLayer(const LayerParameter &layer_param,
  ...
  if (layer_param.type == "ReLU" &&
      layer_param.reluparam.negativeslope > 0.0f) {
    std::shared_ptr<ReLUPlugin> relu_plugin;
    relu_plugin.reset(
        new ReLUPlugin(layer_param.reluparam, inputs[0]->getDimensions()));
...
void RTNet::addConcatLayer(const LayerParameter &layer_param,
                           ...
                           TensorMap *tensor_map,
                           TensorModifyMap *tensor_modify_map) {
  ConcatParameter concat = layer_param.concatparam;
  ...
  CHECK_EQ((size_t)nbInputs, layer_param.bottom.size());
...
void RTNet::addPoolingLayer(const LayerParameter &layer_param,
  ...
  nvinfer1::IPoolingLayer *poolLayer = net->addPooling(
      *inputs[0], pool_type,
      {static_cast<int>(pool.kernelh), static_cast<int>(pool.kernelw)});
  poolLayer->setStride(nvinfer1::DimsHW{static_cast<int>(pool.strideh),
                                        static_cast<int>(pool.stridew)});
  poolLayer->setPadding(nvinfer1::DimsHW{static_cast<int>(pool.padh),
                                         static_cast<int>(pool.padw)});
...
void RTNet::addSliceLayer(const LayerParameter &layer_param,
                          ...
                          TensorMap *tensor_map,
                          TensorModifyMap *tensor_modify_map) {
  CHECK_GT(layer_param.sliceparam.axis, 0);
  std::shared_ptr<SLICEPlugin> slice_plugin;
  slice_plugin.reset(
      new SLICEPlugin(layer_param.sliceparam, inputs[0]->getDimensions()));
...
void RTNet::addInnerproductLayer(const LayerParameter &layer_param,
                                 ...
                                 TensorMap *tensor_map,
                                 TensorModifyMap *tensor_modify_map) {
  InnerProductParameter fc = layer_param.innerproductparam;
  nvinfer1::Weights bias{nvinfer1::DataType::kFLOAT, nullptr, 0};
  if ((*weight_map)[layer_param.name.c_str()].size() == 2) {
    bias = (*weight_map)[layer_param.name.c_str()][1];
  }
  nvinfer1::IFullyConnectedLayer *fcLayer = net->addFullyConnected(
      *inputs[0], fc.numoutput, (*weight_map)[layer_param.name.c_str()][0],
      bias);
...
void RTNet::addScaleLayer(const LayerParameter &layer_param,
  ...
  if (layer_param.type == "Power") {
    auto power_param = layer_param.powerparam;
...
void RTNet::addBatchnormLayer(const LayerParameter &layer_param,
                              ...
                              TensorMap *tensor_map,
                              TensorModifyMap *tensor_modify_map) {
  BatchNormParameter param = layer_param.batchnormparam;
  UNUSED(param);
...
void RTNet::addSoftmaxLayer(const LayerParameter &layer_param,
                            ...
                            TensorMap *tensor_map,
                            TensorModifyMap *tensor_modify_map) {
  if (layer_param.softmaxparam != apollo_msgs::msg::ApolloperceptioninferenceSoftmaxParameter()) {
    std::shared_ptr<SoftmaxPlugin> softmax_plugin;
    softmax_plugin.reset(new SoftmaxPlugin(layer_param.softmaxparam,
...
void RTNet::addEltwiseLayer(const LayerParameter &layer_param,
                            ...
                            TensorMap *tensor_map,
                            TensorModifyMap *tensor_modify_map) {
  auto pair = eltwise_map.find((apollo::perception::inference::EltwiseParameter::EltwiseOp)layer_param.eltwiseparam.operation);
...
void RTNet::addArgmaxLayer(const LayerParameter &layer_param,
  ...
  argmax_plugin.reset(new ArgMax1Plugin(layer_param.argmaxparam,
                                        inputs[0]->getDimensions()));
...
void RTNet::addPermuteLayer(const LayerParameter &layer_param,
                            ...
                            TensorModifyMap *tensor_modify_map) {
  UNUSED(nbInputs);
  CHECK_LE(layer_param.permuteparam.order.size(), nvinfer1::Dims::MAX_DIMS);
  ...
  ACHECK(layer_param.permuteparam.order.at(0) == 0);
  for (size_t i = 1; i < layer_param.permuteparam.order.size(); ++i) {
    int order = layer_param.permuteparam.order.at(i);
...
void RTNet::addReshapeLayer(const LayerParameter &layer_param,
                            ...
                            TensorMap *tensor_map,
                            TensorModifyMap *tensor_modify_map) {
  apollo::perception::inference::DimsCHW dims;
  dims.d[0] = static_cast<int>(layer_param.reshapeparam.shape.dim.at(1));
  dims.d[1] = static_cast<int>(layer_param.reshapeparam.shape.dim.at(2));
  dims.d[2] = static_cast<int>(layer_param.reshapeparam.shape.dim.at(3));
...
void RTNet::addPaddingLayer(const LayerParameter &layer_param,
                            ...
                            TensorMap *tensor_map,
                            TensorModifyMap *tensor_modify_map) {
  nvinfer1::DimsHW pre_dims(layer_param.paddingparam.padt,
                            layer_param.paddingparam.padl);
  nvinfer1::DimsHW post_dims(layer_param.paddingparam.padb,
                             layer_param.paddingparam.padr);
...
void RTNet::addDFMBPSROIAlignLayer(const LayerParameter &layer_param,
  ...
  dfmb_psroi_align_plugin.reset(new DFMBPSROIAlignPlugin(
      layer_param.dfmbpsroipoolingparam, input_dims, nbInputs));
...
void RTNet::addRCNNProposalLayer(const LayerParameter &layer_param,
  ...
  rcnn_proposal_plugin.reset(new RCNNProposalPlugin(
      layer_param.bboxregparam, layer_param.detectionoutputssdparam,
      input_dims));
...
void RTNet::addLayer(const LayerParameter &layer_param,
  ...
  if (layer_param.type == "Convolution") {
    ...
    AINFO_EVERY(60000) << "skip dropout";
...
bool RTNet::loadWeights(const std::string &model_file, WeightMap *weight_map) {
  ...
  for (size_t i = 0; i < net.layer.size(); i++) {
    std::vector<nvinfer1::Weights> lw;
    for (size_t j = 0; j < net.layer.at(i).blobs.size(); j++) {
      // val memory will be released when deconstructor is called
      auto blob = &(net.layer.at(i).blobs.at(j));
      CHECK_EQ(blob->doubledata.size(), 0);
      CHECK_EQ(blob->doublediff.size(), 0);
      CHECK_EQ(blob->diff.size(), 0);
      CHECK_NE(blob->data.size(), 0);
      if (net.layer.at(i).type == "BatchNorm") {
        mergeBN(j, &net.layer.at(i));
      }
      auto wt = loadLayerWeights(blob->data.data(), blob->data.size());
      lw.push_back(wt);
    }
    (*weight_map)[net.layer.at(i).name.c_str()] = lw;
  }
  return true;
}
void RTNet::mergeBN(int index, LayerParameter *layer_param) {
  auto blob = &(layer_param->blobs.at(index));
  CHECK_EQ(blob->doubledata.size(), 0);
  CHECK_EQ(blob->doublediff.size(), 0);
  CHECK_EQ(blob->diff.size(), 0);
  CHECK_NE(blob->data.size(), 0);
  int size = blob->data.size();

  auto scale = (layer_param->blobs.at(2));
  const float scale_factor = scale.data.at(0) == 0 ? 0 : 1 / scale.data.at(0);
  auto epsilon = layer_param->batchnormparam.eps;
  if (index == 0) {
    const BlobProto *var = &(layer_param->blobs.at(index + 1));
    for (int k = 0; k < size; k++) {
      auto data = blob->data.at(k);
      blob->data.at(k) = (static_cast<float>(-data * scale_factor /
                                sqrt(var->data.at(k) * scale_factor + epsilon)));
    }
  } else if (index == 1) {
    for (int k = 0; k < size; k++) {
      blob->data.at(k) = (1.0f / static_cast<float>(sqrt(
                                   blob->data.at(k) * scale_factor + epsilon)));
    }
  }
}
...
RTNet::RTNet(const std::string &net_file, const std::string &model_file,
  ...
  ReadProtoFromFile(net_file, net_param_.get());
}
RTNet::RTNet(const std::string &net_file, const std::string &model_file,
             const std::vector<std::string> &outputs,
             const std::vector<std::string> &inputs,
  ...
  ReadProtoFromBinaryFile(net_file, net_param_.get());
...
bool RTNet::shape(const std::string &name, std::vector<int> *res) {
  ...
  if (tensor_modify_map_.find(name) == tensor_modify_map_.end()) {
    AINFO_EVERY(60000) << "can't get the shape of " << name;
    return false;
  }
  ...
  apollo::perception::inference::DimsCHW dims = static_cast<apollo::perception::inference::DimsCHW &&>(
...
bool RTNet::Init(const std::map<std::string, std::vector<int>> &shapes) {
  ...
  if (gpu_id_ < 0) {
    AINFO_EVERY(60000) << "must use gpu mode";
    return false;
  }
...
bool RTNet::checkInt8(const std::string &gpu_name,
  ...
  if (calibrator == nullptr) {
    AINFO_EVERY(60000) << "Device Works on FP32 Mode.";
    return false;
  }
  for (auto ref : _gpu_checklist) {
    if (ref == gpu_name) {
      AINFO_EVERY(60000) << "Device Works on Int8 Mode.";
      return true;
    }
  }
...
bool RTNet::addInput(const TensorDimsMap &tensor_dims_map,
                     ...
                     TensorMap *tensor_map) {
  CHECK_GT(net_param_->layer.size(), 0);
  input_names_.clear();
...
void RTNet::parse_with_api(
  ...
  for (auto layer_param : order) {
    ...
    for (size_t j = 0; j < layer_param.bottom.size(); j++) {
      CHECK_NOTNULL(tensor_map[tensor_modify_map_[layer_param.bottom.at(j)]]);
      inputs.push_back(tensor_map[tensor_modify_map_[layer_param.bottom.at(j)]]);
    }
    addLayer(layer_param, inputs.data(), layer_param.bottom.size(),
  ...
  for (auto iter = output_names_.begin(); iter != output_names_.end();) {
    if (tensor_map.find(tensor_modify_map_[*iter]) != tensor_map.end()) {
      ...
      //AINFO_EVERY(60000) << "Erase output: " << *iter;
