...
bool UndistortionHandler::set_device(int device) {
  ...
  if (code != cudaSuccess) {
    AERROR_EVERY(60000) << "cudaSetDevice failed: " << cudaGetErrorString(code);
    return false;
  }
  return true;
}
...
bool UndistortionHandler::Init(const std::string &sensor_name, int device) {
  ...
  if (!sensor_manager->IsSensorExist(sensor_name)) {
    AERROR_EVERY(60000) << "Sensor '" << sensor_name << "' not exists!";
    return false;
  }
...
bool UndistortionHandler::Handle(const base::Image8U &src_img,
  ...
  switch (src_img.channels()) {
    ...
    case 3:
      ...
      break;
    default:
      AERROR_EVERY(60000) << "Invalid number of channels: " << src_img.channels();
      return false;
  }

  if (status != NPP_SUCCESS) {
    AERROR_EVERY(60000) << "NPP_CHECK_NPP - status = " << status;
    return false;
  }
