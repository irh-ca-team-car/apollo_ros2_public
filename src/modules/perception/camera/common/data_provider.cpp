  ...
  namespace perception
  {
    namespace camera
    {
      ...
      bool DataProvider::Init(const DataProvider::InitOptions &options)
      {
        ...
        {
          AERROR_EVERY(60000) << "Failed to set device to: " << device_id_;
          return false;
        }

        AINFO_EVERY(60000) << "Initialize uint8 blobs";
        ...
        ACHECK(gray_ != nullptr);
        ACHECK(rgb_ != nullptr);
        ACHECK(bgr_ != nullptr);

        AINFO_EVERY(60000) << "Allocate CPU memory for uint8 blobs";
        gray_->cpu_data();
        rgb_->cpu_data();
        bgr_->cpu_data();

        AINFO_EVERY(60000) << "Allocate GPU memory for uint8 blobs";
        gray_->gpu_data();
        rgb_->gpu_data();
        bgr_->gpu_data();
        ...
        if (options.do_undistortion)
        {
          ...
          {
            AERROR_EVERY(60000) << "Cannot init handler";
            return false;
          }
          ...
          ACHECK(ori_gray_ != nullptr);
          ACHECK(ori_rgb_ != nullptr);
          ACHECK(ori_bgr_ != nullptr);
                                       ...
                                       const std::string &encoding)
      {
...
#ifndef USE_GPU // copy to host memory
        AINFO_EVERY(60000) << "Fill in CPU mode ...";
        if (handler_ != nullptr)
        {
          AERROR_EVERY(60000) << "Undistortion DO NOT support CPU mode!";
          return false;
        }
        if (encoding == "rgb8")
        {
        ...
        {
          AERROR_EVERY(60000) << "Unrecognized image encoding: " << encoding;
        }
#else // copy to device memory directly
        AINFO_EVERY(60000) << "Fill in GPU mode ..." << encoding;
        if (encoding == "rgb8")
        {
          if (handler_ != nullptr)
          {
        ...
        AINFO_EVERY(60000) << "Done! (" << success << ")";
        return success;
      }
  ...
  return true;
                                  ...
                                  base::Image8U *image)
      {
        ...
        case base::Color::GRAY:
          ...
          *image = (*gray_);
          break;
        default:
          AERROR_EVERY(60000) << "Unsupported Color: "
        ...
        if (options.do_crop)
        {
          AINFO_EVERY(60000) << "\tcropping ...";
          //*image = (*image)(options.crop_roi);
        }
        AINFO_EVERY(60000) << "Done!";
        return true;
      }
      ...
      bool DataProvider::to_gray_image()
      {
        if (!gray_ready_)
        {
          ...
          {
            AWARN_EVERY(60000) << "No image data filled yet, return uninitialized blob!";
            return false;
          }
        }
        return true;
