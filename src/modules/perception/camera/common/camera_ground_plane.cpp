...
bool ConvertGround4ToGround3(const float &baseline,
  ...
  if (p[0] > 1e-3) {
    AERROR_EVERY(60000) << "Have roll in the ground plane: " << p[0];
...
GroundPlaneTracker::GroundPlaneTracker(int track_length) {
  if (track_length <= 0) {
    AERROR_EVERY(60000) << "track_length, " << track_length << ", should be positive";
  }
...
bool CameraGroundPlaneDetector::DetetGround(float pitch, float camera_height,
  ...
  if (!plane.empty()) {  // assigned from outside
    ...
    AINFO_EVERY(60000) << "set ground plane from outside: " << plane[0] << ", " << plane[1]
          << ", " << plane[2] << ", " << plane[3];
...
bool CameraGroundPlaneDetector::DetectGroundFromSamples(float *vd, int count_vd,
                                                        float *inlier_ratio) {
  if (vd == nullptr) {
    AERROR_EVERY(60000) << "vd is nullptr";
    return false;
  }
  if (inlier_ratio == nullptr) {
    AERROR_EVERY(60000) << "inlier_ratio is nullptr";
    return false;
  }
  *inlier_ratio = 0.0f;
