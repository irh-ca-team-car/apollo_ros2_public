...
bool LoadAnchors(const std::string &path, std::vector<float> *anchors) {
  ...
  ifs >> num_anchors;
  if (!ifs.good()) {
    AERROR_EVERY(60000) << "Failed to get number of anchors!";
    return false;
  }
  ...
  for (int i = 0; i < num_anchors; ++i) {
    ...
    if (!ifs.good()) {
      AERROR_EVERY(60000) << "Failed to load the " << i << "-th anchor!";
      return false;
    }
  }
  ifs.close();
...
bool LoadTypes(const std::string &path,
  ...
  std::ifstream ifs(path, std::ifstream::in);
  if (!ifs.good()) {
    AERROR_EVERY(60000) << "Type_list not found: " << path;
    return false;
  }
  std::string type;
  AINFO_EVERY(60000) << "Supported types: ";
  while (ifs >> type) {
    if (base::kName2SubTypeMap.find(type) == base::kName2SubTypeMap.end()) {
      AERROR_EVERY(60000) << "Invalid type: " << type;
      return false;
    }
    (*types).push_back(base::kName2SubTypeMap.at(type));
    AINFO_EVERY(60000) << "\t\t" << type;
  }
  AINFO_EVERY(60000) << "\t\t" << (*types).size() << " in total.";
  ifs.close();
  return true;
}
bool LoadExpand(const std::string &path, std::vector<float> *expands) {
  std::ifstream ifs(path, std::ifstream::in);
  if (!ifs.good()) {
    AERROR_EVERY(60000) << "expand_list not found: " << path;
    return false;
  }
  float expand;
  AINFO_EVERY(60000) << "Expand nums [" << path << "]: ";
  while (ifs >> expand) {
    expands->push_back(expand);
    AINFO_EVERY(60000) << "\t\t" << expand;
  }
  ifs.close();
  return true;
}
bool ResizeCPU(const base::Blob<uint8_t> &src_blob,
  ...
  if (origin_channel != dst_blob->shape(3)) {
    AERROR_EVERY(60000) << "channel should be the same after resize.";
    return false;
  }
