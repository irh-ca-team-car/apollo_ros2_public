...
int lane_postprocessor_eval() {
  ...
  if (!common::LoadBrownCameraIntrinsic("params/front_6mm_intrinsics.yaml",
                                        &model)) {
    AERROR_EVERY(60000) << "LoadBrownCameraIntrinsic Error!";
    return -1;
  }
  ...
  AINFO_EVERY(60000) << "Detector: " << detector->Name();
  ...
  for (int i = 0; i < static_cast<int>(imnames.size()); ++i) {
    ...
    AINFO_EVERY(60000) << "Process file: " << FLAGS_file_title;
    ...
    AINFO_EVERY(60000) << "Detector finished!";
    // Postprocess
    ...
    if (FLAGS_lane_line_debug) {
      ...
      AINFO_EVERY(60000) << "detect_laneline_point_set num: "
