...
bool TransformServer::Init(const std::vector<std::string> &camera_names,
  ...
  try {
    ...
    AINFO_EVERY(60000) << trans.translation() << " "
    ...
    AERROR_EVERY(60000) << "load velodyne128 extrisic file error"
    ...
    AERROR_EVERY(60000) << "load velodyne128 extrisic file "
  ...
  for (const auto &yaml_file : extrinsic_filelist) {
    try {
      ...
      if (!AddTransform(child_frame_id, frame_id, trans)) {
        AINFO_EVERY(60000) << "failed to add transform from " << child_frame_id << " to "
              << frame_id;
      }
    } catch (YAML::InvalidNode &in) {
...
bool TransformServer::LoadFromFile(const std::string &tf_input,
                                   float frequency) {
  if (frequency <= 0) {
    AERROR_EVERY(60000) << "Error frequency value:" << frequency;
    return false;
  }
  ...
  AINFO_EVERY(60000) << "Load tf successfully. count: " << tf_.size()
...
void TransformServer::print() {
  for (auto item : edges_) {
    AINFO_EVERY(60000) << "----------------";
    AINFO_EVERY(60000) << item.first;
    AINFO_EVERY(60000) << "edge: ";
    AINFO_EVERY(60000) << "from " << item.second.child_frame_id << " to "
          << item.second.frame_id;
    Eigen::Affine3d trans = item.second.transform;
    Eigen::Quaterniond quat(trans.linear());
    AINFO_EVERY(60000) << "rot: " << quat.x() << " " << quat.y() << " " << quat.z() << " "
          << quat.w();
    AINFO_EVERY(60000) << "trans: " << trans.translation()[0] << " "
