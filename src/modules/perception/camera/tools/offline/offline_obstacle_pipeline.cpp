...
DEFINE_string(params_dir, "/apollo/modules/perception/params",
              "params directory");
...
int work() {
  ...
  AINFO_EVERY(60000) << "config_root: " << FLAGS_config_root;
  init_option.root_dir = FLAGS_config_root;
  AINFO_EVERY(60000) << "config_file: " << FLAGS_config_file;
  ...
  if (!fin.is_open()) {
    AERROR_EVERY(60000) << "Cannot open image list: " << FLAGS_test_list;
    return -1;
  }
  ...
  for (size_t i = 0; i < camera_names.size(); ++i) {
    ...
    AINFO_EVERY(60000) << "Init data_provider for " << camera_names[i];
  }
  ...
  for (const auto &camera_name : camera_names) {
    ...
    AINFO_EVERY(60000) << "#intrinsics of " << camera_name << ": "
  ...
  for (size_t i = 0; i < camera_names.size(); ++i) {
    ...
    if (!transform_server.QueryTransform(camera_names[i], "ground", &c2g)) {
      AINFO_EVERY(60000) << "Failed to query transform from " << camera_names[i]
            << " to ground";
      return -1;
    }
    float camera_ground_height = static_cast<float>(c2g.translation()[2]);
    AINFO_EVERY(60000) << camera_names[i] << " height: " << camera_ground_height;
    ...
    if (FLAGS_base_camera_name == camera_names[i]) {
      ...
      if (!transform_server.QueryTransform(camera_names[i],
        ...
        AINFO_EVERY(60000) << "Failed to query transform";
        return -1;
      }
  ...
  while (fin >> line) {
    ...
    if (temp_strs.size() != 2) {
      AERROR_EVERY(60000) << "invaid format in " << FLAGS_test_list;
    }
    camera_name = temp_strs[0];
    image_name = temp_strs[1];

    AINFO_EVERY(60000) << "image: " << image_name << ", camera_name: " << camera_name;
    ...
    if (FLAGS_image_color == "gray") {
      ...
      AERROR_EVERY(60000) << "Invalid color: " << FLAGS_image_color;
    }
    if (image.data == 0) {
      AERROR_EVERY(60000) << "Cannot read image: " << image_path;
      return -1;
    }
    ...
    AINFO_EVERY(60000) << "Timestamp: " << std::fixed << std::setprecision(10)
          << frame.timestamp;
    ...
    if (FLAGS_base_camera_name == camera_name) {
      ...
      if (!transform_server.QueryTransform(camera_name, FLAGS_base_camera_name,
                                           &trans)) {
        AINFO_EVERY(60000) << "Failed to query transform from " << camera_name << " to "
    ...
    AINFO_EVERY(60000) << "Project Matrix: \n" << frame.project_matrix;
    Eigen::Affine3d pose;
    ...
    if (!transform_server.QueryTransform(camera_name, "novatel", &c2n)) {
      AINFO_EVERY(60000) << "Failed to query transform from " << camera_name
            << " to novatel";
      return -1;
    }
    ...
    if (FLAGS_do_undistortion && (FLAGS_undistortion_save_dir != "") &&
      ...
      AINFO_EVERY(60000) << "Undistorted image saved to : "
    ...
    if (!cyber::common::PathExists(save_dir)) {
      AERROR_EVERY(60000) << "save_dir does not exist : " << save_dir;
    } else {
      std::ofstream myfile;
      ...
      AINFO_EVERY(60000) << "Result saved to : "
            << save_dir + "/" + image_name + FLAGS_image_ext;
    }
  }
  return 0;
