...
bool LoadFromKitti(const std::string &kitti_path, CameraFrame *frame) {
  ...
  if (fp == nullptr) {
    AERROR_EVERY(60000) << "Failed to load object file: " << kitti_path;
    return false;
  }
  while (!feof(fp)) {
    ...
    ret = fscanf(fp, "%254s %f %f %lf %f %f %f %f %f %f %f %lf %lf %lf %f %f",
                 ...
                 &score);
    AINFO_EVERY(60000) << "fscanf return: " << ret;
...
int main() {
  ...
  AINFO_EVERY(60000) << "Init DataProvider ...";
  ...
  AINFO_EVERY(60000) << "Init Detector ...";
  ...
  AINFO_EVERY(60000) << "Init Transformer ...";
  ...
  if (!fin.is_open()) {
    AERROR_EVERY(60000) << "Cannot open test list: " << FLAGS_test_list;
    return -1;
  }

  std::string image_name;
  while (fin >> image_name) {
    AINFO_EVERY(60000) << "image: " << image_name;
    ...
    if (FLAGS_pre_detected_dir != "") {
      ...
      if (!LoadFromKitti(kitti_path, &frame)) {
        AINFO_EVERY(60000) << "loading kitti result failed: " << kitti_path;
        continue;
      }
    } else {
    ...
    if (fp == nullptr) {
      AINFO_EVERY(60000) << "Failed to open result path: " << result_path;
      return -1;
    }
    int obj_id = 0;
