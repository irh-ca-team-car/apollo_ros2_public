...
PlaneMotion::PlaneMotion(int s) {
  ...
  if (mat_motion_sensor_.rows() == 3 && mat_motion_sensor_.cols() == 3) {
    ...
    AERROR_EVERY(60000) << "Unknow motion matrix size : " << mat_motion_sensor_.rows() << " "
...
void PlaneMotion::add_new_motion(double pre_image_timestamp,
  ...
  if (motion_operation_flag != RESET) {
    ...
    while (static_cast<int>(raw_motion_queue_.size()) > buffer_size_ * 10) {
      AWARN_EVERY(60000) << "MotionQueue is too large, try sync motion/image timestep";
      raw_motion_queue_.pop_front();
    }
    ...
    switch (motion_operation_flag) {
      ...
      case ACCUM_PUSH_MOTION:
        ...
        break;
      default:
        AERROR_EVERY(60000) << "motion operation flag:wrong type";
        return;
    }
  } else {
    mot_buffer_->clear();
