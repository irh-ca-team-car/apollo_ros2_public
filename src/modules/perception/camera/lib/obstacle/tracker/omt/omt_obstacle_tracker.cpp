...
bool OMTObstacleTracker::Init(const ObstacleTrackerInitOptions &options) {
  ...
  if (!cyber::common::GetProtoFromASCIIFile(omt_config, &omt_param_)) {
    AERROR_EVERY(60000) << "Read config failed: " << omt_config;
    return false;
  }

  AINFO_EVERY(60000) << "load omt parameters from " << omt_config
        << " \nimg_capability: " << omt_param_.imgcapability
        << " \nlost_age: " << omt_param_.lostage
        << " \nreserve_age: " << omt_param_.reserveage
        << " \nborder: " << omt_param_.border
        << " \ntarget_thresh: " << omt_param_.targetthresh
        << " \ncorrect_type: " << omt_param_.correcttype;

  track_id_ = 0;
  frame_num_ = 0;
  frame_list_.Init(omt_param_.imgcapability);
  gpu_id_ = options.gpu_id;
  similar_map_.Init(omt_param_.imgcapability, gpu_id_);
  similar_.reset(new CosineSimilar);
  ...
  std::string type_change_cost =
      GetAbsolutePath(options.root_dir, omt_param_.typechangecost);
...
bool OMTObstacleTracker::CombineDuplicateTargets() {
  ...
  for (size_t i = 0; i < targets_.size(); ++i) {
    ...
    for (size_t j = i + 1; j < targets_.size(); ++j) {
      ...
      if (hypo.score < omt_param_.targetcombineiouthreshold) {
        continue;
      }
  ...
  for (auto &pair : score_list) {
    ...
    target_del.Clear();
    AINFO_EVERY(60000) << "Target " << target_del.id << " is merged into Target "
...
void OMTObstacleTracker::GenerateHypothesis(const TrackObjectPtrs &objects) {
  ...
  for (size_t i = 0; i < targets_.size(); ++i) {
    ...
    for (size_t j = 0; j < objects.size(); ++j) {
      ...
      if (sa == 0) {
        hypo.score = omt_param_.weightdiffcamera.motion * sm +
                     omt_param_.weightdiffcamera.shape * ss +
                     omt_param_.weightdiffcamera.overlap * so;
      } else {
        hypo.score = (omt_param_.weightsamecamera.appearance * sa +
                      omt_param_.weightsamecamera.motion * sm +
                      omt_param_.weightsamecamera.shape * ss +
                      omt_param_.weightsamecamera.overlap * so);
      }
      ...
      if (sm < 0.045 || hypo.score < omt_param_.targetthresh) {
        continue;
      }
      score_list.push_back(hypo);
    }
  ...
  for (auto &pair : score_list) {
    ...
    used_target[pair.target] = true;
    AINFO_EVERY(60000) << "Target " << target.id << " match " << det_obj->indicator.frame_id
          << " (" << pair.object << ")"
  ...
  for (size_t i = 0; i < objects.size(); ++i) {
    if (!used_[i]) {
      ...
      if (min_tmplt.empty()  // unknown type
          || rect.height > min_tmplt[0] * omt_param_.mininitheightratio) {
        Target target(omt_param_.targetparam);
        target.Add(objects[i]);
        targets_.push_back(target);
        AINFO_EVERY(60000) << "Target " << target.id << " is created by "
...
bool OMTObstacleTracker::Associate2D(const ObstacleTrackerOptions &options,
  ...
  AINFO_EVERY(60000) << "Create " << new_count << " new target";

  for (auto &target : targets_) {
    if (target.lost_age > omt_param_.reserveage) {
      AINFO_EVERY(60000) << "Target " << target.id << " is lost";
      target.Clear();
    } else {
...
bool OMTObstacleTracker::Associate3D(const ObstacleTrackerOptions &options,
  ...
  for (auto &target : targets_) {
    ...
    if (obj->type != base::ObjectType::UNKNOWN_UNMOVABLE) {
      ...
      if (move > sqr(omt_param_.abnormalmovement) * dis) {
        AINFO_EVERY(60000) << "Target " << target.id << " is removed for abnormal movement";
