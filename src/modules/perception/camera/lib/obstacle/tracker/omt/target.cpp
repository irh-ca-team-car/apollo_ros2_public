...
void Target::Init(const omt::TargetParam &param) {
  ...
  world_lwh.SetWindow(param.worldlhwhistory);
  world_lwh_for_unmovable.SetWindow(param.worldlhwhistory);
  image_wh.SetAlpha(param.imagewhupdaterate);
  // TODO(gaohan)  should update variance when predict and update
  image_center.variance_ *= target_param_.imagecenter.initvariance;
  image_center.measure_noise_ *=
      target_param_.imagecenter.measurevariance;
  image_center.process_noise_ *=
      target_param_.imagecenter.processvariance;
  world_center.variance_ *= target_param_.worldcenter.initvariance;
  ...
  type_probs.assign(static_cast<int>(base::ObjectSubType::MAX_OBJECT_TYPE),
                    0.0f);
  world_center_for_unmovable.SetWindow(param.worldlhwhistory);
  world_velocity.SetWindow(param.meanfilterwindow);

  // record history state after kalman filter
  history_world_states_.set_capacity(param.worldstatehistory);

  // record orientation of displacement
  displacement_theta.SetWindow(param.meanfilterwindow);
  direction.SetAlpha(param.directionfilterratio);
  ...
  world_center_const.covariance_ *=
      target_param_.worldcenter.initvariance;
...
void Target::Predict(CameraFrame *frame) {
  ...
  float acc_variance = target_param_.worldcenter.processvariance;
...
void Target::Update3D(CameraFrame *frame) {
  ...
  if (!isLost()) {
    ...
    AINFO_EVERY(60000) << "dir " << id << " " << object->theta << " " << theta;
    object->theta = theta;
    ...
    if (object->type == base::ObjectType::UNKNOWN_UNMOVABLE) {
      ...
      float dis_err = target_param_.worldcenter.measurevariance *
  ...
  if (object->type == base::ObjectType::UNKNOWN_UNMOVABLE) {
    ...
    if (tracked_objects.size() > 10) {
      ...
      if (ratio > target_param_.toolargevelocityratio) {
        world_center.MagicVelocity(vel);
        ADEBUG << "Velocity too large";
      } else if (ratio > target_param_.largevelocityratio) {
        vel(0) = (x(2) + vel(0)) / 2;
        vel(1) = (x(3) + vel(1)) / 2;
        world_center.MagicVelocity(vel);
    ...
    if (speed > target_param_.velocitythreshold) {
  ...
  if (object->type != base::ObjectType::UNKNOWN_UNMOVABLE &&
    ...
    if (avg_vel_norm > target_param_.stablemovingspeed) {
      ...
      stable_moving = displacement_theta.get_variance()(0, 0) <
                      target_param_.displacementthetavar;
    }
...
void Target::UpdateType(CameraFrame *frame) {
  ...
  if (!isLost()) {
    ...
    float alpha = gaussian(
        ...
        1.0f, target_param_.typefiltervar);
...
void Target::ClappingTrackVelocity(const base::ObjectPtr &obj) {
  ...
  if (obj->type == base::ObjectType::VEHICLE) {
    ...
    if (vel_heading_angle_change >
        target_param_.abnormalvelocityheadinganglethreshold) {
...
bool Target::CheckStatic() {
  if (static_cast<int>(history_world_states_.size()) <
      target_param_.mincachedworldstatehistorysize) {
    return false;
  }
  ...
  const int min_vel_size =
      std::min(static_cast<int>(history_world_states_.size()),
               target_param_.mincachedvelocitysize);
  ...
  if (obj_type == base::ObjectType::PEDESTRIAN) {
    small_speed = speed_avg < target_param_.staticspeedthresholdped;
  } else {
    small_speed = speed_avg < target_param_.staticspeedthreshold;
  }
  if (small_speed) {
    return true;
  }
  ...
  bool not_move = false;
  if (static_cast<int>(history_world_states_.size()) >=
      std::max(target_param_.mincachedpositionsize,
               target_param_.calcavgpositionwindowsize)) {
    ...
    const int start_idx =
        ...
        static_cast<int>(target_param_.mincachedpositionsize);
    ...
    if (end_idx - start_idx >= target_param_.calcavgpositionwindowsize &&
        start_idx + target_param_.calcavgpositionwindowsize - 1 <
            static_cast<int>(history_world_states_.size()) &&
        end_idx - target_param_.calcavgpositionwindowsize + 1 >= 0) {
      for (int i = 0; i < target_param_.calcavgpositionwindowsize; ++i) {
      ...
      if (std::fabs(time_diff) > 1e-3) {
        start_position /= target_param_.calcavgpositionwindowsize;
        end_position /= target_param_.calcavgpositionwindowsize;
        move_distance = (end_position - start_position).head(2).norm();
        // check moving speed by type
        double avg_speed = move_distance / time_diff;
        if (obj_type == base::ObjectType::PEDESTRIAN) {
          not_move = avg_speed < target_param_.minmovingavgspeedped;
        } else {
          not_move = avg_speed < target_param_.minmovingavgspeed;
        }
      }
    }
  }
  ...
  if (stddev > target_param_.velocitythetavar) {  // 28.64 degree
    return true;
  }
  return false;
}
...
bool Target::isTracked() const {
  return Size() >= target_param_.trackedlife;
}
