...
void ObstacleReference::Init(const omt::ReferenceParam &ref_param, float width,
  ...
  ref_width_ = static_cast<int>(
      width / static_cast<float>(ref_param.downsampling) + 1);
  ref_height_ = static_cast<int>(
      height / static_cast<float>(ref_param.downsampling) + 1);
...
void ObstacleReference::UpdateReference(const CameraFrame *frame,
  ...
  for (auto &&reference : refs) {
    reference.area *= ref_param_.areadecay;
  }
  for (auto &&target : targets) {
    ...
    if (box.ymax - box.ymin < ref_param_.minallowheight) {
      continue;
    }
    ...
    if (box.ymax >= img_height_ + 1) {
      AERROR_EVERY(60000) << "box.ymax (" << box.ymax << ") is larger than img_height_ + 1 ("
             << img_height_ + 1 << ")";
      return;
    }
    if (box.xmax >= img_width_ + 1) {
      AERROR_EVERY(60000) << "box.xmax (" << box.xmax << ") is larger than img_width_ + 1 ("
             << img_width_ + 1 << ")";
      return;
    }
    ...
    AINFO_EVERY(60000) << "Target: " << target.id
          << " can be Ref. Type: " << base::kSubType2NameMap.at(obj->sub_type);
    int y_discrete =
        static_cast<int>(y / static_cast<float>(ref_param_.downsampling));
    int x_discrete =
        static_cast<int>(x / static_cast<float>(ref_param_.downsampling));
    ...
    if (Contain(object_template_manager_->TypeRefinedByRef(), obj->sub_type)) {
      ...
      if (obj->sub_type != base::ObjectSubType::TRAFFICCONE || height.empty()) {
        ...
        for (auto &&reference : reference_[sensor]) {
          ...
          float thres_dy =
              std::max(static_cast<float>(ref_param_.downsampling) * scale_y,
