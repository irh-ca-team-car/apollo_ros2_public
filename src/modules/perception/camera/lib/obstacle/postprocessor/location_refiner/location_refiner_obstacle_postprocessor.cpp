...
bool LocationRefinerObstaclePostprocessor::Init(
  ...
  {
    AERROR_EVERY(60000) << "Read config failed: " << postprocessor_config;
    return false;
  }

  AINFO_EVERY(60000) << "Load postprocessor parameters from " << postprocessor_config
        << " \nmin_dist_to_camera: "
        << location_refiner_param_.mindisttocamera
        << " \nroi_h2bottom_scale: "
        << location_refiner_param_.roih2bottomscale;
  return true;
}
...
bool LocationRefinerObstaclePostprocessor::Process(
  ...
  if (options.do_refinement_with_calibration_service &&
    ...
    AINFO_EVERY(60000) << "No valid ground plane in the service.";
  }
  float query_plane[4] = {
  ...
  for (auto &obj : frame->detected_objects) {
    ...
    float dist2camera = common::ISqrt(common::ISqr(object_center[0]) +
                                      common::ISqr(object_center[1]));

    // if (dist2camera > location_refiner_param_.mindisttocamera ||
    //     !is_in_rule_roi) {
    ...
    object_center[2] += dimension_hwl[0] / 2;
    ...
    obj->center(1) = -static_cast<double>(object_center[2]);
    obj->center(2) = static_cast<double>(object_center[1]);

    auto distance = obj->center(1);
    //if(fabs(distance) <=15)
    {
       auto angleV = (obj->camera_supplement.box.ymax - obj->camera_supplement.box.ymin)*70.0 /height_image;
       auto circ = 360 * 1.5 / angleV;
       auto r = circ / (2*3.14);
       distance= obj->center(1)= -r;
    }

    auto x = (obj->camera_supplement.box.xmin + obj->camera_supplement.box.xmax) /2;
    auto half_width = width_image/2.0;
    auto val = (x - half_width)/width_image;
    auto angle = (150/2.0)*val;
    obj->center(0)=tan(angle*0.01745329) * distance;

    //obj->center = frame->camera2world_pose * obj->center;
    // << ""
    AINFO_EVERY(6000) << "diff on camera z: " << z_diff_camera;
    AINFO_EVERY(6000) << "Obj center from postprocessor: " << obj->center.transpose();
  }
  return true;
}
