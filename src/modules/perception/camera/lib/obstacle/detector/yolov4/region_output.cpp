...
void filter_bbox(const MinDims &min_dims,
  ...
  AINFO_EVERY(60000) << valid_obj_idx << " of " << total_obj_idx << " obstacles kept";
  objects->resize(valid_obj_idx);
  AINFO_EVERY(60000) << "Number of detected obstacles: " << objects->size();
}
...
void get_objects_gpu(const YoloBlobs &yolo_blobs, const cudaStream_t &stream,
  ...
  const float *ori_data_vec[3] = {
      get_gpu_data(model_param.withbox3d, *yolo_blobs.det1_ori_blob),
      multi_scale
          ? get_gpu_data(model_param.withbox3d, *yolo_blobs.det2_ori_blob)
          : nullptr,
      multi_scale
          ? get_gpu_data(model_param.withbox3d, *yolo_blobs.det3_ori_blob)
          : nullptr};
  const float *dim_data_vec[3] = {
      get_gpu_data(model_param.withbox3d, *yolo_blobs.det1_dim_blob),
      multi_scale
          ? get_gpu_data(model_param.withbox3d, *yolo_blobs.det2_dim_blob)
          : nullptr,
      multi_scale
          ? get_gpu_data(model_param.withbox3d, *yolo_blobs.det3_dim_blob)
          : nullptr};

  // TODO[KaWai]: add 3 scale frbox data and light data.
  const float *lof_data =
      get_gpu_data(model_param.withfrbox, *yolo_blobs.lof_blob);
  const float *lor_data =
      get_gpu_data(model_param.withfrbox, *yolo_blobs.lor_blob);

  const float *area_id_data =
      get_gpu_data(model_param.numareas > 0, *yolo_blobs.area_id_blob);
  const float *visible_ratio_data =
      get_gpu_data(model_param.withratios, *yolo_blobs.visible_ratio_blob);
  const float *cut_off_ratio_data =
      get_gpu_data(model_param.withratios, *yolo_blobs.cut_off_ratio_blob);

  const auto &with_lights = model_param.withlights;
  ...
  for (int i = 0; i < num_candidates_vec.size(); i++) {
    ...
    get_object_kernel<<<block_size, thread_size, 0, stream>>>(
        ...
        model_param.confidencethreshold, light_vis_conf_threshold,
        light_swt_conf_threshold, model_param.withbox3d,
        model_param.withfrbox, model_param.withlights,
        model_param.withratios, multi_scale, model_param.numareas,
  ...
  for (int k = 0; k < num_classes; ++k) {
    apply_nms_gpu(res_box_data, cpu_cls_data + k * all_scales_num_candidates,
                  ...
                  model_param.confidencethreshold, top_k, nms.threshold,
  ...
  for (auto it = indices.begin(); it != indices.end(); ++it) {
    ...
    for (size_t j = 0; j < indice.size(); ++j) {
      ...
      if (scores[idx] < model_param.confidencethreshold) {
        continue;
      }
      ...
      fill_base(obj, bbox);
      fill_bbox3d(model_param.withbox3d, obj, bbox + 4);
      fill_frbox(model_param.withfrbox, obj, bbox + 8);
      fill_lights(model_param.withlights, obj, bbox + 16);
      fill_ratios(model_param.withratios, obj, bbox + 22);
      fill_area_id(model_param.numareas > 0, obj, bbox + 30);
