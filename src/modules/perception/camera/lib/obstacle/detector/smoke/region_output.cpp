...
void get_smoke_objects_cpu(const SmokeBlobs &smoke_blobs,
                     ...
                     int width, int height) {
  UNUSED(types);
  UNUSED(light_vis_conf_threshold);
  UNUSED(light_swt_conf_threshold);
  UNUSED(overlapped);
  UNUSED(idx_sm);
  ...
  for (int i = 0; i < 50; i++) {
    ...
    float score = bbox[13];
    if (score < model_param.confidencethreshold) {
      continue;
    }
    ...
    fill_smoke_bbox3d(model_param.withbox3d, obj, bbox);
