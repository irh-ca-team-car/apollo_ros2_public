...
bool MultiCueObstacleTransformer::Init(
  ...
  if (!cyber::common::GetProtoFromASCIIFile(transformer_config, &multicue_param_))
  {
    AERROR_EVERY(60000) << "Read config failed: " << transformer_config;
    return false;
  }
  AINFO_EVERY(60000) << "Load transformer parameters from " << transformer_config
        << " \nmin dimension: " << multicue_param_.mindimensionval
        << " \ndo template search: " << multicue_param_.checkdimension;
...
void MultiCueObstacleTransformer::SetObjMapperOptions(
  ...
  *theta_ray =
  ...
  obj_mapper_options->check_dimension = multicue_param_.checkdimension;
...
int MultiCueObstacleTransformer::MatchTemplates(base::ObjectSubType sub_type,
  ...
  switch (sub_type) {
    ...
    case base::ObjectSubType::BUS:
      ...
      break;
    default:
      if (min_dimension_val < multicue_param_.mindimensionval) {
        common::IScale3(dimension_hwl, multicue_param_.mindimensionval *
