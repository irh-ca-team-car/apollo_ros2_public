...
bool TrafficLightRecognition::Init(
  ...
  if (!cyber::common::GetProtoFromASCIIFile(proto_path, &recognize_param_)) {
  ...
  classify_quadrate_->Init(recognize_param_.quadratemodel, options.gpu_id,
                           options.root_dir);
  classify_vertical_->Init(recognize_param_.verticalmodel, options.gpu_id,
                           options.root_dir);
  classify_horizontal_->Init(recognize_param_.horizontalmodel,
