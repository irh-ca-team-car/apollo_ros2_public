...
bool TrafficLightDetection::Init(
  ...
  if (!cyber::common::GetProtoFromASCIIFile(proto_path, &detection_param_))
  {
  ...
  AINFO_EVERY(60000) << "TL detection param: " << param_str;*/

  std::string model_root =
      GetAbsolutePath(options.root_dir, detection_param_.modelname);
  AINFO_EVERY(60000) << "model_root " << model_root;

  std::string proto_file =
      GetAbsolutePath(model_root, detection_param_.protofile);
  AINFO_EVERY(60000) << "proto_file " << proto_file;

  std::string weight_file =
      GetAbsolutePath(model_root, detection_param_.weightfile);
  AINFO_EVERY(60000) << "weight_file " << weight_file;

  if (detection_param_.isbgr) {
    data_provider_image_option_.target_color = base::Color::BGR;
    mean_[0] = detection_param_.meanb;
    mean_[1] = detection_param_.meang;
    mean_[2] = detection_param_.meanr;
  } else {
    data_provider_image_option_.target_color = base::Color::RGB;
    mean_[0] = detection_param_.meanr;
    mean_[1] = detection_param_.meang;
    mean_[2] = detection_param_.meanb;
  }

  net_inputs_.push_back(detection_param_.inputblobname);
  net_inputs_.push_back(detection_param_.imparamblobname);
  net_outputs_.push_back(detection_param_.outputblobname);

  AINFO_EVERY(60000) << "net input blobs: "
        << std::accumulate(net_inputs_.begin(), net_inputs_.end(),
                           std::string(""),
                           [](std::string &sum, const std::string &s) {
                             return sum + "\n" + s;
                           });
  AINFO_EVERY(60000) << "net output blobs: "
  ...
  const auto &model_type = detection_param_.modeltype;
  ...
  AINFO_EVERY(60000) << "rt_net_ create succeed";
  rt_net_->set_gpu_id(options.gpu_id);
  AINFO_EVERY(60000) << "set gpu id " << options.gpu_id;
  gpu_id_ = options.gpu_id;

  int resize_height = detection_param_.mincropsize;
  int resize_width = detection_param_.mincropsize;
  max_batch_size_ = detection_param_.maxbatchsize;
  ...
  AINFO_EVERY(60000) << "net init success.";
  ...
  switch (detection_param_.cropmethod) {
    default:
    case 0:
      crop_.reset(new CropBox(detection_param_.cropscale,
                              detection_param_.mincropsize));
      break;
    case 1:
...
bool TrafficLightDetection::Inference(
  ...
  input_img_blob->Reshape(static_cast<int>(batch_num),
                          static_cast<int>(detection_param_.mincropsize),
                          static_cast<int>(detection_param_.mincropsize),
                          3);
  ...
  for (size_t i = 0; i < batch_num; ++i) {
    auto offset = i * param_blob_length_;
    param_data[offset + 0] =
        static_cast<float>(detection_param_.mincropsize);
    param_data[offset + 1] =
        static_cast<float>(detection_param_.mincropsize);
    param_data[offset + 2] = 1;
  }

  AINFO_EVERY(60000) << "reshape inputblob " << input_img_blob->shape_string();

  for (size_t i = 0; i < batch_num; ++i) {
    base::TrafficLightPtr light = lights->at(i);
    base::RectI cbox;
    crop_->getCropBox(img_width, img_height, light, &cbox);
    AINFO_EVERY(60000) << "get crop box success " << cbox.x << " " << cbox.y << " "
    ...
    if (!OutOfValidRegion(cbox, img_width, img_height) && cbox.Area() > 0) {
      ...
      AINFO_EVERY(60000) << "get image data success ";
  ...
  rt_net_->Infer();
  cudaDeviceSynchronize();
  AINFO_EVERY(60000) << "rt_net run success";
...
bool TrafficLightDetection::Detect(const TrafficLightDetectorOptions &options,
  ...
  {
    AINFO_EVERY(60000) << "no lights to detect";
    return true;
  }
  ...
  AINFO_EVERY(60000) << "detection input " << lights_ref.size() << " lights";
  ...
  AINFO_EVERY(60000) << "Dump output Done! Get box num:" << detected_bboxes_.size();
  ...
  AINFO_EVERY(60000) << "start select";
  select_.SelectTrafficLights(detected_bboxes_, &lights_ref);
  AINFO_EVERY(60000) << "select success";

  AINFO_EVERY(60000) << "detection success";
  return true;
}
...
bool TrafficLightDetection::SelectOutputBoxes(
  ...
  AINFO_EVERY(60000) << "output blob size " << output_blob->shape(0) << " "
  ...
  for (int candidate_id = 0; candidate_id < result_box_num; candidate_id++) {
    ...
    if (img_id >= static_cast<int>(crop_box_list.size())) {
      AINFO_EVERY(60000) << "img id " << img_id << " > " << crop_box_list.size();
      continue;
    }
    ...
    if (static_cast<int>(tmp->region.detect_class_id) >= 0) {
      ...
      if (OutOfValidRegion(tmp->region.detection_roi,
        ...
        AINFO_EVERY(60000) << "Invalid width or height or x or y: "
              << tmp->region.detection_roi.width << " | "
              << tmp->region.detection_roi.height << " | "
              << tmp->region.detection_roi.x << " | "
              << tmp->region.detection_roi.y;
        AINFO_EVERY(60000) << " max width " << crop_box_list.at(img_id).width
      ...
      AINFO_EVERY(60000) << "detect roi x " << tmp->region.detection_roi.x << " "
            << tmp->region.detection_roi.y << " "
            << tmp->region.detection_roi.width << " "
            << tmp->region.detection_roi.height;
...
void TrafficLightDetection::ApplyNMS(std::vector<base::TrafficLightPtr> *lights,
                                     double iou_thresh) {
  if (lights == nullptr) {
    AERROR_EVERY(60000) << "lights are not available";
    return;
  }
