...
bool TLPreprocessor::Init(const TrafficLightPreprocessorInitOptions &options) {
  ...
  if (!projection_.Init(projection_init_option)) {
    AERROR_EVERY(60000) << "init multi_camera_projection failed.";
    return false;
  }
  ...
  AINFO_EVERY(60000) << "preprocessor init succeed";
...
bool TLPreprocessor::UpdateCameraSelection(
  ...
  AINFO_EVERY(60000) << "TLPreprocessor Got signal number: " << lights->size()
        << ", ts: " << timestamp;
  if (lights->empty()) {
    AINFO_EVERY(60000) << "No signals, select camera with max focal length: "
  ...
  if (!ProjectLightsAndSelectCamera(pose, option,
    ...
    AERROR_EVERY(60000) << "project_lights_and_select_camera failed, ts: " << timestamp;
  }

  AINFO_EVERY(60000) << "selected_camera_id: " << selected_camera_name_.second;
...
bool TLPreprocessor::SyncInformation(const double image_timestamp,
  ...
  AINFO_EVERY(60000) << "ready to sync information";

  if (!projection_.HasCamera(cam_name)) {
    AERROR_EVERY(60000) << "sync_image failed, "
           << "get invalid camera_name: " << cam_name;
    return false;
  }

  AINFO_EVERY(60000) << "Enter TLPreprocessor::sync_image. proj_ts: " << proj_ts
  ...
  AINFO_EVERY(60000) << "sync_image succeeded.";
...
bool TLPreprocessor::UpdateLightsProjection(
  ...
  AINFO_EVERY(60000) << "clear lights_outside_image_ " << lights_outside_image_.size();

  if (lights->empty()) {
    AINFO_EVERY(60000) << "No lights to be projected";
    return true;
  }

  if (!ProjectLights(pose, camera_name, lights, &lights_on_image_,
                     &lights_outside_image_)) {
    AERROR_EVERY(60000) << "update_lights_projection project lights on " << camera_name
           << " image failed";
    return false;
  }

  if (lights_outside_image_.size() > 0) {
    AERROR_EVERY(60000) << "update_lights_projection failed,"
  ...
  for (const base::TrafficLightPtr &light : lights_on_image_) {
    if (OutOfValidRegion(light->region.projection_roi,
      ...
      AINFO_EVERY(60000) << "update_lights_projection light project out of image region. "
            << "camera_name: " << camera_name;
      return false;
    }
  }

  AINFO_EVERY(60000) << "UpdateLightsProjection success";
  return true;
}
...
bool TLPreprocessor::SetCameraWorkingFlag(const std::string &camera_name,
  ...
  if (!projection_.HasCamera(camera_name)) {
    AERROR_EVERY(60000) << "SetCameraWorkingFlag failed, "
           << "get invalid camera_name: " << camera_name;
    return false;
  }
  camera_is_working_flags_[camera_name] = is_working;
  AINFO_EVERY(60000) << "SetCameraWorkingFlag succeeded, camera_name: " << camera_name
...
bool TLPreprocessor::GetCameraWorkingFlag(const std::string &camera_name,
  ...
  if (!projection_.HasCamera(camera_name)) {
    AERROR_EVERY(60000) << "GetCameraWorkingFlag failed, "
           << "get invalid camera_name: " << camera_name;
    return false;
  }
...
void TLPreprocessor::SelectCamera(
  ...
  auto min_focal_len_working_camera = GetMinFocalLenWorkingCameraName();
  AINFO_EVERY(60000) << "working camera with minimum focal length: "
  ...
  for (size_t cam_id = 0; cam_id < lights_on_image_array->size(); ++cam_id) {
    ...
    if (!GetCameraWorkingFlag(camera_name, &is_working) || !is_working) {
      AINFO_EVERY(60000) << "camera " << camera_name << "is not working";
      continue;
    }
    ...
    if (camera_name != min_focal_len_working_camera) {
      ...
      if (lights_outside_image_array->at(cam_id).size() > 0) {
        AINFO_EVERY(60000) << "light project out of image, "
              << "camera_name: " << camera_name
      ...
      for (const auto light : lights) {
        ...
        if (OutOfValidRegion(light->region.projection_roi,
                             ...
                             option.image_borders_size->at(camera_name))) {
          ok = false;
          AINFO_EVERY(60000) << "light project out of image region, "
  ...
  AINFO_EVERY(60000) << "select_camera selection: " << *selected_camera_name;
}
...
bool TLPreprocessor::ProjectLights(
  ...
  if (lights->empty()) {
    AINFO_EVERY(60000) << "project_lights get empty signals.";
    return true;
  }
  if (!projection_.HasCamera(camera_name)) {
    AERROR_EVERY(60000) << "project_lights get invalid camera_name: " << camera_name;
    return false;
  }
...
bool TLPreprocessor::ProjectLightsAndSelectCamera(
  ...
  if (selected_camera_name == nullptr) {
    AERROR_EVERY(60000) << "selected_camera_name is not available";
    return false;
  }
  if (lights == nullptr) {
    AERROR_EVERY(60000) << "lights is not available";
    return false;
  }
  ...
  for (size_t cam_id = 0; cam_id < num_cameras_; ++cam_id) {
    ...
    if (!ProjectLights(pose, camera_name, lights,
      ...
      AERROR_EVERY(60000) << "select_camera_by_lights_projection project lights on "
