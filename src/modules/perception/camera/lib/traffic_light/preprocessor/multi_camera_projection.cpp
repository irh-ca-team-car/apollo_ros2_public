...
bool MultiCamerasProjection::Init(const MultiCamerasInitOption& options) {
  ...
  for (size_t i = 0; i < options.camera_names.size(); ++i) {
    ...
    if (!(camera_models_[cur_camera_name])) {
      AERROR_EVERY(60000) << "get null camera_model, camera_name: " << cur_camera_name;
      return false;
    }
  ...
  AINFO_EVERY(60000) << "camera_names sorted by descending focal lengths: "
...
bool MultiCamerasProjection::Project(const CarPose& pose,
  ...
  if (!HasCamera(option.camera_name)) {
    AERROR_EVERY(60000) << "no camera: " << option.camera_name;
    return false;
  }
  ...
  bool ret = false;
  AINFO_EVERY(60000) << "project use camera_name: " << option.camera_name;
...
int MultiCamerasProjection::getImageWidth(
  ...
  if (!HasCamera(camera_name)) {
    AERROR_EVERY(60000) << "getImageWidth failed, camera_name: " << camera_name;
    return -1;
  }
...
int MultiCamerasProjection::getImageHeight(
    const std::string& camera_name) const {
  if (!HasCamera(camera_name)) {
    AERROR_EVERY(60000) << "getImageHeight failed, camera_name: " << camera_name;
    return -1;
  }
...
bool MultiCamerasProjection::BoundaryBasedProject(
  ...
  if (camera_model.get() == nullptr) {
    AERROR_EVERY(60000) << "camera_model is not available.";
    return false;
  }
  ...
  if (bound_size < 4) {
    AERROR_EVERY(60000) << "invalid bound_size " << bound_size;
    return false;
  }
