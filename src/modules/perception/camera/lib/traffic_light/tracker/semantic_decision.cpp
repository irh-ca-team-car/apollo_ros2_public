...
bool SemanticReviser::Init(const TrafficLightTrackerInitOptions &options) {
  ...
  if (!cyber::common::GetProtoFromASCIIFile(proto_path, &semantic_param_))
  {
    AERROR_EVERY(60000) << "load proto param failed, root dir: " << options.root_dir;
    return false;
  }

  int non_blink_coef = 2;
  revise_time_s_ = semantic_param_.revisetimesecond;
  blink_threshold_s_ = semantic_param_.blinkthresholdsecond;
  hysteretic_threshold_ = semantic_param_.hystereticthresholdcount;
