...
bool LaneBasedCalibrator::Process(const EgoLane &lane, const float &velocity,
  ...
  if (!IsTravelingStraight(vehicle_yaw_changed)) {
    AINFO_EVERY(60000) << "Do not calibate if not moving straight: "
  ...
  if (!GetVanishingPoint(lane, &vp_cur)) {
    AINFO_EVERY(60000) << "Lane is not valid for calibration.";
    return false;
  }
  ...
  if (!PopVanishingPoint(&vp_work)) {
    AINFO_EVERY(60000) << "Driving distance is not long enough";
    return false;
  }
  ...
  if (!GetPitchFromVanishingPoint(vp_work, &pitch_cur_)) {
    AINFO_EVERY(60000) << "Failed to estimate pitch from vanishing point.";
    return false;
  }
  ...
  if (!AddPitchToHistogram(pitch_cur_)) {
    AINFO_EVERY(60000) << "Calculated pitch is out-of-range.";
    return false;
  }
...
bool LaneBasedCalibrator::GetVanishingPoint(const EgoLane &lane,
  ...
  if (!get_line_seg_left) {
    AINFO_EVERY(60000) << "Left lane is too short.";
    return false;
  }
  ...
  if (!get_line_seg_right) {
    AINFO_EVERY(60000) << "Right lane is too short.";
    return false;
  }
