...
bool LaneLineCalibrator::Calibrate(const CalibratorOptions &options,
  ...
  if (pitch_angle == nullptr) {
    AERROR_EVERY(60000) << "pitch_angle is not available";
    return false;
  }
  EgoLane ego_lane;
  if (!LoadEgoLaneline(*options.lane_objects, &ego_lane)) {
    AINFO_EVERY(60000) << "Failed to get the ego lane.";
    return false;
  }
  ...
  ADEBUG_EVERY(60000) << "c2w transform this frame:\n"
  ...
  if (!is_first_frame_) {
    ...
    ADEBUG_EVERY(60000) << timediff_yawrate_velocity_text;
  }
  ...
  if (updated) {
    *pitch_angle = calibrator_.get_pitch_estimation();
    ...
    AINFO_EVERY(60000) << "#updated pitch angle: " << *pitch_angle;
    AINFO_EVERY(60000) << "#vanishing row: " << vanishing_row;
  }
...
bool LaneLineCalibrator::LoadEgoLaneline(
  ...
  if (ego_lane == nullptr) {
    AERROR_EVERY(60000) << "ego_lane is not available";
    return false;
  }
