...
bool DenselineLanePostprocessor::Init(
  ...
  if (!cyber::common::GetProtoFromASCIIFile(proto_path, &denseline_param)) {
    AERROR_EVERY(60000) << "Failed to load proto param, root dir: " << options.root_dir;
    return false;
  }
  const auto& model_param = denseline_param.modelparam;
  input_offset_x_ = model_param.inputoffsetx;
  input_offset_y_ = model_param.inputoffsety;
  input_crop_width_ = model_param.cropwidth;
  input_crop_height_ = model_param.cropheight;
  AINFO_EVERY(60000) << " offset_x=" << input_offset_x_ << " offset_y=" << input_offset_y_
  ...
  AINFO_EVERY(60000) << "postprocessor_config: " << postprocessor_config;
  if (!cyber::common::GetProtoFromASCIIFile(postprocessor_config,
                                       &lane_postprocessor_param_)) {
    AERROR_EVERY(60000) << "Read config detect_param failed: " << postprocessor_config;
    return false;
  }
  /*std::string param_str;
  google::protobuf::TextFormat::PrintToString(lane_postprocessor_param_,
                                              &param_str);
  AINFO_EVERY(60000) << "lane_postprocessor param: " << param_str;*/
  //
  omit_bottom_line_num_ = lane_postprocessor_param_.omitbottomlinenum;
  laneline_map_score_thresh_ =
      lane_postprocessor_param_.lanelinemapscorethresh;
  laneline_point_score_thresh_ =
      lane_postprocessor_param_.lanelinepointscorethresh;
  laneline_point_min_num_thresh_ =
      lane_postprocessor_param_.lanelinepointminnumthresh;
  cc_valid_pixels_ratio_ = lane_postprocessor_param_.ccvalidpixelsratio;
  laneline_reject_dist_thresh_ =
      lane_postprocessor_param_.lanelinerejectdistthresh;
...
bool DenselineLanePostprocessor::Process2D(
  ...
  AINFO_EVERY(60000) << "[AfterProcess2D]lane_lines_num: " << frame->lane_objects.size();
...
bool DenselineLanePostprocessor::SelectLanecenterCCs(
  ...
  if (lane_ccs_num == 0) {
    AINFO_EVERY(60000) << "lane_ccs_num is 0.";
    return false;
  }
  ...
  for (int i = 0; i < lane_ccs_num; i++) {
    ...
    if (static_cast<int>(pixels.size()) < valid_pixels_num) {
      AINFO_EVERY(60000) << "pixels_size < valid_pixels_num.";
      continue;
    }
  ...
  if (valid_ccs_num == 0) {
    AINFO_EVERY(60000) << "valid_ccs_num is 0.";
    return false;
  }
...
bool DenselineLanePostprocessor::LocateLanelinePointSet(
  ...
  if (channels < net_model_channel_num_) {
    AERROR_EVERY(60000) << "channel (" << channels << ") is less than net channel ("
...
bool DenselineLanePostprocessor::ClassifyLaneCCsPosTypeInImage(
  ...
  if (min_index == -1) {
    AERROR_EVERY(60000) << "min_index=-1";
    return false;
  }
  //  0: ego-lane
  //  1: adj-left lane
...
void DenselineLanePostprocessor::AddImageLaneline(
  ...
  if (avg_dist >= laneline_reject_dist_thresh_) {
    AERROR_EVERY(60000) << "avg_dist>=laneline_reject_dist_thresh_";
    return;
  }
