  ...
  namespace perception
  {
    namespace camera
    {
          ...
          const LanePostprocessorInitOptions &options)
      {
        ...
        if (!GetProtoFromASCIIFile(proto_path, &darkscnn_param))
        {
          AINFO_EVERY(60000) << "Failed to load proto param, root dir: " << options.root_dir;
          return false;
        }
        const auto &model_param = darkscnn_param.modelparam;
        input_offset_x_ = model_param.inputoffsetx;
        input_offset_y_ = model_param.inputoffsety;
        lane_map_width_ = model_param.resizewidth;
        lane_map_height_ = model_param.resizeheight;
        ...
        if (!GetProtoFromASCIIFile(postprocessor_config, &lane_postprocessor_param_))
        {
          AERROR_EVERY(60000) << "Failed to read config detect_param: " << postprocessor_config;
          return false;
        }

        roi_height_ = lane_postprocessor_param_.roiheight;
        roi_start_ = lane_postprocessor_param_.roistart;
        roi_width_ = lane_postprocessor_param_.roiwidth;

        lane_type_num_ = static_cast<int>(spatialLUTind.size());
        AINFO_EVERY(60000) << "lane_type_num_: " << lane_type_num_;
        return true;
      }
          ...
          const LanePostprocessorOptions &options, CameraFrame *frame)
      {
        ...
        // AINFO_EVERY(60000) << "Time for writing: " << microseconds - microseconds_2 << " us";
