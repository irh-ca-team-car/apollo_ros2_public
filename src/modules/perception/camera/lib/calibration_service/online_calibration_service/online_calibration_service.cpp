...
bool OnlineCalibrationService::QueryGroundPlaneInCameraFrame(
  ...
  if (plane_param == nullptr) {
    AERROR_EVERY(60000) << "plane_param is nullptr";
    return false;
  }
  if (!is_service_ready_) {
...
bool OnlineCalibrationService::QueryCameraToGroundHeightAndPitchAngle(
  ...
  if (height == nullptr) {
    AERROR_EVERY(60000) << "height is nullptr";
    return false;
  }
  if (pitch == nullptr) {
    AERROR_EVERY(60000) << "pitch is nullptr";
    return false;
  }
  if (!is_service_ready_) {
    *height = *pitch = 0.0;
...
void OnlineCalibrationService::Update(CameraFrame *frame) {
  if (frame == nullptr) {
    AERROR_EVERY(60000) << "frame is nullptr";
    return;
  }
  ...
  auto iter = name_camera_status_map_.find(sensor_name_);
  AINFO_EVERY(60000) << "camera_ground_height: " << iter->second.camera_ground_height
        << " meter.";
  AINFO_EVERY(60000) << "pitch_angle: " << iter->second.pitch_angle * 180.0 / M_PI
        << " degree.";
  // ACHECK(BuildIndex());
