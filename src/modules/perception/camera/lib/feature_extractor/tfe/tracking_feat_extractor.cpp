...
bool TrackingFeatureExtractor::Init(
  ...
  std::string config_path = cyber::common::GetAbsolutePath(init_options.root_dir, init_options.conf_file);
  if (!cyber::common::GetProtoFromASCIIFile(config_path, &feat_param))
  {
    AERROR_EVERY(60000) << "read proto_config fail";
    return false;
  }
  if (feat_param.extractor.size() != 1) {
    AERROR_EVERY(60000) << "extractor should be 1";
    return false;
  }
  ...
  for (size_t i = 0; i < feat_param.extractor.size(); i++) {
    switch (feat_param.extractor[i].feattype) {
      case tracking_feature::ExtractorParam::FeatureType::ROIPooling:
        init_roipooling(init_options,
                        feat_param.extractor[i].roipoolingparam);
        break;
    }
  }
  if (roi_poolings_.empty()) {
    AERROR_EVERY(60000) << "no proper extractor";
    return false;
  }
...
void TrackingFeatureExtractor::init_roipooling(
  ...
  int pooled_w = param.pooledw;
  int pooled_h = param.pooledh;
  bool use_floor = param.usefloor;
