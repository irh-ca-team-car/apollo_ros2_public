...
void WriteCamera2World(std::ofstream &fout, int frame_num,
  ...
  if (!fout.is_open()) {
    AERROR_EVERY(60000) << "Cannot write Camera2World!";
    return;
  }
  fout << frame_num;
...
void WriteTracking(std::ofstream &fout, int frame_num,
  ...
  if (!fout.is_open()) {
    AERROR_EVERY(60000) << "Cannot write tracking!";
    return;
  }
  char output[500];
...
int WriteDetections(const bool enabled, const std::string &out_path,
  ...
  AINFO_EVERY(60000) << "Write detection to: " << out_path;
  if (!outf.is_open()) {
    AERROR_EVERY(60000) << "Cannot open output file: " << out_path;
    return -1;
  }
  ...
  for (const auto &obj : objects) {
    switch (obj->type) {
      ...
      case base::ObjectType::PEDESTRIAN:
        ...
        break;
      default:
        AERROR_EVERY(60000) << "Unknown object type: " << static_cast<int>(obj->type);
        outf << "Unknown";
    }
...
int WriteDetections(const bool enabled, const std::string &out_path,
  ...
  AINFO_EVERY(60000) << "Feature dim: " << feature_dim;
...
int WriteLanelines(const bool enabled, const std::string &save_path,
  ...
  if (file_save == nullptr) {
    AERROR_EVERY(60000) << "Failed to open lane save path: " << save_path;
    return -1;
  }
  int lane_line_size = static_cast<int>(lane_objects.size());
  AINFO_EVERY(60000) << "Lane line num: " << lane_line_size;
  fprintf(file_save, "[\n");
...
int WriteCalibrationOutput(bool enabled, const std::string &out_path,
  ...
  if (!frame->calibration_service->QueryCameraToGroundHeightAndPitchAngle(
    ...
    AERROR_EVERY(60000) << "Failed to query camera to ground height and pitch.";
    return -1;
  }

  FILE *file_save = fopen(out_path.data(), "wt");
  if (file_save == nullptr) {
    AERROR_EVERY(60000) << "Failed to open output path: " << out_path.data();
    return -1;
  }
...
void WriteFusionTracking(std::ofstream &fout, int frame_num,
                         ...
                         const std::vector<base::ObjectPtr> &tracked_object) {
  if (!fout.is_open()) {
    AERROR_EVERY(60000) << "Failed to write tracking!";
    return;
  }
  AINFO_EVERY(60000) << "Write track results: " << frame_num;
  if (camera_name == "front_12mm") {
    ...
    AERROR_EVERY(60000) << "Unknown camera name: " << camera_name;
  }
}
}  // namespace camera
}  // namespace perception
