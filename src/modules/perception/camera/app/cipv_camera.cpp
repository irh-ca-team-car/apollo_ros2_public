...
bool Cipv::GetEgoLane(const std::vector<base::LaneLine> &lane_objects,
  ...
  for (size_t i = 0; i < lane_objects.size(); ++i) {
    ...
    if (lane_object.pos_type == base::LaneLinePositionType::EGO_LEFT) {
      if (debug_level_ >= 2) {
        AINFO_EVERY(60000) << "[GetEgoLane]LEFT_image_lane_objects[" << i
              << "].curve_image_point_set.size(): " << curve_image_point_size;
        AINFO_EVERY(60000) << "[GetEgoLane]LEFT_ground_lane_objects[" << i
      ...
      if (debug_level_ >= 2) {
        AINFO_EVERY(60000) << "[GetEgoLane]RIGHT_image_lane_objects[" << i
              << "].curve_image_point_set.size(): " << curve_image_point_size;
        AINFO_EVERY(60000) << "[GetEgoLane]RIGHT_ground_lane_objects[" << i
              << "].curve_car_coord_point_set_size: "
              << curve_car_coord_point_set_size;
      }
      if (curve_image_point_size < min_laneline_length_for_cipv_) {
...
bool Cipv::ElongateEgoLane(const std::vector<base::LaneLine> &lane_objects,
  ...
  if (b_left_valid && b_right_valid) {
    ...
    if (debug_level_ >= 2) {
      AINFO_EVERY(60000) << "Both lanes are fine";
    }
    ...
    if (debug_level_ >= 2) {
      AINFO_EVERY(60000) << "Made left lane with offset: " << offset_distance;
    }
    ...
    if (debug_level_ >= 2) {
      AINFO_EVERY(60000) << "Made right lane with offset: " << offset_distance;
    }
  }
  return true;
}
...
bool Cipv::CreateVirtualEgoLane(const float yaw_rate, const float velocity,
  ...
  MakeVirtualEgoLaneFromYawRate(yaw_rate, velocity, offset_distance,
                                ...
                                &egolane_ground->right_line);
  if (debug_level_ >= 2) {
    AINFO_EVERY(60000) << "Made both lane_objects with size of "
...
bool Cipv::FindClosestObjectImage(const std::shared_ptr<base::Object> &object,
  ...
  if (debug_level_ >= 3) {
    AINFO_EVERY(60000) << "object->camera_supplement.box = base::RectF("
          << object->camera_supplement.box.xmin << ", "
  ...
  *distance =
  ...
  if (debug_level_ >= 2) {
    AINFO_EVERY(60000) << "start(" << closted_object_edge->start_point(0) << ", "
          << closted_object_edge->start_point(1) << ")->";
    AINFO_EVERY(60000) << "end(" << closted_object_edge->end_point(0) << ", "
          << closted_object_edge->end_point(1) << ")";
    AINFO_EVERY(60000) << "closest distance: " << *distance;
  }
  return true;
}
...
bool Cipv::FindClosestObjectGround(const std::shared_ptr<base::Object> &object,
                                   ...
                                   LineSegment2Df *closted_object_edge,
                                   float *distance) {
  UNUSED(egolane_ground);
  if (debug_level_ >= 2)
  {
    AINFO_EVERY(60000) << "object->track_id = " << object->track_id;
  }
  float size_x = object->size(0);
  float size_y = object->size(1);
  float size_z = object->size(2);
  ...
  if (debug_level_ >= 3) {
    ...
    AINFO_EVERY(60000) << "object.center(0) = " << object->center(0) << ";";
    AINFO_EVERY(60000) << "object.center(1) = " << object->center(1) << ";";
    AINFO_EVERY(60000) << "object.center(2) = " << object->center(2) << ";";
    AINFO_EVERY(60000) << "pos(0) = " << pos(0) << ";";
    AINFO_EVERY(60000) << "pos(1) = " << pos(1) << ";";
    AINFO_EVERY(60000) << "pos(2) = " << pos(2) << ";";
    AINFO_EVERY(60000) << "object->camera_supplement.local_center(0) = "
          << object->camera_supplement.local_center(0) << ";";
    AINFO_EVERY(60000) << "object->camera_supplement.local_center(1) = "
          << object->camera_supplement.local_center(1) << ";";
    AINFO_EVERY(60000) << "object->camera_supplement.local_center(2) = "
          << object->camera_supplement.local_center(2) << ";";
    AINFO_EVERY(60000) << "theta_ray = " << theta_ray << ";";
    AINFO_EVERY(60000) << "object->camera_supplement.alpha = "
          << object->camera_supplement.alpha << ";";
    AINFO_EVERY(60000) << "theta = " << theta << ";";
    AINFO_EVERY(60000) << "object.anchor_point(0) = " << object->anchor_point(0) << ";";
    AINFO_EVERY(60000) << "object.anchor_point(1) = " << object->anchor_point(1) << ";";
    AINFO_EVERY(60000) << "object.anchor_point(2) = " << object->anchor_point(2) << ";";
    AINFO_EVERY(60000) << "object.direction(0) = " << object->direction(0) << ";";
    AINFO_EVERY(60000) << "object.direction(1) = " << object->direction(1) << ";";
    AINFO_EVERY(60000) << "object.direction(2) = " << object->direction(2) << ";";
    AINFO_EVERY(60000) << "object.size(0) = " << object->size(0) << ";";
    AINFO_EVERY(60000) << "object.size(1) = " << object->size(1) << ";";
    AINFO_EVERY(60000) << "object.size(2) = " << object->size(2) << ";";
  }
  ...
  if (debug_level_ >= 2) {
    AINFO_EVERY(60000) << "P0(" << p[0](0) << ", " << p[0](1) << ")";
    AINFO_EVERY(60000) << "P1(" << p[1](0) << ", " << p[1](1) << ")";
    AINFO_EVERY(60000) << "P2(" << p[2](0) << ", " << p[2](1) << ")";
    AINFO_EVERY(60000) << "P3(" << p[3](0) << ", " << p[3](1) << ")";
  }
  ...
  if (left_index < 0 || right_index < 0 || left_index == right_index) {
    if (debug_level_ >= 2) {
      AINFO_EVERY(60000) << "left_index: " << left_index;
      AINFO_EVERY(60000) << "right_index: " << right_index;
    }
    return false;
  }
  ...
  *distance =
  ...
  if (debug_level_ >= 2) {
    ...
    AINFO_EVERY(60000) << "end(" << closted_object_edge->end_point(0) << ", "
          << closted_object_edge->end_point(1) << ")";
    AINFO_EVERY(60000) << "closest distance to p[" << closest_index << "]("
...
bool Cipv::AreDistancesSane(const float distance_start_point_to_right_lane,
  ...
  if (distance_start_point_to_right_lane > kMaxDistObjectToLaneInMeter) {
    if (debug_level_ >= 1) {
      AINFO_EVERY(60000) << "distance from start to right lane("
  ...
  if (distance_start_point_to_left_lane > kMaxDistObjectToLaneInMeter) {
    if (debug_level_ >= 1) {
      AINFO_EVERY(60000) << "distance from start to left lane("
  ...
  if (distance_end_point_to_right_lane > kMaxDistObjectToLaneInMeter) {
    if (debug_level_ >= 1) {
      AINFO_EVERY(60000) << "distance from end to right lane("
  ...
  if (distance_end_point_to_left_lane > kMaxDistObjectToLaneInMeter) {
    if (debug_level_ >= 1) {
      AINFO_EVERY(60000) << "distance from end to left lane("
  ...
  if (distance > kMaxVehicleWidthInMeter) {
    if (debug_level_ >= 1) {
      AINFO_EVERY(60000) << "width of vehicle (" << distance << " m) is too long";
    }
    return false;
  }
  ...
  // AINFO_EVERY(60000) << "Distances are sane!";
...
bool Cipv::IsPointLeftOfLine(const Point2Df &point,
  ...
  if (cross_product > 0.0f) {
    if (debug_level_ >= 2) {
      AINFO_EVERY(60000) << "point (" << point(0) << ", " << point(1)
...
bool Cipv::IsObjectInTheLaneImage(const std::shared_ptr<base::Object> &object,
  ...
  if (!b_valid_object) {
    if (debug_level_ >= 1) {
      AINFO_EVERY(60000) << "The closest edge of an object is not available";
    }
    return false;
  }
  *object_distance = distance;
  ...
  if (debug_level_ >= 3) {
    AINFO_EVERY(60000) << "egolane_image.left_line.line_point.size(): "
          << egolane_image.left_line.line_point.size();
  }
  if (egolane_image.left_line.line_point.size() <= 1) {
    if (debug_level_ >= 1) {
      AINFO_EVERY(60000) << "No left lane";
    }
    return false;
  }
  ...
  if (closest_index >= 0) {
    ...
    if (debug_level_ >= 3) {
      AINFO_EVERY(60000) << "[Left] closest_index: " << closest_index
            << ", shortest_distance: " << shortest_distance;
      AINFO_EVERY(60000) << "Should be left to be selected";
    }
    if (IsPointLeftOfLine(
      ...
      AINFO_EVERY(60000) << "The left lane is clear";
    }
  }

  if (debug_level_ >= 3) {
    AINFO_EVERY(60000) << "egolane_image.right_line.line_point.size(): "
          << egolane_image.right_line.line_point.size();
  }
  // Check start_point and right lane
  if (egolane_image.right_line.line_point.size() <= 1) {
    if (debug_level_ >= 1) {
      AINFO_EVERY(60000) << "No right lane";
    }
    return false;
  }
  closest_index = -1;
  ...
  if (closest_index >= 0) {
    if (debug_level_ >= 3) {
      AINFO_EVERY(60000) << "[right] closest_index: " << closest_index
            << ", shortest_distance: " << shortest_distance;
      AINFO_EVERY(60000) << "Should be right to be selected";
    }
    // Check if the end point is on the right of the line segment
    if (!IsPointLeftOfLine(
            closted_object_edge.start_point,
      ...
      AINFO_EVERY(60000) << "The right lane is clear";
    }
  }

  if (b_left_lane_clear && b_right_lane_clear) {
    AINFO_EVERY(60000) << "The object is in the ego lane";
  } else {
    AINFO_EVERY(60000) << "The object is out of the ego lane";
  }
...
bool Cipv::IsObjectInTheLaneGround(const std::shared_ptr<base::Object> &object,
  ...
  *object_distance = distance;
  ...
  if (debug_level_ >= 3) {
    AINFO_EVERY(60000) << "egolane_ground.left_line.line_point.size(): "
          << egolane_ground.left_line.line_point.size();
  }
  ...
  if (debug_level_ >= 3) {
    AINFO_EVERY(60000) << "egolane_ground.right_line.line_point.size(): "
...
bool Cipv::DetermineCipv(const std::vector<base::LaneLine> &lane_objects,
  ...
  if (debug_level_ >= 3) {
    AINFO_EVERY(60000) << "Cipv Got SensorObjects with size of " << objects->size();
    AINFO_EVERY(60000) << "Cipv Got lane object with size of " << lane_objects.size();
  }
  ...
  for (int32_t i = 0; i < static_cast<int32_t>(objects->size()); ++i) {
    if (debug_level_ >= 2) {
      AINFO_EVERY(60000) << "objects[" << i << "]->track_id: " << (*objects)[i]->track_id;
    }
    if (IsObjectInTheLane((*objects)[i], egolane_image, egolane_ground,
      ...
      if (debug_level_ >= 2) {
        AINFO_EVERY(60000) << "current cipv_index: " << cipv_index;
      }
    }
  ...
  if (debug_level_ >= 1) {
    AINFO_EVERY(60000) << "old_cipv_index: " << old_cipv_index;
    AINFO_EVERY(60000) << "old_cipv_track_id_: " << old_cipv_track_id_;
  }
  if (cipv_index >= 0) {
    ...
    if (debug_level_ >= 1) {
      AINFO_EVERY(60000) << "final cipv_index: " << cipv_index;
      AINFO_EVERY(60000) << "final cipv_track_id: " << cipv_track_id;
      // AINFO_EVERY(60000) << "CIPV Index is changed from " << old_cipv_index << "th
      // object to "
      //            << cipv_index << "th object.";
      // AINFO_EVERY(60000) << "CIPV Track_ID is changed from " << old_cipv_track_id <<
      // " to "
      //            << cipv_track_id << ".";
    }
  } else {
    if (debug_level_ >= 1) {
      AINFO_EVERY(60000) << "No cipv";
    }
  }
...
bool Cipv::TranformPoint(const Eigen::VectorXf &in,
  ...
  if (in.rows() != motion_matrix.cols()) {
    AERROR_EVERY(60000) << "Matrix mismatch";
  }
...
bool Cipv::CollectDrops(const base::MotionBufferPtr &motion_buffer,
  ...
  if (debug_level_ >= 2) {
    AINFO_EVERY(60000) << " motion_size: " << motion_size;
  }
  if (motion_size <= 0) {
  ...
  if (debug_level_ >= 2) {
    AINFO_EVERY(60000) << "object_trackjectories_.size(): " << object_trackjectories_.size();
  }
  for (auto obj : *objects) {
    int cur_id = obj->track_id;
    if (debug_level_ >= 2) {
      AINFO_EVERY(60000) << "target ID: " << cur_id;
    }
    ...
    if (debug_level_ >= 2) {
      AINFO_EVERY(60000) << "object_trackjectories_[" << cur_id
    ...
    for (std::size_t it = object_trackjectories_[cur_id].size() - 1, count = 0;
      ...
      //               &transformed_pt);
      if (debug_level_ >= 3) {
        AINFO_EVERY(60000) << "(*motion_buffer)[" << motion_size - it - 1 << "].motion:";
        AINFO_EVERY(60000) << motion_buffer->at(motion_size - it - 1).motion;
        AINFO_EVERY(60000) << "accum_motion_buffer[" << motion_size - it - 1 << "] =";
        AINFO_EVERY(60000) << accum_motion_buffer;
        AINFO_EVERY(60000) << "target[" << obj->track_id << "][" << it << "]: ("
  ...
  for (const auto &each_object : object_trackjectories_) {
    ...
    if (!b_found_id && object_trackjectories_[obj_id].size() > 0) {
      ...
      if (debug_level_ >= 2) {
        AINFO_EVERY(60000) << "object_id_skip_count_[" << obj_id
              << " ]: " << object_id_skip_count_[obj_id];
      }
      if (object_id_skip_count_[obj_id] >= kMaxAllowedSkipObject) {
        if (debug_level_ >= 2) {
          AINFO_EVERY(60000) << "Removed obsolete object " << obj_id;
        }
  ...
  if (debug_level_ >= 2) {
    for (auto obj : *objects) {
      int cur_id = obj->track_id;
      AINFO_EVERY(60000) << "obj->track_id: " << cur_id;
      AINFO_EVERY(60000) << "obj->drop_num: " << obj->drop_num;
    }
  }
  return true;
}
...
bool Cipv::image2ground(const float image_x, const float image_y,
  ...
  if (debug_level_ >= 1) {
    AINFO_EVERY(60000) << "p_ground(2) too small :" << p_ground(2);
  }
  return false;
}
...
bool Cipv::ground2image(const float ground_x, const float ground_y,
  ...
  if (debug_level_ >= 1) {
    AINFO_EVERY(60000) << "p_image(2) too small :" << p_image(2);
  }
  return false;
}
