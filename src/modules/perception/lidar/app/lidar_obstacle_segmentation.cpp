...
bool LidarObstacleSegmentation::Init(
  ...
  config.segmentor=currentNode->declare_parameter<std::string>("segmentor", "CNNSegmentation");
  config.usemapmanager=currentNode->declare_parameter<bool>("use_map_manager", true);
  config.useobjectfilterbank=currentNode->declare_parameter<bool>("use_object_filter_bank", true);
  
  segmentor_name_ = config.segmentor;
  use_map_manager_ = config.usemapmanager;
  use_object_filter_bank_ = config.useobjectfilterbank;
  ...
  if (use_map_manager_) {
    ...
    if (!map_manager_.Init(map_manager_init_options)) {
      AINFO_EVERY(60000) << "Failed to init map manager.";
