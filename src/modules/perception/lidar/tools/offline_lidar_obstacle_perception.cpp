 ...
 public:
  ...
  bool setup() {
    ...
    if (!lib::ConfigManager::Instance()->Init()) {
      AERROR_EVERY(60000) << "Failed to init ConfigManage.";
      return false;
    }
    lidar_segmentation_.reset(new LidarObstacleSegmentation);
    if (lidar_segmentation_ == nullptr) {
      AERROR_EVERY(60000) << "Failed to get LidarObstacleSegmentation instance.";
      return false;
    }
    ...
    if (!lidar_segmentation_->Init(segment_init_options_)) {
      AINFO_EVERY(60000) << "Failed to init LidarObstacleSegmentation.";
      return false;
    }
    lidar_tracking_.reset(new LidarObstacleTracking);
    if (lidar_tracking_ == nullptr) {
      AERROR_EVERY(60000) << "Failed to get LidarObstacleTracking instance.";
      return false;
    }
    ...
    if (!common::SensorManager::Instance()->GetSensorInfo(FLAGS_sensor_name,
                                                          &sensor_info_)) {
      AERROR_EVERY(60000) << "Failed to get sensor info, sensor name: " << FLAGS_sensor_name;
      return false;
    }
  ...
  bool run() {
    ...
    if (!common::GetFileList(pcd_folder, ".pcd", &pcd_file_names)) {
      AERROR_EVERY(60000) << "pcd_folder: " << pcd_folder << " get file list error.";
      return false;
    }
    ...
    for (size_t i = 0; i < pcd_file_names.size(); ++i) {
      AINFO_EVERY(60000) << "***************** Frame " << i << " ******************";
      AINFO_EVERY(60000) << pcd_file_names[i];
      ...
      AINFO_EVERY(60000) << "Read point cloud from " << pcd_file_names[i]
            << " with cloud size: " << frame_->cloud->size();
      if (pose_folder != "") {
        std::string pose_file_name = pose_folder + "/" + file_name + ".pose";
        AINFO_EVERY(60000) << "Pose file: " << pose_file_name;
        ...
        if (common::ReadPoseFile(pose_file_name, &frame_->lidar2world_pose,
                                 &idt, &timestamp)) {
          AINFO_EVERY(60000) << "[timestamp]: " << std::setprecision(16) << timestamp;
          frame_->timestamp = timestamp;
        } else {
          AINFO_EVERY(60000) << "Failed to load pose, disable tracking pipeline.";
      ...
      if (segment_result.error_code != LidarErrorCode::Succeed) {
        AINFO_EVERY(60000) << segment_result.log;
        return false;
      }
      if (FLAGS_enable_tracking) {
        AINFO_EVERY(60000) << "Enable tracking.";
        ...
        if (tracking_result.error_code != LidarErrorCode::Succeed) {
          AINFO_EVERY(60000) << tracking_result.log;
          return false;
        }
  ...
  bool WriteObjectsForNewBenchmark(size_t frame_id,
    ...
    if (!fout.is_open()) {
      AERROR_EVERY(60000) << "Failed to open: " << path;
      return false;
    }
...
int main(int argc, char** argv) {
  auto FLAGS_alsologtostderr = 1;
  //google::ParseCommandLineFlags(&argc, &argv, true);
  //google::InitGoogleLogging(argv[0]);

  apollo::perception::lidar::OfflineLidarObstaclePerception test;
  if (!test.setup()) {
    AINFO_EVERY(60000) << "Failed to setup OfflineLidarObstaclePerception";
    return -1;
  }
  return test.run() ? 0 : -1;
}
