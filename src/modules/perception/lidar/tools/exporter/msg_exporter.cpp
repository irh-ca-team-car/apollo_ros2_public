...
void MsgExporter::ImageMessageHandler(
  ...
  double timestamp = img_msg->measurementtime;
  ...
  //    reinterpret_cast<const unsigned char*>(img_msg->disparitydata.data)
  //    : nullptr;
...
void MsgExporter::PointCloudMessageHandler(
    ...
    const std::string& child_frame_id, const std::string& folder) {
  double timestamp = cloud_msg->measurementtime;
  // std::cout << "Receive point cloud message from channel: " << channel <<
  //    " at time: " << FORMAT_TIMESTAMP(timestamp) << std::endl;
  pcl::PointCloud<PCLPointXYZIT> cloud;
  if (cloud_msg->pointsize > 0) {
    cloud.resize(cloud_msg->pointsize);
    for (int i = 0; i < cloud_msg->pointsize; ++i) {
