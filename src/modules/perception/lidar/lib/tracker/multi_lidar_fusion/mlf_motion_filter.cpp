...
bool MlfMotionFilter::Init(const MlfFilterInitOptions& options) {
  ...
  config_file = GetAbsolutePath(config_file, "mlf_motion_filter.json");
  MlfMotionFilterConfig config;
  ACHECK(cyber::common::GetProtoFromASCIIFile(config_file, &config));
  use_adaptive_ = config.useadaptive;
  use_breakdown_ = config.usebreakdown;
  use_convergence_boostup_ = config.useconvergenceboostup;
  init_velocity_variance_ = config.initvelocityvariance;
  init_acceleration_variance_ = config.initaccelerationvariance;
  measured_velocity_variance_ = config.measuredvelocityvariance;
  predict_variance_per_sqrsec_ = config.predictvariancepersqrsec;
  boostup_history_size_minimum_ = config.boostuphistorysizeminimum;
  boostup_history_size_maximum_ = config.boostuphistorysizemaximum;
  converged_confidence_minimum_ = config.convergedconfidenceminimum;
  noise_maximum_ = config.noisemaximum;
  trust_orientation_range_ = config.trustorientationrange;
