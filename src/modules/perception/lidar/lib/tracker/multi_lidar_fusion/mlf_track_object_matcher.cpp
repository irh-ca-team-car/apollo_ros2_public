...
bool MlfTrackObjectMatcher::Init(
  ...
  foreground_matcher_ = BaseBipartiteGraphMatcherRegisterer::GetInstanceByName(
      config.foregroundmathcermethod);
  ACHECK(foreground_matcher_ != nullptr);
  AINFO_EVERY(60000) << "MlfTrackObjectMatcher, fg: " << foreground_matcher_->Name();
  background_matcher_ = BaseBipartiteGraphMatcherRegisterer::GetInstanceByName(
      config.backgroundmatchermethod);
  ACHECK(background_matcher_ != nullptr);
  AINFO_EVERY(60000) << "MlfTrackObjectMatcher, bg: " << background_matcher_->Name();
  ...
  bound_value_ = config.boundvalue;
  max_match_distance_ = config.maxmatchdistance;
  return true;
}
