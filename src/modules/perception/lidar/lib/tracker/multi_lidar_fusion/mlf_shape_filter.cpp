...
bool MlfShapeFilter::Init(const MlfFilterInitOptions& options) {
  ...
  config_file = GetAbsolutePath(config_file, "mlf_shape_filter.json");
  MlfShapeFilterConfig config;
  ACHECK(cyber::common::GetProtoFromASCIIFile(config_file, &config));

  bottom_points_ignore_threshold_ = config.bottompointsignorethreshold;
  top_points_ignore_threshold_ = config.toppointsignorethreshold;
  return true;
}
