...
bool MlfMotionRefiner::Init(const MlfMotionRefinerInitOptions& options) {
  ...
  config_file = GetAbsolutePath(config_file, "mlf_motion_refiner.json");
  MlfMotionRefinerConfig config;
  ACHECK(apollo::cyber::common::GetProtoFromASCIIFile(config_file, &config));
  // read from proto config
  claping_speed_threshold_ = config.clapingspeedthreshold;
  claping_acceleration_threshold_ = config.clapingaccelerationthreshold;
  return true;
}
...
bool MlfMotionRefiner::Refine(const MlfTrackDataConstPtr& track_data,
  ...
  if (need_keep_motion && (!new_object->converged)) {
    ...
    AINFO_EVERY(60000) << "Track_id " << track_data->track_id_
