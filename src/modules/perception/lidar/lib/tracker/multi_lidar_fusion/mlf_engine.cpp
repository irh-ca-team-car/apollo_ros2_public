...
bool MlfEngine::Init(const MultiTargetTrackerInitOptions& options) {
  ...
  AINFO_EVERY(60000) << work_root;
  AINFO_EVERY(60000) << root_path;
  config_file = GetAbsolutePath(work_root, root_path);
  AINFO<< config_file;
  config_file = GetAbsolutePath(config_file, "mlf_engine.json");
  AINFO<< config_file;
  MlfEngineConfig config;
  ACHECK(cyber::common::GetProtoFromASCIIFile(config_file, &config));

  main_sensor_.clear();
  for (size_t i = 0; i < config.mainsensor.size(); ++i) {
    main_sensor_.emplace(config.mainsensor[i]);
  }

  use_histogram_for_match_ = config.usehistogramformatch;
  histogram_bin_size_ = config.histogrambinsize;
  output_predict_objects_ = config.outputpredictobjects;
  reserved_invisible_time_ = config.reservedinvisibletime;
  use_frame_timestamp_ = config.useframetimestamp;
...
bool MlfEngine::Track(const MultiTargetTrackerOptions& options,
  ...
  AINFO_EVERY(60000) << "MlfEngine publish objects: " << frame->tracked_objects.size()
...
void MlfEngine::SplitAndTransformToTrackedObjects(
  ...
  AINFO_EVERY(60000) << "MlfEngine: " << sensor_info.name
...
void MlfEngine::TrackObjectMatchAndAssign(
  ...
  AINFO_EVERY(60000) << "MlfEngine: " + name + " assignments " << assignments.size()
...
void convertPoseToLoc(const Eigen::Affine3d& pose,
  ...
  localization->pose.orientation.qx=p.x();
  localization->pose.orientation.qy=p.y();
  localization->pose.orientation.qz=p.z();
  localization->pose.orientation.qw=p.w();
}
    ...
    for (auto& track_data : *tracks) {
      if (!output_predict_objects_ && track_data->is_current_state_predicted_) {
        ...
        if (!track_data->ToObject(-global_to_local_offset_, frame->timestamp,
                                  tracked_objects[pos])) {
          AERROR_EVERY(60000) << "Tracking failed";
          continue;
        }
        ++pos;
      }
...
//  }
  ...
  if (num_predict != 0) {
    AINFO_EVERY(60000) << "MlfEngine, num_predict: " << num_predict
          << " num_objects: " << num_objects;
    if (num_predict > num_objects) {
      AERROR_EVERY(60000) << "num_predict > num_objects";
      return;
    }
...
void MlfEngine::RemoveStaleTrackData(const std::string& name, double timestamp,
  ...
  AINFO_EVERY(60000) << "MlfEngine: " << name << " remove stale tracks, from "
        << tracks->size() << " to " << pos;
  tracks->resize(pos);
}
