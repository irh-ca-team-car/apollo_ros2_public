...
bool FusedClassifier::Init(const ClassifierInitOptions& options) {
  ...
  config_file = GetAbsolutePath(config_file, "fused_classifier.json");
  FusedClassifierConfig config;
  ACHECK(cyber::common::GetProtoFromFile(config_file, &config));
  temporal_window_ = config.temporalwindow;
  enable_temporal_fusion_ = config.enabletemporalfusion;
  use_tracked_objects_ = config.usetrackedobjects;
  one_shot_fusion_method_ = config.oneshotfusionmethod;
  sequence_fusion_method_ = config.sequencefusionmethod;
...
bool FusedClassifier::Classify(const ClassifierOptions& options,
  ...
  if (enable_temporal_fusion_ && frame->timestamp > 0.0) {
    // sequence fusion
    AINFO_EVERY(60000) << "Combined classifier, temporal fusion";
    ...
    for (auto& object : *objects) {
      ...
      if (tracked_objects.empty()) {
        AERROR_EVERY(60000) << "Find zero-length track, so skip.";
        continue;
      }
      if (object != tracked_objects.rbegin()->second) {
        AERROR_EVERY(60000) << "There must exist some timestamp in disorder, so skip.";
        continue;
      }
      if (!sequence_fuser_->TypeFusion(option_, &tracked_objects)) {
        AERROR_EVERY(60000) << "Failed to fuse types, so break.";
        break;
      }
    }
  } else {
    // one shot fusion
    AINFO_EVERY(60000) << "Combined classifier, one shot fusion";
    for (auto& object : *objects) {
      if (object->lidar_supplement.is_background) {
        object->type_probs.assign(static_cast<int>(ObjectType::MAX_OBJECT_TYPE),
                                  0);
      ...
      if (!one_shot_fuser_->TypeFusion(option_, object)) {
        AERROR_EVERY(60000) << "Failed to fuse types, so continue.";
      }
    }
  }
  return true;
