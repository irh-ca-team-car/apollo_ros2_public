...
bool CCRFOneShotTypeFusion::Init(const TypeFusionInitOption& option) {
  ...
  config_file = GetAbsolutePath(config_file, "ccrf_type_fusion.json");
  CcrfTypeFusionConfig config;
  ACHECK(cyber::common::GetProtoFromFile(config_file, &config));
  std::string classifiers_property_file_path =
      GetAbsolutePath(work_root, config.classifierspropertyfilepath);
  ...
  for (auto& pair : smooth_matrices_) {
    ...
    AINFO_EVERY(60000) << "Source: " << pair.first;
    AINFO_EVERY(60000) << pair.second;
  }
  ...
  AINFO_EVERY(60000) << "Confidence: ";
  AINFO_EVERY(60000) << confidence_smooth_matrix_;
...
bool CCRFSequenceTypeFusion::Init(const TypeFusionInitOption& option) {
  ...
  std::string transition_property_file_path =
      GetAbsolutePath(work_root, config.transitionpropertyfilepath);
  s_alpha_ = config.transitionmatrixalpha;
  ...
  AINFO_EVERY(60000) << "transition matrix";
  AINFO_EVERY(60000) << transition_matrix_;
...
bool CCRFSequenceTypeFusion::FuseWithConditionalProbabilityInference(
    TrackedObjects* tracked_objects) {
  // AINFO_EVERY(60000) << "Enter fuse with conditional probability inference";
  ...
  for (auto& pair : *tracked_objects) {
    ...
    if (!one_shot_fuser_.FuseOneShotTypeProbs(object,
      ...
      AERROR_EVERY(60000) << "Failed to fuse one short probs in sequence.";
      return false;
    }
  }
