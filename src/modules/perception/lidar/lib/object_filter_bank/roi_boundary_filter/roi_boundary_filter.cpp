...
bool ROIBoundaryFilter::Init(const ObjectFilterInitOptions& options) {
  ...
  config_file = GetAbsolutePath(config_file, "roi_boundary_filter.json");
  ROIBoundaryFilterConfig config;
  ACHECK(cyber::common::GetProtoFromFile(config_file, &config));
  distance_to_boundary_threshold_ = config.distancetoboundarythreshold;
  confidence_threshold_ = config.confidencethreshold;
  cross_roi_threshold_ = config.crossroithreshold;
  inside_threshold_ = config.insidethreshold;
  return true;
}
...
bool ROIBoundaryFilter::Filter(const ObjectFilterOptions& options,
                               LidarFrame* frame) {
  if (!frame) {
    AINFO_EVERY(60000) << "Lidar frame is nullptr.";
    return false;
  }
  if (!frame->hdmap_struct) {
    AINFO_EVERY(60000) << "HDMap struct is nullptr.";
    return true;
  }
  if (frame->hdmap_struct->road_boundary.size() +
      ...
      0) {
    AINFO_EVERY(60000) << "Donot find roi polygons, skip boundary filter.";
  ...
  AINFO_EVERY(60000) << "Roi boundary filter, " << objects_valid_flag_.size() << " to "
        << count;
  return true;
}
