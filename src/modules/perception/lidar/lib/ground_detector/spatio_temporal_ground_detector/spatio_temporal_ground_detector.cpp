    ...
    namespace lidar
    {
          ...
          const GroundDetectorInitOptions &options)
      {
        ...
        config_file = apollo::cyber::common::GetAbsolutePath(
            config_file, "spatio_temporal_ground_detector.json");

        // get config params
        SpatioTemporalGroundDetectorConfig config_params;
        ACHECK(GetProtoFromASCIIFile(config_file, &config_params))
            << "Failed to parse SpatioTemporalGroundDetectorConfig config file." << config_file;
        ground_thres_ = config_params.groundthres;
        use_roi_ = config_params.useroi;
        use_ground_service_ = config_params.usegroundservice;

        param_ = new common::PlaneFitGroundDetectorParam;
        param_->roi_region_rad_x = config_params.roiradx;
        param_->roi_region_rad_y = config_params.roirady;
        param_->roi_region_rad_z = config_params.roiradz;
        param_->nr_grids_coarse = config_params.gridsize;
        param_->nr_smooth_iter = config_params.nrsmoothiter;
        ...
        ground_service_content_.Init(
            config_params.roiradx, config_params.roirady,
            config_params.gridsize, config_params.gridsize);
        return true;
      }
                                                ...
                                                LidarFrame *frame)
      {
        ...
        AINFO_EVERY(60000) << "succeed to call ground detector with non ground points "
        ...
        if (use_ground_service_)
        {
          ...
          {
            AINFO_EVERY(60000) << "Failed to find ground service and cannot update.";
          }
        }
        return true;
      }
