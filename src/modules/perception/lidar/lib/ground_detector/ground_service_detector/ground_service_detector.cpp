...
bool GroundServiceDetector::Init(const GroundDetectorInitOptions& options) {
  ...
  config_file = GetAbsolutePath(config_file, "ground_service_detector.json");

  GroundServiceDetectorConfig config;
  ACHECK(cyber::common::GetProtoFromFile(config_file, &config));
  ground_threshold_ = config.groundthreshold;
  ...
  if (ground_service_ == nullptr) {
    AERROR_EVERY(60000) << "Ground service is nullptr, Init scene manager first !";
    return false;
  }
  return true;
}
...
bool GroundServiceDetector::Detect(const GroundDetectorOptions& options,
  ...
  if (!ground_service_content_.IsServiceReady()) {
    AERROR_EVERY(60000) << "service is not ready.";
    return false;
  }
