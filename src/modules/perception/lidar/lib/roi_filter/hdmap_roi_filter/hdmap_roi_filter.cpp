...
bool HdmapROIFilter::Init(const ROIFilterInitOptions& options) {
  ...
  config_file = GetAbsolutePath(config_file, "hdmap_roi_filter.json");
  HDMapRoiFilterConfig config;
  ACHECK(apollo::cyber::common::GetProtoFromASCIIFile(config_file, &config));
  range_ = config.range;
  cell_size_ = config.cellsize;
  extend_dist_ = config.extenddist;
  no_edge_table_ = config.noedgetable;
  set_roi_service_ = config.setroiservice;
  ...
  AINFO_EVERY(60000) << " HDMap Roi Filter Parameters: "
...
bool HdmapROIFilter::Filter(const ROIFilterOptions& options,
  ...
  if (frame->hdmap_struct == nullptr || frame->cloud == nullptr) {
    AERROR_EVERY(60000) << " Input frame data error !";
    return false;
  }
  ...
  if (0 == polygons_world_size) {
    AINFO_EVERY(60000) << " Polygon Empty.";
    return false;
  }
  ...
  if (set_roi_service_) {
    ...
    if (roi_service != nullptr) {
      ...
      AINFO_EVERY(60000) << "Failed to find roi service and cannot update.";
    }
  }
  return ret;
}
