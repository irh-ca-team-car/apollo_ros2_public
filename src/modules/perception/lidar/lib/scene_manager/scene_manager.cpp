  ...
  namespace perception
  {
    namespace lidar
    {
      ...
      bool SceneManager::InitInternal(const SceneManagerInitOptions &options)
      {
        ...
        config_file = GetAbsolutePath(config_file, "scene_manager.json");

        SceneManagerConfig config;
        std::string interparam;

        ACHECK(cyber::common::GetProtoFromASCIIFile(config_file, &config));
      
        services_.clear();
        for (size_t i = 0; i < config.servicename.size(); ++i)
        {
          const auto &name = config.servicename[i];
          SceneServicePtr service(SceneServiceRegisterer::GetInstanceByName(name));
          if (service == nullptr)
          {
            AINFO_EVERY(60000) << "Failed to find scene service: " << name << ", skipped";
            continue;
          }
          if (!service->Init())
          {
            AINFO_EVERY(60000) << "Failed to init scene service: " << name << ", skipped";
            continue;
          }
          services_.emplace(name, service);
          AINFO_EVERY(60000) << "Scene manager add service: " << name;
        }
        initialized_ = true;
        return true;
      }
