...
bool NCut::Init(const NCutParam &param) {
  ...
  if (!_classifier->init()) {
    AERROR_EVERY(60000) << "failed to init FrameClassifierPipeline.";
    return false;
  }
  return true;
}
...
bool NCut::Configure(const NCutParam &ncut_param_) {
  _grid_radius = ncut_param_.gridradius;
  _connect_radius = ncut_param_.connectradius;
  _super_pixel_cell_size = ncut_param_.superpixelcellsize;
  _num_cuts = ncut_param_.numcuts;
  _ncuts_stop_threshold = ncut_param_.ncutsstopthreshold;
  _ncuts_enable_classifier_threshold =
      ncut_param_.ncutsenableclassifierthreshold;
  _sigma_space = ncut_param_.sigmaspace;
  _sigma_feature = ncut_param_.sigmafeature;
  _skeleton_cell_size = ncut_param_.skeletoncellsize;
  _patch_size = ncut_param_.patchsize;
  _overlap_factor = ncut_param_.overlapfactor;
  _felzenszwalb_sigma = ncut_param_.felzenszwalbsigma;
  _felzenszwalb_k = ncut_param_.felzenszwalbk;
  _felzenszwalb_min_size = ncut_param_.felzenszwalbminsize;

  AINFO_EVERY(60000) << "NCut Parameters";
  return true;
}
...
void NCut::Segment(base::PointFCloudConstPtr cloud) {
  ...
  AINFO_EVERY(60000) << "cloud obstacle size in ncut segment is "
...
// visualize_points(pids);
  ...
  AINFO_EVERY(60000) << "super pixels done " << omp_get_wtime() - start_t;
...
#ifdef DEBUG_NCUT
  AINFO_EVERY(60000) << "precompute skeleton and bbox done " << omp_get_wtime() - start_t;
  start_t = omp_get_wtime();
#endif
  // .3 grach cut
  ...
  NormalizedCut(_ncuts_stop_threshold, true, &segment_clusters,
                &segment_labels);
#ifdef DEBUG_NCUT
  AINFO_EVERY(60000) << "normalized_cut done, #segments " << segment_clusters.size()
...
// visualize_segments_from_cluster(_cluster_points);
  ...
  while (!job_stack.empty()) {
    ...
    job_stack.pop();
#ifdef DEBUG_NCUT
    AINFO_EVERY(60000) << "curr size " << curr->size();
...
#ifdef DEBUG_NCUT
      AINFO_EVERY(60000) << "N cut cost is " << cost << ", seg1 size " << seg1->size()
            << ", seg2 size " << seg2->size();
      if (curr->size() < 50) {
        std::cout << "seg1: ";
