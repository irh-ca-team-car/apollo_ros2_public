...
bool NCutSegmentation::Init(const SegmentationInitOptions& options) {
  ...
  if (!Configure(param_file)) {
    AERROR_EVERY(60000) << "failed to load ncut config.";
    return false;
  }
  ...
  if (!_outliers) {
    AERROR_EVERY(60000) << "Failed to reset outliers.";
    return false;
  }
  int num_threads = 1;
#pragma omp parallel
  ...
  worker_.Bind([&]() {
    ...
    AINFO_EVERY(60000) << "lidar 2 world pose " << lidar_frame_ref_->lidar2world_pose(0, 3)
  ...
  AINFO_EVERY(60000) << "NCutSegmentation init success, num_threads: " << num_threads;
  return true;
}
...
bool NCutSegmentation::Configure(std::string param_file) {
  ...
  grid_radius_ = seg_param_.gridradius;
  height_threshold_ = seg_param_.heightthreshold;
  partition_cell_size_ = seg_param_.partitioncellsize;
  vehicle_filter_cell_size_ = seg_param_.vehiclefiltercellsize;
  pedestrian_filter_cell_size_ = seg_param_.pedestrianfiltercellsize;
  outlier_length_ = seg_param_.outlierlength;
  outlier_width_ = seg_param_.outlierwidth;
  outlier_height_ = seg_param_.outlierheight;
  outlier_min_num_points_ = seg_param_.outlierminnumpoints;
  remove_ground_ = seg_param_.removegroundpoints;
  remove_roi_ = seg_param_.removeroi;
  ground_detector_str_ = seg_param_.grounddetector;
  roi_filter_str_ = seg_param_.roifilter;
  ncut_param_ = seg_param_.ncutparam;
  do_classification_ = seg_param_.doclassification;
  AINFO_EVERY(60000) << "NCut Segmentation ";
  return true;
}
...
bool NCutSegmentation::GetConfigs(std::string* param_file) {
  ...
  config_file = GetAbsolutePath(config_file, "ncut.json");
...
#pragma omp parallel
  ...
  AINFO_EVERY(60000) << "input point cloud: " << original_cloud_->size() << " points";
#ifdef DEBUG_NCUT
  VisualizePointCloud(original_cloud_);
#endif

  if (remove_roi_) {
    AINFO_EVERY(60000) << "remove roi and remove ground for ncut segmentation";
    worker_.WakeUp();
    worker_.Join();
  }
  ...
  if (remove_ground_) {
    AINFO_EVERY(60000) << "remove ground";
  ...
  AINFO_EVERY(60000) << "filter ground, elapsed time: " << omp_get_wtime() - start_t
...
#ifdef DEBUG_NCUT
  AINFO_EVERY(60000) << "filter vehicle, elapsed time: " << omp_get_wtime() - start_t
...
#ifdef DEBUG_NCUT
  AINFO_EVERY(60000) << "filter pedestrian, elapsed time: " << omp_get_wtime() - start_t;
  start_t = omp_get_wtime();
  AINFO_EVERY(60000) << "filter pedestrian: " << cloud_after_people_filter->size()
        << " points left";
  VisualizePointCloud(cloud_after_people_filter);
  AINFO_EVERY(60000) << "after filter car/pedestrian #segments " << segments->size();
...
#ifdef DEBUG_NCUT
  AINFO_EVERY(60000) << "partition small regions, elapsed time: "
        << omp_get_wtime() - start_t;
  start_t = omp_get_wtime();
  AINFO_EVERY(60000) << "partition " << cloud_components.size() << " components";
#endif
...
#ifdef DEBUG_NCUT
  AINFO_EVERY(60000) << "filter outlier, elapsed time: " << omp_get_wtime() - start_t;
  start_t = omp_get_wtime();
#endif
  // .6 graph cut each
...
#pragma omp parallel for
  ...
  AINFO_EVERY(60000) << "aggregate results, elapsed time: " << omp_get_wtime() - start_t;
  AINFO_EVERY(60000) << "final #segments " << segments->size();
  AINFO_EVERY(60000) << "final #outliers " << _outliers->size();
...
#ifdef DEBUG_NCUT
  AINFO_EVERY(60000) << "flood fill: " << component_points.size() << " components";
// VisualizeComponents(in_cloud, component_points);
  ...
  for (int i = 0; i < num_components; ++i) {
    if (component_points[i].size() > min_num_points) {
      ...
      AINFO_EVERY(60000) << "before: component id: " << i << ", label: " << label;
      if (filter_pedestrian_only) {
        label =
            (label == "pedestrian" || label == "nonMot") ? label : "unknown";
      }
      AINFO_EVERY(60000) << "after: component id: " << i << ", label: " << label;
      component_labels[i] = label;
    }
  }  // end of for

  AINFO_EVERY(60000) << "classification done";
  ...
  AINFO_EVERY(60000) << "obstacle_filter: filter unknown out, obstacle_components.size = "
