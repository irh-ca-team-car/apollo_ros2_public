...
bool MaskPillarsDetection::Detect(const LidarDetectorOptions& options,
  ...
  if (cudaSetDevice(FLAGS_gpu_id) != cudaSuccess) {
    AERROR_EVERY(60000) << "Failed to set device to gpu " << FLAGS_gpu_id;
    return false;
  }
  ...
  AINFO_EVERY(60000) << "num points before fusing: " << num_points;
  ...
  AINFO_EVERY(60000) << "num points after fusing: " << num_points;
  ...
  AINFO_EVERY(60000) << "PointPillars: "
        << "\n"
