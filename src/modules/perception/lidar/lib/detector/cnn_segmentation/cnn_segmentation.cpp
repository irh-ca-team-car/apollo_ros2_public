  ...
  namespace perception
  {
    namespace lidar
    {
      ...
      bool CNNSegmentation::Init(const SegmentationInitOptions &options)
      {
        ...
        AINFO_EVERY(60000) << "--    param_file: " << param_file;
        AINFO_EVERY(60000) << "--    proto_file: " << proto_file;
        AINFO_EVERY(60000) << "--    weight_file: " << weight_file;
        AINFO_EVERY(60000) << "--    engine_file: " << engine_file;

        // get cnnseg params
        ACHECK(GetProtoFromASCIIFile(param_file, &cnnseg_param_))
            << "Failed to parse CNNSegParam config file." << param_file;
        ACHECK(GetProtoFromASCIIFile(engine_file, &spp_engine_config_))
            << "Failed to parse SppEngine config file." << engine_file;

        // init feature parameters
        const FeatureParam &feature_param = cnnseg_param_.featureparam;
        range_ = feature_param.pointcloudrange;
        width_ = feature_param.width;
        height_ = feature_param.height;
        min_height_ = feature_param.minheight;
        max_height_ = feature_param.maxheight;

        // init inference model
        const NetworkParam &network_param = cnnseg_param_.networkparam;
        std::vector<std::string> output_names;
        output_names.push_back(network_param.instanceptblob);
        output_names.push_back(network_param.categoryptblob);
        output_names.push_back(network_param.confidenceptblob);
        output_names.push_back(network_param.heightptblob);
        output_names.push_back(network_param.headingptblob);
        output_names.push_back(network_param.classptblob);
        std::vector<std::string> input_names;
        input_names.push_back(network_param.featureblob);
        inference_.reset(inference::CreateInferenceByName(cnnseg_param_.modeltype,
                                                          proto_file, weight_file,
                                                          output_names, input_names));
        CHECK_NOTNULL(inference_.get());

        gpu_id_ = (int)cnnseg_param_.gpuid >=(int)0 ? cnnseg_param_.gpuid : -1;
        if (gpu_id_ >= 0)
          BASE_CUDA_CHECK(cudaSetDevice(gpu_id_));
        inference_->set_gpu_id(gpu_id_); // inference sets CPU mode when -1

        std::map<std::string, std::vector<int>> input_shapes;
        auto &input_shape = input_shapes[network_param.featureblob];
        input_shape = {1, 8, height_, width_};
        if (!feature_param.useintensityfeature)
        {
          input_shape[1] -= 2;
        }
        if (!feature_param.useconstantfeature)
        {
          input_shape[1] -= 2;
        }
        ...
        // init blobs
        instance_pt_blob_ = inference_->get_blob(network_param.instanceptblob);
        CHECK_NOTNULL(instance_pt_blob_.get());
        category_pt_blob_ = inference_->get_blob(network_param.categoryptblob);
        CHECK_NOTNULL(category_pt_blob_.get());
        confidence_pt_blob_ =
            inference_->get_blob(network_param.confidenceptblob);
        CHECK_NOTNULL(confidence_pt_blob_.get());
        height_pt_blob_ = inference_->get_blob(network_param.heightptblob);
        CHECK_NOTNULL(height_pt_blob_.get());
        feature_blob_ = inference_->get_blob(network_param.featureblob);
        CHECK_NOTNULL(feature_blob_.get());
        if (cnnseg_param_.doclassification)
        {
          classify_pt_blob_ = inference_->get_blob(network_param.classptblob);
          CHECK_NOTNULL(classify_pt_blob_.get());
        }
        if (cnnseg_param_.doheading)
        {
          heading_pt_blob_ = inference_->get_blob(network_param.headingptblob);
        ...
        // secondary segmentor
        /*if (cnnseg_param_.fillrecallwithncut) {
      ...
      bool CNNSegmentation::InitClusterAndBackgroundSegmentation()
      {
        ...
        // init roi filter
        roi_filter_ =
            BaseROIFilterRegisterer::GetInstanceByName(cnnseg_param_.roifilter);
        ...
        // init spp engine
        SppParams params;
        params.height_gap = spp_engine_config_.heightgap;
        params.confidence_range = cnnseg_param_.confidencerange;
        ...
        spp_data.objectness_threshold = cnnseg_param_.objectnessthresh;
        spp_data.confidence_threshold = cnnseg_param_.confidencethresh;
        spp_data.top_z_threshold = cnnseg_param_.heightthresh;
    ...
    AINFO_EVERY(60000) << "before roi filter";
    if (lidar_frame_ref_->hdmap_struct != nullptr &&
      ...
      AINFO_EVERY(60000) << "Fail to call roi filter, use origin cloud.";
    ...
    AINFO_EVERY(60000) << "after roi filter";
    ...
    AINFO_EVERY(60000) << "Roi-filter time: " << roi_filter_time_
    ...
    return true; });
                                    ...
                                    LidarFrame *frame)
      {
        ...
        if (frame == nullptr)
        {
          AERROR_EVERY(60000) << "Input null frame ptr.";
          return false;
        }
        if (frame->cloud == nullptr)
        {
          AERROR_EVERY(60000) << "Input null frame cloud.";
          return false;
        }
        if (frame->world_cloud == nullptr)
        {
          AERROR_EVERY(60000) << "Input null frame world cloud.";
          return false;
        }
        if (frame->cloud->size() == 0)
        {
          AERROR_EVERY(60000) << "Input none points.";
          return false;
        }
        if (frame->cloud->size() != frame->world_cloud->size())
        {
          AERROR_EVERY(60000) << "Cloud size and world cloud size not consistent.";
          return false;
        }
        ...
        AINFO_EVERY(60000) << "CNNSEG: mapping: " << mapping_time_ << "\t"
          ...
          std::vector<std::shared_ptr<Object>> *objects)
      {
        ...
        AINFO_EVERY(60000) << "Use origin cloud and copy height";
        ...
        if (cnnseg_param_.removegroundpoints)
        {
        ...
        {
          if (clusters[i]->points.size() <= cnnseg_param_.minptsnum &&
              clusters[i]->pixels.size() < cnnseg_param_.minptsnum)
          {
            continue;
          }
        ...
        /*if (cnnseg_param_.fillrecallwithncut && secondary_segmentor) {
                                       ...
                                       std::string *engine_file)
      {
        ...
        config_file = GetAbsolutePath(config_file, "cnnseg.json");

        CNNSegConfig config;
        ACHECK(apollo::cyber::common::GetProtoFromASCIIFile(config_file, &config))
            << "Failed to parse CNNSeg config file" << config_file;
        *proto_file = GetAbsolutePath(work_root, config.protofile);
        *weight_file = GetAbsolutePath(work_root, config.weightfile);
        *param_file = GetAbsolutePath(work_root, config.paramfile);
        *engine_file = GetAbsolutePath(work_root, config.enginefile);
