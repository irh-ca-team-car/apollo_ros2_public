  ...
  namespace perception
  {
    namespace common
    {
      ...
      bool SensorManager::Init()
      {
        ...
        auto configFilePath = apollo::cyber::currentNode->declare_parameter<std::string>("sensor_config", "<root>/conf/perception/sensor_manager.json");
        apollo::cyber::ComponentBase::instance()->replaceRoot(configFilePath);
        AINFO << configFilePath;
        apollo::cyber::common::GetProtoFromASCIIFile(configFilePath, &sensor_list_proto);
        AINFO << sensor_list_proto;

        auto AddSensorInfo = [this](const apollo_msgs::msg::ApolloperceptionSensorMeta &sensor_meta_proto)
        {
          ...
          if (!pair.second)
          {
            AERROR_EVERY(60000) << "Duplicate sensor name error.";
            return false;
          }
          ...
          if (this->IsCamera(sensor_info.type))
          {
            ...
            {
              AERROR_EVERY(60000) << "Failed to load camera intrinsic:" << intrinsic_file;
              return false;
            }
        ...
        for (const apollo_msgs::msg::ApolloperceptionSensorMeta &sensor_meta_proto : sensor_list_proto.sensormeta)
        {
          if (!AddSensorInfo(sensor_meta_proto))
          {
            AERROR_EVERY(60000) << "Failed to add sensor_info: " << sensor_meta_proto.name;
            return false;
          }
        }

        inited_ = true;
        AINFO_EVERY(60000) << "Init sensor_manager success.";
        return true;
      }
                                        ...
                                        SensorInfo *sensor_info) const
      {
        ...
        {
          AERROR_EVERY(60000) << "Nullptr error.";
          return false;
        }
