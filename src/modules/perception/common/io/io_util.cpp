...
bool ReadPoseFile(const std::string &filename, Eigen::Affine3d *pose,
  ...
  if (!fin.is_open()) {
    AERROR_EVERY(60000) << "Failed to open pose file: " << filename;
    return false;
  }
...
bool LoadBrownCameraIntrinsic(const std::string &yaml_file,
  ...
  try {
    ...
    AERROR_EVERY(60000) << "load camera intrisic file " << yaml_file
...
bool LoadOmnidirectionalCameraIntrinsics(
  ...
  if (!node["width"].IsDefined() || !node["height"].IsDefined() ||
    ...
    AINFO_EVERY(60000) << "Invalid intrinsics file for an omnidirectional camera.";
    return false;
  }
...
bool GetFileList(const std::string &path, const std::string &suffix,
  ...
  if (!PathExists(path)) {
    AINFO_EVERY(60000) << path << " not exist.";
    return false;
  }

  for(auto& itr: std::filesystem::recursive_directory_iterator(path)) {
    try {
