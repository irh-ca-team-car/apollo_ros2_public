...
bool HMMatcher::Init() {
  ...
  AINFO_EVERY(60000) << "matcher name: " << name_;
  ...
  config_file = GetAbsolutePath(config_file, "hm_matcher.json");
  // get config params
  MatcherConfig config_params;
  ACHECK(cyber::common::GetProtoFromASCIIFile(config_file, &config_params))
      << "Failed to parse MatcherConfig config file." << config_file;
  double max_match_distance = config_params.maxmatchdistance;
  double bound_match_distance = config_params.boundmatchdistance;
