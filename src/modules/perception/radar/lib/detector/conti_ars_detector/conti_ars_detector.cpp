...
bool ContiArsDetector::Detect(const drivers::ContiRadar& corrected_obstacles,
  ...
  radar_frame->timestamp = corrected_obstacles.apolloheader.timestampsec;
...
void ContiArsDetector::RawObs2Frame(
  ...
  for (const auto radar_obs : corrected_obstacles.contiobs) {
    ...
    radar_object->id = radar_obs.obstacleid;
    radar_object->track_id = radar_obs.obstacleid;
    Eigen::Vector4d local_loc(radar_obs.longitudedist,
                              radar_obs.lateraldist, 0, 1);
    ...
    Eigen::Vector3d local_vel(radar_obs.longitudevel,
                              radar_obs.lateralvel, 0);
    ...
    vel_rms.setZero();
    dist_rms(0, 0) = radar_obs.longitudedistrms;
    dist_rms(1, 1) = radar_obs.lateraldistrms;
    vel_rms(0, 0) = radar_obs.longitudevelrms;
    vel_rms(1, 1) = radar_obs.lateralvelrms;
    ...
    radar_object->velocity_uncertainty =
        (radar2world_rotate * vel_rms * vel_rms.transpose() *
         radar2world_rotate_t)
            .cast<float>();
    double local_obj_theta = radar_obs.oritationangle / 180.0 * PI;
    ...
    radar_object->theta_variance =
        static_cast<float>(radar_obs.oritationanglerms / 180.0 * PI);
    ...
    int cls = radar_obs.obstacleclass;
    ...
    ADEBUG << "obs_id: " << radar_obs.obstacleid << ", "
           << "long_dist: " << radar_obs.longitudedist << ", "
           << "lateral_dist: " << radar_obs.lateraldist << ", "
           << "long_vel: " << radar_obs.longitudevel << ", "
           << "latera_vel: " << radar_obs.lateralvel << ", "
           << "rcs: " << radar_obs.rcs << ", "
           << "meas_state: " << radar_obs.measstate;
  }
}
