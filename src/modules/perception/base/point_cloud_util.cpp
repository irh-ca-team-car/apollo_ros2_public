...
bool GetPointCloudMinareaBbox(const PointFCloud& pc, BoundingCube* box,
  ...
  if (verbose) {
    AINFO_EVERY(60000) << "center = " << mar.center.x << " " << mar.center.y;
    AINFO_EVERY(60000) << "size = " << mar.size.height << " " << mar.size.width;
    AINFO_EVERY(60000) << "yaw = " << mar.angle;
    AINFO_EVERY(60000) << "height = " << max_z - min_z;
  }
  box->x = mar.center.x;
  box->y = mar.center.y;
