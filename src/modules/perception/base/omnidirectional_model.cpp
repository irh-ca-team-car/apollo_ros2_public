...
Eigen::Vector2f OmnidirectionalCameraDistortionModel::Project(
  ...
  if (std::isgreater(point3d[2], 0.f)) {
    AERROR_EVERY(60000) << "The input point (" << point3d
...
bool OmnidirectionalCameraDistortionModel::set_params(
  ...
  if (params.size() < 9) {
    AINFO_EVERY(60000) << "Missing cam2world and world2cam model.";
    return false;
  }

  uint32_t cam2world_order = uint32_t(params(8));
  AINFO_EVERY(60000) << "cam2world order: " << cam2world_order << ", size: " << params.size();
  std::cout<< std::endl;

  if (params.size() < 9 + cam2world_order + 1) {
    AINFO_EVERY(60000) << "Incomplete cam2world model or missing world2cam model.";
    return false;
  }

  uint32_t world2cam_order = uint32_t(params(9 + cam2world_order));
  AINFO_EVERY(60000) << "world2cam order: " << world2cam_order << ", size: " << params.size();
  std::cout<< std::endl;

  if (params.size() < 9 + cam2world_order + 1 + world2cam_order) {
    AINFO_EVERY(60000) << "Incomplete world2cam model.";
    return false;
  }
