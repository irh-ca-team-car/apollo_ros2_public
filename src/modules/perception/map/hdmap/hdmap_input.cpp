...
bool HDMapInput::InitHDMap() {
  ...
  if (!lib::ConfigManager::Instance()->GetModelConfig(model_name,
                                                      &model_config)) {
    AERROR_EVERY(60000) << "Failed to find model: " << model_name;
    return false;
  }
  if (!model_config->get_value("hdmap_sample_step", &hdmap_sample_step_)) {
    AERROR_EVERY(60000) << "hdmap_sample_step not found.";
  ...
  AINFO_EVERY(60000) << "hdmap_file_: " << hdmap_file_;
  if (!apollo::cyber::common::PathExists(hdmap_file_)) {
    AERROR_EVERY(60000) << "Failed to find hadmap file: " << hdmap_file_;
    return false;
  }
  if (hdmap_->LoadMapFromFile(hdmap_file_) != 0) {
    AERROR_EVERY(60000) << "Failed to load hadmap file: " << hdmap_file_;
    return false;
  }

  AINFO_EVERY(60000) << "Load hdmap file: " << hdmap_file_;*/
  return true;
}
...
bool HDMapInput::GetRoiHDMapStruct(
  ...
  if (hdmap_.get() == nullptr) {
    AERROR_EVERY(60000) << "hdmap is not available";
    return false;
  }
  ...
  if (hdmap_->GetRoadBoundaries(point, distance, &road_boundary_vec,
                                &junctions_vec) != 0) {
    AERROR_EVERY(60000) << "Failed to get road boundary, point: ";
    return false;
  }
...
void HDMapInput::DownsamplePoints(const base::PointDCloudPtr& raw_cloud_ptr,
  ...
  AINFO_EVERY(60000) << "Downsample road boundary points from " << raw_cloud_size << " to "
        << polygon_ptr->size();
}
...
bool HDMapInput::GetNearestLaneDirection(const base::PointD& pointd,
  ...
  UNUSED(pointd);
  UNUSED(lane_direction);
  /*if (hdmap_ == nullptr)
  {
    return false;
   }
   apollo::common::PointENU point;
   point.set__x(pointd.x);
   point.set__y(pointd.y);
   point.set__z(pointd.z);
   ...
   int status = hdmap_->GetNearestLane(point, &nearest_lane,
       &nearest_s, &nearest_l);
   if (status != 0) {
     AINFO_EVERY(60000) << "Failed to get nearest lane for point " <<
     point;
     return false;
   }*/
...
bool HDMapInput::GetSignalsFromHDMap(
  ...
  if (hdmap_->GetForwardNearestSignalsOnLane(point, forward_distance,
                                             &forward_signals) != 0) {
    AERROR_EVERY(60000) << "Failed to call HDMap::get_signal. point: ";
    return false;
  }
