  ...
  namespace perception
  {
    namespace onboard
    {
      ...
      void TransformCache::AddTransform(const StampedTransform &transform)
      {
        ...
        if (delt < 0.0)
        {
          AINFO_EVERY(60000) << "ERROR: add earlier transform to transform cache";
          return;
        }
                                          ...
                                          double max_duration)
      {
        ...
        {
          AINFO_EVERY(60000) << "ERROR: query timestamp is " << delt
                << "s later than cached timestamp";
          return false;
        }
        else if (delt < 0.0)
        {
          AINFO_EVERY(60000) << "ERROR: query earlier timestamp than transform cache";
          return false;
        }
        ...
        if (size == 1)
        {
          ...
          AINFO_EVERY(60000) << "use transform at " << transforms_.back().timestamp << " for "
                << timestamp;
        }
        else
        {
          ...
          AINFO_EVERY(60000) << "estimate pose at " << timestamp << " from poses at "
      ...
      {
        sensor2novatel_tf2_frame_id_ = "novatel";
        sensor2novatel_tf2_child_frame_id_ = sensor2novatel_tf2_child_frame_id;
        novatel2world_tf2_frame_id_ = "world";
          ...
          Eigen::Affine3d *novatel2world_trans)
      {
        if (!inited_)
        {
          AWARN << "TransformWrapper not Initialized,"
        ...
        if (sensor2novatel_extrinsics_ == nullptr)
        {
          ...
          *sensor2novatel_extrinsics_ =
          ...
          AINFO_EVERY(60000) << "Get sensor2novatel extrinsics successfully.";
        }
        ...
        *sensor2world_trans = novatel2world * (*sensor2novatel_extrinsics_);
        ...
        AINFO_EVERY(60000) << "Get pose timestamp: " << timestamp << ", pose: \n"
      ...
      bool TransformWrapper::GetExtrinsics(Eigen::Affine3d *trans)
      {
        ...
        {
          AERROR_EVERY(60000) << "TransformWrapper get extrinsics failed";
          return false;
        }
                                        ...
                                        const std::string &child_frame_id)
      {
        ...
        if (!tf2_buffer_->canTransform(frame_id, child_frame_id, query_time, static_cast<float>(FLAGS_obs_tf2_buff_size), &err_string))
        {
          AERROR_EVERY(60000) << "Can not find transform. " << FORMAT_TIMESTAMP(timestamp)
