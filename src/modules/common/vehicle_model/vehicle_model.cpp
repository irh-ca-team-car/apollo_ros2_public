...
namespace apollo
{
    namespace common
    {
        ...
        void VehicleModel::RearCenteredKinematicBicycleModel(
            const apollo_msgs::msg::ApollocommonVehicleModelConfig &vehicle_model_config,
            ...
            VehicleState *predicted_vehicle_state)
        {
            ...
            double dt = vehicle_model_config.rckinematicbicyclemodel.dt;
            ...
            double cur_v = cur_vehicle_state.linearvelocity;
            double cur_a = cur_vehicle_state.linearacceleration;
            double next_x = cur_x;
            double next_y = cur_y;
            ...
            predicted_vehicle_state->set__x(next_x);
            predicted_vehicle_state->set__y(next_y);
            predicted_vehicle_state->set__z(cur_z);
            predicted_vehicle_state->set__heading(next_phi);
            predicted_vehicle_state->set__kappa(cur_vehicle_state.kappa);
            predicted_vehicle_state->set__linearvelocity(next_v);
            predicted_vehicle_state->set__linearacceleration(
                cur_vehicle_state.linearacceleration);
        }

        VehicleState VehicleModel::Predict(const double predicted_time_horizon,
                                           const VehicleState &cur_vehicle_state)
        {
            apollo_msgs::msg::ApollocommonVehicleModelConfig vehicle_model_config;

            ACHECK(cyber::common::GetProtoFromASCIIFile(FLAGS_vehicle_model_config_filename,
            ...
            ACHECK(vehicle_model_config.modeltype !=
                   VehicleModelConfig::COM_CENTERED_DYNAMIC_BICYCLE_MODEL);
            ACHECK(vehicle_model_config.modeltype != VehicleModelConfig::MLP_MODEL);

            VehicleState predicted_vehicle_state;
            if (vehicle_model_config.modeltype ==
