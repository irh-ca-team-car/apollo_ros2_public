...
void MonitorLogger::Publish(const MonitorMessageItem::MessageSource &source,
  ...
  for (const auto &msg_item : messages) {
    auto *monitor_msg_item=&monitor_msg.item.back() ;
    monitor_msg_item->source=source;
    monitor_msg_item->loglevel=msg_item.first;
    monitor_msg_item->msg=msg_item.second;
    monitor_msg.item.push_back(*monitor_msg_item);
  }
...
void MonitorLogger::DoPublish(MonitorMessage *message) const {
  ...
  node_->FillHeader("monitor", *message);
