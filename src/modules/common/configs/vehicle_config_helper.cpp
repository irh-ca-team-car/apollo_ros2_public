...
void VehicleConfigHelper::Init() {
  AINFO << "Init" << FLAGS_vehicle_config_path;
  apollo::common::gflags::init_common(apollo::cyber::ComponentBase::instance());
  AINFO << "Init(Flags init)" << FLAGS_vehicle_config_path;
...
double VehicleConfigHelper::MinSafeTurnRadius() {
  const auto &param = vehicle_config_.vehicleparam;
  double lat_edge_to_center =
      std::max(param.leftedgetocenter, param.rightedgetocenter);
  double lon_edge_to_center =

      std::max(param.frontedgetocenter, param.backedgetocenter);
  return std::sqrt((lat_edge_to_center + param.minturnradius) *
                       (lat_edge_to_center + param.minturnradius) +
...
common::math::Box2d VehicleConfigHelper::GetBoundingBox(
  ...
  const auto &vehicle_param = vehicle_config_.vehicleparam;
  double diff_truecenter_and_pointX = (vehicle_param.frontedgetocenter -
