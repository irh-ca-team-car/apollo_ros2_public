...
namespace apollo
{
    namespace prediction
    {
        ...
        void RightOfWay::Analyze(ContainerManager *container_manager)
        {
            ...
            const ApolloperceptionPerceptionObstacle *pose_obstacle_ptr =
                 ...
                 obstacles_container->curr_frame_considered_obstacle_ids())
            {
                ...
                if (latest_feature_ptr->type != perception::PerceptionObstacle::VEHICLE)
                {
                    continue;
                }
                ADEBUG << "RightOfWay for obstacle [" << latest_feature_ptr->id << "], "
                       << "with lane_sequence_size: "
                       << latest_feature_ptr->lane.lanegraph.lanesequence.size();
                for (auto &lane_sequence : latest_feature_ptr->lane.lanegraph.lanesequence)
                {
                    accumulated_s = 0.0;
                    for (auto lane_segment : lane_sequence.lanesegment)
                    {
                        accumulated_s += lane_segment.ends - lane_segment.starts;
                        // set lower righ_of_way for turn lanes
                        if (lane_segment.laneturntype == 2)
                        { // left_turn
                            lane_sequence.set__rightofway(-20);
                        }
                        else if (lane_segment.laneturntype == 3)
                        { // right_turn
                            lane_sequence.set__rightofway(-10);
                        }
