...
namespace apollo
{
    namespace prediction
    {
        ...
        namespace
        {
            ...
            int NearestFrontObstacleIdOnLaneSequence(const apollo_msgs::msg::ApollopredictionLaneSequence &lane_sequence)
            {
                int nearest_front_obstacle_id = std::numeric_limits<int>::min();
                double smallest_relative_s = std::numeric_limits<double>::max();
                for (const auto &nearby_obs : lane_sequence.nearbyobstacle)
                {
                    if (nearby_obs.s < 0.0 ||
            ...
            int NearestBackwardObstacleIdOnLaneSequence(const apollo_msgs::msg::ApollopredictionLaneSequence &lane_sequence)
            {
        ...
        void ObstaclesPrioritizer::AssignIgnoreLevel()
        {
            ...
            for (const int obstacle_id : obstacle_ids)
            {
                ...
                bool is_pedestrian_like_in_front_near_lanes =
                    ...
                    (latest_feature_ptr->type == perception::PerceptionObstacle::PEDESTRIAN ||
                     latest_feature_ptr->type == perception::PerceptionObstacle::BICYCLE ||
                     latest_feature_ptr->type == perception::PerceptionObstacle::UNKNOWN ||
                     latest_feature_ptr->type == perception::PerceptionObstacle::UNKNOWN_MOVABLE) &&
                    is_near_lane;
                ...
                if (!need_consider)
                {
                    latest_feature_ptr->priority.set__priority(
        ...
        void ObstaclesPrioritizer::AssignCautionLevel()
        {
            ...
            auto storytelling_container =
                container_manager_->GetContainer<StoryTellingContainer>(
                    common::adapter::AdapterConfig::STORYTELLING);
            ...
            const Obstacle &ego_vehicle, ObstaclesContainer *obstacles_container)
        {
            ...
            for (const auto &lane_sequence :
                 ego_latest_feature.lane.lanegraph.lanesequence)
            {
            ...
            const Obstacle &ego_vehicle, ObstaclesContainer *obstacles_container)
        {
            ADCTrajectoryContainer *ego_trajectory_container =
                container_manager_->GetContainer<ADCTrajectoryContainer>(
                    common::adapter::AdapterConfig::PLANNING_TRAJECTORY);
            const Feature &ego_latest_feature = ego_vehicle.latest_feature();
            for (const auto &lane_sequence :
                 ego_latest_feature.lane.lanegraph.lanesequence)
            {
            ...
            const Obstacle &ego_vehicle, ObstaclesContainer *obstacles_container)
        {
                 ...
                 obstacles_container->curr_frame_movable_obstacle_ids())
            {
                ...
                Feature *latest_feature_ptr = obstacle_ptr->mutable_latest_feature();
                if (latest_feature_ptr->type != perception::PerceptionObstacle::PEDESTRIAN)
                {
                    continue;
                }
                ...
                double end_y = start_y + FLAGS_caution_pedestrian_approach_time *
                                             latest_feature_ptr->rawvelocity.y;
            ...
            const Obstacle &ego_vehicle, ObstaclesContainer *obstacles_container)
        {
                   ...
                   FLAGS_caution_obs_max_nums)
            {
                ...
                obstacle_ptr->mutable_latest_feature()->priority.set__priority(
                    ObstaclePriority::NORMAL);
            ...
            for (const auto overlap_ptr : cross_lanes)
            {
                ...
                for (const auto &object : overlap_ptr->overlap().object)
                {
                    if (object.id.id == lane_info_ptr->id().id &&
                        object.overlapinfo.laneoverlapinfo.ends < ego_lane_s_)
                    {
                ...
                for (const auto &object : overlap_ptr->overlap().object)
                {
                    ...
                    double ahead_s = overlap_lane_ptr->total_length() -
                                     object.overlapinfo.laneoverlapinfo.starts;
                    SetCautionBackward(
            ...
            ObstaclesContainer *obstacles_container)
        {
            ...
            while (!lane_info_queue.empty())
            {
                    ...
                    !lane_obstacles[lane_id].empty())
                {
                    ...
                    int obstacle_id = lane_obstacles[lane_id].front().obstacleid;
                    double obstacle_s = lane_obstacles[lane_id].front().lanes;
                    for (const LaneObstacle &lane_obstacle : lane_obstacles[lane_id])
                    {
                        if (lane_obstacle.lanes > obstacle_s)
                        {
                            obstacle_id = lane_obstacle.obstacleid;
                            obstacle_s = lane_obstacle.lanes;
                        }
                    }
                    Obstacle *obstacle_ptr = obstacles_container->GetObstacle(obstacle_id);
                    if (obstacle_ptr == nullptr)
                ...
                for (const auto &pre_lane_id : curr_lane->lane().predecessorid)
                {
