    ...
    namespace prediction
    {
            ...
            ContainerManager *container_manager)
        {
            ...
            auto ego_state_container = container_manager->GetContainer<PoseContainer>(
                common::adapter::AdapterConfig::LOCALIZATION);
            ...
            const LaneInfoPtr &ptr_ego_lane, const common::math::Vec2d &ego_position)
        {
            ...
            if (ptr_left_neighbor_lane == nullptr &&
                ptr_ego_lane->lane().leftboundary != LaneBoundary() &&
                ptr_ego_lane->lane().leftboundary.boundarytype.size() != 0 &&
                ptr_ego_lane->lane().leftboundary.boundarytype.at(0).types.size() != 0 &&
                ptr_ego_lane->lane().leftboundary.boundarytype.at(0).types.at(0) !=
                    hdmap::LaneBoundaryType::CURB)
            {
                const auto &reverse_lanes =
                    ptr_ego_lane->lane().leftneighborreverselaneid;
                std::for_each(
            ...
            ContainerManager *container_manager)
        {
            ...
            for (const auto &overlap_id : junction->junction().overlapid)
            {
                if (PredictionMap::OverlapById(overlap_id.id) != nullptr)
                {
                    ...
                    {
                        if (object.overlapinfo.signaloverlapinfo != 0 ||
                            object.overlapinfo.stopsignoverlapinfo != 0)
                        {
                            need_consider = true;
                        }
                    }
                                                 ...
                                                 const double heading)
        {
            ...
            position_enu.set__x(position.x);
            position_enu.set__y(position.y);
            position_enu.set__z(position.z);
