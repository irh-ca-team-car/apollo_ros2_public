...
void JunctionAnalyzer::SetAllJunctionExits() {
  ...
  for (const auto& overlap_id : junction_info_ptr_->junction().overlapid) {
    ...
    for (const auto& object : overlap_info_ptr->overlap().object) {
      if (object.overlapinfo!= apollo_msgs::msg::ApollohdmapObjectOverlapInfooverlapInfo()) {
        const std::string& lane_id = object.id.id;
        auto lane_info_ptr = PredictionMap::LaneById(lane_id);
        double s = object.overlapinfo.laneoverlapinfo.ends;
        if (s + FLAGS_junction_exit_lane_threshold <
          ...
          junction_exit.set__exitlaneid(lane_id);
          junction_exit.exitposition.set__x(position.x);
          junction_exit.exitposition.set__y(position.y);
          junction_exit.set__exitheading(lane_info_ptr->Heading(s));
          junction_exit.set__exitwidth(lane_info_ptr->GetWidth(s));
...
std::vector<JunctionExit> JunctionAnalyzer::GetJunctionExits(
  ...
  while (!lane_info_queue.empty()) {
    ...
    for (const auto& succ_lane_id : curr_lane->lane().successorid) {
...
const JunctionFeature& JunctionAnalyzer::GetJunctionFeature(
  ...
  junction_feature.set__junctionid(GetJunctionId());
  junction_feature.set__junctionrange(ComputeJunctionRange());
  ...
  for (const auto& junction_exit : junction_exits) {
    junction_feature.junctionexit.push_back(junction_exit);
  }
  junction_feature.enterlane.set__laneid(start_lane_id);
  junction_feature.startlaneid.push_back(start_lane_id);
...
JunctionFeature JunctionAnalyzer::GetJunctionFeature(
  ...
  for (const std::string& start_lane_id : start_lane_ids) {
    ...
    if (!initialized) {
      merged_junction_feature.set__junctionid(junction_feature.junctionid);
      merged_junction_feature.set__junctionrange(
          junction_feature.junctionrange);
      initialized = true;
    }
    for (const JunctionExit& junction_exit : junction_feature.junctionexit) {
      if (junction_exits_map.find(junction_exit.exitlaneid) ==
          junction_exits_map.end()) {
        junction_exits_map[junction_exit.exitlaneid] = junction_exit;
      }
    }
  }
  for (const auto& exit : junction_exits_map) {
    merged_junction_feature.startlaneid.push_back(exit.first);
    merged_junction_feature.junctionexit.push_back(exit.second);
  }
...
double JunctionAnalyzer::ComputeJunctionRange() {
  CHECK_NOTNULL(junction_info_ptr_);
  if (!(junction_info_ptr_->junction().polygon.point.size() >0) ||
      junction_info_ptr_->junction().polygon.point.size() < 3) {
