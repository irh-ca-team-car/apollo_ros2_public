...
bool PredictionMap::GetProjection(
  ...
  apollo::common::math::Vec2d v;
  v.set_x(pos.x());
  v.set_y(pos.y());
  return lane_info->GetProjection(v, s, l);
}
...
bool PredictionMap::IsVirtualLane(const std::string& lane_id) {
  ...
  using LaneBoundary = apollo::hdmap::LaneBoundary;
  const auto &lane = lane_info->lane();
  bool left_virtual = lane.leftboundary != LaneBoundary() &&
                      lane.leftboundary.virtuall;
  bool right_virtual = lane.rightboundary != LaneBoundary() &&
                       lane.rightboundary.virtuall;
...
bool PredictionMap::OnVirtualLane(const Eigen::Vector2d& point,
  ...
  hdmap_point.set__x(point[0]);
  hdmap_point.set__y(point[1]);
...
bool PredictionMap::InJunction(const Eigen::Vector2d& point,
  ...
  for (const auto junction_info : junction_infos) {
    if (junction_info == nullptr || junction_info->junction().polygon ==  apollo::hdmap::Polygon()) {
      continue;
    }
...
bool PredictionMap::IsLaneInJunction(
  ...
  if (lane_info->lane().junctionid != apollo::hdmap::Id() &&
      lane_info->lane().junctionid.id == junction_id) {
    return true;
  }
...
void PredictionMap::NearbyLanesByCurrentLanes(
  ...
  if (lanes.empty()) {
    ...
    for (auto& lane_ptr : lanes) {
      ...
      for (auto& lane_id : lane_ptr->lane().leftneighborforwardlaneid) {
      ...
      for (auto& lane_id : lane_ptr->lane().rightneighborforwardlaneid) {
        const std::string& id = lane_id.id;
        if (lane_ids.find(id) != lane_ids.end()) {
          continue;
        }
...
std::shared_ptr<const LaneInfo> PredictionMap::GetLeftNeighborLane(
  ...
  for (const auto& lane_id :
       ptr_ego_lane->lane().leftneighborforwardlaneid) {
...
std::shared_ptr<const LaneInfo> PredictionMap::GetRightNeighborLane(
  ...
  std::vector<std::string> neighbor_ids;
  for (const auto& lane_id :
       ptr_ego_lane->lane().rightneighborforwardlaneid) {
    neighbor_ids.push_back(lane_id.id);
  }
...
bool PredictionMap::IsLeftNeighborLane(
  ...
  for (const auto& left_lane_id :
       curr_lane->lane().leftneighborforwardlaneid) {
...
bool PredictionMap::IsRightNeighborLane(
  ...
  for (auto& right_lane_id :
       curr_lane->lane().rightneighborforwardlaneid) {
...
bool PredictionMap::IsSuccessorLane(std::shared_ptr<const LaneInfo> target_lane,
  ...
  for (const auto& successor_lane_id : curr_lane->lane().successorid) {
...
bool PredictionMap::IsPredecessorLane(
  ...
  for (const auto& predecessor_lane_id : curr_lane->lane().predecessorid) {
