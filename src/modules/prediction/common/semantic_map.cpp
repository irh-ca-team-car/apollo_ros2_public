...
bool ValidFeatureHistory(const ObstacleHistory& obstacle_history,
  ...
  if (obstacle_history.feature.size() == 0) {
    return false;
  }
  ...
  const Feature& feature = obstacle_history.feature.at(0);
...
void SemanticMap::RunCurrFrame(
  ...
  ego_feature_ = obstacle_id_history_map.at(FLAGS_ego_vehicle_id).feature.at(0);
  if (!FLAGS_enable_async_draw_base_image) {
    ...
    task_future_ = std::async(&SemanticMap::DrawBaseMapThread, this);
...
void SemanticMap::DrawRoads(const common::PointENU& center_point,
  ...
  for (const auto& road : roads) {
    for (const auto& section : road->road().section) {
      ...
      for (const auto& edge : section.boundary.outerpolygon.edge) {
        if (edge.type == 2) {  // left edge
          for (const auto& segment : edge.curve.segment) {
            for (const auto& point : segment.curvetype.linesegment.point) {
...
void SemanticMap::DrawLanes(const common::PointENU& center_point,
  ...
  for (const auto& lane : lanes) {
    ...
    for (const auto& segment : lane->lane().centralcurve.segment) {
      for (size_t i = 0; i < segment.curvetype.linesegment.point.size() - 1; ++i) {
        const auto &p0 =
            GetTransPoint(segment.curvetype.linesegment.point.at(i).x,
                          segment.curvetype.linesegment.point.at(i).y, base_x, base_y);
        const auto &p1 = GetTransPoint(segment.curvetype.linesegment.point.at(i + 1).x,
                                       segment.curvetype.linesegment.point.at(i + 1).y,
                                       base_x, base_y);
        double theta = atan2(segment.curvetype.linesegment.point.at(i + 1).y -
                                 segment.curvetype.linesegment.point.at(i).y,
    ...
    if (lane->lane().type == 2 && lane->lane().leftboundary.virtuall &&
        lane->lane().rightboundary.virtuall) {
      continue;
    }
    // Draw lane's left_boundary
    for (const auto& segment : lane->lane().leftboundary.curve.segment) {
      for (size_t i = 0; i < segment.curvetype.linesegment.point.size() - 1; ++i) {
        const auto &p0 =
            GetTransPoint(segment.curvetype.linesegment.point.at(i).x,
                          segment.curvetype.linesegment.point.at(i).y, base_x, base_y);
    ...
    for (const auto& segment :
         lane->lane().rightboundary.curve.segment) {
      for (size_t i = 0; i < segment.curvetype.linesegment.point.size() - 1; ++i) {
        const auto &p0 =
            GetTransPoint(segment.curvetype.linesegment.point.at(i).x,
                          segment.curvetype.linesegment.point.at(i).y, base_x, base_y);
...
void SemanticMap::DrawPoly(const Feature& feature, const cv::Scalar& color,
                           ...
                           cv::Mat* img) {
  std::vector<cv::Point> polygon;
  for (auto& polygon_point : feature.polygonpoint) {
    polygon.push_back(std::move(
...
void SemanticMap::DrawHistory(const ObstacleHistory& history,
  ...
  for (int i = history.feature.size() - 1; i >= 0; --i) {
    const Feature& feature = history.feature.at(i);
    ...
    if (feature.id == FLAGS_ego_vehicle_id) {
      ...
      if (feature.polygonpoint.size() == 0) {
...
cv::Mat SemanticMap::CropByHistory(const ObstacleHistory& history,
  ...
  const Feature& curr_feature = history.feature.at(0);
