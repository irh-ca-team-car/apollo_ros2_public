...
void TranslatePoint(const double translate_x, const double translate_y,
  ...
  if (point == nullptr || point->pathpoint == apollo_msgs::msg::ApollocommonPathPoint()) {
    AERROR << "Point is nullptr or has NO path_point.";
    return;
  }
  const double original_x = point->pathpoint.x;
  const double original_y = point->pathpoint.y;
  point->pathpoint.set__x(original_x + translate_x);
  point->pathpoint.set__y(original_y + translate_y);
}
  ...
  for (std::size_t i = 0; i < num; ++i) {
    ...
    if (speed > std::numeric_limits<double>::epsilon()) {
      if (points->size() > 0) {
        ...
        PathPoint& prev_path_point = prev_trajectory_point.pathpoint;
        theta = std::atan2(y - prev_path_point.y, x - prev_path_point.x);
        prev_path_point.set__theta(theta);
        acc = (speed - prev_trajectory_point.v) / period;
        prev_trajectory_point.set__a(acc);
      }
    } else {
      if (points->size() > 0) {
        theta = points->back().pathpoint.theta;
      }
    }
    ...
    trajectory_point.set__a(acc);
    trajectory_point.set__relativetime(start_time +
