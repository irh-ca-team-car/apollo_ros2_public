...
double ValidationChecker::ProbabilityByCentripetalAcceleration(
    const apollo_msgs::msg::ApollopredictionLaneSequence& lane_sequence, const double speed) {
  ...
  for (size_t i = 0; i < lane_sequence.pathpoint.size(); ++i) {
    const auto& path_point = lane_sequence.pathpoint.at(i);
...
bool ValidationChecker::ValidCentripetalAcceleration(
  ...
  for (size_t i = 0; i + 1 < trajectory_points.size(); ++i) {
    ...
    double time_diff = std::abs(p1.relativetime - p0.relativetime);
    if (time_diff < FLAGS_double_precision) {
      continue;
    }

    double theta_diff = std::abs(common::math::NormalizeAngle(
        p1.pathpoint.theta - p0.pathpoint.theta));
...
bool ValidationChecker::ValidTrajectoryPoint(
  ...
  return trajectory_point.pathpoint != apollo_msgs::msg::ApollocommonPathPoint() &&
      (!std::isnan(trajectory_point.pathpoint.x)) &&
      (!std::isnan(trajectory_point.pathpoint.y)) &&
      (!std::isnan(trajectory_point.pathpoint.theta)) &&
      (!std::isnan(trajectory_point.v)) &&
      (!std::isnan(trajectory_point.a)) &&
      (!std::isnan(trajectory_point.relativetime));
}
