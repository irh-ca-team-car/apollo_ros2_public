...
namespace apollo
{
  namespace prediction
  {
    ...
    bool MessageProcess::InitContainers(ContainerManager *container_manager)
    {
      ...
      if (!cyber::common::GetProtoFromASCIIFile(FLAGS_prediction_adapter_config_filename,
                                                &adapter_conf))
      {
        ...
        ScenarioManager *scenario_manager)
    {
      ...
      auto ptr_storytelling_container =
          container_manager->GetContainer<StoryTellingContainer>(
              apollo::common::adapter::AdapterConfig::STORYTELLING);
      ...
      if (ptr_ego_vehicle != nullptr)
      {
        ...
        if (perception_obstacles.apolloheader != apollo::common::Header() &&
            perception_obstacles.apolloheader.timestampsec != 0)
        {
          ADEBUG << "Correcting " << std::fixed << std::setprecision(6)
                 << ptr_ego_vehicle->timestamp << " to " << std::fixed
                 << std::setprecision(6)
                 << perception_obstacles.apolloheader.timestampsec;
          perception_obs_timestamp = perception_obstacles.apolloheader.timestampsec;
        }
      ...
      if (scenario.type == Scenario::JUNCTION && scenario.junctionid != std::string())
      {
        ...
        PredictionObstacles *const prediction_obstacles)
    {
      ...
      if (FLAGS_prediction_offline_mode == PredictionConstants::kDumpFeatureProto)
      {
             ...
             ptr_obstacles_container->curr_frame_movable_obstacle_ids())
        {
          ...
          if (obstacle_ptr->latest_feature() == apollo::prediction::Feature())
          {
    ...
    void MessageProcess::OnPlanning(ContainerManager *container_manager,
                                    const apollo_msgs::msg::ApolloplanningADCTrajectory &adc_trajectory)
    {
      auto ptr_ego_trajectory_container =
          container_manager->GetContainer<ADCTrajectoryContainer>(
              apollo::common::adapter::AdapterConfig::PLANNING_TRAJECTORY);
      ...
      ADEBUG << "Received a planning message [" << adc_trajectory
             << "].";
                                        ...
                                        const Stories &story)
    {
      ...
      ADEBUG << "Received a storytelling message [" << story
             << "].";
    }
    ...
    {
      (void)prediction_conf;
      (void)container_manager;
      (void)evaluator_manager;
      (void)predictor_manager;
      (void)scenario_manager;
      (void)record_filepath;
