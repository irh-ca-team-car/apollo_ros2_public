    ...
    namespace prediction
    {
                                          ...
                                          ObstaclesContainer *obstacles_container)
        {
            ...
            for (size_t i = 0; i < lane_graph_ptr->lanesequence.size(); ++i)
            {
                ...
                {
                    lane_sequence_ptr.set__probability(0.0);
                ...
                if (lane_sequence_ptr.vehicleonlane)
                {
        ...
        void CruiseMLPEvaluator::ExtractFeatureValues(
            Obstacle *obstacle_ptr, apollo_msgs::msg::ApollopredictionLaneSequence *lane_sequence_ptr,
            std::vector<double> *feature_values)
        {
            // Sanity checks.
            CHECK_NOTNULL(obstacle_ptr);
            ...
            const Obstacle *obstacle_ptr, std::vector<double> *feature_values)
        {
            ...
            for (std::size_t i = 0; i < obstacle_ptr->history_size(); ++i)
            {
                ...
                else
                {
                    ...
                    ADEBUG << feature.lane.currentlanefeature.size();
                }
                ...
                //if (feature.has_velocity_heading())
                {
                    vel_heading_history[i] =
                        WorldAngleToObjAngle(feature.velocityheading, obs_curr_heading);
                    if (i != 0)
                    {
        ...
        void CruiseMLPEvaluator::SetInteractionFeatureValues(
            ...
            apollo_msgs::msg::ApollopredictionLaneSequence *lane_sequence_ptr, std::vector<double> *feature_values)
        {
            (void)obstacle_ptr;
            ...
            forward_obstacle.set__s(FLAGS_default_s_if_no_obstacle_in_lane_sequence);
            forward_obstacle.set__l(FLAGS_default_l_if_no_obstacle_in_lane_sequence);
            backward_obstacle.set__s(-FLAGS_default_s_if_no_obstacle_in_lane_sequence);
            backward_obstacle.set__l(FLAGS_default_l_if_no_obstacle_in_lane_sequence);

            for (const auto &nearby_obstacle : lane_sequence_ptr->nearbyobstacle)
            {
                if (nearby_obstacle.s < 0.0)
                {
                    if (nearby_obstacle.s > backward_obstacle.s)
                    {
                        backward_obstacle.set__id(nearby_obstacle.id);
                        backward_obstacle.set__s(nearby_obstacle.s);
                        backward_obstacle.set__l(nearby_obstacle.l);
                    }
                }
                else
                {
                    ...
                    {
                        forward_obstacle.set__id(nearby_obstacle.id);
                        forward_obstacle.set__s(nearby_obstacle.s);
                        forward_obstacle.set__l(nearby_obstacle.l);
                    }
                }
            }
        ...
        void CruiseMLPEvaluator::SetLaneFeatureValues(
            const Obstacle *obstacle_ptr, const apollo_msgs::msg::ApollopredictionLaneSequence *lane_sequence_ptr,
            std::vector<double> *feature_values)
        {
            // Sanity checks.
            feature_values->clear();
            ...
            apollo_msgs::msg::ApollopredictionLaneSequence *lane_sequence_ptr)
        {
            ...
            lane_sequence_ptr->set__probability(apollo::common::math::Sigmoid(
                static_cast<double>(probability_tensor.accessor<float, 2>()[0][0])));
            lane_sequence_ptr->set__timetolanecenter(
