...
namespace apollo
{
    namespace prediction
    {
                                             ...
                                             ObstaclesContainer *obstacles_container)
        {
            ...
            Trajectory &trajectory = latest_feature_ptr->predictedtrajectory.emplace_back();
            trajectory.set__probability(1.0);
            ...
            for (int i = 0; i < 30; ++i)
            {
                ...
                if (i > 0)
                {
                    const auto &last_point = trajectory.trajectorypoint.at(i - 1).pathpoint;
                    prev_x = last_point.x;
                    prev_y = last_point.y;
                }
                TrajectoryPoint &point = trajectory.trajectorypoint.emplace_back();
                double dx = static_cast<double>(torch_output[0][i][0]);
                double dy = static_cast<double>(torch_output[0][i][1]);

                double heading = latest_feature_ptr->velocityheading;
                Vec2d offset(dx, dy);
                ...
                if (torch_output_tensor.sizes()[2] == 5)
                {
                    ...
                    point.gaussianinfo.set__sigmax(sigma_x);
                    point.gaussianinfo.set__sigmay(sigma_y);
                    point.gaussianinfo.set__correlation(corr);
                ...
                { // use origin heading for the first second
                    point.pathpoint.set__theta(
                        latest_feature_ptr->velocityheading);
                }
                else
                {
                    point.pathpoint.set__theta(
                        std::atan2(trajectory.trajectorypoint.at(i).pathpoint.y -
                                       trajectory.trajectorypoint.at(i - 1).pathpoint.y,
                                   trajectory.trajectorypoint.at(i).pathpoint.x -
                                       trajectory.trajectorypoint.at(i - 1).pathpoint.x));
                }
                point.set__relativetime(static_cast<double>(i) *
                                         FLAGS_prediction_trajectory_time_resolution);
                if (i == 0)
                {
                    point.set__v(latest_feature_ptr->speed);
                }
                else
                {
                    double diff_x = point_x - prev_x;
                    double diff_y = point_y - prev_y;
                    point.set__v(std::hypot(diff_x, diff_y) /
                                 FLAGS_prediction_trajectory_time_resolution);
                }
            }
        ...
        {   
            ...
            {  
                ADEBUG << "CUDA is not available";
                torch_vehicle_model_ =
                    torch::jit::load(FLAGS_torch_vehicle_semantic_lstm_cpu_file, device_);
                    ADEBUG << "flag2" << FLAGS_torch_pedestrian_semantic_lstm_cpu_file;
