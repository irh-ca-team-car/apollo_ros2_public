    ...
    namespace prediction
    {
                                             ...
                                             std::vector<Obstacle *> dynamic_env)
        {
            ...
            if ((latest_feature_ptr->lane == Lane()) ||
                (latest_feature_ptr->lane.lanegraphordered == LaneGraph()))
            {
                AERROR << "Obstacle [" << id << "] has no lane graph.";
                return false;
            }
            LaneGraph *lane_graph_ptr =
                &latest_feature_ptr->lane.lanegraphordered;
            ...
            ADEBUG << "There are " << lane_graph_ptr->lanesequence.size()
                ...
                PredictionConstants::kDumpDataForLearning)
            {
                ...
                if (latest_feature_ptr->junctionfeature != JunctionFeature())
                {
        ...
        {
            for (const apollo_msgs::msg::ApollopredictionLaneSequence &lane_sequence : lane_graph.lanesequence)
            {
                string_feature_values->push_back("|");
                for (int i = lane_sequence.adclanesegmentidx;
                     i < static_cast<int>(lane_sequence.lanesegment.size()); ++i)
                {
                    string_feature_values->push_back(lane_sequence.lanesegment.at(i).laneid);
                }
            }
            return true;
        }
            ...
            const Obstacle *obstacle_ptr, std::vector<double> *feature_values)
        {
            ...
            double obs_curr_heading = obs_curr_feature.velocityheading;
                 ...
                 ++i)
            {
                ...
                {
                    vel_heading_history[i] = feature.velocityheading;
                    if (i != 0)
                    {
                ...
                {
                    if (j >= (size_t)max_num_poly_pt)
                    {
                        break;
                    }
                    polygon_points_history[i][j].first = feature.polygonpoint.at(j).x;
                    polygon_points_history[i][j].second = feature.polygonpoint.at(j).y;
                }
            }
        ...
        bool LaneScanningEvaluator::ExtractStaticEnvFeatures(
            ...
            std::vector<int> *lane_sequence_idx_to_remove)
        {
            (void)lane_sequence_idx_to_remove;
            // Sanity checks.
            CHECK_NOTNULL(lane_graph_ptr);
            feature_values->clear();
            ...
            for (size_t i = 0; i < lane_graph_ptr->lanesequence.size(); ++i)
            {
                ...
                const apollo_msgs::msg::ApollopredictionLaneSequence &lane_sequence = lane_graph_ptr->lanesequence.at(i);

                // Extract features from backward lane-points.
                size_t count = 0;
                std::vector<double> backward_feature_values;
                for (int j = lane_sequence.adclanesegmentidx; j >= 0; --j)
                {
                    if (count >= SINGLE_LANE_FEATURE_SIZE * BACKWARD_LANE_POINTS_SIZE)
                    {
                        break;
                    }
                    const LaneSegment &lane_segment = lane_sequence.lanesegment.at(j);
                    int k_starting_idx = lane_segment.lanepoint.size() - 1;
                    if (j == lane_sequence.adclanesegmentidx)
                    {
                        k_starting_idx = std::min(lane_segment.adclanepointidx,
                                                  (int)lane_segment.lanepoint.size() - 1);
                    }
                    for (int k = k_starting_idx; k >= 0; --k)
                    {
                        ...
                        const auto &lane_point = lane_segment.lanepoint.at(k);
                ...
                count = 0;
                for (size_t j = lane_sequence.adclanesegmentidx;
                     j < lane_sequence.lanesegment.size(); ++j)
                {
                    ...
                    if (j == (size_t)lane_sequence.adclanesegmentidx)
                    {
                        k_starting_idx = std::min(lane_segment.adclanepointidx,
                                                  (int)lane_segment.lanepoint.size() - 1);
                    }
                    for (size_t k = k_starting_idx; k < lane_segment.lanepoint.size(); ++k)
                    {
                        if (count >= SINGLE_LANE_FEATURE_SIZE * LANE_POINTS_SIZE)
                        {
                            break;
            ...
            torch::jit::script::Module torch_model, Feature *feature_ptr)
        {
            ...
            for (size_t i = 0; i < SHORT_TERM_TRAJECTORY_SIZE; ++i)
            {
                ...
                Vec2d offset(dx, dy);
                Vec2d rotated_offset = offset.rotate(feature_ptr->velocityheading);
                double point_x = feature_ptr->position.x + rotated_offset.x();
                double point_y = feature_ptr->position.y + rotated_offset.y();
                point.pathpoint.set__x(point_x);
                point.pathpoint.set__y(point_y);
                point.set__relativetime(static_cast<double>(i) *
                                        FLAGS_prediction_trajectory_time_resolution);
                feature_ptr->shorttermpredictedtrajectorypoints.push_back(point);
