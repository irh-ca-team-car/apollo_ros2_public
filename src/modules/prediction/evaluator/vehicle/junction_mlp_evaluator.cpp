...
namespace apollo
{
    namespace prediction
    {
                                            ...
                                            ObstaclesContainer *obstacles_container)
        {
            ...
            if (
                latest_feature_ptr->junctionfeature.junctionexit.size() < 1)
            {
            ...
            if (latest_feature_ptr->junctionfeature.junctionexit.size() > 1)
            {
            ...
            {
                latest_feature_ptr->junctionfeature.junctionmlpprobability.push_back(prob);
            }
            ...
            for (const JunctionExit &junction_exit :
                 latest_feature_ptr->junctionfeature.junctionexit)
            {
                double x =
                    junction_exit.exitposition.x - latest_feature_ptr->position.x;
                double y =
                    junction_exit.exitposition.y - latest_feature_ptr->position.y;
                double angle =
                    std::atan2(y, x) - std::atan2(latest_feature_ptr->rawvelocity.y,
                                                  latest_feature_ptr->rawvelocity.x);
                double d_idx = (angle / (2.0 * M_PI) + 1.0 / 24.0) * 12.0;
                int idx = static_cast<int>(floor(d_idx >= 0 ? d_idx : d_idx + 12));
                junction_exit_prob[junction_exit.exitlaneid] = probability[idx];
            }
            for (size_t i = 0; i < lane_graph_ptr->lanesequence.size(); ++i)
            {
                ...
                {
                    if (junction_exit_prob.find(lane_segment.laneid) !=
                        junction_exit_prob.end())
                    {
                        lane_sequence_ptr.set__probability(
                            junction_exit_prob[lane_segment.laneid]);
                    }
                }
            }
            return true;
            ...
            Obstacle *obstacle_ptr, std::vector<double> *feature_values)
        {
            ...
            double obs_curr_heading = feature.velocityheading;
            bool has_history = false;
            std::vector<std::pair<double, double>> pos_history(
                FLAGS_junction_historical_frame_length, std::make_pair(0.0, 0.0));

            if (obstacle_ptr->history_size() > (size_t)FLAGS_junction_historical_frame_length)
            {
                has_history = true;
                for (std::size_t i = 0; i < (size_t)FLAGS_junction_historical_frame_length; ++i)
                {
            ...
            feature_values->push_back(feature.junctionfeature.junctionrange);
            if (has_history)
            {
            ...
            std::vector<double> *const feature_values)
        {
            ...
            *feature_values = std::vector<double>(4, 0.0);
            ...
            apollo::common::math::Vec2d ego_relative_velocity(ego_velocity.x,
                                                              ego_velocity.y);
            ego_relative_velocity.rotate(-obstacle_feature.velocityheading);
            ...
            Obstacle *obstacle_ptr, std::vector<double> *feature_values)
        {
            ...
            const Feature &feature_ptr = obstacle_ptr->latest_feature();

            double heading = std::atan2(feature_ptr.rawvelocity.y,
                                        feature_ptr.rawvelocity.x);

            std::string junction_id = feature_ptr.junctionfeature.junctionid;
            double junction_range = feature_ptr.junctionfeature.junctionrange;
            for (int i = 0; i < 12; ++i)
            {
                feature_values->push_back(0.0);
                feature_values->push_back(0.0);
            ...
            int num_junction_exit = feature_ptr.junctionfeature.junctionexit.size();
            for (int i = 0; i < num_junction_exit; ++i)
            {
                const JunctionExit &junction_exit =
                    feature_ptr.junctionfeature.junctionexit.at(i);
                double x = junction_exit.exitposition.x - feature_ptr.position.x;
                double y = junction_exit.exitposition.y - feature_ptr.position.y;
                double diff_x = std::cos(-heading) * x - std::sin(-heading) * y;
                double diff_y = std::sin(-heading) * x + std::cos(-heading) * y;
                double diff_heading =
                    apollo::common::math::AngleDiff(heading, junction_exit.exitheading);
