...
namespace apollo
{
    namespace prediction
    {
            ...
            ObstaclesContainer *obstacles_container)
        {
            ...
            auto *lane_graph = &feature_ptr->lane.lanegraph;

            int num_lane_sequence = lane_graph->lanesequence.size();
            ...
            {
                const auto &lane_sequence = lane_graph->lanesequence.at(i);
            ...
            CHECK_EQ(posteriors.size(),
                     lane_graph->lanesequence.size());
            for (int i = 0; i < num_lane_sequence; ++i)
            {
                double normalized_posterior =
                    posteriors[i] / (posterior_sum + FLAGS_double_precision);
                lane_graph->lanesequence.at(i).set__probability(normalized_posterior);
            ...
            if (largest_posterior > probability_threshold)
            {
                for (int i = 0; i < num_lane_sequence; ++i)
                {
                    ...
                    if (lane_sequence.stopsign != StopSign())
                    {
                        double stop_acceleration = 0.0;
                        double stop_distance = lane_sequence.stopsign.lanesequences -
                                               lane_sequence.lanes;
            ...
            {
                const auto &sequence = lane_graph->lanesequence.at(best_seq_idx);
                ...
                std::vector<TrajectoryPoint> points;
                DrawTrajectory(*obstacle, lane_graph->lanesequence.at(best_seq_idx),
                               best_lon_acceleration,
                               FLAGS_prediction_trajectory_time_length,
                               FLAGS_prediction_trajectory_time_resolution, &points);
                Trajectory trajectory = GenerateTrajectory(points);
            ...
            const double time_resolution)
        {
            ...
            for (const TrajectoryPoint &point : adc_trajectory.trajectorypoint)
            {
                if (point.relativetime + FLAGS_double_precision > curr_timestamp)
                {
            ...
            std::vector<TrajectoryPoint> *trajectory_points)
        {
            ...
            for (size_t i = 0; i < total_num; ++i)
            {
                ...
                trajectory_point.set__a(lon_acceleration);
        ...
        double InteractionPredictor::CentripetalAccelerationCost(
            const ApollopredictionLaneSequence &lane_sequence, const double speed,
            const double acceleration)
        {
            ...
            const double acceleration)
        {
            ...
            double cost_abs_sum = 0.0;
            double cost_sqr_sum = 0.0;
            int num_lane_segment = lane_sequence.lanesegment.size();

            double remained_s = lane_sequence.lanesegment.at(0).starts;
            int lane_seg_idx = 0;
            double prev_s = 0.0;

            for (const TrajectoryPoint &adc_trajectory_point : adc_trajectory_)
            {
                double relative_time = adc_trajectory_point.relativetime;
                double curr_s =
                ...
                while (lane_seg_idx < num_lane_segment)
                {
                    const LaneSegment &lane_segment =
                        lane_sequence.lanesegment.at(lane_seg_idx);
                    const std::string &lane_id = lane_segment.laneid;
                    ...
                    if (remained_s < lane_length)
                    {
                        ...
                        double adc_x = adc_trajectory_point.pathpoint.x;
                        double adc_y = adc_trajectory_point.pathpoint.y;
            ...
            const ADCTrajectoryContainer *adc_trajectory_container)
        {
            ...
            return lane_sequence.rightofway < 0;
        }
