...
using apollo_msgs::msg::ApollopredictionObstacleConf;
...
void PredictorManager::Init(const PredictionConf& config) {
  for (const auto& conf : config.obstacleconf) {
    
    switch (conf.obstacletype) {
      ...
      case perception::PerceptionObstacle::PEDESTRIAN: {
        pedestrian_predictor_ = (ObstacleConf::PredictorType)conf.predictortype;
        break;
      }
...
void PredictorManager::Run(
  ...
  prediction_obstacles_ = PredictionObstacles();
...
void PredictorManager::PredictObstacles(
    ...
    const ADCTrajectoryContainer* adc_trajectory_container,
    ObstaclesContainer* obstacles_container) {
  for (const ApolloperceptionPerceptionObstacle &perception_obstacle :
       perception_obstacles.perceptionobstacle)
  {
    ...
    if (obstacle != nullptr) {
      ...
      prediction_obstacle.set__timestamp(perception_obstacle.timestamp);
      prediction_obstacle.set__isstatic(true);
    }

    prediction_obstacle.set__predictedperiod(
        FLAGS_prediction_trajectory_time_length);
    prediction_obstacle.perceptionobstacle=(
        perception_obstacle);

    prediction_obstacles_.predictionobstacle.push_back(
        prediction_obstacle);
  }
}
...
void PredictorManager::PredictObstaclesInParallel(
  ...
  for (const auto& perception_obstacle :
    ...
    if (obstacle == nullptr) {
      ...
      prediction_obstacle_ptr->set__isstatic(true);
      prediction_obstacle_ptr->set__timestamp(perception_obstacle.timestamp);
    } else {
       ...
       perception_obstacles.perceptionobstacle)
  {
    ...
    prediction_obstacle_ptr->set__predictedperiod(
        FLAGS_prediction_trajectory_time_length);
    prediction_obstacle_ptr->perceptionobstacle=(
        perception_obstacle);
    prediction_obstacles_.predictionobstacle.push_back(
...
void PredictorManager::PredictObstacle(
  ...
  prediction_obstacle->set__timestamp(obstacle->timestamp());
  ...
  if (obstacle->ToIgnore()) {
    ...
    prediction_obstacle->priority.set__priority(
...
void PredictorManager::InitVehiclePredictors(const ApollopredictionObstacleConf& conf) {
  switch (conf.obstaclestatus) {
    case ObstacleConf::ON_LANE: {
      if (conf.prioritytype == ObstaclePriority::CAUTION) {
        vehicle_on_lane_caution_predictor_ = (ObstacleConf::PredictorType)conf.predictortype;
      } else {
        vehicle_on_lane_predictor_ = (ObstacleConf::PredictorType)conf.predictortype;
      }
      break;
    }
    case ObstacleConf::OFF_LANE: {
      vehicle_off_lane_predictor_ = (ObstacleConf::PredictorType)conf.predictortype;
      break;
    }
    case ObstacleConf::IN_JUNCTION: {
      if (conf.prioritytype == ObstaclePriority::CAUTION) {
        vehicle_in_junction_caution_predictor_ = (ObstacleConf::PredictorType)conf.predictortype;
      } else {
        vehicle_in_junction_predictor_ = (ObstacleConf::PredictorType)conf.predictortype;
      }
      break;
    }
    default: { break; }
...
void PredictorManager::InitCyclistPredictors(const ApollopredictionObstacleConf& conf) {
  switch (conf.obstaclestatus) {
    case ObstacleConf::ON_LANE: {
      cyclist_on_lane_predictor_ = (ObstacleConf::PredictorType)conf.predictortype;
      break;
    }
    case ObstacleConf::OFF_LANE: {
      cyclist_off_lane_predictor_ = (ObstacleConf::PredictorType)conf.predictortype;
      break;
    }
    default: { break; }
  }
...
void PredictorManager::InitDefaultPredictors(const ApollopredictionObstacleConf &conf)
{
  switch (conf.obstaclestatus) {
    case ObstacleConf::ON_LANE: {
      default_on_lane_predictor_ = (ObstacleConf::PredictorType)conf.predictortype;
      break;
    }
    case ObstacleConf::OFF_LANE: {
      default_off_lane_predictor_ = (ObstacleConf::PredictorType) conf.predictortype;
      break;
    }
    default: { break; }
  }
