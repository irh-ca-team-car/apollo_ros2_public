    ...
    namespace prediction
    {
            ...
            std::vector<TrajectoryPoint> *trajectory_points)
        {
            ...
            const std::array<double, 2> start_x = {feature.position.x,
                                                   feature.rawvelocity.x};
            const std::array<double, 2> end_x = {
                junction_exit.exitposition.x,
                std::cos(junction_exit.exitheading) * speed};
            const std::array<double, 2> start_y = {feature.position.y,
                                                   feature.rawvelocity.y};
            const std::array<double, 2> end_y = {
                junction_exit.exitposition.y,
                std::sin(junction_exit.exitheading) * speed};
            ...
            while (t <= exit_time)
            {
                ...
                path_point.set__x(EvaluateCubicPolynomial(x_coeffs, t, 0));
                path_point.set__y(EvaluateCubicPolynomial(y_coeffs, t, 0));
                path_point.set__z(0.0);
                path_point.set__theta(std::atan2(EvaluateCubicPolynomial(y_coeffs, t, 1),
                                                EvaluateCubicPolynomial(x_coeffs, t, 1)));
                TrajectoryPoint trajectory_point;
                trajectory_point.pathpoint=(path_point);
                trajectory_point.set__v(std::hypot(EvaluateCubicPolynomial(x_coeffs, t, 1),
                                                  EvaluateCubicPolynomial(y_coeffs, t, 1)));
                trajectory_point.set__relativetime(t);
                trajectory_points->emplace_back(std::move(trajectory_point));
                t += period;
            }
            std::string lane_id = junction_exit.exitlaneid;
            std::shared_ptr<const LaneInfo> lane_info = PredictionMap::LaneById(lane_id);
            Eigen::Vector2d pos(junction_exit.exitposition.x,
                                junction_exit.exitposition.y);
            double lane_s = 0.0;
            double lane_l = 0.0;
            double theta = M_PI;
            ...
            while (t <= total_time)
            {
                ...
                TrajectoryPoint trajectory_point;
                PathPoint path_point;
                path_point.set__x(pos.x());
                path_point.set__y(pos.y());
                path_point.set__z(0.0);
                ...
                while (lane_s > PredictionMap::LaneById(lane_id)->total_length())
                {
                    ...
                    if (PredictionMap::LaneById(lane_id)->lane().successorid.size() < 1)
                    {
                        return;
                    }
                    // TODO(all) consider the logic to choose successor_id
                    lane_id = PredictionMap::LaneById(lane_id)->lane().successorid.at(0).id;
                }
                t += period;
            }
        }
            ...
            const Feature &feature)
        {
            ...
            if (feature.junctionfeature.junctionexit.size() < 1 ||
                feature.junctionfeature.junctionmlpprobability.size() != 12)
            {
            ...
            for (int i = 0; i < 12; ++i)
            {
                if (feature.junctionfeature.junctionmlpprobability.at(i) > max_prob)
                {
                    max_prob = feature.junctionfeature.junctionmlpprobability.at(i);
                    max_idx = i;
                }
            }
            std::vector<JunctionExit> junction_exits;
            for (const JunctionExit &junction_exit :
                 feature.junctionfeature.junctionexit)
            {
                double x = junction_exit.exitposition.x - feature.position.x;
                double y = junction_exit.exitposition.y - feature.position.y;
                double angle = std::atan2(y, x) - std::atan2(feature.rawvelocity.y,
                                                             feature.rawvelocity.x);
