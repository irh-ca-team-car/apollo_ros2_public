            ...
            ObstaclesContainer *obstacles_container)
        {
            ...
            for (int i = 0; i < num_lane_sequence; ++i)
            {
                ...
                if (sequence.lanesegment.empty() ||
                    sequence.lanesegment.at(0).lanepoint.empty())
                {
 ...
 */
            ...
            std::vector<TrajectoryPoint> *points)
        {
            ...
            double vel_heading = feature.velocityheading;
            ...
            double lane_heading = lane_sequence.lanesegment.at(0).lanepoint.at(0).heading;
            double s0 = 0.0;
            ...
            size_t lane_segment_index = lane_sequence.adclanesegmentidx;
            std::string lane_id =
            ...
            for (size_t i = 0; i < total_num; ++i)
            {
                ...
                trajectory_point.set__v(lane_speed);
                trajectory_point.set__a(lane_acc);
        ...
        std::pair<double, double> MoveSequencePredictor::ComputeLonEndState(
            const std::array<double, 3> &init_s, const ApollopredictionLaneSequence &lane_sequence)
        {
            // Get the maximum kappa of the lane.
            double max_kappa = 0.0;
            double s_at_max_kappa = 0.0;
            for (size_t i = 0; i < lane_sequence.pathpoint.size(); ++i)
            {
                const PathPoint &path_point = lane_sequence.pathpoint.at(i);
            ...
            const Obstacle &obstacle, const ApollopredictionLaneSequence &lane_sequence)
        {
            ...
            const Feature &feature = obstacle.latest_feature();
            const auto &first_lane_point =
                lane_sequence.lanesegment.at(0).lanepoint.at(0);
            double v_x = feature.velocity.x;
            double v_y = feature.velocity.y;
            double lane_heading = first_lane_point.heading;
            double lane_l = first_lane_point.relativel;
