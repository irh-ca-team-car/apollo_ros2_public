...
namespace apollo
{
    namespace prediction
    {
        ...
        int Predictor::NumOfTrajectories(const Obstacle &obstacle)
        {
            ...
            return obstacle.latest_feature().predictedtrajectory.size();
        }
            ...
            const std::vector<TrajectoryPoint> &points)
        {
            ...
            trajectory.trajectorypoint = {points.begin(), points.end()};
            return trajectory;
        }
                                            ...
                                            Obstacle *obstacle_ptr)
        {
            ...
            for (int i = start_index; i < num; ++i)
            {
                obstacle_ptr->mutable_latest_feature()
                    ->predictedtrajectory.at(i)
                    .set__probability(prob);
            }
        }
            ...
            Obstacle *obstacle)
        {
            for (auto &predicted_trajectory :
                 obstacle->mutable_latest_feature()->predictedtrajectory)
            {
            ...
            Trajectory *trajectory)
        {
            ...
            int num_of_point = trajectory->trajectorypoint.size();
            if (num_of_point == 0)
            {
                return false;
            }
            ...
            double vehicle_heading = feature.velocityheading;
            double forward_length =
                std::fmax(vehicle_length / 2.0 - FLAGS_distance_beyond_junction, 0.0);

            double front_x = trajectory->trajectorypoint.at(0).pathpoint.x +
                             forward_length * std::cos(vehicle_heading);
            double front_y = trajectory->trajectorypoint.at(0).pathpoint.y +
                             forward_length * std::sin(vehicle_heading);
            PathPoint front_point;
            front_point.set__x(front_x);
            front_point.set__y(front_y);
            bool front_in_junction =
                adc_trajectory_container.IsPointInJunction(front_point);

            const PathPoint &start_point = trajectory->trajectorypoint.at(0).pathpoint;
            ...
            {
                const PathPoint &point = trajectory->trajectorypoint.at(index).pathpoint;
            ...
            {
                trajectory->trajectorypoint.pop_back();
            }
            return true;
        }
