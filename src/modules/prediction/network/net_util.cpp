    ...
    namespace prediction
    {
        namespace network
        {
            ...
            bool LoadTensor(const TensorParameter &tensor_pb, Eigen::MatrixXf *matrix)
            {
                ...
                if (tensor_pb.shape.size() < 2)
                {
                    ADEBUG << "Load tensor size: (1, " << tensor_pb.shape.at(0) << ")";
                    matrix->resize(1, tensor_pb.shape.at(0));
                    for (int i = 0; i < tensor_pb.shape.at(0); ++i)
                    {
                        (*matrix)(0, i) = static_cast<float>(tensor_pb.data.at(i));
                    }
                    return true;
                }
                ADEBUG << "Load tensor size: (" << tensor_pb.shape.at(0) << ", "
                       << tensor_pb.shape.at(1) << ")";
                CHECK_EQ(tensor_pb.shape.size(), 2);
                matrix->resize(tensor_pb.shape.at(0), tensor_pb.shape.at(1));
                for (int i = 0; i < tensor_pb.shape.at(0); ++i)
                {
                    for (int j = 0; j < tensor_pb.shape.at(1); ++j)
                    {
                        (*matrix)(i, j) =
                            static_cast<float>(tensor_pb.data.at(i * tensor_pb.shape.at(1) + j));
                    }
                }
                return true;
            }
            ...
            bool LoadTensor(const TensorParameter &tensor_pb, Eigen::VectorXf *vector)
            {
                ...
                ADEBUG << "Load tensor size: (" << tensor_pb.shape.at(0) << ", 1)";
                CHECK_EQ(tensor_pb.shape.size(), 1);
                if (tensor_pb.shape.size() == 1)
                {
                    vector->resize(tensor_pb.shape.at(0));
                    for (int i = 0; i < tensor_pb.shape.at(0); ++i)
                    {
                        (*vector)(i) = static_cast<float>(tensor_pb.data.at(i));
                    }
                }
                return true;
            }
            ...
            {
                if (tensor_pb.data.empty() || tensor_pb.shape.size() != 3)
                {
                    AERROR << "Fail to load the necessary fields!";
                    return false;
                }
                int num_depth = tensor_pb.shape.at(0);
                int num_row = tensor_pb.shape.at(1);
                int num_col = tensor_pb.shape.at(2);
                CHECK_EQ(tensor_pb.data.size(),(size_t)( num_depth * num_row * num_col));
                ...
                for (int k = 0; k < num_depth; ++k)
                {
                    ...
                    for (int i = 0; i < num_row; ++i)
                    {
                        ...
                        {
                            matrix(i, j) = tensor_pb.data.at(tensor_pb_index);
                            ++tensor_pb_index;
                        }
                    }
