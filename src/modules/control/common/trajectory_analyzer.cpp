  ...
  namespace control
  {
    namespace
    {
      ...
      double PointDistanceSquare(const TrajectoryPoint &point, const double x,
                                 const double y)
      {
        const double dx = point.pathpoint.x - x;
        const double dy = point.pathpoint.y - y;
        return dx * dx + dy * dy;
      }
    ...
    {
      header_time_ = planning_published_trajectory->apolloheader.timestampsec;
      // seq_num_ = planning_published_trajectory->apolloheader().sequence_num();

      for (size_t i = 0; i < planning_published_trajectory->trajectorypoint.size();
           ++i)
      {
        trajectory_points_.push_back(
            planning_published_trajectory->trajectorypoint.at(i));
      }
    }
    ...
    {
      if (trajectory_points_.size() == (size_t)0)
      {
        AERROR_EVERY(60000) << "No trajectory point";
        return TrajectoryPointToPathPoint(TrajectoryPoint()); ;
      }
      ...
      if (index_start == index_end ||
          std::fabs(trajectory_points_[index_start].pathpoint.s -
                    trajectory_points_[index_end].pathpoint.s) <= kEpsilon)
      {
        ...
        const double t) const
    {
      ...
      {
        return point.relativetime < relative_time;
      };
      ...
      else
      {
        ...
        if (it_low->relativetime - t < t - it_lower->relativetime)
        {
          return *it_low;
        }
        return *it_lower;
                                                       ...
                                                       const double y) const
    {
      ...
      {
        double px = common::math::lerp(p0.pathpoint.x, p0.pathpoint.s,
                                       p1.pathpoint.x, p1.pathpoint.s, s);
        double py = common::math::lerp(p0.pathpoint.y, p0.pathpoint.s,
                                       p1.pathpoint.y, p1.pathpoint.s, s);
        double dx = px - x;
        double dy = py - y;
        return dx * dx + dy * dy;
      };

      PathPoint p = p0.pathpoint;
      double s = common::math::GoldenSectionSearch(dist_square, p0.pathpoint.s,
                                                   p1.pathpoint.s);
      p.set__s(s);
      p.set__x(common::math::lerp(p0.pathpoint.x, p0.pathpoint.s,
                                  p1.pathpoint.x, p1.pathpoint.s, s));
      p.set__y(common::math::lerp(p0.pathpoint.y, p0.pathpoint.s,
                                  p1.pathpoint.y, p1.pathpoint.s, s));
      p.set__theta(common::math::slerp(p0.pathpoint.theta, p0.pathpoint.s,
                                       p1.pathpoint.theta, p1.pathpoint.s,
                                       s));
      // approximate the curvature at the intermediate point
      p.set__kappa(common::math::lerp(p0.pathpoint.kappa, p0.pathpoint.s,
                                      p1.pathpoint.kappa, p1.pathpoint.s,
                                      s));
      return p;
    }
        ...
        const double rear_to_com_distance)
    {
      ...
      for (size_t i = 0; i < trajectory_points_.size(); ++i)
      {
        auto com = ComputeCOMPosition(rear_to_com_distance,
                                      trajectory_points_[i].pathpoint);
        trajectory_points_[i].pathpoint.set__x(com.x());
        trajectory_points_[i].pathpoint.set__y(com.y());
      }
    }
