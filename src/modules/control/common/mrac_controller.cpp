...
void MracController::Init(const MracConf &mrac_conf,
  ...
  model_order_ = mrac_conf.mracmodelorder;
  // Initialize the saturation limits
  bound_ratio_ = mrac_conf.mracsaturationlevel;
...
Status MracController::SetReferenceModel(const MracConf &mrac_conf) {
  ...
  if (((mrac_conf.referencetimeconstant < Epsilon && model_order_ == 1)) ||
      ((mrac_conf.referencenaturalfrequency < Epsilon &&
        model_order_ == 2))) {
    const auto error_msg = absl::StrCat(
        "mrac controller error: reference model time-constant parameter: ",
        mrac_conf.referencetimeconstant,
        "and natural frequency parameter: ",
        mrac_conf.referencenaturalfrequency,
  ...
  tau_reference_ = mrac_conf.referencetimeconstant;
  wn_reference_ = mrac_conf.referencenaturalfrequency;
  zeta_reference_ = mrac_conf.referencedampingratio;
  tau_clamping_ = mrac_conf.clampingtimeconstant;
  adaption_clamping_enabled = (tau_clamping_ >= Epsilon);
  return Status::OK();
}

Status MracController::SetAdaptionModel(const MracConf &mrac_conf) {
  const int p_size = mrac_conf.adaptionmatrixp.size();
  const int x_size = mrac_conf.adaptionstategain.size();
  const int aw_size = mrac_conf.antiwindupcompensationgain.size();
  ...
  for (int i = 0; i < model_order_; ++i) {
    gamma_state_adaption_(i, i) =
        (i <= x_size - 1) ? mrac_conf.adaptionstategain.at(i)
                          : mrac_conf.adaptionstategain.at(x_size - 1);
    gain_anti_windup_(i, i) =
        (i <= aw_size - 1)
            ? mrac_conf.antiwindupcompensationgain.at(i)
            : mrac_conf.antiwindupcompensationgain.at(aw_size - 1);
    for (int j = 0; j < model_order_; ++j) {
      matrix_p_adaption_(i, j) =
          mrac_conf.adaptionmatrixp.at(i * model_order_ + j);
    }
  }
  gamma_input_adaption_(0, 0) = mrac_conf.adaptiondesiredgain;
  gamma_nonlinear_adaption_(0, 0) = mrac_conf.adaptionnonlineargain;
  return Status::OK();
}
...
void MracController::EstimateInitialGains(const LatencyParam &latency_param) {
  const double rise_time_estimate =
      latency_param.deadtime + latency_param.risetime;
  const double settling_time_estimate =
      latency_param.deadtime + latency_param.settlingtime;
