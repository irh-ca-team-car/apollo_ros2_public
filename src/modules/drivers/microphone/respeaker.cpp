...
namespace apollo
{
    namespace drivers
    {
        namespace microphone
        {
            ...
            void Respeaker::init(
                const std::shared_ptr<const MicrophoneConfig_> &microphone_config)
            {
                if (microphone_config->microphonemodel != 1)
                {
                ...
                stream_ptr_->init_stream(
                    microphone_config->samplerate, microphone_config->channeltype.size(),
                    microphone_config->chunk, device_index,
                    get_format_from_width(microphone_config->samplewidth));
            }
            ...
            PaDeviceIndex Respeaker::get_respeaker_index() const
            {
