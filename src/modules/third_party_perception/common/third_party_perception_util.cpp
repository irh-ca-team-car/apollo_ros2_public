...
namespace apollo
{
    namespace third_party_perception
    {
        ...
        void FillPerceptionPolygon(ApolloperceptionPerceptionObstacle & perception_obstacle,
                                   ...
                                   const double heading)
        {
            ...
            for (int i = 0; i < 4; ++i)
            {
                auto ptr = perception_obstacle.polygonpoint.emplace_back();
                ptr.set__x(
        ...
        Point SLtoXY(const double x, const double y, const double theta)
        {
            ...
            converted_point.set__x(x * std::cos(theta) + y * std::sin(theta));
            converted_point.set__y(x * std::sin(theta) - y * std::cos(theta));
        ...
        double GetNearestLaneHeading(const Point &point)
        {
            ...
            if (hdmap == nullptr)
            {
                AERROR << "Failed to get nearest lane for point " << point;
                return -1.0;
            }

            PointENU point_enu;
            point_enu.set__x(point.x);
            point_enu.set__y(point.y);
            point_enu.set__z(point.z);
