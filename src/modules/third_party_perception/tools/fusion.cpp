...
std::vector<Vec2d> PerceptionObstacleToVectorVec2d(
    const ApolloperceptionPerceptionObstacle &obstacle)
{
  std::vector<Vec2d> result;
  for (const auto& vertex : obstacle.polygonpoint) {
    result.emplace_back(vertex.x, vertex.y);
  }
  return result;
}

bool HasOverlap(const ApolloperceptionPerceptionObstacle &obstacle_1,
                const ApolloperceptionPerceptionObstacle &obstacle_2)
{
...
bool HasOverlap(const ApolloperceptionPerceptionObstacle &obstacle,
                const PerceptionObstacles &obstacles)
{
  for (const auto& current_obstacle : obstacles.perceptionobstacle) {
...
PerceptionObstacles EyeRadarFusion(const PerceptionObstacles& eye_obstacles,
  ...
  for (auto& eye_obstacle :
       (eye_obstacles_fusion.perceptionobstacle)) {
    for (auto& radar_obstacle :
         (radar_obstacles_fusion.perceptionobstacle)) {
      if (HasOverlap(eye_obstacle, radar_obstacle)) {
        eye_obstacle.set__confidence(0.99);
        eye_obstacle.velocity=(radar_obstacle.velocity);
      }
    }
  }
