...
namespace filter {
  ...
  bool IsPreserved(const RadarObstacle &radar_obstacle)
  {
    ...
    double nearest_l =
        GetLateralDistanceToNearestLane(radar_obstacle.absoluteposition);
...
RadarObstacles FilterRadarObstacles(const RadarObstacles& radar_obstacles) {
  ...
  for (const auto& iter : radar_obstacles.radarobstacle) {
    if (IsPreserved(iter.data)) {
      filtered_radar_obstacles.radarobstacle.push_back(iter);
    }
  }
  filtered_radar_obstacles.apolloheader=(radar_obstacles.apolloheader);
