...
Fusion::Fusion() {
  ...
  score_params_ = ft_config_manager_ptr_->fusion_params_.params.scoreparams;
  switch (score_params_.confidencelevel) {
...
bool Fusion::CheckDisScore(const base::Object &in1_ptr,
  ...
  *score = 2.5 * std::max(0.0, score_params_.maxmatchdistance - dis);
  return true;
}
...
bool Fusion::CheckTypeScore(const base::Object &in1_ptr,
  ...
  if (in1_ptr.sub_type == in2_ptr.sub_type) {
    ...
    same_prob *= score_params_.probscale;
  }
  *score *= same_prob;
  return true;
}
