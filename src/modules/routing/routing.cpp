...
namespace apollo
{
    namespace routing
    {
            ...
            const RoutingRequest &routing_request)
        {
            ...
            for (size_t i = 0; i < routing_request.waypoint.size(); ++i)
            {
                LaneWaypoint lane_waypoint(routing_request.waypoint.at(i));
                if (lane_waypoint.id!=std::string())
                {
                    continue;
                }
                ...
                for (size_t j = 0; j < lanes.size(); ++j)
                {
                    ...
                    if (j == 0)
                    {
                        auto &waypoint_info = fixed_request.waypoint.at(i);
                        waypoint_info.set__id(lanes[j]->id().id);
                        waypoint_info.set__s(s);
                    }
                    else
                    {
                        // additional candidate lanes
                        LaneWaypoint new_lane_waypoint(lane_waypoint);
                        new_lane_waypoint.set__id(lanes[j]->id().id);
                        new_lane_waypoint.set__s(s);
            ...
            for (const auto &m : additional_lane_waypoint_map)
            {
                ...
                for (size_t i = 0; i < cur_size; ++i)
                {
                    ...
                    for (const auto &lane_waypoint : m.second)
                    {
                        ...
                        auto &waypoint_info = new_request.waypoint.at(m.first);
                        waypoint_info.set__id(lane_waypoint.id);
                        waypoint_info.set__s(lane_waypoint.s);
            ...
            {
                ADEBUG << "Fixed routing request:" << fixed_request;
            }
        ...
        {
            const auto &routing_request = routing_response->routingrequest;
            const bool has_parking_info = routing_request.parkinginfo != ParkingInfo();
            const bool has_parking_id =
                has_parking_info && routing_request.parkinginfo.parkingspaceid!= std::string();
            ...
            //  ID successfully
        if (has_parking_info && routing_request.parkinginfo.parkingpoint != PointENU())
            {
                const PointENU parking_point =
                    routing_request.parkinginfo.parkingpoint;
                std::string parking_space_id;
                if (GetParkingID(parking_point, &parking_space_id))
                {
                    routing_response->routingrequest
                        .parkinginfo.set__parkingspaceid(parking_space_id);
                    return true;
                }
            }
                              ...
                              RoutingResponse *const routing_response)
        {
            ...
            AINFO << "Get new routing request:" << routing_request;
            ...
            for (const auto &fixed_request : fixed_requests)
            {
                ...
                if (navigator_ptr_->SearchRoute(fixed_request, &routing_response_temp))
                {
                    ...
                    {
                        *routing_response=(routing_response_temp);
            ...
            AWARN <<"Routing failed! " << routing_response->status.msg;
            return false;
        }
