...
namespace apollo
{
    namespace routing
    {
        ...
        bool ContainsKey_TNS(std::unordered_set<const TopoNode *> objs,const TopoNode* key)
        {
            for (auto &&i : objs)
            {
                if (i == key)
                                             ...
                                             PassageInfo *const curr_passage)
        {
            ...
            while (allowed_to_explore)
            {
                     ...
                     curr_passage->nodes.front().GetTopoNode()->InFromPreEdge())
                {
                    ...
                    if (ContainsKey_TNS(node_set_of_curr_passage, pred_node))
                    {
                        continue;
                    }
                                            ...
                                            PassageInfo *const curr_passage)
        {
            ...
            while (allowed_to_explore)
            {
                     ...
                     curr_passage->nodes.back().GetTopoNode()->OutToSucEdge())
                {
                    ...
                    if (ContainsKey_TNS(node_set_of_curr_passage, succ_node))
                    {
                        continue;
                    }
            ...
            Passage & passage)
        {
            ...
            {
                LaneSegment &seg = passage.segment.emplace_back();
                seg.set__id(it->LaneId());
                seg.set__starts(it->StartS());
                seg.set__ends(it->EndS());
            }
        }
            ...
            const TopoRangeManager &range_manager, RoutingResponse *const result)
        {
            ...
            result->set__mapversion(map_version);
            result->measurement.set__distance(CalculateDistance(nodes));
            result->routingrequest=(request);
            return true;
        }
        ...
        {
            auto &road = result->road.emplace_back();

            road.set__id(passages[start_index.first].nodes[start_index.second].RoadId());
            for (std::size_t i = start_index.first;
                 i <= end_index.first && i < passages.size(); ++i)
            {
                auto &passage = road.passage.emplace_back();
                const size_t node_start_index =
                    (i == start_index.first ? std::max((std::size_t)0, start_index.second) : 0);
                ...
                const size_t node_end_index =
                    (i == end_index.first ? std::min(end_index.second, passages[i].nodes.size() - 1) : passages[i].nodes.size() - 1);
                ...
                {
                    passage.set__changelanetype(FORWARD);
                    passage.set__canexit(true);
                }
                else
                {
                    passage.set__changelanetype(passages[i].change_lane_type);
                    passage.set__canexit(i == end_index.first);
                }
            }
        }
