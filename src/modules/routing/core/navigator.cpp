    ...
    namespace routing
    {
        ...
        namespace
        {
            ...
            bool ShowRequestInfo(const RoutingRequest &request, const TopoGraph *graph)
            {
                ...
                auto tmp2 = request.blacklistedlane;
                ...
                for (auto &&bl : tmp2)
                {
                    ...
                    AINFO << "Black point:\tlane id: " << bl.id
                          << " start_s: " << bl.starts << " end_s: " << bl.ends
                          << " length: " << node->Length();
                }
            ...
            {
                for (auto &&point : request.waypoint)
                {
            ...
            {
                error_code->errorcode=(error_code_id);
        ...
        {
            Graph graph;
            if (!cyber::common::GetProtoFromASCIIFile(topo_file_path, &graph))
            {
                                    ...
                                    RoutingResponse *const response)
        {
            ...
            if (!Init(request, graph_.get(), &way_nodes, &way_s))
            {
                SetErrorCode(ErrorCode::ROUTING_ERROR_NOT_READY,
                             "Failed to initialize navigator!", &response->status);
                return false;
            }
            ...
            SetErrorCode(ErrorCode::OK, "Success!", &response->status);
