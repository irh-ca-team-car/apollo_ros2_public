...
void ConvertOutRange(const std::vector<CurveRange>& range_vec,
...
TopoNode::TopoNode(const Node& node)
  ...
  ACHECK(pb_node_.length > kLenghtEpsilon)
      << "Node length is invalid in pb: " << pb_node_;
  Init();
  origin_node_ = this;
}
...
void TopoNode::Init() {
  ...
  ConvertOutRange(pb_node_.leftout, start_s_, end_s_,
  ...
  ConvertOutRange(pb_node_.rightout, start_s_, end_s_,
...
bool TopoNode::FindAnchorPoint() {
  ...
  for (const auto& seg : CentralCurve().segment) {
    total_size += seg.curvetype.linesegment.point.size();
  }
  double rate = (StartS() + EndS()) / 2.0 / Length();
  size_t anchor_index = static_cast<size_t>(total_size * rate);
  for (const auto& seg : CentralCurve().segment) {
    if (anchor_index < seg.curvetype.linesegment.point.size()) {
      SetAnchorPoint(seg.curvetype.linesegment.point.at(anchor_index));
      return true;
    }
    anchor_index -= seg.curvetype.linesegment.point.size();
  }
  return false;
}
...
bool TopoNode::IsVirtual() const { return pb_node_.isvirtual; }

const std::string& TopoNode::LaneId() const { return pb_node_.laneid; }

const std::string& TopoNode::RoadId() const { return pb_node_.roadid; }

const hdmap::Curve& TopoNode::CentralCurve() const {
  return pb_node_.centralcurve;
}
...
const TopoEdge* TopoNode::GetInEdgeFrom(const TopoNode* from_node) const {
  for (auto &&i : in_edge_map_)
  {
      if(i.first == from_node)
        return i.second;
  }
  return nullptr;
}
...
TopoEdge::TopoEdge(const apollo_msgs::msg::ApolloroutingEdge &edge, const TopoNode *from_node,
                   const TopoNode *to_node)
    : pb_edge_(edge), from_node_(from_node), to_node_(to_node) {}

TopoEdge::~TopoEdge() {}

const apollo_msgs::msg::ApolloroutingEdge& TopoEdge::PbEdge() const { return pb_edge_; }
...
const std::string& TopoEdge::FromLaneId() const {
  return pb_edge_.fromlaneid;
}

const std::string& TopoEdge::ToLaneId() const { return pb_edge_.tolaneid; }

TopoEdgeType TopoEdge::Type() const {
  if (pb_edge_.directiontype == Edge::LEFT) {
    return TET_LEFT;
  }
  if (pb_edge_.directiontype == Edge::RIGHT) {
    return TET_RIGHT;
  }
  return TET_FORWARD;
}
