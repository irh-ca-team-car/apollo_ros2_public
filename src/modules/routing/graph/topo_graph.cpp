...
bool TopoGraph::LoadNodes(const Graph& graph) {
  ...
  for (const auto& node : graph.node) {
    node_index_map_[node.laneid] = static_cast<int>(topo_nodes_.size());
    std::shared_ptr<TopoNode> topo_node;
    topo_node.reset(new TopoNode(node));
    road_node_map_[node.roadid].insert(topo_node.get());
...
bool TopoGraph::LoadEdges(const Graph& graph) {
  ...
  for (const auto& edge : graph.edge) {
    const std::string& from_lane_id = edge.fromlaneid;
    const std::string& to_lane_id = edge.tolaneid;
...
bool TopoGraph::LoadGraph(const Graph& graph) {
  ...
  map_version_ = graph.hdmapversion;
  map_district_ = graph.hdmapdistrict;
