...
namespace apollo
{
  ...
  {
    bool CheckPointDistanceInThreshold(apollo_msgs::msg::ApollocommonPointENU point_a,
                                       apollo_msgs::msg::ApollocommonPointENU point_b, double distance)
    {
    ...
    {
      cycle_ = cycle_routing_task.cyclenum;
      auto waypoints = cycle_routing_task.routingrequest.waypoint;
      waypoint_num_ = waypoints.size();
      begin_point_ = waypoints[0];
      end_point_ = waypoints[waypoint_num_ - 1];
      is_allowed_to_route_ = true;
      original_routing_request_ = cycle_routing_task.routingrequest;
      //   map_service is dreamview related
        ...
        routing::RoutingRequest *new_routing_request)
    {
      ...
      if (is_allowed_to_route_)
      {
                                          ...
                                          FLAGS_threshold_for_destination_check))
        {
          ...
          new_routing_request = &original_routing_request_;
          auto cur_point = new_routing_request->waypoint[0];
          // //   map_service is dreamview related
      ...
      else
      {
                                          ...
                                          FLAGS_threshold_for_destination_check))
        {
          AINFO << "GetNewRouting: reach end point"
                << "Remaining cycles: " << cycle_;
          new_routing_request->waypoint.clear();
          auto *cur_point = &new_routing_request->waypoint.emplace_back();
          // map_service is dreamview related
          if (!map_service_->ConstructLaneWayPointWithHeading(
                  pose.position.x, pose.position.y, pose.heading,
                  cur_point))
          ...
          auto *next_point = &new_routing_request->waypoint.emplace_back();
