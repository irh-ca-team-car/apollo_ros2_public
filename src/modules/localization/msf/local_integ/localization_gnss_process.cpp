...
void LocalizationGnssProcess::RawObservationProcess(
  ...
  if (raw_obs.receiverid==0) {
    AERROR << "Obs data being invalid if without receiver id!";
    return;
  }
  const double unix_to_gps = 315964800;
  const double sec_per_week = 604800;
  double leap_second_s = gnss_solver_->get_leap_second(raw_obs.gnssweek,
                                                       raw_obs.gnssseconds);

  double sys_secs_to_gnss =
      Clock::NowInSeconds() - unix_to_gps + leap_second_s;
  double obs_secs =
      raw_obs.gnssseconds + raw_obs.gnssweek * sec_per_week;
  double obs_delay = sys_secs_to_gnss - obs_secs;

  double obs_xyz[3] = {0.0};
  if (raw_obs.positionx!=0 && raw_obs.positiony!=0 &&
      raw_obs.positionz!=0) {
    obs_xyz[0] = raw_obs.positionx;
    obs_xyz[1] = raw_obs.positiony;
    obs_xyz[2] = raw_obs.positionz;
  }
  ...
  snprintf(
      ...
      raw_obs.receiverid, raw_obs.gnssseconds, raw_obs.satobsnum,
  ...
  double new_obs_time =
      raw_obs.gnssseconds + raw_obs.gnssweek * second_per_week;
...
void LocalizationGnssProcess::RawEphemerisProcess(
  ...
  auto gnss_orbit = msg;
  if (gnss_orbit.gnsstype == drivers::gnss::GnssType::GLO_SYS) {
     ...
     */
    double leap_sec =
        gnss_solver_->get_leap_second(gnss_orbit.glonassorbit.weeknum,
                                      gnss_orbit.glonassorbit.toe);
    double toe = gnss_orbit.glonassorbit.toe - leap_sec;
    gnss_orbit.glonassorbit.toe=toe;
    gnss_orbit.glonassorbit.weekseconds=toe;
  }
...
bool LocalizationGnssProcess::DuplicateEph(
  ...
  drivers::gnss::GnssType t_type = static_cast<drivers::gnss::GnssType>(raw_eph.gnsstype);
  ...
  if (t_type == drivers::gnss::GnssType::GLO_SYS) {
    sat_prn = raw_eph.glonassorbit.slotprn;
    week_num = raw_eph.glonassorbit.weeknum;
    toe = raw_eph.glonassorbit.toe;
  } else {
    sat_prn = raw_eph.kepplerorbit.satprn;
    week_num = raw_eph.kepplerorbit.weeknum;
    toe = raw_eph.kepplerorbit.toe;
  }
