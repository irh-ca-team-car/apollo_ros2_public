...
bool MeasureRepublishProcess::NovatelBestgnssposProcess(
  ...
  ADEBUG << "MeasureDataRepublish Debug Log: bestgnsspos msg: "
         << "[time:" << measure->time << "]"
         << "[x:" << measure->gnss_pos.longitude * RAD_TO_DEG << "]"
         << "[y:" << measure->gnss_pos.latitude * RAD_TO_DEG << "]"
         << "[z:" << measure->gnss_pos.height << "]"
         << "[std_x:" << bestgnsspos_msg.longitudestddev << "]"
         << "[std_y:" << bestgnsspos_msg.latitudestddev << "]"
         << "[std_z:" << bestgnsspos_msg.heightstddev << "]"
         << "[position_type:" << bestgnsspos_msg.soltype << "]";
...
void MeasureRepublishProcess::GnssLocalProcess(
  ...
  *measure = measure_data;
  ...
  ADEBUG << "MeasureDataRepublish Debug Log: rtkgnss msg: "
...
bool MeasureRepublishProcess::LidarLocalProcess(
  ...
  measure_data.time = lidar_local_msg.measurementtime;
  ...
  double longitude_var = lidar_local_msg.uncertainty.positionstddev.x;
  double latitude_var = lidar_local_msg.uncertainty.positionstddev.y;
  double yaw_var = lidar_local_msg.uncertainty.orientationstddev.z;
  ...
  *measure = measure_data;
  ...
  ADEBUG << "MeasureDataRepublish Debug Log: lidarLocal msg: "
         << "[time:" << measure_data.time << "]"
         << "[x:" << measure_data.gnss_pos.longitude * RAD_TO_DEG << "]"
         << "[y:" << measure_data.gnss_pos.latitude * RAD_TO_DEG << "]"
         << "[z:" << measure_data.gnss_pos.height << "]"
...
void MeasureRepublishProcess::TransferXYZFromBestgnsspose(
  ...
  measure->time = bestgnsspos_msg.measurementtime;
  if (is_trans_gpstime_to_utctime_) {
  ...
  measure->gnss_pos.height =
      bestgnsspos_msg.heightmsl + bestgnsspos_msg.undulation;

  measure->variance[0][0] =
      bestgnsspos_msg.longitudestddev * bestgnsspos_msg.longitudestddev;
  measure->variance[1][1] =
      bestgnsspos_msg.latitudestddev * bestgnsspos_msg.latitudestddev;
  measure->variance[2][2] =
      bestgnsspos_msg.heightstddev * bestgnsspos_msg.heightstddev;
...
bool MeasureRepublishProcess::GnssHeadingProcess(
  ...
  int solution_status = heading_msg.solutionstatus;
  int position_type = heading_msg.positiontype;
  ...
  *status = position_type;
  ...
  measure_data->time = heading_msg.measurementtime;
  if (is_trans_gpstime_to_utctime_) {
  ...
  if (delta_time_between_height < 1.0) {
    AINFO << "the heading time and delta time: "
  ...
  double heading_std = heading_msg.headingstddev;
  ...
  AINFO << "novatel heading is: " << measure_data->time
        << " " << gnss_yaw;
  if (gnss_yaw > 180) {
...
bool MeasureRepublishProcess::CheckBestgnssPoseXYStd(
  ...
  if ((bestgnsspos_msg.longitudestddev > GNSS_XY_STD_THRESHOLD) ||
      (bestgnsspos_msg.latitudestddev > GNSS_XY_STD_THRESHOLD)) {
    AWARN << "the position std is large: "
          << bestgnsspos_msg.longitudestddev << " "
          << bestgnsspos_msg.latitudestddev;
    return false;
  }
  return true;
}
...
bool MeasureRepublishProcess::CheckBestgnssposeStatus(
    const GnssBestPose& bestgnsspos_msg) {
  int gnss_solution_status = static_cast<int>(bestgnsspos_msg.solstatus);
  int gnss_position_type = static_cast<int>(bestgnsspos_msg.soltype);
