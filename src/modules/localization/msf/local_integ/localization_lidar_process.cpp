...
int LocalizationLidarProcess::GetResult(LocalizationEstimate* lidar_local_msg) {
  ...
  lidar_local_msg->measurementtime=pre_location_time_;

  apollo::common::Header* headerpb = &lidar_local_msg->apolloheader;
  headerpb->timestampsec=pre_location_time_;

  apollo::localization::Pose* posepb = &lidar_local_msg->pose;
  apollo::common::PointENU* position = &posepb->position;
  position->x=location_.translation()(0);
  position->y=location_.translation()(1);
  position->z=location_.translation()(2);

  apollo::common::Quaternion* quaternion = &posepb->orientation;
  Eigen::Quaterniond quat(location_.linear());
  quaternion->qx=quat.x();
  quaternion->qy=quat.y();
  quaternion->qz=quat.z();
  quaternion->qw=quat.w();

  apollo::localization::Uncertainty* uncertainty =
      &lidar_local_msg->uncertainty;

  apollo::common::Point3D* position_std_dev =
      &uncertainty->positionstddev;
  position_std_dev->x=location_covariance_(0, 0);
  position_std_dev->y=location_covariance_(1, 1);
  position_std_dev->z=0.0;
  ...
  orientation_std_dev->x=0.0;
  orientation_std_dev->y=0.0;
  orientation_std_dev->z=yaw_covariance;

  MsfStatus* msf_status = &lidar_local_msg->msfstatus;
  msf_status->locallidarstatus=local_lidar_status_;
  msf_status->locallidarquality=local_lidar_quality_;
...
bool LocalizationLidarProcess::GetPredictPose(const double lidar_time,
  ...
  if (latest_imu_time - lidar_time > imu_lidar_max_delay_time_) {
    AERROR << "LocalizationLidar GetPredictPose: "
           << "Lidar msg too old! "
