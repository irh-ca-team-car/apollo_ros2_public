...
void LocalizationInteg::TransferImuRfu(const drivers::gnss::Imu &imu_msg,
  ...
  double measurement_time = TimeUtil::Gps2Unix(imu_msg.measurementtime);
  imu_rfu->measurement_time = measurement_time;
  imu_rfu->fb[0] = imu_msg.linearacceleration.x * FLAGS_imu_rate;
  imu_rfu->fb[1] = imu_msg.linearacceleration.y * FLAGS_imu_rate;
  imu_rfu->fb[2] = imu_msg.linearacceleration.z * FLAGS_imu_rate;

  imu_rfu->wibb[0] = imu_msg.angularvelocity.x * FLAGS_imu_rate;
  imu_rfu->wibb[1] = imu_msg.angularvelocity.y * FLAGS_imu_rate;
  imu_rfu->wibb[2] = imu_msg.angularvelocity.z * FLAGS_imu_rate;
}
...
void LocalizationInteg::TransferImuFlu(const drivers::gnss::Imu &imu_msg,
  ...
  imu_flu->fb[0] = -imu_msg.linearacceleration.y * FLAGS_imu_rate;
  imu_flu->fb[1] = imu_msg.linearacceleration.x * FLAGS_imu_rate;
  imu_flu->fb[2] = imu_msg.linearacceleration.z * FLAGS_imu_rate;

  imu_flu->wibb[0] = -imu_msg.angularvelocity.y * FLAGS_imu_rate;
  imu_flu->wibb[1] = imu_msg.angularvelocity.x * FLAGS_imu_rate;
  imu_flu->wibb[2] = imu_msg.angularvelocity.z * FLAGS_imu_rate;
}
