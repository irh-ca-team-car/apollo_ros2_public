...
void LocalizationIntegProcess::RawImuProcess(const ImuData &imu_msg) {
  ...
  if (delta_time > 0.1) {
    AERROR << "the imu message loss more than 10, "
           << "the pre time and current time: " << pre_imu_time << " "
           << cur_imu_time;
  } else if (delta_time < 0.0) {
    AERROR << "received imu message's time is eary than last imu message, "
           << "the pre time and current time: " << pre_imu_time << " "
           << cur_imu_time;
  }
  ...
  if (delta_system_time > 0.1) {
    AERROR << "the imu message loss more than 10 according to system time, "
  ...
  if (sins_->IsSinsAligned()) {
    ...
    if (cur_imu_time - 0.5 > pre_imu_time) {
      AINFO << "SINS has completed alignment!";
      pre_imu_time = cur_imu_time;
    }
  } else {
    delay_output_counter_ = 0;
    if (cur_imu_time - 0.5 > pre_imu_time) {
      AINFO << "SINS is aligning!";
      pre_imu_time = cur_imu_time;
    }
  }
...
void LocalizationIntegProcess::GetResult(IntegState *state,
  ...
  *state = integ_state_;
  ...
  if (*state != IntegState::NOT_INIT) {
    ADEBUG << "IntegratedLocalization Debug Log: integ_pose msg: "
           << "[time:" << ins_pva_.time << "]"
  ...
  apollo::common::Header *headerpb_loc = &localization->apolloheader;
  apollo::localization::Pose *posepb_loc = &localization->pose;

  localization->measurementtime=ins_pva_.time;
  headerpb_loc->timestampsec=apollo::cyber::Clock::NowInSeconds();

  apollo::common::PointENU *position_loc = &posepb_loc->position;
  apollo::common::Quaternion *quaternion = &posepb_loc->orientation;
  UTMCoor utm_xy;
  FrameTransform::LatlonToUtmXY(ins_pva_.pos.longitude, ins_pva_.pos.latitude,
                                &utm_xy);
  position_loc->x=utm_xy.x;
  position_loc->y=utm_xy.y;
  position_loc->z=ins_pva_.pos.height;

  quaternion->qx=ins_pva_.qbn[1];
  quaternion->qy=ins_pva_.qbn[2];
  quaternion->qz=ins_pva_.qbn[3];
  quaternion->qw=ins_pva_.qbn[0];

  apollo::common::Point3D *velocitylinear =
      &posepb_loc->linearvelocity;
  velocitylinear->x=ins_pva_.vel.ve;
  velocitylinear->y=ins_pva_.vel.vn;
  velocitylinear->z=ins_pva_.vel.vu;

  apollo::common::Point3D *eulerangles = &posepb_loc->eulerangles;
  eulerangles->x=ins_pva_.att.pitch;
  eulerangles->y=ins_pva_.att.roll;
  eulerangles->z=ins_pva_.att.yaw;

  posepb_loc->heading=ins_pva_.att.yaw;

  apollo::localization::Uncertainty *uncertainty =
      &localization->uncertainty;
  apollo::common::Point3D *position_std_dev =
      &uncertainty->positionstddev;
  position_std_dev->x=std::sqrt(pva_covariance_[0][0]);
  position_std_dev->y=std::sqrt(pva_covariance_[1][1]);
  position_std_dev->z=std::sqrt(pva_covariance_[2][2]);

  apollo::common::Point3D *linear_velocity_std_dev =
      &uncertainty->linearvelocitystddev;
  linear_velocity_std_dev->x=std::sqrt(pva_covariance_[3][3]);
  linear_velocity_std_dev->y=std::sqrt(pva_covariance_[4][4]);
  linear_velocity_std_dev->z=std::sqrt(pva_covariance_[5][5]);

  apollo::common::Point3D *orientation_std_dev =
      &uncertainty->orientationstddev;
  orientation_std_dev->x=std::sqrt(pva_covariance_[6][6]);
  orientation_std_dev->y=std::sqrt(pva_covariance_[7][7]);
  orientation_std_dev->z=std::sqrt(pva_covariance_[8][8]);
}
...
void LocalizationIntegProcess::StartThreadLoop() {
  keep_running_ = true;
  measure_data_queue_size_ = 150;
  (void)std::async(&LocalizationIntegProcess::MeasureDataThreadLoop, this);
}
...
bool LocalizationIntegProcess::CheckIntegMeasureData(
  ...
  ADEBUG << "IntegratedLocalization Debug Log: measure data: "
