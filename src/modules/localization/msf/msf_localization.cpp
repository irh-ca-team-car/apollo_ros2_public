...
void MSFLocalization::InitParams() {
  ...
  auto localization_timer_ = currentNode->create_wall_timer(std::chrono::milliseconds(10), std::bind(&MSFLocalization::OnLocalizationTimer, this));
}
...
void MSFLocalization::OnRawImu(
  ...
  auto *status_headerpb = &status.apolloheader;
  status_headerpb->timestampsec=
      result.localization().apolloheader.timestampsec;
  status.fusionstatus=
      static_cast<MeasureState>(result.integ_status().integ_state);
  status.statemessage=result.integ_status().state_message;
  status.measurementtime=result.localization().measurementtime;
...
void MSFLocalization::CompensateImuVehicleExtrinsic(
  ...
  // calculate orientation_vehicle_world
  apollo::localization::Pose *posepb_loc = &local_result->pose;
  ...
  posepb_loc->heading=common::math::QuaternionToHeading(
  ...
  common::math::EulerAnglesZXYd euler_angle(
      quat_vehicle_world.w(), quat_vehicle_world.x(), quat_vehicle_world.y(),
      quat_vehicle_world.z());
  eulerangles->x=euler_angle.pitch();
  eulerangles->y=euler_angle.roll();
  eulerangles->z=euler_angle.yaw();
}
