...
void NDTLocalization::Init() {
  ...
  ADEBUG << "LoadLidarExtrinsic" << lidar_extrinsics_file;
  bool success =
...
void NDTLocalization::OdometryCallback(
  ...
  double odometry_time = odometry_msg->apolloheader.timestampsec;
  ...
  if (odometry_msg->localization!=apollo_msgs::msg::ApollolocalizationPose()) {
...
void NDTLocalization::FillLocalizationMsgHeader(
  ...
  //header->sequence_num=++localization_seq_num_;
}
...
void NDTLocalization::ComposeLocalizationEstimate(
  ...
  *localization = apollo_msgs::msg::ApollolocalizationLocalizationEstimate();
  ...
  localization->measurementtime=odometry_msg->apolloheader.timestampsec;
  auto mutable_pose = &localization->pose;
  mutable_pose->position.x=pose.translation().x();
  mutable_pose->position.y=pose.translation().y();
  mutable_pose->position.z=pose.translation().z();

  Eigen::Quaterniond quat(pose.linear());
  mutable_pose->orientation.qw=quat.w();
  mutable_pose->orientation.qx=quat.x();
  mutable_pose->orientation.qy=quat.y();
  mutable_pose->orientation.qz=quat.z();
  double heading =
  ...
  mutable_pose->eulerangles.x=euler.pitch();
  mutable_pose->eulerangles.y=euler.roll();
  mutable_pose->eulerangles.z=euler.yaw();

  const auto& odometry_pose = odometry_msg->localization;
  if (odometry_pose.linearvelocity != apollo_msgs::msg::ApollocommonPoint3D()) {
    mutable_pose->linearvelocity=
        odometry_pose.linearvelocity;
  }
  if (odometry_pose.linearacceleration != apollo_msgs::msg::ApollocommonPoint3D()) {
    mutable_pose->linearacceleration=
        odometry_pose.linearacceleration;
  }
  if (odometry_pose.angularvelocity != apollo_msgs::msg::ApollocommonPoint3D()) {
    mutable_pose->angularvelocity=
        odometry_pose.angularvelocity;
  }
  if (odometry_pose.linearaccelerationvrf != apollo_msgs::msg::ApollocommonPoint3D()) {
    mutable_pose->linearaccelerationvrf=
        odometry_pose.linearaccelerationvrf;
  }
  if (odometry_pose.angularvelocityvrf != apollo_msgs::msg::ApollocommonPoint3D()) {
    mutable_pose->angularvelocityvrf=
        odometry_pose.angularvelocityvrf;
  }
}
...
void NDTLocalization::ComposeLidarResult(double time_stamp,
  ...
  *localization = apollo_msgs::msg::ApollolocalizationLocalizationEstimate();
  ...
  localization->measurementtime=time_stamp;
  auto mutable_pose = &localization->pose;
  mutable_pose->position.x=pose.translation().x();
  mutable_pose->position.y=pose.translation().y();
  mutable_pose->position.z=pose.translation().z();
...
bool NDTLocalization::QueryPoseFromTF(double time, Eigen::Affine3d* pose) {
  ...
  geometry_msgs::msg::TransformStamped query_transform;
  try {
  ...
  Eigen::Quaterniond tmp_quat(query_transform.transform.rotation.w,
                              query_transform.transform.rotation.x,
                              query_transform.transform.rotation.y,
                              query_transform.transform.rotation.z);
...
void NDTLocalization::LidarMsgTransfer(
  ...
  lidar_frame->measurement_time =
      cyber::Time(msg->measurementtime).ToSecond();
  if (ndt_debug_log_flag_) {
...
bool NDTLocalization::FindNearestOdometryStatus(
  ...
  for (auto itr = odometry_status_list.begin();
    ...
    double diff = std::abs(itr->apolloheader.timestampsec - odometry_timestamp);
