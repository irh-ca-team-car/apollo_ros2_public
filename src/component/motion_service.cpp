...
bool MotionService::Init() {
  ...
  motion_service_param.cameranames=declare_parameter<std::string>("camera_names","front_6mm");
  motion_service_param.inputcamerachannelnames=declare_parameter<std::string>("input_camera_channel_names","/apollo/sensor/camera/front_6mm/image");
  motion_service_param.inputlocalizationchannelname=declare_parameter<std::string>("input_localization_channel_name","/apollo/localization/pose");
  motion_service_param.outputtopicchannelname=declare_parameter<std::string>("output_topic_channel_name","/apollo/perception/motion_service");
...
void MotionService::PublishEvent(const double timestamp) {
  ...
  for (int k = static_cast<int>(motion_buffer.size()) - 1; k >= 0; k--) {
    ...
    motion_service_msg->vehiclestatus.push_back(v_status_msg);
