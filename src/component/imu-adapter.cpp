  ...
  namespace drivers
  {
    namespace adapter
    {
      ...
      bool IMUAdapterComponent::Init()
      {
        ...
        AINFO << "input_topic:" << input_topic;
        AINFO << "output_topic:" << output_topic;
        AINFO << "output_topic_c:" << output_topic_c;
      ...
      void IMUAdapterComponent::Proc(const std::shared_ptr<sensor_msgs::msg::Imu> imu_msg)
      {
        ...
        if(imu_msg->header.stamp.sec < this->get_clock()->now().seconds() - 1800) return;
        AINFO_EVERY(60000)<< "Receiving IMU INFO";
        apollo::drivers::gnss::Imu imu_apollo;

        imu_apollo.apolloheader.frameid = imu_msg->header.frame_id;
        imu_apollo.apolloheader.timestampsec = this->get_clock()->now().seconds();
        ...
        imu_apollo2.apolloheader.timestampsec = this->get_clock()->now().seconds();
