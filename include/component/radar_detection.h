...
class RadarDetectionComponent : public cyber::Component {
 public:
  RadarDetectionComponent() : Component("RadarDetectionComponent") {
        seq_num_=0;
  ...
  bool Init() override;
  bool Proc(const std::shared_ptr<ContiRadar>& message);
 ...
 private:
  ...
  std::shared_ptr<apollo::cyber::WriterJson<SensorFrameMessage>> writer_;
  std::shared_ptr<cyber::Reader<ContiRadar>> contiradar_reader_;
};
