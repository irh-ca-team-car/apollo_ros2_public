...
class MSFLocalizationComponent final
 ...
 public:
  MSFLocalizationComponent() : Component("MSFLocalizationComponent") {}
  ~MSFLocalizationComponent() = default;

  bool Init() override;

  bool Proc(const std::shared_ptr<drivers::gnss::Imu>& imu_msg);
...
class LocalizationMsgPublisher {
 public:
  explicit LocalizationMsgPublisher(cyber::ComponentBase *node);
