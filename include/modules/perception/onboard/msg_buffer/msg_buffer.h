  ...
  namespace perception
  {
    namespace onboard
    {
      ...
      void MsgBuffer<T>::Init(const std::string &channel, const std::string &name)
      {
        ...
        node_ = apollo::cyber::ComponentBase::instance();
      ...
      {
        std::lock_guard<std::mutex> lock(buffer_mutex_);
        double timestamp = msg->measurementtime;
      ...
      int MsgBuffer<T>::LookupNearest(double timestamp, ConstPtr *msg)
      {
        ...
        if (!init_)
        {
          AERROR_EVERY(60000) << "msg buffer is uninitialized.";
          return false;
        }
        if (buffer_queue_.empty())
        {
          AERROR_EVERY(60000) << "msg buffer is empty.";
          return false;
        }
        if (buffer_queue_.front().first - FLAGS_obs_buffer_match_precision >
            timestamp)
        {
          AERROR_EVERY(60000) << "Your timestamp (" << timestamp
      ...
      int MsgBuffer<T>::LookupLatest(ConstPtr *msg)
      {
        ...
        if (!init_)
        {
          AERROR_EVERY(60000) << "Message buffer is uninitialized.";
          return false;
        }
        if (buffer_queue_.empty())
        {
          AERROR_EVERY(60000) << "Message buffer is empty.";
          return false;
        }
                                     ...
                                     std::vector<ObjectPair> *msgs)
      {
        ...
        if (buffer_queue_.front().first - FLAGS_obs_buffer_match_precision >
            timestamp)
        {
          AERROR_EVERY(60000) << "Your timestamp (" << timestamp << ") is earlier than the oldest "
        ...
        if (buffer_queue_.back().first + FLAGS_obs_buffer_match_precision <
            timestamp)
        {
          AERROR_EVERY(60000) << "Your timestamp (" << timestamp << ") is newer than the latest "
