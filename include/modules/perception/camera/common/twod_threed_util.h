...
T CalAngleDiff(const T a1, const T a2) {
  ...
  T diff2 = a1 - a2 + common::Constant<T>::PI() * 2;
  diff2 = diff2 < 0 ? -diff2 : diff2;
  T diff3 = a1 - a2 - common::Constant<T>::PI() * 2;
...
inline T GetSharpAngle(const T &a, const T &b) {
  ...
  const T pi = common::Constant<T>::PI();
  T angle = std::abs(a - b);
  if (angle > pi) {
    angle -= pi;
  }
...
T GetJaccardIndex(const T *bbox_ref, const T &x_min, const T &y_min,
  ...
  T union_area = (y_max - y_min) * (x_max - x_min) +
                 ...
                 inter_area;
  return inter_area * common::IRec(union_area);
}
