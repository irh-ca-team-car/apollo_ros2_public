...
class Int8EntropyCalibrator : public IInt8EntropyCalibrator {
 public:
  Int8EntropyCalibrator(
      ...
      bool read_cache, std::string network) noexcept;

  virtual ~Int8EntropyCalibrator();
  int getBatchSize() const noexcept{ return stream_.getBatchSize(); }

  bool getBatch(void *bindings[], const char *names[], int nbBindings) throw() override;

  const void *readCalibrationCache(size_t &length) throw() override;

  void writeCalibrationCache(const void *cache, size_t length) throw() override;
