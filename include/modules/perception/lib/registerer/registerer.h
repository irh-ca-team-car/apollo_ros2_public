  ...
  public:                                                             \
    ...
    {                                                                 \
      ...
      FactoryMap::iterator iter = map.find(name);                     \
          AERROR_EVERY(60000) << "GetInstanceByName:" << name;                           \
      if (iter == map.end())                                          \
      {                                                               \
        for (auto c : map)                                            \
        {                                                             \
          AERROR_EVERY(60000) << "Instance:" << c.first;                           \
        }                                                             \
        AERROR_EVERY(60000) << "Get instance " << name << " failed.";              \
        return nullptr;                                               \
      }                                                               \
      Any object = iter->second->NewInstance();                       \
      AINFO_EVERY(60000) << "Get instance " << name << " succeeded.";              \
...
#define PERCEPTION_REGISTER_CLASS(clazz, name)                              \
  ...
  bool RegisterFactory##name()                                              \
  {                                                                         \
    ::apollo::perception::lib::FactoryMap &map =                            \
        ::apollo::perception::lib::GlobalFactoryMap()[#clazz];              \
    if (map.find(#name) == map.end())                                       \
      map[#name] = new ObjectFactory##name();                               \
    AINFO_EVERY(60000) << "Registered RegisterFactory" #name;                            \
    return true;                                                            \
  }                                                                         \
  bool RegisterdFactory##name = RegisterFactory##name();
