...
class OpenSpaceInfo {
 public:
  ...
  apollo::planningInternal::Debug *mutable_debug() { return debug_; }

  void set_debug(apollo::planningInternal::Debug *debug) { debug_ = debug; }

  const apollo::planningInternal::Debug &debug() const { return *debug_; }

  const apollo::planningInternal::Debug debug_instance() const {
    return debug_instance_;
  }

  apollo::planningInternal::Debug *mutable_debug_instance()
  {
    return &debug_instance_;
  }
  ...
  void sync_debug_instance() {
    ...
    // vectors.
    if (!debug_->planningdata.openspace.obstacles.empty()) {
      debug_instance_.planningdata
          .openspace
          .obstacles.clear();
    }
  ...
  void RecordDebug(apollo::planningInternal::Debug *ptr_debug);
