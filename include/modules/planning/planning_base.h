...
class PlanningBase {
 public:
  ...
  explicit PlanningBase(const std::shared_ptr<DependencyInjector> &);

  virtual ~PlanningBase();

  virtual apollo::common::Status Init(const apollo_msgs::msg::ApolloplanningPlanningConfig &);

  virtual std::string Name() const = 0;

  virtual void RunOnce(const LocalView &local_view,
                       apollo_msgs::msg::ApolloplanningADCTrajectory*) = 0;
 ...
 protected:
  virtual void FillPlanningPb(const double timestamp,
                              apollo_msgs::msg::ApolloplanningADCTrajectory*);
