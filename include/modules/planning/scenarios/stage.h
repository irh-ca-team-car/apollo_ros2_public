...
class Stage {
 public:
  ...
  Stage(const apollo_msgs::msg::ApolloplanningStageConfig& config,
        const std::shared_ptr<DependencyInjector>& injector);

  virtual ~Stage() = default;

  const apollo_msgs::msg::ApolloplanningStageConfig& config() const { return config_; }

  int stage_type() const { return config_.stagetype; }
...
#define DECLARE_STAGE(NAME, CONTEXT)                          \
  class NAME : public Stage {                                 \
   public:                                                    \
    explicit NAME(const StageConfig& config)  \
        : Stage(config) {}                                    \
