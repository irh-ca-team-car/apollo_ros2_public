...
class SubmoduleOutput {
 public:
  ...
  void set_curr_scenario(const apollo_msgs::msg::ApollopredictionScenario &scenario);

  const std::vector<Obstacle>& curr_frame_obstacles() const;

  const Obstacle& GetEgoVehicle() const;

  const std::vector<apollo_msgs::msg::ApolloperceptionPerceptionObstacle> &
  ...
  const apollo_msgs::msg::ApollopredictionScenario& curr_scenario() const { return curr_scenario_; }
