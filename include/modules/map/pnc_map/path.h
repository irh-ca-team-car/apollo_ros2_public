...
struct LaneWaypoint {
  ...
  LaneWaypoint(LaneInfoConstPtr lane, const double s)
      : lane((lane)), s(s) { CHECK_NOTNULL(lane); };
  LaneWaypoint(LaneInfoConstPtr lane, const double s, const double l)
      : lane((lane)), s(s), l(l){CHECK_NOTNULL(lane);} LaneInfoConstPtr lane = nullptr;
  double s = 0.0;
  double l = 0.0;
...
struct LaneSegment {
  ...
  LaneSegment(LaneInfoConstPtr lane, const double start_s, const double end_s)
      : lane(lane), start_s(start_s), end_s(end_s) { CHECK_NOTNULL(lane); }
